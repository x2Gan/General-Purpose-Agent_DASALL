...
constexpr std::string_view kGenerateStage = "llm.ollama_native.generate";
constexpr std::string_view kHealthStage = "llm.ollama_native.health_check";
constexpr std::string_view kChatPath = "/api/chat";
constexpr std::string_view kTagsPath = "/api/tags";
...
[[nodiscard]] bool config_has_required_values(const LLMAdapterConfig& config) {
  return config.adapter_family == kOllamaNativeFamily && !config.adapter_id.empty() &&
...
[[nodiscard]] std::optional<std::string> extract_json_object_field(
    std::string_view payload,
    std::string_view key,
    std::size_t offset = 0U) {
  return extract_json_wrapped_field(payload, key, '{', '}', offset);
...
OllamaAdapter::OllamaAdapter(std::shared_ptr<ILLMTransport> transport)
    : transport_(std::move(transport)) {}

bool OllamaAdapter::init(const LLMAdapterConfig& config) {
...
AdapterCallResult OllamaAdapter::generate(const contracts::LLMRequest& request) {
  if (!is_ready()) {
  ...
  if (!validation.ok || !request.request_mode.has_value() ||
    ...
    return make_failure_result(ResultCode::ValidationFieldMissing,
                               validation.ok
                                   ? "ollama adapter only supports unary generate() in 026"
  ...
  if (!transport_request.has_consistent_values()) {
    return make_failure_result(ResultCode::ValidationFieldMissing,
                               "ollama adapter failed to materialize a valid transport request",
                               false,
                               std::string(kGenerateStage));
  }
...
StreamSessionRef OllamaAdapter::stream_generate(const contracts::LLMRequest&, IStreamObserver*) {
  return StreamSessionRef{.session_id = "ollama-native-streaming-not-implemented"};
}

HealthStatus OllamaAdapter::health_check() {
  if (!is_ready()) {
    return HealthStatus{
        .ready = false,
        .degraded = true,
        .message = "ollama adapter is not initialized",
    };
  }
  ...
  if (!request.has_consistent_values()) {
    return HealthStatus{
        .ready = false,
        .degraded = true,
        .message = "ollama adapter failed to materialize health probe request",
    };
  }
  ...
  if (!response.has_consistent_values()) {
    return HealthStatus{
        .ready = false,
        .degraded = true,
        .message = "ollama health probe returned an inconsistent transport response",
    };
  }
  ...
  if (response.status_code == 429U || response.status_code == 503U) {
    return HealthStatus{
        .ready = true,
        .degraded = true,
        .message = "ollama endpoint degraded: http " + std::to_string(response.status_code),
    };
  }
  ...
  return HealthStatus{
      .ready = false,
      .degraded = true,
      .message = "ollama endpoint unavailable: " + resolve_transport_error_message(response),
  };
}

bool OllamaAdapter::is_ready() const {
  return transport_ != nullptr && config_.has_value() && config_has_required_values(*config_);
}

std::string OllamaAdapter::resolve_model_id(std::string_view route_key) const {
  if (route_key.empty()) {
    return {};
  }
...
LLMTransportRequest OllamaAdapter::make_chat_request(const contracts::LLMRequest& request) const {
  ...
  return LLMTransportRequest{
      ...
      .url = join_url(config_->base_url, kChatPath),
...
LLMTransportRequest OllamaAdapter::make_health_request() const {
  if (!is_ready()) {
    return {};
  }
  ...
  return LLMTransportRequest{
      ...
      .url = join_url(config_->base_url, kTagsPath),
      .auth_ref = config_->auth_ref,
      .header_refs = config_->header_refs,
      .base_url_alias = config_->base_url_alias,
      .snapshot_version = config_->snapshot_version,
...
AdapterCallResult OllamaAdapter::map_chat_response(const contracts::LLMRequest& request,
  ...
  normalized_response.finish_reason = extract_json_string_field(response.body, "done_reason");

  const auto message_json = extract_json_object_field(response.body, "message");
  const std::string_view message_payload =
      message_json.has_value() ? std::string_view(*message_json) : std::string_view(response.body);
  const auto tool_calls_json = extract_json_array_field(message_payload, "tool_calls");
  const auto content = extract_json_string_field(message_payload, "content");
  ...
  const auto prompt_tokens = extract_json_uint_field(response.body, "prompt_eval_count");
  const auto completion_tokens = extract_json_uint_field(response.body, "eval_count");
  if (prompt_tokens.has_value() && completion_tokens.has_value()) {
    const auto total_tokens = static_cast<std::uint32_t>(*prompt_tokens + *completion_tokens);
  ...
  result.provider_diagnostics.reasoning_content =
      extract_json_string_field(message_payload, "thinking").value_or("");
...
AdapterCallResult OllamaAdapter::make_failure_result(contracts::ResultCode result_code,
                                                     std::string message,
                                                     bool retryable,
  ...
  error.source_ref.ref_id = config_.has_value() ? config_->adapter_id : "ollama-native";
