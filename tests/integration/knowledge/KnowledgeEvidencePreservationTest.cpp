  ...
  [[nodiscard]] SparseIndexSearchResult search(
    ...
    if (sqlite3_prepare_v2(connection_, sql.c_str(), -1, &statement, nullptr) != SQLITE_OK) {
      ...
      result.error = dasall::knowledge::make_knowledge_error_info(
          ...
          "knowledge_evidence_preservation.search");
      return result;
    }
...
[[nodiscard]] KnowledgeQuery make_query() {
  ...
  query.request_id = "req-knowledge-evidence-019";
  query.session_id = "session-knowledge-evidence-019";
...
[[nodiscard]] CorpusDescriptor make_descriptor() {
  ...
  descriptor.active_snapshot_id = "snapshot-knowledge-evidence-019";
...
[[nodiscard]] IndexManifest make_manifest() {
  ...
  manifest.snapshot_id = "snapshot-knowledge-evidence-019";
...
struct KnowledgeEvidenceHarness {
  ...
  KnowledgeEvidenceHarness()
    ...
    assert_true(catalog_replaced,
                "knowledge evidence preservation harness should install a consistent corpus descriptor");

    auto snapshot = std::make_shared<IndexSnapshot>();
    snapshot->manifest = make_manifest();
    snapshot->checksum = "checksum-knowledge-evidence-019";
    ...
    assert_true(snapshot_swapped,
                "knowledge evidence preservation harness should install an active lexical snapshot");

    auto service = std::make_unique<KnowledgeServiceFacade>(make_service_deps());
    assert_true(service->init(config),
                "knowledge evidence preservation harness should initialize the facade with a consistent config");
...
[[nodiscard]] KnowledgeRetrieveResult retrieve_result(
    dasall::knowledge::IKnowledgeService& service,
    const KnowledgeQuery& query) {
  return service.retrieve(query);
}

void test_knowledge_evidence_preservation_keeps_structured_refs_on_happy_path() {
  KnowledgeEvidenceHarness harness;
  const auto result = retrieve_result(*harness.knowledge_service, make_query());

  assert_true(result.ok,
              "knowledge evidence preservation gate should complete successfully through the service interface");
  assert_true(result.has_consistent_values(),
              "knowledge evidence preservation gate should preserve retrieve result invariants");
  assert_true(result.evidence.has_value(),
              "knowledge evidence preservation gate should return an evidence bundle on the happy path");
  assert_true(!result.evidence->degraded,
              "knowledge evidence preservation gate should keep the happy-path evidence non-degraded");
  assert_true(!result.retrieval_evidence_refs.empty(),
              "knowledge evidence preservation gate should project structured evidence refs alongside text evidence");
  assert_equal(std::string{"ADR-0001#policy"},
               result.retrieval_evidence_refs.front().source_ref,
               "knowledge evidence preservation gate should preserve the citation ref in structured evidence");
  assert_equal(std::string{"fresh"},
               result.retrieval_evidence_refs.front().freshness,
               "knowledge evidence preservation gate should preserve freshness in structured evidence");
  assert_true(!result.evidence->context_projection.empty() &&
                  result.evidence->context_projection.front().find("ADR-0001#policy") != std::string::npos,
              "knowledge evidence preservation gate should preserve citation-bearing text projection");
}

void test_knowledge_evidence_preservation_rejects_stale_snapshot_without_leaking_evidence() {
  KnowledgeEvidenceHarness harness;
  harness.now_ms = 70000;

  const auto result = retrieve_result(*harness.knowledge_service, make_query());
  assert_true(!result.ok,
              "knowledge evidence preservation gate should fail closed when the snapshot is stale without opt-in");
  assert_true(result.has_consistent_values(),
              "knowledge evidence preservation gate should keep the rejected result shape consistent");
  assert_true(result.error.has_value(),
              "knowledge evidence preservation gate should surface an explicit stale rejection error");
  assert_equal(static_cast<int>(KnowledgeErrorCode::IndexStaleRejected),
               result.error->details.code.value_or(-1),
               "knowledge evidence preservation gate should map stale rejection to IndexStaleRejected");
  assert_true(!result.evidence.has_value(),
              "knowledge evidence preservation gate should not return an evidence bundle on stale rejection");
  assert_true(result.retrieval_evidence_refs.empty(),
              "knowledge evidence preservation gate should not leak structured refs on stale rejection");
}
...
int main() {
  try {
    test_knowledge_evidence_preservation_keeps_structured_refs_on_happy_path();
    test_knowledge_evidence_preservation_rejects_stale_snapshot_without_leaking_evidence();
