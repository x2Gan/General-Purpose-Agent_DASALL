...
void test_sqlite_schema_migrator_applies_v001_and_reports_up_to_date_status() {
  ...
  assert_true(table_exists(connection.get(), "schema_migrations"),
              "schema migrator should keep the migration ledger table available");
  ...
  assert_equal(6, query_count(connection.get(), "SELECT COUNT(*) FROM schema_migrations"),
               "fresh database should record all bundled migrations through V006");

  const auto migration_status = migrator.status(connection.get());
  assert_equal(6, migration_status.current_version,
               "migration status should report the applied V006 version");
  assert_equal(6, migration_status.target_version,
               "migration status should report V006 as the current target version");
  assert_true(migration_status.up_to_date,
              "migration status should report the database as up-to-date after V006");
