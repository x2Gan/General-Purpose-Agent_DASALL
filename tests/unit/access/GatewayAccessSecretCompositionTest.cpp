...
void gateway_access_options_receive_runtime_secret_manager() {
  ...
  assert_true(options.ownership_secret_manager.get() == secret_manager.get(),
              "gateway app composition should project RuntimeDependencySet.secret_manager into the Access ownership seam");
}

void gateway_access_options_clear_secret_manager_when_dependency_set_is_missing() {
  dasall::access::GatewayAccessPipelineOptions options;
  ...
  assert_true(options.ownership_secret_manager == nullptr,
              "gateway app composition should not fabricate an ownership secret manager when runtime dependencies are absent");
}
...
int main() {
  try {
    gateway_access_options_receive_runtime_secret_manager();
    gateway_access_options_clear_secret_manager_when_dependency_set_is_missing();
