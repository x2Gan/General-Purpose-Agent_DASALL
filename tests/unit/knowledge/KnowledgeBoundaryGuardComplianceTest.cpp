...
#error "KnowledgeBoundaryGuardComplianceTest requires DASALL_REPO_ROOT"
#endif
...
void test_knowledge_sources_do_not_include_forbidden_subsystem_privates() {
  using dasall::tests::support::assert_true;

  const std::filesystem::path knowledge_root = repo_root() / "knowledge";
  std::vector<std::filesystem::path> files = collect_source_files(knowledge_root / "include");
  const std::vector<std::filesystem::path> src_files = collect_source_files(knowledge_root / "src");
  ...
  const std::vector<Violation> violations = scan_include_lines(
      files,
      {
          ...
          "ILLMAdapter",
          "ILLMTransport",
          "LLMProductionFactory",
          "ProviderDescriptor",
      });

  assert_true(
      violations.empty(),
      "knowledge sources must not include llm/memory/runtime/tools/apps/access private implementation headers or boundary owners:\n" +
          summarize_violations(violations));
}

void test_knowledge_target_does_not_link_or_include_forbidden_subsystems_in_cmake() {
  using dasall::tests::support::assert_true;

  const std::vector<Violation> violations = scan_cmake_forbidden_tokens(
      repo_root() / "knowledge" / "CMakeLists.txt",
      {
          "dasall_llm",
          "dasall_memory",
          "dasall_runtime",
  ...
  assert_true(
      violations.empty(),
      "dasall_knowledge CMake target must not link or include forbidden subsystem private roots:\n" +
          summarize_violations(violations));
}

void test_knowledge_sources_do_not_take_on_context_runtime_or_llm_provider_owners() {
  using dasall::tests::support::assert_true;
  ...
  assert_true(
      violations.empty(),
      "knowledge implementation must not take on ContextPacket assembly, runtime recovery/orchestration, or concrete llm provider ownership:\n" +
          summarize_violations(violations));
}

void test_knowledge_public_surface_does_not_expose_context_runtime_or_llm_provider_types() {
  using dasall::tests::support::assert_true;

  const std::vector<std::filesystem::path> files = collect_source_files(repo_root() / "knowledge" / "include");
  ...
  assert_true(
      violations.empty(),
      "knowledge public headers must not expose context assembly, runtime recovery/orchestration, or llm provider owner types:\n" +
          summarize_violations(violations));
}
...
int main() {
  try {
    test_knowledge_sources_do_not_include_forbidden_subsystem_privates();
    test_knowledge_target_does_not_link_or_include_forbidden_subsystems_in_cmake();
    test_knowledge_sources_do_not_take_on_context_runtime_or_llm_provider_owners();
    test_knowledge_public_surface_does_not_expose_context_runtime_or_llm_provider_types();
