...
3. 当前 llm owner 代码/文档侧缺口已不再是 placeholder 实现、边界回归防线或当前态回写；`LLM-FIX-007` 已把 external provider 长稳态 / L6 soak 拆成 provider jitter、network loss、secret rotate、retry budget exhaustion 与 observability trend 的可执行 slice，因此剩余开放项主要收敛为 `FULLINT-TODO-019` 的 current release candidate rerun，以及按 007 deliverable 执行的 soak slices。
...
| 剩余开放项 | 已收敛 | FULLINT-TODO-019；external provider soak execution backlog | `LLMBoundaryGuardComplianceTest` 已自动守住 llm/source boundary；`FULLINT-TODO-019` owner 当前候选版本 rerun / artifact archive，`LLM-FIX-007` 已冻结 provider jitter / network loss / secret rotate / retry budget exhaustion / observability trend 的执行矩阵 |
...
| L6 soak / chaos / provider confidence | 未完成 | timeout/retry/fallback/route stability/observability 只证明 failure-handling basis，不等于长稳态或生产置信度；`LLM-FIX-007` 已把执行项拆成 provider jitter、network loss、secret rotate、retry budget exhaustion 与 observability trend，但真实证据仍待执行 | Medium | P1 |
...
| LLMGenerateRequest | llm/include/LLMGenerateRequest.h | 供 Runtime 调用 LLMManager 的模块公共对象，承载 stage/task_type、required pre-route `LLMRequest` hint 与独立 `prompt_release_id_override` | 保持 module-local |
...
8. `LLMGenerateRequest.request.model_route` 是 manager handoff 上的 required pre-route hint，用于让 PromptPipeline/PromptComposer 在不提前拥有最终 route ownership 的前提下得到稳定预算输入；它不是 `PromptQuery.model_family` 的别名。
9. `LLMGenerateRequest.prompt_release_id_override` 用于显式 PromptRegistry selector，必须与 `request.prompt_id/request.prompt_version` 这组响应审计锚点分离，避免输入 selector 与输出 audit 字段复用同一槽位。
10. ModelSelectionHint 用于把“场景智能切换”收敛为显式输入，而不是让 adapter 或调用代码内部猜测用户意图。
11. thinking mode 的私有字段如 `reasoning_content` 只属于 provider-local 归一化范围，不进入 shared contracts。
...
4. prompt_release_id 若由 Runtime 显式指定，则通过 `LLMGenerateRequest.prompt_release_id_override` 传入，优先级高于自动选择，并与 `request.prompt_id/request.prompt_version` 的输出审计锚点解耦。
...
3. 离线评测场景下的 response-stage judge 变体应继续落在 responder 家族内部，通过 `task_type=judge_main_chain` 和可选 `prompt_release_id_override` 选择具体 release；它服务 `WP-COG-GAP-018` 这类 batch eval / regression job，但不得新增 `judge` canonical stage，也不得绕开 PromptRegistry / PromptPolicy 的 trusted-source 与 allowlist 治理。
...
4. **v1.1 P1-2 / 043 闭环：route hint 时序澄清**：shared `PromptComposeRequest.model_route` 在 compose 阶段仍是 optional 字段，但 manager handoff 上的 `LLMGenerateRequest.request.model_route` 已冻结为 **required pre-route hint**。LLMManager 在进入 PromptPipeline 前把该 hint 写入 compose request，用于预算感知装配；最终 provider route 仍在 `ModelRouter.resolve()` 后由 adapter request 覆写为 concrete route key。该 hint 只服务预算/装配，不得被误当作 `PromptQuery.model_family` selector。
...
sequenceDiagram
  ...
  alt Over budget
    ...
    RT->>LM: generate(...)
    LM-->>RT: LLMManagerResult(code=PolicyDenied, governance_disposition=OverBudget, safe_to_replan=true)
...
关键约束：
1. over-budget 不允许在 llm 内直接做二次语义裁剪；LLMManager 只能把 `OverBudget`/`RequireRecompose` 通过 `governance_disposition + safe_to_replan` 回流 Runtime。
...
13. WP-COG-GAP-012 后，`model_profile.stage_routes` 与 `LLMGenerateRequest.stage` 的 canonical key 集合固定为 `perception`、`planning`、`execution`、`reflection`、`response`；`planner`、`responder`、`reasoning` 只能作为 task_type、Prompt 包目录名或历史 profile-source alias，不得作为投影后的 stage route key。`perception` 现在是独立 canonical stage，用于 intent classification / entity extraction 的 prompt release、route 和 schema 治理，不再折叠进 `planning`。
...
4. manager 不得把 `OverBudget`/`RequireRecompose` 折叠成“只有 `PolicyDenied` 的普通 deny”；shared result code 若暂时不够细，只能通过 module-local result 语义补足。
...
2. 它的执行顺序必须固定为：接收 Runtime handoff（含 required pre-route hint 与可选 explicit prompt release override） -> 调用 PromptPipeline -> 调用 ModelRouter -> 通过 AdapterRegistry 获取 adapter handle -> 执行调用（含计时、取消、错误分类，v1.1 P2-1 已合并进 LLMManager） -> 交给 ResponseNormalizer -> 交给 UsageAggregator（v1.1 GAP-2）-> 记录 observability -> 必要时按 degrade_policy 走 fallback。
...
2. 覆盖它是否严格遵守“只编排、不越权”的边界，例如 route hint 为空时失败而不是自行补 route，PromptPolicy deny 时不继续调用 adapter，OverBudget/RequireRecompose 时保留 governance disposition 与 `safe_to_replan`。
