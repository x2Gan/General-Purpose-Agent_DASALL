...
owner 划分约束如下：SessionTask / StepTask 生命周期与 waiting/cancel/timeout/retry/resume 语义由 Runtime owner；WorkerTask 的拆分、SubTaskGraph、WorkerLease 与结果汇聚由 Multi-Agent owner；services 只上报 execution facts 和 progress，不拥有顶层任务主控；共享对象边界由 contracts 冻结。
...
- [DASALL Agent 架构设计文档](#dasall-agent-架构设计文档)
  ...
  - [5. 关键子系统设计](#5-关键子系统设计)
    ...
    - [5.5 Execution Control 子系统](#55-execution-control-子系统)
      ...
      - [5.5.2 能力范围](#552-能力范围)
      - [5.5.3 风险治理](#553-风险治理)
    - [5.6 Communication 跨域能力](#56-communication-跨域能力)
...
1. Access Gateway、Request Normalizer：归属 Product & Access Layer，对应 `access/` 共享 core；`apps/gateway` 等入口壳层只负责装配与协议绑定。
...
> 对齐说明：在工程归属上，协同子域编排、Worker 管理、Agent Registry、租约与结果汇聚能力继续收敛到 multi_agent 模块；但其顶层主控权仍归 runtime/Agent Control Plane，不形成第二套全局 Orchestrator。
...
> 对齐说明：在工程归属上，本层对应 platform 模块，是 Linux/POSIX/HAL 抽象、能力切片和外设/HAL 受控接入的统一落点；它提供平台事实和句柄语义，但不持有业务级调度、审批、恢复或认知决策权。
...
这三个子域是最容易在实现阶段被“下沉消失”的部分，因此在本版原始架构文档中明确保留为 Runtime、Services、Platform、Multi-Agent 详细设计的上位约束。

需要特别说明的是：当前第 5 章仍保留 Execution Control、Communication、Task 等专题章节，用于聚焦高风险执行、通信归属和任务生命周期等关键问题；但在总体架构口径上，它们已不再等同于与 contracts、profiles、multi_agent、platform 并列的顶层工程模块。
...
6. TaskRequest、TaskState、WorkerLease、SubTaskGraph：统一表达任务提交、生命周期、子任务租约和任务图边界。
...
| Task Manager | 顶层任务生命周期、等待态与取消/超时语义管理 | TaskRequest | TaskState | Event Bus |
...
在工程归属上，Multi-Agent 是独立核心子系统，对应 multi_agent 模块；但它承接的是协同子域编排与 Worker 管理，不拥有第二套顶层主控权。
...
### 5.6 Communication 跨域能力

Communication 在本版原始架构文档中的定位，是一组必须被显式识别的跨域通信关注点，而不是独立于 Access、LLM、Capability Services、Platform 之外再新增一个顶层工程模块。它用于回答“系统通过哪些通道与外部世界发生交互，以及这些通道分别由谁负责”，而不是引入第二套通信主控。

其 owner 划分固定如下：

1. Access Channel：HTTP、WebSocket、MQTT、CLI、串口入口等产品入口通道，由 Access 子系统负责协议适配、认证鉴权、输入归一化和结果发布。
2. Service Channel：进程内接口、IPC、RPC 等服务调用通道，由 Capability Services 子系统持有命令、查询、订阅等服务语义边界，由 Platform 层持有 IPC、网络传输、句柄和 I/O 事实。
3. Field Channel：UART、GPIO、I2C、SPI、CAN、Modbus 等现场接口，由 Platform 层及后续 HAL 子域负责现场 I/O、总线和驱动接入；上层只消费受控抽象，不直接处理底层时序与协议细节。
4. LLM Channel：HTTP、gRPC、本地推理 API 等模型接入通道，由 LLM 子系统负责 Cloud / LAN / Local provider 协议适配、模型路由和发送前治理。
5. Runtime 对上述四类通道不拥有底层协议 owner 身份，只持有调用顺序、deadline、取消、降级和背压治理权。

需要特别说明的是，“串口”在 Access Channel 与 Field Channel 中都可能出现，但语义不同：若串口用于人机交互、上位机接入或调试入口，则其入口协议语义归 Access；若串口承担现场设备链路、UART 时序或总线通信，则其底层事实归 Platform / HAL。
...
5. 若串口被用作用户或上位机入口，其入口协议语义仍归 Access；底层 UART 句柄、时序和驱动细节仍归 Platform / HAL。
6. 工程上由 `access/` 承载共享接入 core，`apps/*` 只保留入口壳层与装配职责。
...
> 对齐说明：本节保留 Task 专题章节，用于聚焦任务生命周期、等待态和共享对象边界；但 owner 划分按跨域能力冻结，不再意味着当前工程必须单列独立的 task/ 顶层目录。

#### 5.11.1 职责

1. 管理 SessionTask / StepTask 的生命周期推进：创建、排队、运行、等待、恢复、终止。
2. 管理顶层任务的取消、超时、重试和等待态语义，并把状态变更以事件形式投递给 Runtime/Event Bus。
3. WorkerLease、SubTaskGraph 和结果汇聚等协同子域能力由 Multi-Agent owner；Task Manager 不重复持有第二套协同主控。
4. 执行事实和进度回报由 services 输出；Task Manager 消费这些事实并推进顶层任务状态，而不直接拥有执行后端。

Task owner 矩阵按以下口径冻结：

| Task 控制点 | Runtime / Task Manager | Multi-Agent | Services | Contracts | 唯一 Owner | 说明 |
|---|---|---|---|---|---|---|
...
> 说明：按当前 owner 划分，Task Manager 应先实现在 runtime 模块内；只有当 Task 在工程上形成独立目录、稳定接口和详细设计闭环时，才适合再独立为 task 目录。
...
1. Reactor：用于网络 I/O 和事件多路复用（Access 入口，以及 Service / Field 通道的底层 I/O）。
...
10. multi_agent：协同子域编排、Worker 管理、Agent Registry 与结果汇聚；不拥有第二套顶层主控权。
11. platform：Linux/POSIX/HAL 抽象、能力切片与外设/HAL 受控接入。
