...
1. `Type=simple`
2. `Environment=DASALL_DAEMON_PROFILE_ID=desktop_full`
3. `EnvironmentFile=-/etc/default/dasall-daemon`
4. `ExecStartPre=/usr/sbin/dasall-daemon --validate-only --profile-id=${DASALL_DAEMON_PROFILE_ID} --config-file /etc/dasall/daemon.json`
5. `ExecStart=/usr/sbin/dasall-daemon --profile-id=${DASALL_DAEMON_PROFILE_ID} --config-file /etc/dasall/daemon.json`
6. `User=dasall`
7. `Group=dasall`
8. `RuntimeDirectory=dasall`
9. `StateDirectory=dasall`
10. `UMask=007`
...
3. `Environment=` 提供 `desktop_full` 保底值，`EnvironmentFile=` 提供运维可编辑的 profile 选择入口；v1 中该文件只应承载 `DASALL_DAEMON_PROFILE_ID`。
4. `ExecStartPre` 与 `ExecStart` 必须共享同一组 `--profile-id` + `--config-file` 输入，确保 validate-only 与正式启动看到完全一致的 package-mode 组合。
5. `/etc/dasall/daemon.json` 仍然只承载 `daemon.*` 覆盖，不承载 `profile_id`；当前 parser/loader 也只支持这一边界。
6. `Type=simple` 是 v1 稳妥基线；待 daemon 真正具备对外 supervisor notify 语义后，再评估切到 `Type=notify`。
7. 不应通过 `WorkingDirectory=/usr/share/dasall` 等方式补洞；一旦需要依赖 WorkingDirectory 才能启动，说明安装态路径收敛没有完成。
...
1. profile 选择入口：`/etc/default/dasall-daemon` 中的 `DASALL_DAEMON_PROFILE_ID`
2. daemon 覆盖配置文件位置：`/etc/dasall/daemon.json`，且它只承载 `daemon.*`
3. 配置校验命令：`. /etc/default/dasall-daemon && /usr/sbin/dasall-daemon --validate-only --profile-id="${DASALL_DAEMON_PROFILE_ID}" --config-file /etc/dasall/daemon.json`
...
4. `EnvironmentFile` 在 v1 只拥有 `DASALL_DAEMON_PROFILE_ID` 这一 profile 入口，不额外承载 `daemon.*` 覆盖键。
...
原因：当前代码只支持通过 `--profile-id` 选择 profile，尚不支持在 `/etc/dasall/daemon.json` 中声明 profile；因此 `/etc/default/dasall-daemon` 的 owner 只应是 profile 选择本身。
...
3. `/etc/default/dasall-daemon` 在这份 unit 中只负责注入 `DASALL_DAEMON_PROFILE_ID`；`daemon.*` 覆盖仍归 `/etc/dasall/daemon.json`。
4. 这份 unit 仍然依赖 7.10.8 中列出的路径收敛项已经完成，否则 `ExecStartPre` 会先失败。
