...
7. **测试与验收出口**：推荐单测 `MemoryMaintenanceRetentionTest.cpp`（Turn/Fact/Experience 过期清理 + quarantine 清理）、`MemoryMaintenanceCheckpointTest.cpp`（checkpoint 执行 + starvation 检测）；集成测试 `MemoryMaintenanceIntegrationTest.cpp`（完整周期 + vector rebuild）；build-tree 验收命令 `ctest --test-dir build-ci -R "MemoryMaintenance.*Test" --output-on-failure`。installed authoritative 口径固定为本机 package smoke 调用 `/usr/lib/dasall/dasall-memory-maintenance-proof --json`，并将结果落盘为 `memory-maintenance-proof.json`；该证据用于安装态 checkpoint / retention / quarantine cleanup 正向路径，不外推 qemu / soak。自 2026-06-02 起，memory owner 另提供 `scripts/packaging/validate_memory_installed_or_qemu.sh` 作为 installed gate wrapper：该脚本复用 `pkg_smoke_install.sh --explicit-start-check` 生成的 `run-first.json`、`run-second.json`、`memory-proof.json` 与 `memory-maintenance-proof.json`，并汇总为 `~/.cache/dasall/memory/installed-evidence/latest.json`，其中 `checks.init`、`checks.open_store`、`checks.prepare_context`、`checks.write_back`、`checks.maintenance` 作为 memory owner 的统一二值证据；可选 `-- <virt cmd>` 仅用于 qemu chaining，不改变本机 authoritative 口径。
