...
11. 代码现状：infra/CMakeLists.txt、infra/include/health/、infra/src/{InfraServiceFacade.cpp,InfraErrorCode.cpp,audit/,plugin/,tracing/}、infra/src/health/（空目录）
...
|---|---|---|---|---|
| health 模块可编译并承载能力 | 部分实现 | health 公共接口已落盘，但 health 实现尚未接入构建 | High | P0 |
| health 目录与实现文件 | 缺失 | infra/src/health/ 为空，缺少核心实现 | High | P0 |
| health 对外接口头文件 | 部分实现 | infra/include/health 已落盘 IHealthMonitor/IHealthProbe/IHealthPolicy 等接口头 | High | P0 |
| 健康状态对象与事件语义 | 部分实现 | HealthSnapshot/HealthTransition/ProbeResult/RecoveryHint 已冻结，运行时实现与映射链仍待补齐 | High | P0 |
...
2. health 当前已具备公共接口与对象冻结结果，缺口集中在 health 实现目录。
...
|---|---|---|---|---|---|---|
| 引入健康统一入口与生命周期 | 新增 HealthMonitorFacade + IHealthMonitor | 将健康能力收敛为单入口，避免散落调用 | infra/include/health/IHealthMonitor.h; infra/src/health/HealthMonitorFacade.cpp | unit: HealthMonitorFacadeTest | cmake --build build-ci --target dasall_infra && ctest --test-dir build-ci -R HealthMonitorFacadeTest | 依赖 contracts ResultCode/ErrorInfo |
...
2. infra/include/health/IHealthMonitor.h
...
| HLT-T002 | Not Started | 新增 IHealthMonitor 与 HealthMonitorFacade 骨架 | 基础设施详细设计 6.2/6.3 | infra/include/health/IHealthMonitor.h; infra/src/health/HealthMonitorFacade.cpp | HealthMonitorFacadeTest | ctest --test-dir build-ci -R HealthMonitorFacadeTest | 支持注册与查询快照 |
