...
1. **职责**：基于 lexical index 执行关键词/BM25/FTS 类召回，并应用 corpus filter、metadata filter、language filter 和 sentence-window 邻域补齐策略。
...
1. **职责**：持有 active snapshot 的只读引用，提供 lexical search 和 manifest 查询；是查询路径的唯一索引入口。
...
5. **关键执行流**：
   ...
   2. 在快照上执行 lexical search 与 metadata filter。
...
1. **职责**：管理 shadow build、apply batch、snapshot-and-swap 和 last-known-good 回退；是索引写路径的唯一 owner。
...
5. **关键执行流**：
   ...
   2. 在 shadow 空间构建或更新 lexical/vector 索引。
...
|---|---|---|---|
| lexical index snapshot | 内存或 SQLite FTS5（待定） | `IndexManifest.format_version` | 格式不兼容时 rebuild；不支持 down migration |
...
| KNO-B03 | Blocker | 词法索引实现技术尚未确定（SQLite FTS5 / 自研倒排 / 三方库） | lexical-only P0 无法编码 | 在 Phase K1 前选定单一实现并补最小 PoC | 先用内存索引夹具做 smoke |
...
| KNO-R09 | Risk | edge 设备 BM25 10k+ chunks 延迟超预算 | `edge_balanced` 下 lexical recall 可能超出 `request_deadline_ms` | Phase K1 补 10k chunk latency benchmark | 减小 top-k 或限制 corpus 规模；极端情况 `edge_minimal` 保持 `knowledge=false` |
...
2. lexical index 技术栈是优先选 SQLite FTS5、轻量倒排，还是引入第三方检索库，需要结合交叉编译和体积约束决策。
...
1. 新增 `KNO-TODO-001`：收敛 knowledge/include 最小 public surface，建立 lexical-only 最小闭环。
2. 新增 `KNO-TODO-002`：冻结 vector bridge 窄接口（含 `IQueryEncoder`），补 hybrid recall 设计与 profile compatibility 证据。
3. 新增 `KNO-TODO-003`：补齐 ingest/index/version ledger 设计与最小测试语料。
4. 新增 `KNO-TODO-004`：建立 `tests/unit/knowledge` 与 `tests/integration/knowledge` 接线并满足 `ctest -N` discoverability。
5. 新增 `KNO-TODO-005`：建立 knowledge 专项 deliverables、Gate 证据和 worklog 回写路径。
6. 新增 `KNO-TODO-006`：建立 retrieval quality golden set（≥30 条），补齐 MRR/NDCG@k/Recall@k 回归基线。
7. 新增 `KNO-TODO-007`：在 `edge_balanced` 上执行 10k chunk BM25 latency benchmark，确认 `request_deadline_ms` 预算可行。
