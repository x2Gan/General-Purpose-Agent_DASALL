...
| LOG-C002 | 架构文档 8.5/9.4 | Must | 日志必须携带 request_id、session_id、trace_id，并支持本地落盘和按 trace/session/request 诊断拉取 | 核心对象/输出 |
...
4. First-party application events：对 cognition 这类一方业务日志，生产级 checkpoint 应保持稳定 `EventName + pipeline/step/outcome + correlation ids + resolved_route/failure_category/error_type` 结构；失败/降级与成功事件共用同一字段骨架，敏感 payload 继续通过 allowlist 与 redaction 约束，不因调试诉求放宽落盘边界。
...
| LogQueryService | 按 trace_id/session_id/request_id 生成受控 diagnostics artifact |
...
| LogQueryService | LogQueryRequest + LogQueryAccessContext | diagnostics/local artifact consumer | 只接受 trace/session/request 精确 selector，并输出本地 artifact_ref |
...
| LogQueryRequest | query_id, selector_kind, selector_value, start_ts_ms, end_ts_ms, max_records | selector_kind 仅允许 TraceId/SessionId/RequestId，必须带有序时间窗，禁止自由查询语法 | 只消费 LogContext 已冻结标识语义，不新增 contracts 对象 |
...
| infra.logging.export.enable_diag_pull | true | 默认/Profile/部署 | 是否允许按 trace/session/request 导出 |
...
`LogQueryService` 的职责限定为“在本地已脱敏日志中，按 `trace_id`、`session_id` 或 `request_id` 生成受控 diagnostics artifact”。它不是通用检索引擎，不提供远程上传、全文搜索、任意 attr 扫描或跨模块二次授权。

| 项目 | 冻结结论 |
|---|---|
| 对外接口 | `query(const LogQueryRequest&, const LogQueryAccessContext&) -> LogQueryResult` |
| 查询对象 | `LogQueryRequest` 必填 `query_id`、`selector_kind`、`selector_value`、`start_ts_ms`、`end_ts_ms`、`max_records`；`selector_kind` 首版仅允许 `TraceId`、`SessionId` 或 `RequestId`，且必须三选一 |
...
冻结结果：LOG-BLK-005 的真实缺口是 query schema、allow 证明和导出限制未写成正式设计，而不是“按 trace/session/request 诊断拉取”能力本身不成立；后续实现可直接按 `LogQueryRequest` / `LogQueryAccessContext` / `LogQueryResult` 边界推进。
...
2. persisted reader 只允许读取本地 redacted runtime log 与同目录 rotation family；首版 selector 固定为精确 `trace_id` / `session_id` / `request_id` + 有序时间窗，不新增全文检索、任意 attr 扫描、cursor DSL、remote upload 或跨模块二次授权。
...
2. memory 当前已由 `MemoryObservability::make_log_attrs()` 建立 ordinary log allowlist，只保留 `request_id/session_id/trace_id/stage/profile_id` 与 bounded enum/count/bool/diagnostic 摘要字段，禁止 raw context body、retrieval payload、summary text 与 embedding payload 进入 ordinary log。
...
1. `MemoryObservability::make_log_attrs()` 现改为 allowlist 过滤，只允许 `warning_count`、`warning`、`warning_codes`、`degraded`、`result_code`、`failure_reason`、`fact_count`、`experience_count`、`conflict_count`、`partial`、`retryable`、`turn_id`、`summary_id`、冲突摘要字段、lifecycle 摘要字段以及 maintenance 计数/布尔字段进入 ordinary log。
2. raw `summary_text`、`goal_summary`、`latest_observation_digest_summary`、`agent_response`、`fact_text`、retrieval evidence 正文与其他未知 field 现不再进入 memory ordinary log attrs；memory owner 继续只通过 request/session/trace/stage correlation 与 owner-safe count/bool/enum/diagnostic 摘要暴露状态。
3. `MemoryManager` 现对 `init.completed`、`init.failed`、`init.skipped`、`shutdown.completed`、`shutdown.skipped` 发射 lifecycle telemetry，并以 `storage_backend`、`vector_enabled`、`auto_schedule`、`lifecycle_state`、`result_code`、`failure_reason` 说明初始化/关闭阶段状态；这些字段只表达配置与生命周期摘要，不泄漏 store path、query payload 或 memory body。
4. `WritebackCoordinator` 现把 turn validation fail-fast 路径纳入 `writeback.failed` telemetry，输出 `warning_codes=writeback_turn_invalid` 与 contracts guard 返回的安全 `failure_reason`；该路径补齐了此前“不进入存储主链就缺少日志”的静默失败缺口。
5. focused build-tree 证据现固定为 `MemoryProductionLoggingIntegrationTest` 与更新后的 `MemoryObservabilityBridgeTest`：前者在 temp `state_root/logging/runtime.log` 上验证 lifecycle、`writeback.completed`、`context.assembled`、`maintenance.completed` 落盘，并用 `FileLogReader + LogQueryService` 按 `session_id`、`trace_id`、`request_id` 产出 redacted query artifact；后者继续守住 `LoggingFacade` in-memory dispatch、lifecycle/audit action 与 payload allowlist。
6. 当前 memory logging 结论已到 L2/L3 build-tree persisted/query owner evidence，但不外推成 installed/package authoritative proof；package artifact 与跨子系统 e2e 继续留给 `INF-LOG-SYS-FIX-007` / `INF-LOG-FIX-011`。本轮不使用 qemu / kvm。
