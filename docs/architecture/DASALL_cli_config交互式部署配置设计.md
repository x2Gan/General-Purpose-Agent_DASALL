...
后续 Build 任务必须同时覆盖两层命名：源码 target / 测试可继续使用 `dasall-cli`，包交付文档、manpage、postinst next steps 和 operator workflow 应以安装态 `dasall` 为主。本文在未特别说明时使用 `dasall-cli config` 代表该命令族的语义面。
...
|---|---|---|---|
| 安装态 socket canonical path | `/run/dasall/daemon.sock`、`/run/dasall/control.sock` 或其它唯一值 | CLI 默认、daemon 示例、DPKG 配置与 wizard 默认值继续漂移 | package/defaults 任务；daemon/CLI 默认 endpoint 复核 |
| socket 权限模型 | 选择 `0600 root/sudo-only` 或 `0660 dasall group`，不能同时承诺 | `OperatorAccessPage` 加组后仍无法访问 daemon socket | platform/UnixIpcProvider、DaemonSocketPolicy、systemd unit、integration tests |
| 命令安装名 | 明确开发态 `dasall-cli` 与安装态 `/usr/bin/dasall` 映射 | 文档、manpage、postinst 和测试命令不一致 | CLI parser/help、packaging install rule、docs |
...
### 3.4 当前最高风险：socket 权限口径冲突

当前工程存在一个需要优先收敛的冲突：daemon/UDS 安全 hardening 已把真实 filesystem socket 收敛为 `0600`，而 Ubuntu 打包与本设计又希望通过 `dasall` 组支持非 root CLI 操作者访问。

这两种模型都可以成立，但不能同时对外承诺：

1. 如果选择 `0600`，则 P0 的 OperatorAccessPage 只能提示 root / sudo 路径，不能说“加入 `dasall` 组即可使用 CLI”。
2. 如果选择 `0660`，则必须把 `UnixIpcProvider`、`DaemonSocketPolicy`、stale cleanup、socket mode integration test、systemd `Group=dasall` / `UMask` 全部改成 package-mode 一致模型。
3. 在该口径未冻结前，OperatorAccessPage 必须 capability-gated；最多展示“operator access model pending”，不能执行 usermod 或承诺权限立即可用。
...
1. 若最终选择 group access 模型，`config` 应明确询问是否把操作者加入 `dasall` 组；若选择 root/sudo-only 模型，则不展示加组主路径。
2. `config` 应明确询问是否 `enable --now`，而不是默认自动启服务。
3. 修改组成员后要提示重新登录会话，而不是假设即时生效。
...
1. `SecretBootstrapWriter` 应是 bootstrap-only 接缝，不并入 `ISecretManager` consumer-facing ABI。
...
1. ToolSkillPage 在 owner surface 未冻结前必须隐藏或只读。
2. 后续若开放可编辑项，必须显示 bundle/source、允许的 tool allowlist、profile constraints、撤销方式和 audit 结果。
3. `config` 不得绕过 tools 自己的 policy gate、SkillRegistry、PluginExtensionBridge 或未来 deployment overlay owner。
...
本页必须 capability-gated。

当前建议：

1. 如果 tools/skills 只具备 internal runtime，但没有稳定的 operator-facing deployment config surface，则本页默认隐藏。
2. 如果后续冻结了 skill bundle / builtin tool / plugin extension 的 enable list，本页再升级为可编辑页面。
3. 在 capability 未就绪前，最多做只读摘要，绝不伪造“默认安装的 skills/tools 可任意开关”的交互。

#### 6.2.6 OperatorAccessPage

本页在 socket 权限模型支持 group access 时，负责两个系统层面动作：

1. 是否把某个本地操作者加入 `dasall` 组。
2. 是否提示“需重新登录 shell 会话后权限才生效”。

安全要求：

1. 必须明确提示 `dasall` 组具有较高本地控制权限。
2. 不通过放宽 socket 为 `0666` 解决权限问题。
3. 在 socket 权限模型未冻结为 group-access-compatible 前，本页只能只读提示，不能执行加组动作。
4. 若最终选择 `0660 dasall group` 模型，本页必须展示当前 socket mode、当前用户是否属于 `dasall` 组、是否需要重新登录会话。
5. 若最终选择 `0600 root/sudo-only` 模型，本页必须删除“加组后可访问”的主路径，只保留 sudo/root 运维提示。
...
`DaemonConfigFileStore` 必须以事务化方式写 canonical 文件，不能直接覆盖目标文件。P0 写入规则建议冻结为：

1. 读取当前文件时保留原始权限、owner/group、换行风格和未知字段。
2. 写入前生成 desired model，并先执行本地 schema 校验。
3. 写入到同目录临时文件，设置目标权限，写完后 `fsync` 文件与父目录。
4. 用 atomic rename 替换目标文件。
5. 替换前保留有限数量的 `.bak` 或 `.last-known-good`，但备份不得包含 secret 明文。
6. 写入后立即执行 `dasall-daemon --validate-only --profile-id <id> --config-file /etc/dasall/daemon.json`。
7. 若 validate 失败，必须回滚到上一个可用版本，并在 summary 中标记 `apply_failed_rolled_back`。
...
| `SecretBootstrapWriter` | `infra/src/secret/` 或 `apps/cli/src/config/bridge/` | 把初始 secret 以 secure buffer 方式导入 file backend，不暴露为普通 CLI flag |
...
4. 初始导入成功后，后续消费仍通过 `ISecretManager` 标准查询面完成。
...
| operator group membership 变化 | 写系统组并提示重新登录 |
...
1. `file_writes`
2. `secret_writes`
3. `service_reload_required`
4. `service_restart_required`
5. `service_start_requested`
6. `service_enable_requested`
7. `manual_followups`
...
| blocker | 影响 | 设计应对 |
|---|---|---|
| socket path / socket permission 模型未统一 | OperatorAccessPage、service unit、CLI 默认 endpoint 和 package 文档无法同时成立 | P0 前先冻结 canonical path 与 `0600`/`0660` 模型；未冻结前只读提示，不执行加组动作 |
| 开发态命令名与安装态命令名存在双层语义 | `dasall-cli config` 与 `/usr/bin/dasall config` 文档、测试、manpage 容易分叉 | 明确源码 target 与安装命令映射，测试覆盖两层入口 |
| `ISecretManager` 无初始写入入口 | LLM key 首装无法真正落盘 | 新增 bootstrap-only secret import seam，不改坏既有读接口 |
...
| tools/skills 部署激活面未冻结 | 无法稳定提供 “默认安装 skills/tools” 的 operator surface | ToolSkillPage capability-gated，首版默认隐藏或只读 |
...
1. `ToolSkillPage` 从只读升级为可编辑。
2. 允许选择 default bundles / allowlist / importer source。
3. 继续遵守 capability-gated，不为未安装能力展示伪选项。
...
1. 包安装后无需任何额外文档跳转，用户只需运行一次 `dasall-cli config` 即可完成当前稳定范围内的首装引导。
...
2. 冻结 socket permission model：`0600 root/sudo-only` 或 `0660 dasall group` 二选一。
3. 冻结命令安装名映射：开发态 `dasall-cli`，安装态 `/usr/bin/dasall`。
