...
| TRC-C005 | ADR-005 | Must | contracts 与关键边界冻结前，不得用 tracer 设计反向改写架构结论 | 设计治理 |
| TRC-C006 | ADR-006 | Must-Not | tracer 不接管上下文语义装配和 Prompt 渲染，仅消费上下文标识并做追踪 | 职责边界 |
| TRC-C007 | ADR-007 | Must-Not | tracer 不做失败语义判断与恢复准入，只记录恢复相关 span 证据 | 异常语义 |
| TRC-C008 | ADR-008 | Must | tracer 不拥有主控调度权，只记录 orchestrator/coordinator/worker 链路 | 多 Agent 边界 |
...
| TRC-C012 | 编码规范 3.7 | Should | 新增公共 tracer 接口需同步 unit/integration/contract 测试 | 测试策略 |
...
2. infra/include/ 为空，尚无 ITracer/TracerProvider 等对外接口。
3. infra/CMakeLists.txt 当前仅编译 src/placeholder.cpp。
4. infra 子系统文档存在整体设计和 logging 模块详细设计，tracer 子模块详细设计缺失。

### 3.2 现状-目标差距表

| 设计目标 | 当前状态 | 差距描述 | 风险等级 | 修复优先级 |
|---|---|---|---|---|
| 提供统一 ITracerProvider/ITracer 接口 | 缺失 | 调用方无统一入口，易各自埋点 | High | P0 |
| 支持 Span 生命周期（start/end/event/status） | 缺失 | 无追踪主链路，无法构建 trace tree | High | P0 |
| 上下文传播（in-process + 跨模块） | 缺失 | trace_id/span_id 无法稳定传递 | High | P0 |
...
| 测试基线（unit/integration/failure） | 缺失 | 无法验证行为稳定性 | High | P0 |

### 3.3 风险冲突识别

| 冲突类型 | 描述 | 影响 | 风险等级 |
|---|---|---|---|
| 边界冲突 | tracer 侵入 runtime 恢复裁定逻辑 | 违反 ADR-007，形成双控制点 | High |
| 语义冲突 | tracer 自定义跨模块共享对象字段 | 与 contracts 冻结对象冲突 | High |
...
| OTel Trace SDK | 强调 ForceFlush/Shutdown、批处理队列、并发安全、采样策略 | 在 tracer 模块中必须显式定义 flush/shutdown 与批处理参数 |
...
| 子组件 | 职责 |
|---|---|
| TracingFacade | tracer 对外统一入口，提供 create/start/stop/flush |
| TracerProviderImpl | 持有 tracer 配置、采样器、processor、exporter 管线 |
...
| TracerProviderImpl | TraceConfig/Profile 配置 | TracerImpl/处理管线 | 配置更新对新旧 tracer 生效策略显式定义 |
...
约束：
1. 不允许 tracer 直接依赖 runtime/cognition 实现类。
...
|---|---|---|---|---|---|---|
| 建立 tracer 对外接口层 | 新增 ITracerProvider/ITracer/ISpan/ITraceContextPropagator | 先冻结调用面，避免上层直连实现 | infra/include/tracing/*.h | unit: TracingInterfaceCompileTest | cmake --build build-ci --target dasall_infra | 依赖 contracts 既有标识语义 |
| 建立 provider + tracer + span 最小闭环 | 新增 TracerProviderImpl/TracerImpl/SpanImpl | 打通 start/end/context 的主链路 | infra/src/tracing/TracerProviderImpl.cpp 等 | unit: SpanLifecycleTest | ctest --test-dir build-ci -R SpanLifecycleTest --output-on-failure | 无 |
...
|---|---|---|---|---|---|---|---|
| TRC-M1 接口冻结 | Not Started | 新增并冻结 tracer 对外接口 | 架构 5.10 + 蓝图 6 | infra/include/tracing/*.h | TracingInterfaceCompileTest | cmake --build build-ci --target dasall_infra | 接口可编译且无上层实现依赖 |
