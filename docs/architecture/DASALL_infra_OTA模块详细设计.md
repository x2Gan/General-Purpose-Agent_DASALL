...
4. docs/todos/infrastructure/DASALL_infrastructure子系统专项TODO.md 中 OTA 阻塞与任务记录
...
| OTA-C016 | docs/todos/infrastructure/DASALL_infrastructure子系统专项TODO.md INF-TODO-015 / INF-BLK-05 | Must | 必须先冻结 UpgradePlan、Package、rollback token、签名算法与存储规范，再解除接口阻塞 | 设计补齐 |
...
|---|---|---|---|---|
| infra/ota 目录与源码落盘 | 部分实现 | ota public headers 与编译测试已落盘，infra/src/ota 仍无实现文件 | High | P0 |
| IOTAManager 接口头文件 | 部分实现 | infra/include/ota/IOTAManager.h 已落盘，manager 运行时实现仍待补齐 | High | P0 |
| OTA 私有对象模型 | 部分实现 | OTATypes 已冻结 UpgradePlan、PackageDescriptor、RollbackToken、InstallEvidence 等对象，执行链仍待实现 | High | P0 |
...
| OTA 测试基线 | 部分实现 | unit/contract 基线已接入，integration/failure injection 仍待补齐 | Medium | P0 |
| 构建入口 | 部分实现 | infra/CMakeLists.txt 已接入真实源码；ota 具体实现尚未接入构建 | High | P0 |

证据：

1. infra/CMakeLists.txt 已接入 core/audit/plugin/tracing 等真实源码。
2. infra/include/ota 已落盘 IOTAManager、OTATypes 及相关 OTA 公共头文件。
3. infra/src/ota 当前为空目录。
4. docs/todos/infrastructure/DASALL_infrastructure_ota组件专项TODO.md 已完成 IOTAManager 与 OTATypes 冻结交付；当前剩余阻塞集中在签名/存储/回滚执行闭环实现。
...
| 建立 IOTAManager 对外稳定入口 | 新增 IOTAManager.h | 先固定 precheck/apply/rollback/query_status 语义 | infra/include/ota/IOTAManager.h | unit: OTAInterfaceCompileTest | cmake --build build-ci --target dasall_infra && ctest --test-dir build-ci -R OTAInterfaceCompileTest | 阻塞：package/signature/token 冻结 |
...
1. infra/include/ota/IOTAManager.h
...
|---|---|---|---|---|---|---|---|
| OTA-M1 对象与接口冻结 | Not Started | 新增并冻结 OTATypes 与 IOTAManager 最小骨架 | 本文 6.5、6.6、专项 TODO INF-TODO-015 | infra/include/ota/IOTAManager.h; infra/include/ota/* | unit: OTATypesCompileTest; OTAInterfaceCompileTest | cmake --build build-ci --target dasall_infra | 对象字段、接口签名、错误码映射入口稳定且可编译 |
...
2. 在 docs/todos/infrastructure/DASALL_infrastructure子系统专项TODO.md 中为 OTA 拆出对象冻结、验签骨架、slot 切换、回滚测试四类原子任务。
