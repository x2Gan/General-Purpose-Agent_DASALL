...
11. 代码现状：infra/CMakeLists.txt、infra/include/watchdog/、infra/src/{InfraServiceFacade.cpp,InfraErrorCode.cpp,audit/,plugin/,tracing/}、infra/src/health（空）
...
|---|---|---|---|---|
| watchdog 模块可编译并承载能力 | 部分实现 | watchdog 公共接口已落盘，但 watchdog 服务实现仍未接入构建 | High | P0 |
| watchdog 接口头文件体系 | 部分实现 | infra/include/watchdog 已落盘 IWatchdogService/IHeartbeatSource/ITimeoutPolicy 与对象头，服务实现仍缺失 | High | P0 |
...
| 测试基线（unit/integration/failure） | 部分实现 | unit 基线已接入，integration/failure 仍待补齐 | Medium | P0 |

证据：
1. infra/CMakeLists.txt 已纳入 core/audit/plugin/tracing 等真实源码。
2. watchdog 当前已具备公共接口与对象冻结结果，缺口集中在 watchdog 实现目录。
3. infra/include/watchdog 已落盘 IWatchdogService、IHeartbeatSource、ITimeoutPolicy 与对象头文件。
...
建议文件：
1. infra/include/watchdog/IWatchdogService.h
2. infra/include/watchdog/IHeartbeatSource.h
3. infra/include/watchdog/ITimeoutPolicy.h
...
|---|---|---|---|---|---|---|---|
| WDG-T001 | Not Started | 新增 IWatchdogService 接口并接入 infra 构建 | Blueprint 4.3 + 工程规范 | infra/include/watchdog/IWatchdogService.h; infra/CMakeLists.txt | WatchdogInterfaceCompileTest | cmake --build build-ci --target dasall_infra | 构建通过且可链接 |
