...
| ToolPluginExtensionConsumer | 消费 `PluginExtensionBridge::rebuild_extension_catalog()` 的 source delta，把 builtin/MCP/skill 扩展自动分发到 ToolRegistry、CapabilityDiscovery、SkillRegistry | tools/src/bridge/ToolPluginExtensionConsumer.cpp |
...
| PluginExtensionBridge | ActivePluginSet、LoadResult.handle_ref、plugin export table | PluginBuiltinProviderView、MCPServerLaunchSpec、SkillAssetRef source-scoped snapshot | IPluginManager、ToolPluginLifecycleBridge |
| ToolPluginExtensionConsumer | PluginExtensionDelta、plugin unload source | ToolRegistry descriptor delta、CapabilityDiscovery server delta、SkillRegistry asset delta | ToolRegistry、CapabilityDiscovery、PluginSkillBundleImporter、SkillRegistry |
...
4. 公共/内部接口：内部建议具备 `ToolPluginLifecycleBridge::synchronize_active_plugins()`、`ToolPluginLifecycleBridge::on_plugin_loaded()`、`ToolPluginLifecycleBridge::on_plugin_unloaded()` 作为 lifecycle 入口，再由 `PluginExtensionBridge::rebuild_extension_catalog()`、`PluginExtensionBridge::on_plugin_loaded()`、`PluginExtensionBridge::on_plugin_unloaded()`、`emit_builtin_delta()`、`emit_mcp_delta()`、`emit_skill_delta()` 维护 source-scoped snapshot；snapshot 分发继续由新增 `ToolPluginExtensionConsumer::on_plugin_loaded()`、`ToolPluginExtensionConsumer::on_plugin_unloaded()` 收口，对 builtin 调 `ToolRegistry::apply_plugin_extension_delta()`，对 stdio MCP 调 `CapabilityDiscovery::on_plugin_delta()`，对 skill bundle 调 importer + `SkillRegistry::revoke_source()/register_asset()`。
5. 关键执行流：读取 active plugin set，逐个解析 tools-specific export table；lifecycle adapter 对 `Loaded` / `Active` plugin 自动同时送入 `PluginExtensionBridge` 与 `ToolPluginExtensionConsumer`，对 disabled / safe-mode / unload source 做 revoke；consumer 复用与 snapshot 同源的 `PluginExtensionDelta`，把 builtin 导出发送 registry delta，把 stdio MCP 导出发送 discovery delta，把 skill bundle 导出发送 importer / registry delta。
6. 失败与回退语义：单个 plugin 的导出不合法时只隔离该 plugin，不拖垮全局 bridge；consumer 任一子域应用失败时必须撤回整个 source，避免出现 “bridge snapshot 已见、registry/discovery/skill 半可见” 的悬挂状态；`CapabilityDiscovery` 清理 plugin source 时只能撤销 source-owned MCP bindings，不能误删同 source 的 plugin descriptors。
7. 测试与验收出口：推荐单测为 `PluginExtensionBridgeTest.cpp`、`PluginExtensionBridgeUnloadTest.cpp`、`PluginExtensionBridgeConcurrencyTest.cpp`、`ToolRegistryTest.cpp`；lifecycle focused integration 保留 `ToolPluginLifecycleBridgeIntegrationTest.cpp`；snapshot 分发层的集成验收继续以 `ToolPluginStdioMCPIntegrationTest`、`ToolPluginSkillBundleIntegrationTest` 与新增 `ToolPluginExtensionEndToEndIntegrationTest.cpp` 为主出口。
