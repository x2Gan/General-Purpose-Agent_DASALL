...
| 范畴 | 当前事实 | 对 TUI 的含义 |
|---|---|---|
| `apps/tui` | 当前不存在独立目录和 target | 必须先做 prototype/fake-only substrate |
...
| 本地 daemon/access | daemon、UDS/IIPC、AccessGateway、readiness、run/status/cancel/diag/knowledge 等本地控制面已可复用 | 正式 TUI 可复用 daemon 主链，但必须新增 TUI projection seam |
...
4. 退出后不跨启动恢复旧会话。
5. `/clear` 建议作为“清空当前前台 transcript + 新建当前进程内 session”的候选行为，最终需在 Phase 0 冻结。
...
| `/clear` | 清空当前前台视图或新建当前进程内 session，待冻结 | TUI + session seam |
...
产品待决点：最终用户版 TUI 是否必须支持非 root 普通用户、是否需要 user-level daemon/socket、是否允许 TUI 只作为 root/sudo operator UI。该问题会影响命令迁移、Debian 文档和安全模型，必须在正式迁移前解清。
...
实现时优先定义 TUI 私有的 `NextTurnPreference` 和 `TuiModelRouteProjection`，不要直接让 UI 持有 ModelCatalogEntry 或 RuntimePolicySnapshot 的内部对象。
...
当前 `ModelSelectionHint` 更接近 runtime/llm 对任务复杂度、延迟、预算和 reasoning 需要的提示，不覆盖 TUI 需要表达的 provider/model pin 与显式 depth preference。当前 runtime 构造 LLM request 时也没有把 selection hint 打通。

正式链路需要分三步补齐：

1. TUI 内部先只维护 `NextTurnPreference` 草稿，并在 fake route catalog 中验证 UI 行为。
2. Access/Runtime 冻结可归一化的“用户下一轮模型偏好”承载位置，明确它是否进入 request context、client capabilities、profile override 或新增受控 projection。
3. LLM route 层扩展受控 hint 或 policy input，由 `ModelRouter` 完成最终裁定，并通过 `TuiModelRouteProjection` 回显实际生效 route。

在第 2 步完成前，TUI 不得承诺真实 provider/model pin 生效。
...
TUI 只能消费面向 UI 的 projection，不消费 runtime/access/llm 内部对象 dump。建议投影边界如下：
...
2. 通过 `cmake/DASALLThirdParty.cmake` 统一管理依赖来源，优先遵循仓库既有 submodule/local cache/FetchContent fallback 策略，并固定版本或 commit。
3. TUI target 只以 private dependency 链接 `ftxui::ftxui` 或更细粒度组件，禁止上游模块链接或 include FTXUI。
...
6. 若 FTXUI 打包路径与 Debian policy 冲突，正式命令迁移必须延后，不得用未验证的 vendored 依赖进入安装态。
...
3. 核心数据：消费 daemon endpoint、request envelope、timeout policy；输出 TUI projection DTO。
4. 接口：`submit_turn()`, `query_status()`, `query_route_catalog()`, `close_session()`。
5. 执行流：TUI DTO -> platform IIPC -> daemon/access -> projection DTO -> data source。
6. 失败语义：socket 不存在、permission denied、timeout、malformed response 都必须映射成稳定错误码。
...
5. 执行流：route catalog -> 可选项过滤展示 -> 用户选择 -> NextTurnPreference -> submit turn。
6. 失败语义：不可用 provider/model 显示 disabled reason；Pin 不满足 allowlist/health 时 fail-closed。
...
| TUI DTO | `TuiSessionView`、`TuiStatusProjection`、`TuiTurnReceipt` | `apps/tui` + access projection seam | 待评估 | 可跨 fake/daemon source 复用 |
| Access/Daemon Projection | daemon readiness、status receipt、route catalog view | access/daemon | 可能 | 需先证明 CLI/gateway 也需要 |
...
| Phase 6 | LLM selector 真实数据链 | `NextTurnPreference` 承载位置、route catalog projection、ModelRouter 裁定回显 | Phase 4/5 |
...
| TUI-DES-004 | FTXUI 只作为 renderer adapter private dependency | `FtxuiRendererAdapter.*`、`TuiDesignTokens.h` | `TuiRenderSnapshotTest` | `ctest --preset vscode-linux-ninja -R "TuiRenderSnapshot" --output-on-failure` |
...
| TUI-RISK-002 | root/sudo-only operator 模型与默认人机 TUI 冲突 | 普通用户启动 `dasall` 可能只能看到 permission denied | 正式命令迁移前冻结 user-level daemon/socket 或 root operator TUI 口径 |
...
| TUI-RISK-005 | ModelSelectionHint 当前链路不支持 pin/depth preference | selector UI 与真实行为不一致 | 先 fake-only，后续按 Access/Runtime/LLM 三段补链路 |
...
| TUI-OQ-001 | `/clear` 是清空视图还是新建当前进程内 session | 影响 session seam 和用户心智 | Phase 0 冻结；prototype 可先 fake 本地行为 |
...
| TUI-OQ-004 | TUI 是否要支持普通用户无 sudo 的默认体验 | 影响 daemon socket、Debian 文档和安全模型 | 正式命令迁移前必须解清 |
