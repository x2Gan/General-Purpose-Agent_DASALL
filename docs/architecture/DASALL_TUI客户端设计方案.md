...
| 范畴 | 当前事实 | 对 TUI 的含义 |
|---|---|---|
| `apps/tui` | 当前已接入 non-installed `dasall_tui_prototype` target；`apps/tui/src/main.cpp` 默认保持 fake-only no-daemon placeholder，并在 FTXUI target 已解析时仅追加 private link helper；`apps/tui/src/data/TuiProjectionTypes.h` 已落 module-local projection DTO 基线，`apps/tui/src/data/ITuiDataSource.h` 已落 data source seam 的 request/result/interface 基线，`apps/tui/src/data/FakeScenarioCatalog.h` / `FakeTuiDataSource.h/.cpp` 已落六个 deterministic scenario 与纯内存 replay 语义，`apps/tui/src/model/TuiAction.h` / `apps/tui/src/model/TuiScreenModel.h` / `apps/tui/src/model/TuiReducer.h/.cpp` 已落 typed MVU model/action/reducer 基线，`apps/tui/src/command/TuiSlashCommandParser.h/.cpp` 已落最小 slash parser 与 help metadata，`apps/tui/src/view/TuiComposer.h/.cpp`、`TuiInputHistory.h`、`TuiModelSelector.h/.cpp` 已分别落 composer 状态机、输入历史与 selector fake 交互基线，`tests/unit/tui/TuiReducerTransitionTest.cpp`、`TuiDataSourceContractTest.cpp`、`TuiFakeScenarioCatalogTest.cpp`、`FakeTuiDataSourceTest.cpp`、`TuiSlashCommandParserTest.cpp`、`TuiComposerTest.cpp`、`TuiComposerHistoryTest.cpp`、`TuiModelSelectorTest.cpp` 与 `TuiRouteCatalogFilterTest.cpp` 已分别守住 reducer、data source seam、scenario catalog、fake replay、slash parser、composer 与 selector focused 路径 | `TUI-TODO-016~020` 可复用 prototype substrate、DTO、data source seam、deterministic fake replay、model/action/reducer、slash parser、composer 与 selector 基线继续演进，但正式 `dasall-tui` / bare `dasall` 迁移仍待后续 gate |
...
| TUI 测试拓扑 | `tests/unit/tui`、`tests/integration/tui`、`tests/fixtures/tui/golden` 已物化，顶层 CMake 可发现 `TuiScreenModelTest` / `TuiReducerTransitionTest` / `ITuiDataSourceContractTest` / `TuiFakeScenarioCatalogTest` / `TuiFakeDataSourceTest` / `TuiSlashCommandParserTest` / `TuiComposerTest` / `TuiComposerHistoryTest` / `TuiModelSelectorTest` / `TuiRouteCatalogFilterTest` / `TuiPrototypeSmokeTest` / `TuiPrototypeBuildSmokeTest`，且 label 已区分 `unit` / `integration` / `snapshot` | 后续 `TUI-TODO-016~020` 可以复用稳定的 discoverability、fake replay baseline、slash parser 基线、composer 状态机基线、selector fake 交互基线与 prototype build smoke 入口，不必边实现边补测试注册 |
