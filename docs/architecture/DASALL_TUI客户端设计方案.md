...
| 当前实现成熟度 | Not Ready | 当前仓库已有 fake-only TUI substrate 与十九项 L2 基线，但 formal app/session/route/release path 仍未闭环；session open/close、真实状态投影、模型偏好真链路与命令迁移继续后置 |
...
### 4.4 当前实现基线（2026-05-23）

当前仓库具备部分可复用底座，但距离正式 TUI 仍有明确缺口。后续 TODO 拆分必须以该基线为准，避免把“可复用主链存在”误解为“TUI 已可直接接入”。

| 范畴 | 当前事实 | 对 TUI 的含义 |
|---|---|---|
| `apps/tui` | 当前已接入 non-installed `dasall_tui_prototype` target；`apps/tui/src/app/TuiApp.h/.cpp` 已把 terminal probe、`FakeTuiDataSource`、reducer、composer、selector 与 renderer 接成 fake-only app loop，并通过 `dasall_tui_prototype_core` 供 prototype / integration tests 复用；`apps/tui/src/main.cpp` 已从 fake-only no-daemon placeholder 切到真实 prototype app path；`apps/tui/src/data/TuiProjectionTypes.h` 已落 module-local projection DTO 基线，`apps/tui/src/data/ITuiDataSource.h` 已落 data source seam 的 request/result/interface 基线，`apps/tui/src/data/FakeScenarioCatalog.h` / `FakeTuiDataSource.h/.cpp` 已落六个 deterministic scenario 与纯内存 replay 语义，`apps/tui/src/model/TuiAction.h` / `apps/tui/src/model/TuiScreenModel.h` / `apps/tui/src/model/TuiReducer.h/.cpp` 已落 typed MVU model/action/reducer 基线，`apps/tui/src/command/TuiSlashCommandParser.h/.cpp` 已落最小 slash parser 与 help metadata，`apps/tui/src/view/TuiComposer.h/.cpp`、`TuiInputHistory.h`、`TuiModelSelector.h/.cpp`、`TuiTranscriptView.h/.cpp`、`TuiStatusPanel.h/.cpp`、`TuiDesignTokens.h`、`TuiLayoutMetrics.h` 已分别落 composer 状态机、输入历史、selector fake 交互、transcript view、status panel、design tokens 与 layout metrics 基线，`apps/tui/src/terminal/TuiTerminalCapabilityProbe.h/.cpp`、`FtxuiRendererAdapter.h/.cpp` 已落 terminal capability probe 与 renderer adapter 基线，`tests/unit/tui/TuiReducerTransitionTest.cpp`、`TuiDataSourceContractTest.cpp`、`TuiFakeScenarioCatalogTest.cpp`、`FakeTuiDataSourceTest.cpp`、`TuiSlashCommandParserTest.cpp`、`TuiComposerTest.cpp`、`TuiComposerHistoryTest.cpp`、`TuiModelSelectorTest.cpp`、`TuiTranscriptViewTest.cpp`、`TuiStatusPanelTest.cpp`、`TuiDesignTokensTest.cpp`、`TuiMainLayoutSnapshotTest.cpp`、`TuiTerminalCapabilityProbeTest.cpp` 与 `TuiRouteCatalogFilterTest.cpp` 已分别守住 reducer、data source seam、scenario catalog、fake replay、slash parser、composer、transcript、status、tokens/metrics、main-layout snapshot、terminal probe 与 selector focused 路径 | `TUI-TODO-020` 已闭合 fake-only app orchestration，后续 021 可以直接复用稳定的 prototype 调用上下文；正式 `dasall-tui` / bare `dasall` 迁移仍待后续 gate |
...
| TUI 测试拓扑 | `tests/unit/tui`、`tests/integration/tui`、`tests/fixtures/tui/golden` 已物化，顶层 CMake 可发现 `TuiScreenModelTest` / `TuiReducerTransitionTest` / `ITuiDataSourceContractTest` / `TuiFakeScenarioCatalogTest` / `TuiFakeDataSourceTest` / `TuiSlashCommandParserTest` / `TuiComposerTest` / `TuiComposerHistoryTest` / `TuiModelSelectorTest` / `TuiTranscriptViewTest` / `TuiStatusPanelTest` / `TuiDesignTokensTest` / `TuiMainLayoutSnapshotTest` / `TuiTerminalCapabilityProbeTest` / `TuiRouteCatalogFilterTest` / `TuiAppStartupTest` / `TuiPrototypeSmokeTest` / `TuiPrototypeBuildSmokeTest`，且 label 已区分 `unit` / `integration` / `snapshot` | `TUI-TODO-020` 已利用稳定的 discoverability、fake replay baseline、slash parser、composer、selector、transcript/status、renderer snapshot 与 terminal probe 基线完成 fake-only startup/smoke 验证；后续不必再把 app loop 留在 topology placeholder 阶段 |
...
截至 2026-05-23，`apps/tui/src/ipc/TuiIpcController.h/.cpp` 已落 controller baseline：header 已冻结五个 operation 的 request payload、response payload、`TuiIpcOutcome`、per-operation timeout policy 与 `TuiIpcController` 方法面；`.cpp` 已实现 platform IIPC roundtrip、JSON serialization、stable reason-code normalization 与 private test seam，`TuiIpcControllerTest` / `TuiIpcPermissionDeniedTest` 已守住 `socket_missing`、`permission_denied`、`timeout`、`schema_mismatch`、`malformed_response` 的 machine-readable 暴露。
...
5. 执行流：prototype 从 `FakeScenarioCatalog` 提供的六个 deterministic scenario（`golden_ready`、`planning_tools`、`needs_confirm`、`recovering`、`route_switch`、`narrow_cjk`）取数，并通过 `FakeTuiDataSource` 统一回放 session、receipt、route catalog 与 event batches；正式实现经 `TuiIpcController` 访问 daemon projection。
6. 失败语义：fake source 不允许网络/IPC；未知场景、非法 cursor、session 不匹配都必须通过 machine-readable `TuiDataSourceIssue` fail-closed；daemon source 将连接失败、权限失败、超时映射为 user-visible reason code。
7. 测试出口：`TuiFakeScenarioCatalogTest`、`TuiFakeDataSourceTest`、`DaemonTuiDataSourceContractTest`；验收 `ctest --preset vscode-linux-ninja -R "Tui(FakeScenarioCatalog|FakeDataSource|DaemonDataSource)" --output-on-failure`。
...
5. 执行流：screen model -> layout metrics -> canonical render frame -> FTXUI screen 或 deterministic snapshot backend -> terminal/snapshot。
6. 失败语义：布局不可容纳时进入 narrow/fallback；snapshot diff 需人工确认后更新 golden。
7. 测试出口：`TuiDesignTokensTest`、`TuiMainLayoutSnapshotTest`、`TuiRenderSnapshotTest`；当前 baseline 至少要覆盖 120x36 ready、80x24 narrow CJK、selector modal 与 busy draft，并允许在 default-off resolver 下先用 deterministic snapshot backend 验证布局；验收 `ctest --preset vscode-linux-ninja -R "Tui(DesignTokens|MainLayoutSnapshot|RenderSnapshot)" --output-on-failure`。
...
| TUI-DES-004 | FTXUI 只作为 renderer adapter private dependency | `FtxuiRendererAdapter.*`、`TuiDesignTokens.h`、`TuiLayoutMetrics.h` | `TuiDesignTokensTest`、`TuiMainLayoutSnapshotTest`、`TuiRenderSnapshotTest` | `ctest --preset vscode-linux-ninja -R "Tui(DesignTokens|MainLayoutSnapshot|RenderSnapshot)" --output-on-failure` |
| TUI-DES-005 | composer 是核心交互组件 | `TuiComposer.*`、`TuiInputHistory.*` | `TuiComposerTest`、`TuiComposerHistoryTest` | `ctest --preset vscode-linux-ninja -R "TuiComposer" --output-on-failure` |
