...
| TUI 测试拓扑 | `tests/unit/tui`、`tests/integration/tui`、`tests/fixtures/tui/golden` 已物化，顶层 CMake 可发现 `TuiScreenModelTest` / `TuiReducerTransitionTest` / `TuiComposerTest` / `TuiPrototypeSmokeTest` / `TuiPrototypeBuildSmokeTest`，且 label 已区分 `unit` / `integration` / `snapshot` | 后续 `TUI-TODO-008~020` 可以复用稳定的 discoverability 与 prototype build smoke 入口，不必边实现边补测试注册 |
