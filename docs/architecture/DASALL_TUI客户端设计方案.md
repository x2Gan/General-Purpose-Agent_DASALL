...
| 范畴 | 当前事实 | 对 TUI 的含义 |
|---|---|---|
| `apps/tui` | 当前已存在目录与 FTXUI private-link helper，但尚不存在独立 TUI target | 仍需先做 prototype/fake-only substrate，不能把 helper 误读为 app 已接线 |
...
| TUI 测试拓扑 | `tests/unit/tui`、`tests/integration/tui`、`tests/fixtures/tui/golden` 已物化，顶层 CMake 可发现 `TuiScreenModelTest` / `TuiReducerTransitionTest` / `TuiComposerTest` / `TuiPrototypeSmokeTest`，且 label 已区分 `unit` / `integration` / `snapshot` | 后续 `TUI-TODO-007~020` 可以复用稳定的 discoverability 入口，不必边实现边补测试注册 |
