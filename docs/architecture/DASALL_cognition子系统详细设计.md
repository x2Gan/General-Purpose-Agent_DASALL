...
| cognition.perception.llm_enabled | true | perception stage 是否优先走 LLM structured dual-path；`edge_minimal` 默认可关 | WP-COG-GAP-012；关闭时仍保留 canonical perception stage 与规则路径 |
...
5. 关键执行流：先读取 GoalContract 的成功判据与约束，再结合 ContextPacket 和 BeliefState 产出本地 rule perception；若 `StageExecutionPlan.perception_llm_enabled=true`，则通过 CognitionLlmBridge 请求 `schema://cognition/perception/v1` 的结构化提取，并依次执行 raw schema 校验、typed projection 与 perception invariant 校验；当 bridge payload 有效时，其结果可以成为 authoritative perception source，但仍需与 rule perception 比较 `task_type` 与 `requires_clarification`，一旦分歧则优先升级 AskClarification；若 LLM 路径关闭或不可用，则按配置进入规则降级；最后由 StageOutputValidator 收口为 PerceptionResult。
6. 失败与降级语义：输入边界不满足时返回 cognition.invalid_input；llm 不可用、schema/projection/invariant 失败且 `rule_fallback_enabled=true` 时允许规则感知降级；若 authoritative perception 自身要求澄清，或与 rule perception 在 `task_type` / `requires_clarification` 上冲突，则必须先走 clarification，而不是直接把不一致结果交给 Planner/Reasoner；当 `perception_llm_enabled=false` 时仍保留 canonical perception stage，只是跳过 LLM 分支。
7. 测试与验收出口：建议单测与集成为 tests/unit/cognition/PerceptionEngineTest.cpp、tests/unit/cognition/StageOutputValidatorPerceptionInvariantTest.cpp、tests/unit/cognition/PerceptionLlmDualPathTest.cpp、tests/integration/cognition/CognitionPerceptionStructuredOutputIntegrationTest.cpp；建议验收命令为 ctest --test-dir build-ci -R "Perception(Engine|LlmDualPath)|PerceptionInvariant|CognitionPerceptionStructured" --output-on-failure。
...
3. 核心数据定义：围绕 ReasoningRequest、ReasoningFrame、CandidateDecisionScore、ActionDecision、DecisionConfidenceBand 建模；ReasoningRequest 允许由 Runtime additive 地携带 `available_tool_descriptors`，表达本轮可见工具的 descriptor 子集，但该字段只服务于 cognition owner 内的候选筛选与 explainability，不提升为 ToolRequest authority。ActionDecision 至少显式表达 decision_kind、selected_node_id、rationale、confidence、clarification_needed、clarification_question、tool_intent_hint、delegate_hint、response_outline、diagnostics，其中 diagnostics 只用于模块内审计与 explainability 标记，不扩张为共享 contracts。
4. 公共/内部接口：公共面保持 IReasoner::decide()；内部建议具备 score_candidates()、evaluate_clarification_need()、build_direct_response_decision()、build_delegate_hint()、project_response_outline()、validate_decision_thresholds() 以及 rule-based tool candidate prefilter seam。
5. 关键执行流：先读取当前活动节点与 plan open questions；若 PerceptionResult 或 PlanGraph 显示信息不足，则优先走 AskClarification；否则结合 latest Observation、BeliefState、GoalContract 与 Runtime 透传的 `available_tool_descriptors` 对“执行当前节点”“直接回复”“提示委派”“终止收敛”等候选路径评分。对 `ExecuteAction` 路径，Reasoner/DecisionProjector 必须先基于 tool name、display name、tags、perception tool entity、goal/objective 关键词做 deterministic rule prefilter，把候选工具面收敛到 top-K，再从该子集中选择 `tool_intent_hint.tool_name`；候选规模只通过低基数 diagnostics 暴露，不把候选 descriptor 集合提升为 ToolRequest authority。原始 heuristic 分数在阈值裁定前必须经过 `reasoner.candidate_weights` 的 profile bias 校准，再按 clarification_threshold、direct_response_threshold、delegate gate 做裁定。当 `budget_utilization >= 0.8` 时，Reasoner 不再落入 ExecuteAction 路径，而是显式经 `DirectResponse` 或 `ConvergeSafe` 分支收口，并在 `ActionDecision.diagnostics` 记录 `budget_pressure_decision_path:<direct_response|converge_safe>` 供审计使用；输出的 ActionDecision 仍需包含 rationale 和 response_outline。
...
|---|---|---|---|
| `cognition.perception.v1` / `perception` | `PerceptionResult` | `schema_version`、`intent_summary`、`task_type`、`entities`、`constraints_digest`、`ambiguities`、`clarification_questions`、`confidence`、`requires_clarification`；其中 `entities[].name`、`entities[].value`、`entities[].confidence` 必填 | 首版 fail-closed；raw schema 只冻结顶层与实体必填字段，ambiguity / clarification 一致性由 invariant 层收口 |
...
5. 关键执行流：CognitionFacade 在每个入口构造统一的 StageTelemetryContext；各阶段在开始、完成、失败、降级时把必要字段送入 CognitionTelemetry；Telemetry 先执行 redaction，再统一转发到 infra 的日志、指标、trace、audit 抽象，并允许通过 owner 内部 sink seam 追加 build-ci/replay 这类测试期 recorder。`stage.completed` 事件除 `decision_kind` / `confidence` 外，还必须在 bridge usage 可用时携带 `prompt_tokens`、`completion_tokens`、`total_cost`、`finish_reason` 四个聚合字段；这些字段对齐 OpenTelemetry GenAI semconv 的 usage / finish reason 观测口径，并被视为可保留的低敏聚合值，redaction 只能裁剪 raw prompt、provider payload、reasoning trace 等敏感内容，不能剥离这些 usage/cost facts。该 recorder 只能消费 request / bridge payload / result 的结构化语义字段，不能绕过 redaction 直接落 raw prompt、provider-private payload 或恢复审计事实；request_id、trace_id、decision_kind、fallback_mode 等字段必须可贯通到 Runtime 与 LLM 侧的关联分析。若启用 `ReplayTraceRecorderConfig.failure_sampling`，recorder 还可在不影响主 replay.trace 全量落盘的前提下，按 owner 内部 deterministic sampling rule 额外复制同一 request 的 replay corpus 到 `failure_samples/<category>/` 目录，仅允许三类低基数语义标签：`cognition.schema_violation`（匹配 result trace 中的 `structured_projection.schema_violation:*`）、`reflection.abort_safe`（匹配 `decision_kind=AbortSafe`）与 `response.fallback_used`（匹配 `fallback_used=true`）；采样判定必须仅依赖 request_id/stage/event/category，不得引入 raw payload 内容哈希或跨请求全局状态。`WP-COG-GAP-018` 的离线 judge regression 继续复用同一 redacted replay seam：周期作业以 `tests/data/cognition/replay/` 作为 curated success-chain split，可选接入 `failure_samples/*` 作为 supplemental failure split；judge 输入必须只携带 case summary、rubric 与 trace bundle 三类 redacted 字段，并通过 llm request tags 中既有 `user_goal`、`constraints`、`session_summary` 槽位送入 response-stage PromptComposer，而不是为评测作业引入第二套 prompt compose surface 或旁路 raw trace 文件内容。
...
7. 测试与验收出口：owner 级单测为 tests/unit/cognition/CognitionTelemetryFieldsTest.cpp、tests/unit/cognition/CognitionTelemetryRedactionTest.cpp、tests/unit/cognition/CognitionTelemetryFailureIsolationTest.cpp、tests/unit/cognition/CognitionReplayRegressionTest.cpp、tests/unit/cognition/CognitionFailureSamplingTest.cpp；2026-05-16 另补 tests/integration/cognition/CognitionProductionTelemetryIntegrationTest.cpp，验证 live audit/metrics/trace provider 已不再停留于 no-op sink，且 `stage.completed` / `stage.failed` / `response.degraded` 在 production-style sink 上保持 redaction。Replay regression 额外固定 tests/data/cognition/replay/ 下的 golden trace 数据集，覆盖 decide / reflect / build 的 request、bridge payload、result 序列；failure sampling test 额外固定 `cognition.schema_violation`、`reflection.abort_safe`、`response.fallback_used` 三类 failure corpus copy 的正负路径。`WP-COG-GAP-018` 追加的离线 judge regression 必须提供一个非主路径 runner，消费上述 replay/failure split，输出 `judge-cases.jsonl`、`judge-results.jsonl`、`judge-report.json`、`judge-report.md` 与 `status.txt` 五类 artifacts，并由 `tests/integration/cognition/CognitionLlmJudgeRegressionTest.cpp` 与 `scripts/ci/cognition_llm_judge_regression.sh` 共同守门。建议验收命令为 ctest --test-dir build-ci -R "CognitionTelemetry(Fields|Redaction|FailureIsolation)Test|Cognition(FailureSampling|ReplayRegression|LlmJudgeRegression)Test|CognitionProductionTelemetryIntegrationTest" --output-on-failure。
...
| Schema evolution / OpenAPI / gRPC | `StageSchemaRegistry` 冻结 `cognition.perception.v1`、`cognition.plan.v1`、`cognition.reasoning.v1`、`cognition.reflection.v1`、`cognition.response.v1` 的 version、required / optional 与 unknown field 策略 | schema drift 必须由 registry 与 focused tests 显式拦截，不能散落在 Facade 或 tests 私有常量 |
