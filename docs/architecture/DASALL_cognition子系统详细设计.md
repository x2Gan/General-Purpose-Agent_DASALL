...
| ActionDecision | decision_kind、selected_node_id、rationale、confidence、clarification_needed、clarification_question、tool_intent_hint、delegate_hint、response_outline | 不直接等于 ToolRequest，不包含 provider/tool execution 细节 |
...
3. 核心数据定义：围绕 StageLlmCallRequest、StageLlmCallResult、StageModelHint、StageSchemaSpec、StageBudgetHint、LlmFailureProjection 建模；StageLlmCallRequest 至少显式表达 stage_name、schema_kind、model_hint、deadline_ms、structured_output_requirement、redaction_requirement。
4. 公共/内部接口：内部建议具备 invoke_stage()、build_llm_request()、derive_budget_hint()、normalize_llm_response()、project_llm_failure()；公共面仍通过各阶段组件间接消费，不单独提升为共享接口。
5. 关键执行流：阶段组件先构造 StageLlmCallRequest 与 StageSchemaSpec，桥接层再把阶段语义、schema 约束和预算提示投影到 llm 公共接口；调用 ILLMManager 后回收结果对象，剥离 provider-private 字段，形成 StageLlmCallResult，并把 response warnings、budget warnings 和 redaction 标记一并带给 StageOutputValidator。
6. 失败与降级语义：provider 不可用、adapter failure、超时和空结果都必须映射为统一 ErrorInfo，而不是通过异常控制主流程；桥接层不做本地重试，只回传失败事实；若 llm 仅返回 warning 而未违反 schema，则 warning 透传给阶段结果，不在桥接层做二次裁剪。
7. 测试与验收出口：建议单测为 tests/unit/cognition/CognitionLlmBridgeProjectionTest.cpp、tests/unit/cognition/CognitionLlmBridgeErrorMappingTest.cpp、tests/unit/cognition/CognitionLlmBridgeBudgetHintTest.cpp；建议验收命令为 ctest --test-dir build-ci -R "CognitionLlmBridge(Projection|ErrorMapping|BudgetHint)Test" --output-on-failure。
...
5. 关键执行流：CognitionFacade 在每个入口构造统一的 StageTelemetryContext；各阶段在开始、完成、失败、降级时把必要字段送入 CognitionTelemetry；Telemetry 再统一转发到 infra 的日志、指标、trace、audit 抽象，并保证 request_id、trace_id、decision_kind、fallback_mode 等字段可贯通到 Runtime 与 LLM 侧的关联分析。
...
7. 测试与验收出口：建议单测为 tests/unit/cognition/CognitionTelemetryFieldsTest.cpp、tests/unit/cognition/CognitionTelemetryRedactionTest.cpp、tests/unit/cognition/CognitionTelemetryFailureIsolationTest.cpp；建议验收命令为 ctest --test-dir build-ci -R "CognitionTelemetry(Fields|Redaction|FailureIsolation)Test" --output-on-failure。
...
struct StageModelHint {
  std::string stage_name;          // perception, planning, reasoning, reflection, response
  ...
  bool requires_reasoning_trace;   // 是否需要 reasoning_content（仅 planning/reflection 阶段可能需要）
...
| 阶段 | 默认 capability_tier | 默认 requires_structured_output | 默认 requires_reasoning_trace |
|---|---|---|---|
| Perception | Lightweight / Standard | true | false |
| Planning | Standard / Advanced | true | true（仅 advanced 计划） |
| Reasoning | Standard | true | false |
| Reflection | Advanced / ReasoningHeavy | true | true |
| Response | Standard | false（自然语言输出） | false |

约束：

1. StageModelHint 保持 cognition 模块内类型，不进入共享契约。
2. CognitionLlmBridge 负责把 StageModelHint 投影到 llm 公共接口的 LLMGenerateRequest 对应字段。
...
|---|---|---|---|---|
| cognition.llm_unavailable | ErrorInfo(cognition.llm_unavailable, stage) | Runtime 检查 stage 降级策略；若允许则通知 cognition 走降级路径，否则进入 RecoveryManager | 否——LLM 子系统已自行处理 provider 不可用并返回统一错误；Runtime 不需要"回报 LLM" | CognitionLlmBridge 从 ILLMManager 收到的已是归一化错误 |
...
| cognition.stage_timeout | ErrorInfo(cognition.stage_timeout, stage, elapsed_ms) | Runtime 从 StageExecutionPlan 检查是否有下一候选路径；否则进入 RecoveryManager | 否 | 超时裁定由门面层做；LLM 调用超时由 LLM adapter 处理 |
...
2. 超时不等于取消——底层 LLM 调用可能仍在进行中（由 LLM adapter 管理生命周期）。
3. 超时后的阶段结果即使后续到达也应被丢弃，不得影响下一次请求。
...
| budget_utilization ≥ 0.8 | Planner 只允许 1-2 节点浅层计划；Reasoner 优先 DirectResponse 或 ConvergeSafe；Reflection 只允许一次轮次 | 临近极限 |
