...
User / App / Remote API / Terminal Event
 ...
 Cloud LLM    LAN LLM    Local LLM
                |
                v
 Knowledge / Services / Execution Control
                |
                v
...
6. Memory 子系统。
7. Knowledge 子系统。
8. Execution Control 子系统（含 Execution Service）。
9. Communication 子系统。
10. Task 子系统。
...
6. Compensation Manager：处理补偿和回滚。
...
2. 在失败时按 LIFO 顺序执行回滚。
3. 对补偿失败单独上报高优先级审计事件。
4. 把补偿结果回写 Experience Memory，供后续策略优化。
