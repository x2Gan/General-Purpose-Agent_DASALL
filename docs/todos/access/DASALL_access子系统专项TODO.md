...
当前结论：Access 详设已经满足 L3/L2 混合拆分条件。`InboundPacket`、`SubjectIdentity`、`AccessDecisionProof`、`AccessErrorCode`、`RuntimeDispatchRequest`、`PublishEnvelope`、`AsyncTaskReceipt`、`IAccessGateway`、`IProtocolAdapter`、`IAdmissionController`、`IAccessRuntimeBridge` 已具备接口级或数据结构级拆分证据；但 `IAccessRuntimeBridge` 与 runtime 真接口、`AccessBootstrapConfig` 落盘形态、`runtime_override` / `diagnostics.pull` 入口 schema、gateway 首版 transport 选型、shared streaming lifecycle、IIPC peer identity 接缝仍有前置收敛要求。因此本专项 TODO 采用“补设计解阻 -> 公共接口与骨架 -> 主链实现 -> 测试与 Gate 收口”的四段编排，默认以 daemon 常驻本地控制面承载 Access 主链，CLI 作为独立进程通过 IIPC/UDS 接入；gateway HTTP 路径受 transport 选型门控推进。
...
| `AccessPolicyGate` | 6.7.1、6.14.7 | L3 | taxonomy、override/diagnostics gate、fail-closed 规则明确 | override/diagnostics schema 仍待外部协同 | 先完成 submit/query/cancel 主路径，override/diagnostics 走 blocker |
...
| CLI client / HTTP adapters | 6.14.8、6.20 | L2 | 本地控制面与远程入口边界、首版测试出口明确 | gateway transport 选型未冻结 | 先做 CLI client，再做 HTTP unary/async |
...
| override / diagnostics schema 未冻结 | 高危入口越权或无法 Gate | ACC-TODO-003 | override / diagnostics 一律 deny-by-default |
| gateway transport 选型未定 | HTTP adapter 接口和线程模型返工 | ACC-TODO-004 | HTTP 仅做选型后的最小 unary/async 范围 |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| ACC-TODO-001 | NotStarted | 补齐 access-runtime bridge sidecar seam | Access 详设 6.6、6.7、6.18、12.2-1；CrossModuleDataProjectionMatrix 6 | `RuntimeDispatchRequest`、`IAccessRuntimeBridge`、access sidecar 投影 | L2 | Access 详设；本 TODO；必要时 runtime seam 说明文档 | `RuntimeDispatchRequest`、`RuntimeInvokeContext`、`IAccessRuntimeBridge::dispatch/cancel` | 文档一致性 | `rg -n "IAccessRuntimeBridge|RuntimeDispatchRequest|Access sidecar|cancel\(" docs/architecture/DASALL_access子系统详细设计.md docs/ssot/CrossModuleDataProjectionMatrix.md docs/todos/access/DASALL_access子系统专项TODO.md` | 无 | ACC-BLK-001 | runtime-facing public seam 与 access sidecar 投影口径唯一 | 更新后的 Access 详设；本 TODO；交付物说明文档 | bridge 输入输出、取消路径与 sidecar owner 可被后续 Build 任务直接引用，且不再存在两套 runtime seam 说法 |
| ACC-TODO-002 | NotStarted | 补齐 AccessBootstrapConfig schema 与治理投影源 | Access 详设 6.11、12.2-2；蓝图 3.2；计划文档阶段 K | 6.11 `AccessBootstrapConfig`、runtime governance views | L2 | Access 详设；本 TODO | `AccessBootstrapConfig`、`AccessAuthView`、`AccessAdmissionView`、`AccessPublishView`、`AccessRuntimeGovernanceView` | 文档一致性 | `rg -n "AccessBootstrapConfig|runtime_budget|timeout_policy|ops_policy|infra.security_policy" docs/architecture/DASALL_access子系统详细设计.md docs/todos/access/DASALL_access子系统专项TODO.md` | 无 | ACC-BLK-002 | 配置 source-of-truth、热更新边界与 immutable view 规则冻结 | 更新后的 Access 详设；本 TODO；交付物说明文档 | 启动事实配置、运行治理投影和 override 边界均具唯一口径，可直接进入 `AccessConfigAdapter` Build |
| ACC-TODO-003 | Blocked | 补齐 override 与 diagnostics 入口 schema | Access 详设 6.7.1、6.11.4、6.12、12.2-8；infra 相关详设交互约束 | `access.runtime_override.apply`、`access.diagnostics.pull` | L1 | Access 详设；本 TODO | `OverrideSourceFact`、`DiagnosticsSelectorFact`、artifact size / selector schema | 文档一致性 | `rg -n "runtime_override|diagnostics.pull|selector|artifact|allow proof" docs/architecture/DASALL_access子系统详细设计.md docs/todos/access/DASALL_access子系统专项TODO.md` | 无 | ACC-BLK-003 | infra/config 与 infra/diagnostics 明确 typed patch / artifact selector 口径 | 更新后的 Access 详设；本 TODO；阻塞记录 | override / diagnostics 可形成可测试 schema；在此之前 Build 任务保持 deny-by-default |
| ACC-TODO-004 | NotStarted | 收敛 gateway 首版 transport 选型与 HTTP-only 边界 | Access 详设 6.14.8、12.2-3、12.2-5；蓝图 3.2 | HTTP unary + async receipt 首版边界 | L1 | Access 详设；本 TODO；`apps/gateway/CMakeLists.txt` 方案说明 | `HttpProtocolAdapter`、`HealthProbeHandler`、HTTP thread model | 文档一致性 | `rg -n "HTTP unary|WebSocket|MQTT|transport|gateway" docs/architecture/DASALL_access子系统详细设计.md docs/todos/access/DASALL_access子系统专项TODO.md` | 无 | ACC-BLK-004 | 首版明确只支持 HTTP unary + async receipt，transport 选型冻结 | 更新后的 Access 详设；本 TODO；选型记录 | gateway 首版范围与 transport 线程模型单一，不再允许 HTTP/WS/MQTT 三路并行启动 |
...
| ID | 状态 | 任务标题 | 来源依据 | 设计锚点 | 粒度等级 | 代码目标 | 目标函数/接口/数据结构 | 测试目标 | 验收命令 | 前置依赖 | 阻塞项 | 解阻条件 | 交付物 | 完成判定 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| ACC-TODO-006 | NotStarted | 新增 access include、CMake 与测试拓扑骨架 | Access 详设 8.1、9.1、9.2；当前代码现状 | `access/include`、`tests/unit/access`、`tests/integration/access` discoverability | L2 | `access/CMakeLists.txt`、`tests/unit/access/CMakeLists.txt`、`tests/integration/access/CMakeLists.txt`、`tests/unit/CMakeLists.txt`、`tests/integration/CMakeLists.txt` | `AccessInterfaceSurfaceTest` discoverability、`ctest -N` discoverability | `cmake --build build-ci --target dasall_access && ctest --test-dir build-ci -N` | 无 | 无 | — | 更新后的 CMake；空骨架测试文件；交付物说明文档 | `tests/unit/access`、`tests/integration/access` 被顶层接入，`ctest -N` 可发现 Access 测试入口 |
| ACC-TODO-007 | NotStarted | 定义 AccessErrorCode 与 AccessError | Access 详设 6.7、6.17 | `AccessErrorCode` 100-999 码段、`AccessError` | L3 | `access/include/AccessErrors.h`、`access/src/ProtocolErrorMapper.cpp` | `AccessErrorCodeTest`、`AccessErrorMappingTest` | `cmake --build build-ci --target dasall_unit_tests && ctest --test-dir build-ci -R "AccessErrorCodeTest|AccessErrorMappingTest" --output-on-failure` | 006 | 无 | — | `access/include/AccessErrors.h`；相关单测；交付物说明文档 | Access 错误码域完整落盘，且 HTTP/CLI 映射有自动化断言 |
| ACC-TODO-008 | NotStarted | 定义 AccessGatewayState 并扩展 IAccessGateway 生命周期接口 | Access 详设 6.7、6.15 | `AccessGatewayState`、`shutdown()`、`state()`、`is_ready()` | L3 | `access/include/IAccessGateway.h`、`access/include/AccessTypes.h` | `AccessInterfaceSurfaceTest`、`AccessGatewayLifecycleTest` | `cmake --build build-ci --target dasall_unit_tests && ctest --test-dir build-ci -R "AccessInterfaceSurfaceTest|AccessGatewayLifecycleTest" --output-on-failure` | 006 | 无 | — | 更新后的 `IAccessGateway.h`、`AccessTypes.h`；相关单测；交付物说明文档 | `IAccessGateway` 与详设 lifecycle surface 一致，且 Ready/Draining/ShutDown 语义可被测试引用 |
| ACC-TODO-009 | NotStarted | 定义 SubjectIdentity、AccessDecisionProof 与 dispatch/publish/receipt supporting types | Access 详设 6.6、6.7、6.19 | `SubjectIdentity`、`AccessDecisionProof`、`RuntimeDispatchRequest/Result`、`PublishEnvelope`、`AsyncTaskReceipt` | L3 | `access/include/AccessTypes.h` | `AccessSupportingTypesTest`、`PublishEnvelopeTypesTest`、`AsyncTaskReceiptTypesTest` | `cmake --build build-ci --target dasall_unit_tests && ctest --test-dir build-ci -R "AccessSupportingTypesTest|PublishEnvelopeTypesTest|AsyncTaskReceiptTypesTest" --output-on-failure` | 006 | ACC-BLK-001 | 完成 001 | 更新后的 `AccessTypes.h`；相关单测；交付物说明文档 | Access supporting types 与详设字段、contracts 边界、ownership 规则一致，后续组件无需再自造局部结构 |
| ACC-TODO-010 | NotStarted | 定义 IAdmissionController 与 AccessAdmissionResult | Access 详设 6.7、6.14.7 | `IAdmissionController::admit/release_ticket/record_completion` | L3 | `access/include/IAdmissionController.h`、`access/include/AccessTypes.h` | `AdmissionControllerSurfaceTest` | `cmake --build build-ci --target dasall_unit_tests && ctest --test-dir build-ci -R "AdmissionControllerSurfaceTest" --output-on-failure` | 006、009 | 无 | — | 新增 `IAdmissionController.h`；相关单测；交付物说明文档 | Admission surface 能表达 admit/replay/conflict/release/completion 语义 |
| ACC-TODO-011 | NotStarted | 对齐 IAccessRuntimeBridge 的 dispatch/cancel 公共面 | Access 详设 6.7、6.18.2 | `IAccessRuntimeBridge::dispatch/cancel` | L3 | `access/include/IAccessRuntimeBridge.h`、`access/include/AccessTypes.h` | `RuntimeBridgeSurfaceTest` | `cmake --build build-ci --target dasall_unit_tests && ctest --test-dir build-ci -R "RuntimeBridgeSurfaceTest" --output-on-failure` | 001、006、009 | ACC-BLK-001 | 完成 001 | 更新后的 `IAccessRuntimeBridge.h`；相关单测；交付物说明文档 | dispatch/cancel 两条 surface 与 async/timeout/cancel 语义一致 |
| ACC-TODO-012 | NotStarted | 定义 AccessBootstrapConfig 与治理视图 supporting surface | Access 详设 6.11、6.14.8 | `AccessBootstrapConfig`、`Access*View` | L2 | `access/include/AccessTypes.h`、`access/src/AccessConfigAdapter.cpp` 占位 | `AccessConfigProjectionTest` | `cmake --build build-ci --target dasall_unit_tests && ctest --test-dir build-ci -R "AccessConfigProjectionTest" --output-on-failure` | 002、006 | ACC-BLK-002 | 完成 002 | 更新后的 `AccessTypes.h`、配置投影占位；交付物说明文档 | 启动事实与运行治理 view 均具明确数据形状，供组合根和内部组件消费 |

### 6.3 配置 / 主链 / 入口实现任务

| ID | 状态 | 任务标题 | 来源依据 | 设计锚点 | 粒度等级 | 代码目标 | 目标函数/接口/数据结构 | 测试目标 | 验收命令 | 前置依赖 | 阻塞项 | 解阻条件 | 交付物 | 完成判定 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| ACC-TODO-013 | NotStarted | 实现 ProtocolAdapterRegistry | Access 详设 6.3、6.14.7 | `register_adapter()`、`resolve_decoder()`、`resolve_encoder()`、`revoke_source()` | L2 | `access/src/ProtocolAdapterRegistry.cpp` | `ProtocolAdapterRegistryTest`、`ProtocolAdapterRegistryConflictTest` | `cmake --build build-ci --target dasall_unit_tests && ctest --test-dir build-ci -R "ProtocolAdapterRegistryTest|ProtocolAdapterRegistryConflictTest" --output-on-failure` | 006、009 | 无 | — | `ProtocolAdapterRegistry.cpp`、相关单测；交付物说明文档 | entry binding 可注册/查找/冲突拒绝且无持锁 I/O |
| ACC-TODO-014 | NotStarted | 实现 SubjectResolver | Access 详设 6.7.2、6.14.7 | `resolve()`、`derive_channel_ref()`、`build_challenge_plan()` | L3 | `access/src/SubjectResolver.cpp` | `SubjectResolverTest`、`SubjectResolverLocalTrustedTest`、`SubjectResolverChallengeTest` | `cmake --build build-ci --target dasall_unit_tests && ctest --test-dir build-ci -R "SubjectResolver(Test|LocalTrustedTest|ChallengeTest)" --output-on-failure` | 009、012、013 | 无 | — | `SubjectResolver.cpp`、相关单测；交付物说明文档 | 本地 trusted、远程 challenge、identity conflict 三路都可二值断言 |
| ACC-TODO-015 | NotStarted | 实现 AuthenticatorChain | Access 详设 6.3、6.14.7 | `authenticate()`、`select_chain()`、`map_failure_reason()` | L3 | `access/src/AuthenticatorChain.cpp` | `AuthenticatorChainTest`、`AuthenticatorChainChallengeTest`、`AuthenticatorChainSecretFailureTest` | `cmake --build build-ci --target dasall_unit_tests && ctest --test-dir build-ci -R "AuthenticatorChain(Test|ChallengeTest|SecretFailureTest)" --output-on-failure` | 012、014 | 无 | — | `AuthenticatorChain.cpp`、相关单测；交付物说明文档 | credential success、challenge、secret backend failure 三路都能显式 fail-closed |
| ACC-TODO-016 | NotStarted | 实现 AccessPolicyGate | Access 详设 6.7.1、6.14.7 | `evaluate_submit()`、`evaluate_task_query()`、`evaluate_override_request()` | L3 | `access/src/AccessPolicyGate.cpp` | `AccessPolicyGateTest`、`AccessPolicyOverrideGateTest`、`AccessPolicyBackendFailureTest` | `cmake --build build-ci --target dasall_unit_tests && ctest --test-dir build-ci -R "AccessPolicy(GateTest|OverrideGateTest|BackendFailureTest)" --output-on-failure` | 003、009、015 | ACC-BLK-003 | 完成 003 | `AccessPolicyGate.cpp`、相关单测；交付物说明文档 | submit/query path 可直接授权；override path 未冻结前保持拒绝且有显式审计 reason |
| ACC-TODO-017 | NotStarted | 实现 AdmissionController | Access 详设 6.13、6.14.7、6.21 | `admit()`、`acquire_inflight_ticket()`、`check_idempotency()`、`release_ticket()`、`record_completion()` | L3 | `access/src/AdmissionController.cpp` | `AdmissionControllerTest`、`RateLimitGateTest`、`IdempotencyGuardTest`、`AdmissionReplayHitTest` | `cmake --build build-ci --target dasall_unit_tests && ctest --test-dir build-ci -R "AdmissionControllerTest|RateLimitGateTest|IdempotencyGuardTest|AdmissionReplayHitTest" --output-on-failure` | 010、012 | 无 | — | `AdmissionController.cpp`、相关单测；交付物说明文档 | busy/conflict/replay-hit/admit 四类路径均可自动断言，且不做 silent loss |
| ACC-TODO-018 | NotStarted | 实现 RequestValidator | Access 详设 6.16、6.17 | `validate_packet()`、`validate_payload_limits()`、`validate_headers()` | L3 | `access/src/RequestValidator.cpp` | `RequestValidatorTest`、`RequestValidatorPayloadLimitTest`、`RequestValidatorInjectionTest` | `cmake --build build-ci --target dasall_unit_tests && ctest --test-dir build-ci -R "RequestValidator(Test|PayloadLimitTest|InjectionTest)" --output-on-failure` | 009 | 无 | — | `RequestValidator.cpp`、相关单测；交付物说明文档 | L1/L2/L3 输入验证规则落盘，payload 超限与注入尝试不会进入 RuntimeBridge |
| ACC-TODO-019 | NotStarted | 实现 RequestNormalizer | Access 详设 6.8、6.16、6.21.3、6.14.8 | `normalize()`、`ensure_trace_ids()`、`project_agent_request()`、`build_publish_context()` | L3 | `access/src/RequestNormalizer.cpp` | `RequestNormalizerTest`、`RequestNormalizerIdentityProjectionTest`、`RequestNormalizerConstraintProjectionTest`、`RequestNormalizerContractCompatibilityTest` | `cmake --build build-ci --target dasall_unit_tests dasall_contract_tests && ctest --test-dir build-ci -R "RequestNormalizer(Test|IdentityProjectionTest|ConstraintProjectionTest|ContractCompatibilityTest)|AgentRequestContractTest|AgentResultContractTest" --output-on-failure` | 009、014、017、018 | 无 | — | `RequestNormalizer.cpp`、相关单测；交付物说明文档 | `AgentRequest` 投影不污染 contracts，`request_id/session_id/trace_id` 与 `constraint_set` 白名单投影均可断言 |
| ACC-TODO-020 | NotStarted | 实现 RuntimeBridge | Access 详设 6.7、6.9、6.18、6.14.8 | `dispatch()`、`cancel()`、`map_runtime_result()`、`map_runtime_reject()` | L2 | `access/src/RuntimeBridge.cpp` | `RuntimeBridgeTest`、`RuntimeBridgeAsyncAcceptTest`、`RuntimeBridgeRejectMappingTest` | `cmake --build build-ci --target dasall_unit_tests && ctest --test-dir build-ci -R "RuntimeBridge(Test|AsyncAcceptTest|RejectMappingTest)" --output-on-failure` | 001、011、019 | ACC-BLK-001 | 完成 001 | `RuntimeBridge.cpp`、相关单测；交付物说明文档 | sync/accepted async/reject 三出口全部可测，cancel 仅转发不自裁定 |
| ACC-TODO-021 | NotStarted | 实现 ResultPublisher 与 ProtocolErrorMapper | Access 详设 6.10、6.17、6.14.8 | `build_envelope()`、`map_protocol_status()`、`emit_publish()` | L3 | `access/src/ResultPublisher.cpp`、`access/src/ProtocolErrorMapper.cpp` | `ResultPublisherTest`、`ProtocolErrorMapperTest`、`ResultPublisherChannelFailureTest` | `cmake --build build-ci --target dasall_unit_tests && ctest --test-dir build-ci -R "ResultPublisherTest|ProtocolErrorMapperTest|ResultPublisherChannelFailureTest" --output-on-failure` | 007、009、013、019、020 | 无 | — | `ResultPublisher.cpp`、`ProtocolErrorMapper.cpp`、相关单测；交付物说明文档 | `AgentResult` -> protocol response 映射稳定，channel unavailable 不吞掉上游业务结果 |
| ACC-TODO-022 | NotStarted | 实现 AsyncTaskRegistry 与 ResultReplayCache | Access 详设 6.10、6.19、6.21、6.14.8 | `register_async_accept()`、`query_receipt()`、`validate_ownership()`、`put()/lookup()` | L3 | `access/src/AsyncTaskRegistry.cpp`、`access/src/ResultReplayCache.cpp` | `AsyncTaskRegistryTest`、`AsyncTaskRegistryOwnershipTest`、`AsyncTaskRegistryExpiryTest`、`ResultReplayCacheTest`、`ResultReplayCacheEvictionTest` | `cmake --build build-ci --target dasall_unit_tests && ctest --test-dir build-ci -R "AsyncTaskRegistry(Test|OwnershipTest|ExpiryTest)|ResultReplayCache(Test|EvictionTest)" --output-on-failure` | 009、017、020、021 | ACC-BLK-006 | deployment secret、constant-time compare 和 TTL 规则明确 | `AsyncTaskRegistry.cpp`、`ResultReplayCache.cpp`、相关单测；交付物说明文档 | accepted async、owner mismatch、TTL expired、replay cache eviction 都可自动断言 |
| ACC-TODO-023 | NotStarted | 实现 AccessObservabilityBridge | Access 详设 6.12、6.14.8 | `emit_request_received()`、`emit_auth_failed()`、`emit_policy_denied()`、`emit_dispatch_result()`、`emit_publish_failed()` | L2 | `access/src/AccessObservabilityBridge.cpp` | `AccessObservabilityBridgeTest`、`AccessObservabilityFieldSetTest` | `cmake --build build-ci --target dasall_unit_tests && ctest --test-dir build-ci -R "AccessObservabilityBridgeTest|AccessObservabilityFieldSetTest" --output-on-failure` | 012、014、015、016、017、020、021 | 无 | — | `AccessObservabilityBridge.cpp`、相关单测；交付物说明文档 | auth failed / policy denied / publish failed 事件字段齐备，桥接失败不改变业务判定 |
| ACC-TODO-024 | NotStarted | 实现 AccessGateway facade 与优雅关闭 | Access 详设 6.14.7、6.15 | `submit()`、`publish_result()`、`shutdown()`、`run_submit_pipeline()` | L2 | `access/src/AccessGateway.cpp` | `AccessGatewayFacadeTest`、`AccessGatewayRejectPathTest`、`AccessGatewayAsyncReceiptTest`、`AccessGatewayLifecycleTest` | `cmake --build build-ci --target dasall_unit_tests && ctest --test-dir build-ci -R "AccessGateway(FacadeTest|RejectPathTest|AsyncReceiptTest|LifecycleTest)" --output-on-failure` | 008、013、014、015、016、017、018、019、020、021、022、023 | 无 | — | `AccessGateway.cpp`、相关单测；交付物说明文档 | submit 主链、reject path、async receipt path、drain/shutdown path 全部可独立验证 |
| ACC-TODO-025 | NotStarted | 实现 CliIpcClient 与 apps/cli 纯客户端组合根 | CLI 本地控制面详设 1.2、6.2、6.4；Access 详设 6.14.8、8.1 | `CliCommandParser`、`CliIpcClient`、`CliOutputFormatter`、CLI composition root | L2 | `apps/cli/src/CliCommandParser.cpp`、`apps/cli/src/CliIpcClient.cpp`、`apps/cli/src/CliOutputFormatter.cpp`、`apps/cli/src/main.cpp`、`apps/cli/CMakeLists.txt` | `CliIpcClientTest`、`CliIpcClientUnavailableTest`、`CliOutputFormatterTest` | `cmake --build build-ci --target dasall_cli dasall_unit_tests && ctest --test-dir build-ci -R "CliIpcClientTest|CliIpcClientUnavailableTest|CliOutputFormatterTest" --output-on-failure` | 013、018、024、038 | 无 | — | 更新后的 `apps/cli` 代码与 CMake、相关单测；交付物说明文档 | CLI 仅作为本地控制面客户端，不 direct link runtime，所有请求经 IIPC/UDS 进入 daemon/access 主链 |
| ACC-TODO-026 | NotStarted | 实现 HttpProtocolAdapter 与 apps/gateway unary/async 组合根 | Access 详设 6.14.8、6.20、8.1 | `decode_http_request()`、`encode_http_response()`、gateway composition root | L2 | `apps/gateway/src/HttpProtocolAdapter.cpp`、`apps/gateway/src/main.cpp` | `HttpProtocolAdapterTest`、`HttpProtocolAdapterErrorMappingTest` | `cmake --build build-ci --target dasall_gateway dasall_unit_tests && ctest --test-dir build-ci -R "HttpProtocolAdapterTest|HttpProtocolAdapterErrorMappingTest" --output-on-failure` | 004、013、018、021、024 | ACC-BLK-004 | 完成 004 | `HttpProtocolAdapter.cpp`、更新后的 `apps/gateway/src/main.cpp`、相关单测；交付物说明文档 | HTTP unary + async receipt 首版主链可用，且不承诺 WS/MQTT |
| ACC-TODO-027 | NotStarted | 实现 TaskQueryHandler 与 ownership validation path | Access 详设 6.18.2、6.19.2、8.1 | `query_receipt()`、`validate_ownership()`、cancel/query path | L2 | `apps/gateway/src/TaskQueryHandler.cpp`、`access/src/AsyncTaskRegistry.cpp` | `AccessTaskQueryHandlerTest`、`AsyncTaskRegistryOwnershipTest` | `cmake --build build-ci --target dasall_gateway dasall_unit_tests && ctest --test-dir build-ci -R "AccessTaskQueryHandlerTest|AsyncTaskRegistryOwnershipTest" --output-on-failure` | 016、020、022、026 | ACC-BLK-006 | deployment secret 与 ownership token 规则稳定 | `TaskQueryHandler.cpp`、相关单测；交付物说明文档 | 非 owner query/cancel 被拒绝，owner query 返回 pending/completed/expired 明确状态 |
| ACC-TODO-028 | NotStarted | 实现 HealthProbeHandler 与 HTTP 安全头/CORS Gate | Access 详设 6.20、6.20.3 | `/health/live`、`/health/ready`、`/health/startup`、安全头/CORS | L2 | `apps/gateway/src/HealthProbeHandler.cpp`、`apps/gateway/src/HttpProtocolAdapter.cpp` | `AccessHealthProbeHandlerTest`、`AccessHttpSecurityHeadersTest` | `cmake --build build-ci --target dasall_gateway dasall_unit_tests && ctest --test-dir build-ci -R "AccessHealthProbeHandlerTest|AccessHttpSecurityHeadersTest" --output-on-failure` | 004、012、026 | ACC-BLK-004 | 完成 004 | `HealthProbeHandler.cpp`、相关单测；交付物说明文档 | health endpoint 二值返回、不经 Admission，且安全头/CORS 行为受配置白名单约束 |
| ACC-TODO-029 | NotStarted | 实现 DaemonProtocolAdapter 与 apps/daemon 本地控制面组合根 | daemon 本地控制面详设 1.2、6.1、6.2；Access 详设 6.14.9、6.15.3、8.1 | `DaemonProtocolAdapter`、`DaemonBootstrap`、local peer uid trusted boundary | L2 | `access/src/daemon/DaemonProtocolAdapter.cpp`、`apps/daemon/src/DaemonBootstrap.cpp`、`apps/daemon/src/main.cpp` | `DaemonProtocolAdapterTest`、`DaemonProtocolAdapterLocalTrustedTest`、`CliDaemonPingIntegrationTest` | `cmake --build build-ci --target dasall_daemon dasall_unit_tests && ctest --test-dir build-ci -R "DaemonProtocolAdapterTest|DaemonProtocolAdapterLocalTrustedTest|CliDaemonPingIntegrationTest" --output-on-failure` | 013、014、015、017、019、024、037 | ACC-BLK-007 | 完成 037 | `DaemonProtocolAdapter.cpp`、`DaemonBootstrap.cpp`、更新后的 `apps/daemon/src/main.cpp`、相关测试；交付物说明文档 | daemon 作为本地 Access owner 可承接 CLI 请求，local trusted 判定基于 peer identity 且不绕过 Admission |
| ACC-TODO-030 | NotStarted | 实现 SimulatorProtocolAdapter 与 apps/simulator 确定性刺激组合根 | Access 详设 6.14.9、6.15.3、8.1 | `SimulatorProtocolAdapter`、deterministic subject stub | L2 | `apps/simulator/src/SimulatorProtocolAdapter.cpp`、`apps/simulator/src/main.cpp` | `SimulatorProtocolAdapterTest`、`SimulatorProtocolAdapterDeterministicTest` | `cmake --build build-ci --target dasall_simulator dasall_unit_tests && ctest --test-dir build-ci -R "SimulatorProtocolAdapterTest|SimulatorProtocolAdapterDeterministicTest" --output-on-failure` | 013、014、017、019、024 | 无 | — | `SimulatorProtocolAdapter.cpp`、更新后的 `apps/simulator/src/main.cpp`、相关单测；交付物说明文档 | simulator 入口只在测试/工厂 profile 下启用，subject stub 行为可重复 |
...
| ID | 状态 | 任务标题 | 来源依据 | 设计锚点 | 粒度等级 | 代码目标 | 目标函数/接口/数据结构 | 测试目标 | 验收命令 | 前置依赖 | 阻塞项 | 解阻条件 | 交付物 | 完成判定 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| ACC-TODO-037 | NotStarted | 补齐 IIPC peer identity 接缝并固化 LocalPeerUidFact 输入 | daemon 本地控制面详设 6.3.3；Access 详设 6.14.9、6.15.3 | `IIPC::describe_peer()`、`PeerIdentitySnapshot`、`LocalPeerUidFact` | L2 | `platform/include/IIPC.h`、`platform/src/linux/UnixIpcProvider.cpp`、`access/src/daemon/DaemonProtocolAdapter.cpp` | `UnixIpcProviderPeerIdentityTest`、`DaemonProtocolAdapterLocalTrustedTest` | `cmake --build build-ci --target dasall_platform dasall_unit_tests && ctest --test-dir build-ci -R "UnixIpcProviderPeerIdentityTest|DaemonProtocolAdapterLocalTrustedTest" --output-on-failure` | 006 | ACC-BLK-007 | IIPC peer identity public seam 冻结并可返回 uid/gid/pid 事实 | 平台接口补口与验证用例；交付物说明文档 | daemon 本地 trusted 判定不再依赖隐式假设，缺失 peer identity 时可 fail-closed |
| ACC-TODO-038 | NotStarted | 纠正 apps/cli 依赖方向并固定 CLI -> IIPC/UDS -> daemon 路径 | CLI 本地控制面详设 5.1、6.2；Access 详设 6.14.8 | `CliIpcClient`、CLI CMake link set、direct runtime link removal | L2 | `apps/cli/CMakeLists.txt`、`apps/cli/src/main.cpp`、`apps/cli/src/CliIpcClient.cpp` | `CliIpcClientTest`、`CliDaemonPingIntegrationTest` | `cmake --build build-ci --target dasall_cli dasall_daemon dasall_unit_tests && ctest --test-dir build-ci -R "CliIpcClientTest|CliDaemonPingIntegrationTest" --output-on-failure` | 006 | 无 | — | 更新后的 `apps/cli` 依赖与验证用例；交付物说明文档 | CLI target 不 direct link runtime，命令链路通过 daemon/access 主链收敛 |
...
| 阶段 | 任务 | 编排建议 | 说明 |
|---|---|---|---|
| A 补设计与解阻 | 001 ~ 005 | 001 / 002 / 004 可并行；003、005 外部协同阻塞 | 先冻结 seam、config 和 v1 范围，再允许接口和组合根落盘 |
...
|---|---|---|---|---|---|
| ACC-BLK-001 | 详设 12.2-1 | `IAccessRuntimeBridge` 与 runtime 真接口尚未最终冻结 | 001、009、011、020 | runtime-facing public seam 与 sidecar adapter 方案单一化 | Bridge 只允许停留在 design freeze，不宣称 runtime live route ready |
| ACC-BLK-002 | 详设 12.2-2 | `AccessBootstrapConfig` 落盘形态与 source-of-truth 未定 | 002、012、028 | bootstrap config 来源、热更新边界和 snapshot fingerprint 冻结 | `AccessConfigAdapter` 仅能实现 deny-oriented / placeholder view |
| ACC-BLK-003 | 详设 12.2-8 | override / diagnostics artifact selector、大小约束和 transport schema 未与 infra 对齐 | 003、016、035 | infra/config 与 infra/diagnostics 给出 typed schema 与 actor proof 要求 | override / diagnostics 一律 deny-by-default，并审计 |
| ACC-BLK-004 | 详设 12.2-3、12.2-5 | gateway 首版 transport 选型和线程模型未冻结 | 004、026、028 | 明确 HTTP unary + async receipt 的 transport 和线程模型 | gateway 只做选型后的最小范围，不并推 WS/MQTT |
...
| ACC-BLK-004 | 中风险 | 是，对 gateway 主线阻断 | CLI 可先推进，HTTP 需选型后进入 Build |
...
| 延后 Gate | stream / WS / MQTT / diagnostics pull / override schema | Blocked only | 防止未冻结能力被错误标记为 ready |
...
2. 若 gateway transport 选型无法在当前阶段完成，回退策略是先交付 CLI/daemon unary + async receipt，并将 HTTP 维持在组合根占位或最小 stub。
...
当前可直接进入执行的范围是：ACC-TODO-001、002、004、006~034、037、038。ACC-TODO-003 与 ACC-TODO-005 仍属于外部协同阻塞项，ACC-TODO-035 与 ACC-TODO-036 受其收口约束，不能被写成 Done-ready Build 结论。默认先落地 CLI -> IIPC/UDS -> daemon -> access 主链，HTTP/gateway 在 transport 冻结后并行推进。
...
3. L1 / Blocked：StreamGateway、WS/MQTT adapters、override/diagnostics transport schema。

### 11.3 后续建议

1. 先完成 001 / 002 / 004 和 006~012、037、038，保证 Access surface、CMake、discoverability、peer identity seam 与 CLI 依赖方向不再漂移。
...
3. 先打通 CLI/daemon 本地控制面主链，再并行打开 HTTP/gateway 与 simulator；stream / WS / MQTT 与 override / diagnostics 保持 Blocked，直到外部边界冻结。
