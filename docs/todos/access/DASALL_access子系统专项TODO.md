...
| ownership token 的多实例同步 authoritative 结论仍未闭合 | P1 | ACC-TODO-051 | 保持 HMAC-SHA256 + current/previous key window + secret missing fail-closed，不把 focused receipt 结果外推为规模化结论 |
...
| ACC-TODO-025 | Done | 实现 CliIpcClient 与 apps/cli 纯客户端组合根 | CLI 本地控制面详设 1.2、6.2、6.4；Access 详设 6.14.8、8.1 | `CliCommandParser`、`CliIpcClient`、`CliOutputFormatter`、CLI composition root | L2 | `apps/cli/src/CliCommandParser.cpp`、`apps/cli/src/CliIpcClient.cpp`、`apps/cli/src/CliOutputFormatter.cpp`、`apps/cli/src/main.cpp`、`apps/cli/CMakeLists.txt` | `CliIpcClientTest`、`CliIpcClientUnavailableTest`、`CliOutputFormatterTest` | `cmake --build build-ci --target dasall-cli dasall_unit_tests && ctest --test-dir build-ci -R "CliIpcClientTest|CliIpcClientUnavailableTest|CliOutputFormatterTest" --output-on-failure` | 013、018、024、038 | 无 | — | 更新后的 `apps/cli` 代码与 CMake、相关单测；交付物说明文档 | CLI 仅作为本地控制面客户端，不 direct link runtime，所有请求经 IIPC/UDS 进入 daemon/access 主链 |
...
| ACC-TODO-029 | Done | 实现 DaemonProtocolAdapter 与 apps/daemon 本地控制面组合根 | daemon 本地控制面详设 1.2、6.1、6.2；Access 详设 6.14.9、6.15.3、8.1 | `DaemonProtocolAdapter`、`DaemonBootstrap`、local peer uid trusted boundary | L2 | `access/src/daemon/DaemonProtocolAdapter.cpp`、`apps/daemon/src/DaemonBootstrap.cpp`、`apps/daemon/src/main.cpp` | `DaemonProtocolAdapterTest`、`DaemonProtocolAdapterLocalTrustedTest`、`CliDaemonPingIntegrationTest` | `cmake --build build-ci --target dasall-daemon dasall_unit_tests && ctest --test-dir build-ci -R "DaemonProtocolAdapterTest|DaemonProtocolAdapterLocalTrustedTest|CliDaemonPingIntegrationTest" --output-on-failure` | 013、014、015、017、019、024、037 | ACC-BLK-007 | 完成 037 | `DaemonProtocolAdapter.cpp`、`DaemonBootstrap.cpp`、更新后的 `apps/daemon/src/main.cpp`、相关测试；交付物说明文档 | daemon 作为本地 Access owner 可承接 CLI 请求，local trusted 判定基于 peer identity 且不绕过 Admission |
...
| ACC-TODO-033 | Done | 新增 async receipt、ownership、cancel 与本地入口单元门 | Access 详设 6.18、6.19、6.21、9.1 | async receipt、ownership、cancel、daemon/simulator local entry | L2 | `tests/unit/access/AsyncTaskRegistry*.cpp`、`AccessTaskQueryHandlerTest.cpp`、`DaemonProtocolAdapter*.cpp`、`SimulatorProtocolAdapter*.cpp` | async/local-entry unit gates | `cmake --build build-ci --target dasall_unit_tests && ctest --test-dir build-ci -R "AsyncTaskRegistry|ResultReplayCache|AccessTaskQueryHandlerTest|DaemonProtocolAdapter|SimulatorProtocolAdapter" --output-on-failure` | 022、027、029、030、037 | ACC-BLK-006 ✅ 已解阻、ACC-BLK-007 ✅ 已解阻 | HMAC/rotation window 与 peer identity 规则稳定 | async/local-entry unit tests；交付物说明文档 | owner mismatch、expired receipt、cancel not applicable、daemon local trusted、simulator deterministic 行为全部可断言 |
...
| ACC-TODO-044 | Done | P0 | 修复 HTTP adapter 首版 decode/encode 与输入安全 | P0-5、R4 | `apps/gateway/src/HttpProtocolAdapter.*`、gateway submit route | HTTP decode 固定 `entry_type="gateway"`、`protocol_kind="http_unary"`；使用结构化 JSON/parser wrapper；校验 method、path、content-type、body-size、idempotency-key；仅投影白名单 header 到 `request_context` | `HttpProtocolAdapterTest`、`HttpProtocolAdapterErrorMappingTest`、新增 `HttpProtocolAdapterStructuredDecodeTest`、`HttpProtocolAdapterSecurityInputTest`、`GatewaySubmitRouteContractTest` | `cmake --build build/vscode-linux-ninja --target dasall_gateway && ctest --test-dir build/vscode-linux-ninja -R "HttpProtocolAdapterTest|HttpProtocolAdapterErrorMappingTest|HttpProtocolAdapterStructuredDecodeTest|HttpProtocolAdapterSecurityInputTest|GatewaySubmitRouteContractTest|GatewayAccessSubmitCompositionTest|AccessObservabilityMainChainIntegrationTest|AccessPublishFailureAuditTest|AccessHealthReadinessIntegrationTest" --output-on-failure` | 026、039、041 | HTTP 入口生成的 packet 可通过 validator/normalizer；转义、嵌套、超限、非法 content-type 与 header 注入均 fail-closed；交付物：[ACC-TODO-044-HttpProtocolAdapter-structured-decode与输入安全收敛.md](/home/gangan/DASALL/docs/todos/access/deliverables/ACC-TODO-044-HttpProtocolAdapter-structured-decode与输入安全收敛.md) |
| ACC-TODO-045 | Done | P1 | 实现 AccessConfigAdapter 生产级 profile/config 投影 | P1-1、R3 | `access/src/AccessConfigAdapter.*`、profiles/config bridge | 将 bootstrap config 与 profile/policy snapshot 投影为 immutable `AccessAuthView`、`AccessAdmissionView`、`AccessPublishView`、`AccessRuntimeGovernanceView`；支持 fingerprint、last-known-good、hot update invalidation；非法 schema fail init | 新增 `AccessConfigAdapterProjectionTest`、`AccessConfigAdapterInvalidSchemaTest`、`AccessProfileCompatibilityTest` | `cmake --build build/vscode-linux-ninja --target dasall_access_config_adapter_projection_unit_test dasall_access_config_adapter_invalid_schema_unit_test dasall_access_daemon_profile_compatibility_integration_test && ctest --test-dir build/vscode-linux-ninja -R "AccessConfigAdapterProjectionTest|AccessConfigAdapterInvalidSchemaTest|AccessProfileCompatibilityTest" --output-on-failure` | 002、012、039 | 五档 baseline profile 下投影稳定；非法 bootstrap/runtime snapshot 不会使用散落默认值进入 Ready；交付物：[ACC-TODO-045-AccessConfigAdapter-生产级profile-config投影.md](/home/gangan/DASALL/docs/todos/access/deliverables/ACC-TODO-045-AccessConfigAdapter-生产级profile-config投影.md) |
| ACC-TODO-046 | Done | P1 | 将 async ownership token 改为正式 HMAC 与 secret rotation | P1-2、R3 | `access/src/AsyncTaskRegistry.*`、infra/secret seam | 使用 infra/secret deployment secret；token 包含 `key_id/issued_at/expires_at/receipt_id/actor_ref/request_id`；HMAC-SHA256 + base64url；constant-time compare；支持 current/previous key 验证；缺 secret 时禁用 async 或 init fail-closed | 新增 `AsyncTaskRegistryHmacOwnershipTest`、`AsyncTaskRegistrySecretRotationTest`、`AsyncTaskRegistryMissingSecretFailClosedTest` | `cmake --build build/vscode-linux-ninja --target dasall_access_async_task_registry_ownership_unit_test dasall_access_async_task_registry_missing_secret_fail_closed_unit_test && ctest --test-dir build/vscode-linux-ninja -R "AsyncTaskRegistryHmacOwnershipTest|AsyncTaskRegistrySecretRotationTest|AsyncTaskRegistryMissingSecretFailClosedTest" --output-on-failure` | 022、039 | ACC-BLK-006 ✅ 已解阻 | 不再使用 `std::hash` 或静态默认 secret；current/previous key 验证、missing secret fail-closed 与 query/receipt 兼容路径已自动验证；交付物：[ACC-TODO-046-Async-ownership-token正式HMAC与secret-rotation.md](/home/gangan/DASALL/docs/todos/access/deliverables/ACC-TODO-046-Async-ownership-token正式HMAC与secret-rotation.md) |
| ACC-TODO-047 | Done | P1 | 引入 IAccessPolicyEvaluator 并接入 infra/policy 生产适配 | P1-3、R3 | `access/src/AccessPolicyGate.*`、infra/policy adapter | 定义 `IAccessPolicyEvaluator` seam；production 实现调用 infra/policy；snapshot backend 仅保留为 unit fake；policy input 包含 subject/channel/environment/operation/target/fingerprint；backend unavailable fail-closed | 新增 `AccessPolicyEvaluatorIntegrationTest`、`AccessPolicyBackendUnavailableIntegrationTest`、`AccessPolicyInputAttributeTest` | `cmake --build build/vscode-linux-ninja --target dasall_access_policy_evaluator_integration_test dasall_access_observability_main_chain_integration_test dasall_access_policy_input_attribute_unit_test dasall_access_policy_gate_unit_test dasall_access_policy_backend_failure_unit_test dasall_access_policy_override_gate_unit_test && ctest --test-dir build/vscode-linux-ninja -R "AccessPolicyEvaluatorIntegrationTest|AccessPolicyBackendUnavailableIntegrationTest|AccessObservabilityMainChainIntegrationTest|AccessPolicyInputAttributeTest|AccessPolicyGateTest|AccessPolicyBackendFailureTest|AccessPolicyOverrideGateTest" --output-on-failure` | 016、039、045 | deny-by-default 与 per-request authorization 已在生产 seam 中成立；策略失败 fail-closed 已具 focused integration 证据；交付物：[ACC-TODO-047-AccessPolicyGate-IAccessPolicyEvaluator与infra-policy生产适配.md](/home/gangan/DASALL/docs/todos/access/deliverables/ACC-TODO-047-AccessPolicyGate-IAccessPolicyEvaluator与infra-policy生产适配.md) |
...
| D R3 P1 安全治理 | 045 → 046 → 047 → 048 | config 投影、HMAC ownership 与 policy evaluator 已闭合；observability reject trace / 更广主链字段仍待补齐 | 当前 `Gate-INT-08` 只证明最小 focused 安全边界已可执行，不外推为全部安全治理已闭合 |
...
| ACC-BLK-006 | 已解阻（2026-05-14）：046 已将 ownership token 收口为 HMAC-SHA256 + base64url，并固定 current/previous key 验证、deployment secret materialization 与 secret missing fail-closed；多实例 authoritative sync 继续保留在风险口径，不外推规模化 receipt 结论 | 051 | 无；后续仅需在 051/release polish 中继续约束 multi-instance sync 结论不越权 | 证据回链到 `access/src/AsyncTaskRegistry.cpp`、`access/src/AccessGatewayFactory.cpp` 与 [docs/todos/access/deliverables/ACC-TODO-046-Async-ownership-token正式HMAC与secret-rotation.md](/home/gangan/DASALL/docs/todos/access/deliverables/ACC-TODO-046-Async-ownership-token正式HMAC与secret-rotation.md) |
...
| ACC-BLK-010 | 交付评审 P1-3/P1-4 | observability 与 ownership 的更广生产级安全治理矩阵仍未完全闭合；当前 `Gate-INT-08` 只把 focused ingress、policy fail-closed 与 ownership fail-closed 证据收敛为最小可执行边界 | 048、051 | ACC-TODO-048 完成，并形成更广安全治理矩阵 | async receipt/query/cancel 与当前 policy focused gate 只能作为 focused functionality / fail-closed 证据，不得外推为全部安全可交付 |
...
| ACC-BLK-006 | 已解阻（046 收口） | 否；focused receipt/query/cancel 路径已有 HMAC/rotation/fail-closed 证据 | 多实例 authoritative sync 仍仅作为 051 风险口径，不外推规模化 ready |
...
| ACC-BLK-010 | 中风险 | 否，但阻断安全可交付结论 | P0 unary focused gate 与 production policy seam 已闭合，最终更广安全可交付仍需 048、051 继续收口 |
...
| ACC-R12 | 交付评审 P1-2 | ownership token 的多实例同步或 rotation window 退化回弱保证 | 把 focused HMAC/rotation 结果外推为规模化 authoritative 结论，或 secret missing 时继续放出 accepted async | 保持 HMAC-SHA256 + base64url + current/previous key window；多实例 sync 仅作为 051 风险收口，不提前宣称 ready |
...
1. 继续按 `observability reject trace / publish evidence -> release polish` 的顺序推进 ACC-TODO-048、051。
