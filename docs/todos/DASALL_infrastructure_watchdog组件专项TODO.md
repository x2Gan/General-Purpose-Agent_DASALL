...
| 证据对象 | 当前状态 | 结论 |
|---|---|---|
| infra/CMakeLists.txt | 仅 src/placeholder.cpp | watchdog 尚未接入构建 |
| infra/include/ | 空目录 | watchdog 对外接口未落盘 |
| infra/src/ | 无 watchdog 子目录实现 | watchdog 主链路未落盘 |
| tests/CMakeLists.txt | 仅 mocks/unit/contract | integration 顶层未接入 |
| tests/unit/CMakeLists.txt | 未接入 infra 子目录 | watchdog unit 发现性缺失 |
...
| RecoveryHintRequest | watchdog 设计 6.5/6.8；ADR-007 | L2 | 建议语义边界明确 | runtime 输入结构冻结不足 | 先冻结对象，再做边界 contract |
...
| tests/integration/failure/profile | watchdog 设计 9.1/9.2；代码现状 | L0 | 用例与 gate 已给出 | tests 顶层 integration 未接线 | 先解阻测试拓扑，再推进 |
...
| integration/failure/profile 测试注册 | 9.1/9.2；现状 | 测试与门禁 | WDG-TODO-022 | tests 顶层 integration 解阻后推进 |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| WDG-TODO-001 | Not Started | 定义 IWatchdogService 接口头文件 | watchdog 设计 6.6/6.7；编码规范 3.7 | 6.6 IWatchdogService | L3 | infra/include/watchdog/IWatchdogService.h | init/start/stop/register_entity/unregister_entity/heartbeat/snapshot | unit：接口可编译；contract：返回 ResultCode/ErrorInfo 可映射 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci --target dasall_infra | 无 | 无 | 无 | 接口头文件、编译记录 | 仅当方法集合与 6.6 一致且无跨模块实现依赖时完成 |
| WDG-TODO-002 | Not Started | 定义 IHeartbeatSource 接口头文件 | watchdog 设计 6.6 | 6.6 IHeartbeatSource | L3 | infra/include/watchdog/IHeartbeatSource.h | emit_heartbeat/describe | unit：接口编译测试；contract：实体描述不越权扩写 contracts | cmake -S . -B build-ci -G Ninja && cmake --build build-ci --target dasall_infra | 无 | 无 | 无 | 接口头文件、编译记录 | 仅当心跳源最小接口冻结并可被 Facade 引用时完成 |
| WDG-TODO-003 | Not Started | 定义 ITimeoutPolicy 接口头文件 | watchdog 设计 6.6/6.8 | 6.6 ITimeoutPolicy | L3 | infra/include/watchdog/ITimeoutPolicy.h | evaluate(candidate,history) | unit：接口编译测试；unit：输入输出对象可绑定 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci --target dasall_infra | 无 | 无 | 无 | 接口头文件、编译记录 | 仅当判级接口独立于执行链路并可编译时完成 |
| WDG-TODO-004 | Not Started | 定义 WatchedEntityDescriptor 数据结构 | watchdog 设计 6.5 | 6.5 WatchedEntityDescriptor | L3 | infra/include/watchdog/WatchedEntityDescriptor.h | entity_id/entity_type/owner_module/criticality/timeout_ms/grace_ms | unit：字段完整性与唯一性前置校验 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci && ctest --test-dir build-ci --output-on-failure -L unit | 无 | 无 | 无 | 头文件、单测 | 仅当字段与设计表一致且唯一性规则可测时完成 |
| WDG-TODO-005 | Not Started | 定义 HeartbeatSample 数据结构 | watchdog 设计 6.5/6.3 | 6.5 HeartbeatSample | L3 | infra/include/watchdog/HeartbeatSample.h | entity_id/heartbeat_ts/deadline_ts/seq_no | unit：乱序样本识别与旧样本判定辅助校验 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci && ctest --test-dir build-ci --output-on-failure -L unit | WDG-TODO-004 | 无 | 无 | 头文件、单测 | 仅当字段完整且可支撑乱序与过旧语义校验时完成 |
| WDG-TODO-006 | Not Started | 定义 TimeoutDecision 数据结构 | watchdog 设计 6.5/6.8 | 6.5 TimeoutDecision | L3 | infra/include/watchdog/TimeoutDecision.h | entity_id/timeout_level/consecutive_miss/reason_code/evidence_ref | unit：等级枚举与连续失败字段可校验；contract：reason_code 映射边界 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci && ctest --test-dir build-ci --output-on-failure -L "unit|contract" | WDG-TODO-005 | 无 | 无 | 头文件、unit/contract 测试 | 仅当等级与原因码语义可二值验证时完成 |
| WDG-TODO-007 | Not Started | 定义 WatchdogSnapshot 数据结构 | watchdog 设计 6.5/6.7 | 6.5 WatchdogSnapshot | L3 | infra/include/watchdog/WatchdogSnapshot.h | total_entities/timed_out_entities/degraded_entities/scan_lag_ms/ts | unit：快照字段一致性与版本单调语义校验 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci && ctest --test-dir build-ci --output-on-failure -L unit | WDG-TODO-004 | 无 | 无 | 头文件、单测 | 仅当快照字段完整且可被 snapshot() 接口消费时完成 |
| WDG-TODO-008 | Not Started | 定义 RecoveryHintRequest 边界对象 | watchdog 设计 6.5/6.8；ADR-007 | 6.5 RecoveryHintRequest | L2 | infra/include/watchdog/RecoveryHintRequest.h | reason_code/target_ref/suggested_action/evidence_ref | contract：不含执行句柄字段；unit：字段完整性 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci --target dasall_contract_tests && ctest --test-dir build-ci --output-on-failure -L contract | WDG-TODO-006 | WDG-BLK-03 | runtime 恢复建议输入结构冻结 | 头文件、contract 测试 | 仅当 contract 能阻断执行字段进入对象时完成 |
| WDG-TODO-009 | Not Started | 实现 WatchdogServiceFacade 生命周期骨架 | watchdog 设计 6.2/6.7 | 6.2 WatchdogServiceFacade；6.7 主流程 | L3 | infra/src/watchdog/WatchdogServiceFacade.cpp | init/start/stop/snapshot | unit：未初始化、已启动、优雅停机路径 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci --target dasall_infra && ctest --test-dir build-ci --output-on-failure -L unit | WDG-TODO-001、004、007 | 无 | 无 | Facade 骨架、单测 | 仅当生命周期路径均可判定成功/失败且不越权到恢复执行时完成 |
| WDG-TODO-010 | Not Started | 实现 HeartbeatRegistry 注册治理骨架 | watchdog 设计 6.2/6.3 | 6.2 HeartbeatRegistry | L3 | infra/src/watchdog/HeartbeatRegistry.cpp | register_entity/unregister_entity/query_entity | unit：重复注册拒绝、未找到注销、上限检查 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci && ctest --test-dir build-ci --output-on-failure -L unit | WDG-TODO-004、009 | 无 | 无 | Registry 骨架、单测 | 仅当重复注册返回 INF_E_WATCHDOG_ENTITY_DUPLICATE 且测试通过时完成 |
| WDG-TODO-011 | Not Started | 实现 HeartbeatIngestor 采集骨架 | watchdog 设计 6.2/6.3/6.8 | 6.2 HeartbeatIngestor；6.3 输入输出 | L3 | infra/src/watchdog/HeartbeatIngestor.cpp | ingest(sample) | unit：乱序与过旧样本处理；failure：心跳风暴下丢弃策略可观测 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci && ctest --test-dir build-ci --output-on-failure -L unit | WDG-TODO-005、010 | 无 | 无 | Ingestor 骨架、单测 | 仅当 stale 样本可判定并映射 INF_E_WATCHDOG_HEARTBEAT_STALE 时完成 |
| WDG-TODO-012 | Blocked | 实现 DeadlineWheel 扫描骨架 | watchdog 设计 6.2/6.7；11.1 | 6.2 DeadlineWheel；6.7 步骤 4 | L2 | infra/src/watchdog/DeadlineWheel.cpp | tick_collect_due(now_ts)/scan_once() | unit：到期候选筛选；failure：扫描超期识别 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci && ctest --test-dir build-ci --output-on-failure -L unit | WDG-TODO-005、010 | WDG-BLK-01 | platform monotonic clock + scheduler 抽象冻结 | 扫描骨架或阻塞记录 | 仅当时钟与调度抽象冻结后，任务从 Blocked 转 Not Started |
| WDG-TODO-013 | Not Started | 实现 TimeoutPolicyEngine 判级骨架 | watchdog 设计 6.2/6.8/6.9 | 6.2 TimeoutPolicyEngine | L3 | infra/src/watchdog/TimeoutPolicyEngine.cpp | evaluate(candidate,history) | unit：grace 窗口与连续失败阈值；failure：warning->critical 升级 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci && ctest --test-dir build-ci --output-on-failure -L unit | WDG-TODO-003、006、011 | 无 | 无 | 策略骨架、单测 | 仅当判级结果可重复验证且升级路径符合策略表时完成 |
...
| WDG-TODO-015 | Not Started | 实现 WatchdogAuditBridge 审计桥接骨架 | watchdog 设计 6.2/6.8/6.10 | 6.2 WatchdogAuditBridge | L3 | infra/src/watchdog/WatchdogAuditBridge.cpp | write_timeout_audit(decision) | unit：critical/fatal 强制审计；failure：审计写失败可观测 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci && ctest --test-dir build-ci --output-on-failure -L unit | WDG-TODO-006 | 无 | 无 | 审计桥接骨架、单测 | 仅当关键超时不会静默丢审计且失败可计数时完成 |
| WDG-TODO-016 | Not Started | 实现 WatchdogMetricsBridge 指标桥接骨架 | watchdog 设计 6.2/6.10 | 6.2 WatchdogMetricsBridge；6.10 指标清单 | L3 | infra/src/watchdog/WatchdogMetricsBridge.cpp | record_timeout/record_scan_lag/record_publish_fail | unit：指标名与标签集合一致性；failure：safe_mode 计数 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci && ctest --test-dir build-ci --output-on-failure -L unit | WDG-TODO-007、013 | 无 | 无 | 指标桥接骨架、单测 | 仅当 7 项核心指标均有对应采样入口且测试通过时完成 |
| WDG-TODO-017 | Blocked | 实现 RecoveryRequestEmitter 边界守卫骨架 | watchdog 设计 6.2/6.8；ADR-007；11.1 | 6.2 RecoveryRequestEmitter | L2 | infra/src/watchdog/RecoveryRequestEmitter.cpp | emit_recovery_hint(decision)/sanitize_payload() | contract：建议与执行分离；unit：evidence_ref 完整性 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci --target dasall_contract_tests && ctest --test-dir build-ci --output-on-failure -L contract | WDG-TODO-008、013 | WDG-BLK-03 | runtime 恢复建议输入结构与边界 contract 模板冻结 | 发射器骨架或阻塞记录 | 仅当 contract 模板可阻断执行字段后可解阻 |
...
| WDG-TODO-019 | Not Started | 定义 WatchdogErrors 私有错误码域与映射 | watchdog 设计 6.6/9.1；编码规范 3.6 | 6.6 错误语义 | L3 | infra/include/watchdog/WatchdogErrors.h | 7 个 INF_E_WATCHDOG_* 错误码与映射入口 | contract：映射 contracts::ResultCode 稳定；unit：枚举稳定 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci --target dasall_contract_tests && ctest --test-dir build-ci --output-on-failure -L contract | WDG-TODO-001、006 | 无 | 无 | 错误码头文件、映射测试 | 仅当 7 个错误码均可追溯到设计条目且 contract 测试通过时完成 |
...
| WDG-TODO-022 | Blocked | 注册 watchdog integration/failure/profile 测试入口 | watchdog 设计 9.1/9.2；代码现状 | 9.1 Integration/Failure/Compatibility | L0 | tests/CMakeLists.txt、tests/integration/infra/watchdog/、tests/stress/ | integration：超时闭环；failure：发布失败/扫描滞后/心跳风暴；profile：差异一致性 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci && ctest --test-dir build-ci -N | WDG-TODO-020、021 | WDG-BLK-05 | tests 顶层 integration 接线与标签规范冻结 | 测试注册改动或阻塞记录 | 仅当 tests 顶层接入 integration 并可发现 watchdog 测试后可解阻 |
| WDG-TODO-023 | Not Started | 回写 watchdog 门禁结果与交付证据 | watchdog 设计 9.2/11.1；规范 6.2 | 9.2 Gate；11 风险阻塞 | L2 | docs/todos/DASALL_infrastructure_watchdog组件专项TODO.md | process test：门禁状态、阻塞变化、回退执行证据 | ctest --test-dir build-ci -N && ctest --test-dir build-ci --output-on-failure -L unit && ctest --test-dir build-ci --output-on-failure -L contract | WDG-TODO-021 | 无 | 无 | 更新后的 TODO 文档证据段 | 仅当每个 Gate 有通过/失败结论及对应命令证据时完成 |
...
| E 集成与兼容门禁 | WDG-TODO-022 | 串行 | integration 拓扑解阻后再推进 |
...
|---|---|---|---|---|
| WDG-BLK-01 | platform monotonic clock 与 scheduler 抽象未冻结 | WDG-TODO-012 | platform 给出稳定时钟/定时接口 | 先冻结最小时钟与调度接口文档并回链到 watchdog |
| WDG-BLK-02 | 事件总线最小发布接口未统一 | WDG-TODO-014 | publish_timeout 的输入输出与失败语义冻结 | 先定义最小 publish 接口与失败返回码 |
| WDG-BLK-03 | runtime 恢复建议输入结构与 contract 模板未冻结 | WDG-TODO-008、017 | RecoveryHintRequest 边界模板冻结 | 在 contract 测试中新增建议-执行分离模板 |
| WDG-BLK-04 | profile 中 watchdog 键名与覆盖优先级未冻结 | WDG-TODO-018 | infra.watchdog.* 键命名冻结并评审通过 | 先补 profile 键名规范，再落配置合并逻辑 |
| WDG-BLK-05 | tests 顶层 integration 未接线 | WDG-TODO-022 | tests/CMakeLists.txt 纳入 integration 并固化标签 | 新增 add_subdirectory(integration) 与标签规范 |
...
1. integration/failure/profile 门禁在 WDG-BLK-05 解阻前不作为必过基线。
