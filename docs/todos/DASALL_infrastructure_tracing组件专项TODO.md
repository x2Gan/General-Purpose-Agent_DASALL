...
11. docs/todos/DASALL_infrastructure子系统专项TODO.md
12. docs/todos/DASALL_infrastructure_logging组件专项TODO.md
...
|---|---|---|---|---|
| TRC-C001 | 架构 3.4.7/5.10；tracer 设计 2.1 | Must | infra 必须提供 trace 能力并与 logging/metrics/audit 协同 | 任务必须覆盖主链路、桥接与可观测 |
...
| TRC-C004 | ADR-006 | Must-Not | tracer 不接管上下文语义装配与 Prompt 渲染 | ContextPropagation 仅做标识注入/提取 |
| TRC-C005 | ADR-007 | Must-Not | tracer 不做失败语义判定与恢复准入 | 仅记录失败证据并输出降级状态 |
| TRC-C006 | ADR-008 | Must | tracer 不拥有调度权，仅记录 orchestrator/coordinator/worker 链路 | 审计与追踪任务只做证据记录 |
| TRC-C007 | contracts 冻结计划；tracer 设计 5.3/6.5 | Must-Not | 不把采样器、批处理、导出协议策略写入 contracts | 所有实现策略留在 infra 私有对象 |
...
| TRC-C010 | tracer 设计 6.9 | Must | 配置支持默认/Profile/部署/运行时覆盖 | 配置任务必须明确层级覆盖与回退策略 |
...
1. 已有明确核心接口清单：ITracerProvider、ITracer、ISpan、ITraceContextPropagator、ISpanExporter、ITraceSampler（tracer 设计 6.6）。
2. 已有核心对象字段：TraceContext、SpanDescriptor、SpanEndResult、SamplingDecision、ExportBatchReport、TraceModuleSnapshot（tracer 设计 6.5）。
3. 已有主流程与异常流程：正常流程 7 步、异常分类 4 类与恢复动作（tracer 设计 6.7/6.8）。
4. 已有错误码域：TRC_E_PROVIDER_NOT_READY、TRC_E_INVALID_CONTEXT、TRC_E_QUEUE_FULL、TRC_E_EXPORT_TIMEOUT、TRC_E_EXPORT_FAILURE、TRC_E_SHUTDOWN_TIMEOUT、TRC_E_CONFIG_INVALID（tracer 设计 6.6）。
5. 已有落盘建议与测试出口：infra/include/tracing、infra/src/tracing、tests/unit/infra/tracing、tests/integration/infra/tracing（tracer 设计 8.1、7）。
...
|---|---|---|---|---|---|
| ITracerProvider | tracer 设计 6.6 | L3 | 方法名、职责、flush/shutdown 语义完整 | 返回状态对象细节未与 contracts 映射成文 | 直接拆接口冻结 + 最小实现 |
| ITracer | tracer 设计 6.6/6.7 | L3 | start_span/with_active_span/current_context 语义完整 | with_active_span 回调异常封装策略 | 直接拆接口 + 生命周期测试 |
| ISpan | tracer 设计 6.6/6.5 | L3 | set_attribute/add_event/set_status/end/get_context 语义完整 | attrs value 类型约束细则 | 直接拆接口 + Span 行为测试 |
| ITraceContextPropagator | tracer 设计 6.6/6.3 | L3 | inject/extract 明确，invalid/noop 语义明确 | carrier 统一容器类型未冻结 | 先冻结接口，carrier 以前置声明占位 |
| TraceContext | tracer 设计 6.5 | L3 | 字段与 invalid/noop 约束明确 | trace_state 编码规范细节缺失 | 直接拆数据结构 |
| SamplingPolicyEngine | tracer 设计 6.2/6.3/6.9 | L3 | 采样模式、输入输出、配置键完整 | 远程采样不在首版范围 | 直接拆函数级任务（本地采样） |
| SpanProcessorPipeline | tracer 设计 6.2/6.3/6.7 | L2 | OnStart/OnEnd 管线职责、非阻塞约束明确 | 处理链插件注册机制未展开 | 先做单链路实现，插件化后置 |
| BatchSpanBuffer | tracer 设计 6.2/6.8/6.9 | L3 | queue/批量/延迟/溢出策略完整 | 线程模型参数细节缺失 | 直接拆队列策略任务 |
| SpanExporterAdapter | tracer 设计 6.2/6.3/6.9 | L2 | noop/file/otlp 预留与失败语义明确 | OTLP 首版是否启用未冻结 | 先做 noop/file，OTLP 标记 Blocked |
| TraceMetricsBridge | tracer 设计 6.2/6.10 | L1 | 指标名清单完整 | metrics 接口签名未冻结 | 标记 Blocked，先补桥接接口设计 |
| TraceAuditBridge | tracer 设计 6.2/6.10 | L1 | 审计事件清单明确 | audit 写入接口签名未冻结 | 标记 Blocked，先补桥接接口设计 |
| tests/integration tracing 注册点 | tracer 设计 8.1/9.1；tests 现状 | L0 | 设计建议存在 | tests 顶层无 integration 注册路径 | 先补测试拓扑后再拆集成任务 |
...
|---|---|---|---|---|
| 对外接口层 ITracerProvider/ITracer/ISpan/Propagator | tracer 设计 6.6 | 接口 | TRC-TODO-001、TRC-TODO-002、TRC-TODO-003、TRC-TODO-004 | 先冻结调用面，阻止上层直连实现细节 |
| 核心对象与错误码域 | tracer 设计 6.5/6.6 | 数据结构/错误处理 | TRC-TODO-005、TRC-TODO-006、TRC-TODO-007 | 先固化语义对象与错误码，再推进实现 |
| 生命周期主链路 | tracer 设计 6.7 | 流程 | TRC-TODO-008、TRC-TODO-009 | provider/tracer/span 分拆成单目标任务 |
| 上下文传播 | tracer 设计 6.3/6.6/6.8 | 适配器 | TRC-TODO-010 | 聚焦 inject/extract 与 invalid/noop 语义 |
| 采样策略 | tracer 设计 6.2/6.3/6.9 | 流程/配置 | TRC-TODO-011 | 先落本地采样策略，不推进远程采样 |
| 批处理导出管线 | tracer 设计 6.2/6.7/6.8/6.9 | 流程 | TRC-TODO-012、TRC-TODO-013 | queue 与 exporter 分任务，避免过载 |
| 异常降级与可观测 | tracer 设计 6.8/6.10 | 错误处理/门禁 | TRC-TODO-014、TRC-TODO-015 | 失败注入与降级路径独立验收 |
| 配置与 Profile 裁剪 | tracer 设计 6.9；蓝图 5 | 配置 | TRC-TODO-016 | 键名、默认值、覆盖层级明确落盘 |
| CMake 与测试门禁 | tracer 设计 7/8/9；当前代码现状 | 测试/门禁 | TRC-TODO-017、TRC-TODO-018、TRC-BLK-003 | 构建接线与 unit/contract 可先做，integration 先解阻 |
| metrics/audit 桥接 | tracer 设计 6.10 | 桥接/门禁 | TRC-BLK-001、TRC-BLK-002 | 外部接口未冻结，必须先补设计 |
| OTLP 导出 | tracer 设计 6.9/7/11 | 导出 | TRC-BLK-004 | 首版导出器选型未冻结，不默认推进 |
...
| TRC-TODO-021 | Not Started | ARC-02 | 将 tracing 任务纳入仓库级 Blocked-first gate 流程，禁止在 Blocked 未解时推进实现任务 | scripts/ci/infra_gate.sh, docs/todos/DASALL_infrastructure_tracing组件专项TODO.md | process test：默认执行 gate 时存在 Blocked 即失败；解阻窗口需显式 ALLOW_BLOCKED=1 | bash scripts/ci/infra_gate.sh | TRC-TODO-018 | 仅当 tracing 执行流程与 gate 绑定，并形成可重复执行记录时完成 |
