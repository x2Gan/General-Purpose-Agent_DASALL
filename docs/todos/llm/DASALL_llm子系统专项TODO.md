...
当前状态更新（2026-05-17 / LLM-FIX-007）：4.3 的“缺失证据”字段仍只用于解释专项 TODO 启动期为何按 001~042 排布，不代表当前实现缺口。`LLMBoundaryGuardComplianceTest` 已显式守住 ADR-006/007/008 的 source boundary；本轮又新增 [docs/todos/llm/deliverables/LLM-FIX-007-external-provider长稳态与L6-soak执行计划.md](deliverables/LLM-FIX-007-external-provider%E9%95%BF%E7%A8%B3%E6%80%81%E4%B8%8EL6-soak%E6%89%A7%E8%A1%8C%E8%AE%A1%E5%88%92.md)，把 provider jitter、network loss、secret rotate、retry budget exhaustion 与 observability trend 拆成可执行验收项。因此 llm owner 当前不再缺 soak owner / plan；当前剩余跨链执行项继续收敛为 `FULLINT-TODO-019` 的 current release candidate rerun，以及按 007 deliverable 执行的 external provider soak slices。
