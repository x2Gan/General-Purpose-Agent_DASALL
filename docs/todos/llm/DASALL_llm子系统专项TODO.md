...
最近更新时间：2026-05-16（完成 LLM-TODO-001/002/003/004/005/006/007/008/009/010/011/012/013/014/015/016/017/018/019/020/021/022/023/024/025/026/027/028/029/030/031/032/033/034/035/038/039/040/041/042/043，且 LLM-FIX-001 已在 module-local 边界内完成 streaming 生命周期实现与 focused tests；阶段 B 公共接口冻结、阶段 C 测试夹具升级、阶段 D Prompt/Provider 资产基线、阶段 E PromptRegistry/TokenEstimator/TemplateRenderer/PromptComposer/PromptPolicy/PromptPipeline、阶段 F ModelRouter/AdapterRegistry/CallExecution/ResponseNormalizer/UsageAggregator/LLMManager、阶段 G Provider 注入/adapter skeleton/observability、阶段 H 关键 integration、阶段 I Gate 证据收口与阶段 I+ 评审缺陷修复已落盘）
阶段：Detailed Design -> Special TODO
适用范围：llm/
当前结论：阶段 I Gate 证据已闭合，阶段 I+ 的 LLM-TODO-043 已完成，阶段 J 的 LLM-TODO-036 先完成 streaming 生命周期设计冻结，后续又由 2026-05-16 的 LLM-FIX-001 在不新增 shared `StreamHandle` / 不修改 `contracts/` 的前提下落地 module-local `StreamSessionRegistry`、manager streaming 执行链、OpenAI-compatible SSE/delta merge 与 focused tests；LLM-TODO-037 已完成 shared supporting object admission 评审并给出 No-Go for shared 结论。module-local 公共接口、Prompt/Provider 资产、Prompt 三段治理、路由与 unary 主链路已完成，manager 输入边界、OverBudget/RequireRecompose 回流语义以及 streaming preference 错误映射均已经 unit / contract / integration 回归验证。shared ModelRoute / PromptPolicyDecision / StreamHandle 继续保持 module-local/deferred，不新增 shared 头文件；后续若重启 admission，必须重新提交消费者矩阵、迁移窗口、compatibility contract tests 与 Go/No-Go 评审。
...
当前状态更新（2026-05-20 / LLM-GAP-004）：4.3 的“缺失证据”字段仍只用于解释专项 TODO 启动期为何按 001~042 排布，不代表当前实现缺口。`LLMBoundaryGuardComplianceTest` 已显式守住 ADR-006/007/008 的 source boundary；[docs/todos/llm/deliverables/LLM-FIX-007-external-provider长稳态与L6-soak执行计划.md](deliverables/LLM-FIX-007-external-provider%E9%95%BF%E7%A8%B3%E6%80%81%E4%B8%8EL6-soak%E6%89%A7%E8%A1%8C%E8%AE%A1%E5%88%92.md) 已把 provider jitter、network loss、secret rotate、retry budget exhaustion 与 observability trend 拆成可执行验收项；[docs/todos/llm/deliverables/LLM-GAP-004-local-installed-soak-blocker-closeout.md](deliverables/LLM-GAP-004-local-installed-soak-blocker-closeout.md) 保留 2026-05-19 历史 blocker ledger；本轮新增 [docs/todos/llm/deliverables/LLM-GAP-004-local-installed-positive-closeout.md](deliverables/LLM-GAP-004-local-installed-positive-closeout.md)，记录用户 smoke 与 5 次本机 installed run 均返回 completed / `llm.origin=deepseek-prod/` / 无 `agent.dataset`。因此 llm owner 当前不再缺 soak owner / plan、blocker ledger 或本机 installed positive evidence；L6 soak 仍只按 007 的 SOAK-01~05 后续升级。
...
| LLM-TODO-042 | Done | 验证 asset-only Provider instance onboarding integration | 详细设计 6.10.1、6.14、9.3；LLM-C023 | 6.14 新 Provider 接入流程；9.3 integration 路径 | L2 | tests/integration/llm/LLMProviderAssetOnboardingIntegrationTest.cpp | 对既有 adapter family，仅新增 provider package、auth_ref、profile route 即可使 provider instance 被选中，无需新增 adapter 代码 | integration：LLMProviderAssetOnboardingIntegrationTest 通过 | `cmake -S . -B build-ci -G "Unix Makefiles" && cmake --build build-ci --target dasall_integration_tests && ctest --test-dir build-ci -R LLMProviderAssetOnboardingIntegrationTest --output-on-failure` | LLM-TODO-014、LLM-TODO-020、LLM-TODO-025、LLM-TODO-029、LLM-TODO-041 | LLM-BLK-007 | ProviderConfig 投影与既有 family adapter skeleton 已可复用 | asset-only onboarding integration 用例 | 仅当既有 family 下的 provider instance 能通过“只加资产 + profile route”路径被发现并启用，且 profile 未声明时不会被隐式激活时完成 |
| LLM-TODO-036 | Done | 补齐 streaming 生命周期设计并落地 llm internal 实现 | 详细设计 7.2、10.1、12.1；v1.0 指引 | 7.2 shared StreamHandle 缺失；10.1 streaming 风险 | L0 | 无新增 shared 头文件；保持 llm/include/stream/StreamSessionRef.h 为 module-local；已落地 `StreamSessionRegistry`、`LLMManager::stream_generate()`、`OpenAICompatibleAdapter::stream_generate()` 与 focused tests | StreamSessionRef、stream_generate 生命周期、bounded session / cancel / cleanup | unit/integration：`StreamSessionLifecycleTest`、`LLMStreamingIntegrationTest`、`CognitionLlmBridgeErrorMappingTest` 覆盖 registry/adapter/manager/bridge 路径 | `Build_CMakeTools(buildTargets=["dasall_stream_session_lifecycle_unit_test","dasall_cognition_llm_bridge_error_mapping_unit_test","dasall_llm_streaming_integration_test"])`；`RunCtest_CMakeTools(tests=["StreamSessionLifecycleTest","LLMStreamingIntegrationTest","CognitionLlmBridgeErrorMappingTest"])` | LLM-TODO-011、LLM-TODO-024 | 无 | LLM-BLK-005 已通过 streaming lifecycle 设计评审与 focused implementation 解阻；shared `StreamHandle` admission 仍继续后置 | docs/todos/llm/deliverables/LLM-TODO-036-streaming生命周期设计并后置实现设计收敛.md、docs/architecture/DASALL_llm子系统详细设计.md、llm/src/stream/StreamSessionRegistry.cpp、llm/src/LLMManager.cpp、llm/src/adapters/OpenAICompatibleAdapter.cpp、tests/unit/llm/StreamSessionLifecycleTest.cpp、tests/integration/llm/LLMStreamingIntegrationTest.cpp、tests/unit/cognition/CognitionLlmBridgeErrorMappingTest.cpp、CMake/TODO/worklog 回写 | 仅当 streaming 不影响 unary 验收、生命周期语义被冻结、且 llm internal implementation 与 focused tests 均落地时完成 |
| LLM-TODO-037 | Done | 评审 shared supporting object admission | 详细设计 7.2、10.1、12.1 | 7.2 暂不可直接映射项；10.1 breaking risk | L0 | 无新增 shared 头文件；继续保持 llm/include/route/ResolvedModelRoute.h、llm/include/prompt/PromptPolicyDecision.h、llm/include/stream/StreamSessionRef.h 为 module-local；形成 No-Go for shared admission 结论 | shared ModelRoute、shared PromptPolicyDecision、shared StreamHandle admission 候选 | contract：现有 LLMRequest / LLMResponse / PromptComposeRequest / PromptComposeResult 契约测试不回退 | `cmake --build build-ci --target dasall_contract_tests && ctest --test-dir build-ci --output-on-failure -L contract` | LLM-TODO-005、LLM-TODO-006、LLM-TODO-007、LLM-TODO-008、LLM-TODO-009、LLM-TODO-010、LLM-TODO-011、LLM-TODO-029、LLM-TODO-031、LLM-TODO-035、LLM-TODO-036 | 无 | LLM-BLK-006 已通过消费者矩阵、迁移窗口、contract gate 与 No-Go 评审结论解阻；后续 admission 重启需另起 contracts 任务 | docs/todos/llm/deliverables/LLM-TODO-037-shared-supporting-object-admission评审设计收敛.md、docs/architecture/DASALL_llm子系统详细设计.md、TODO/worklog 回写 | 仅当评审给出清晰 Go/No-Go 且 contract gate 不回退时完成 |
...
| LLM-GATE-10 | streaming / shared admission 评审门 | 阶段 J 前 | Go/No-Go 评审完成，解阻条件明确，contract gate 不回退 | 036 已冻结并完成 module-local streaming implementation；037 已完成 admission 评审并给出 No-Go for shared，三个候选对象继续 module-local/deferred |
...
| LLM-BLK-005 | 已解阻：StreamHandle / streaming 生命周期未冻结 | LLM-TODO-036 | 已完成取消、ownership、bounded session、backpressure 语义补设计；后续 LLM-FIX-001 已在 llm 内部落地 `StreamSessionRegistry`、OpenAI-compatible SSE/delta merge 与 focused streaming tests | LLM-TODO-036 已补充 streaming 设计与评审门，LLM-FIX-001 已完成 module-local 实现收口 | 继续保持 shared `StreamHandle` admission 后置；真实 streaming 已可在 llm 内部执行，但不作为 shared contracts 开放前提 |
| LLM-BLK-006 | 已解阻：shared ModelRoute / PromptPolicyDecision / StreamHandle admission 未冻结 | LLM-TODO-037 | 已形成消费者矩阵、迁移窗口、contract 验证基线与 No-Go for shared 结论 | LLM-TODO-037 已完成 admission 评审 | 继续保持 module-local supporting types，不发起 shared contracts 变更；若未来重启 admission，另起 contracts 任务 |
...
1. 状态：Done；042 已新增 [tests/integration/llm/LLMProviderAssetOnboardingIntegrationTest.cpp](../../../tests/integration/llm/LLMProviderAssetOnboardingIntegrationTest.cpp)，在真实 `ProviderCatalogRepository + AdapterRegistry + PromptPipeline + LLMManager` 闭环中补齐 asset-only Provider instance onboarding 证据，并通过 [tests/integration/llm/CMakeLists.txt](../../../tests/integration/llm/CMakeLists.txt) 将新用例接入 llm integration discoverability。
...
1. 状态：Done；038 已统一回写 llm 专项 Gate、阶段 G/H 证据链、工具回退策略与残余 blocker 结论，并补齐 036/037 的当前可执行性评估。
...
5. gate 结论：`LLM-GATE-01` 到 `LLM-GATE-10` 已全部闭合。036 已完成 llm internal streaming implementation，037 已完成 shared admission No-Go 评审，LLM-FIX-002 / 003 也已分别补齐 production provider family 注册与 production observability / audit sink，因此当前不再把阶段 J 误判为“待解阻阶段”；真正剩余的是 release 证据补齐与边界回归防线，而不是 shared admission 或 production sink 必做项。
...
7. 子系统结论：当前 `runtime/`、`apps/`、`cognition/` 与 `tools/` 依旧没有共享 `StreamHandle` consumer 的准入证据，因此后续 owner 不是这些子系统去消费 shared stream handle，而是 llm 继续在 module-local 范围内推进 release 证据与边界回归防线，并保持 shared admission 边界不回退。
8. 后继任务：038 已闭环；036/037 的 current-state 结论已固定为“module-local implemented / shared admission No-Go”，`LLM-FIX-003` 与 `LLM-FIX-004` 也已完成。下一步直接串行进入 `LLM-FIX-005`，补齐边界回归防线。
...
1. 状态：Done；036 先把 streaming lifecycle 从抽象 blocker 收敛为明确的设计 Gate，随后 2026-05-16 的 LLM-FIX-001 在不新增 shared `StreamHandle` 的前提下完成了 module-local 实现收口，因此当前结论是“llm internal stream-ready / shared admission still No-Go”。
2. 设计交付：[docs/todos/llm/deliverables/LLM-TODO-036-streaming生命周期设计并后置实现设计收敛.md](deliverables/LLM-TODO-036-streaming%E7%94%9F%E5%91%BD%E5%91%A8%E6%9C%9F%E8%AE%BE%E8%AE%A1%E5%B9%B6%E5%90%8E%E7%BD%AE%E5%AE%9E%E7%8E%B0%E8%AE%BE%E8%AE%A1%E6%94%B6%E6%95%9B.md)。
3. 代码交付：[llm/src/stream/IStreamObserver.h](../../../llm/src/stream/IStreamObserver.h)、[llm/src/stream/StreamSessionRegistry.h](../../../llm/src/stream/StreamSessionRegistry.h)、[llm/src/stream/StreamSessionRegistry.cpp](../../../llm/src/stream/StreamSessionRegistry.cpp)、[llm/src/LLMManager.cpp](../../../llm/src/LLMManager.cpp)、[llm/src/LLMManager.h](../../../llm/src/LLMManager.h)、[llm/src/adapters/OpenAICompatibleAdapter.cpp](../../../llm/src/adapters/OpenAICompatibleAdapter.cpp)、[llm/src/adapters/OpenAICompatibleAdapter.h](../../../llm/src/adapters/OpenAICompatibleAdapter.h)、[tests/unit/llm/StreamSessionLifecycleTest.cpp](../../../tests/unit/llm/StreamSessionLifecycleTest.cpp)、[tests/integration/llm/LLMStreamingIntegrationTest.cpp](../../../tests/integration/llm/LLMStreamingIntegrationTest.cpp)、[tests/unit/cognition/CognitionLlmBridgeErrorMappingTest.cpp](../../../tests/unit/cognition/CognitionLlmBridgeErrorMappingTest.cpp)。
4. 设计回链：[docs/architecture/DASALL_llm子系统详细设计.md](../../architecture/DASALL_llm%E5%AD%90%E7%B3%BB%E7%BB%9F%E8%AF%A6%E7%BB%86%E8%AE%BE%E8%AE%A1.md) 新增 7.2.1，冻结 session state、cancel、ownership、bounded session、backpressure 与 cleanup 语义。
...
8. 后继任务：LLM-TODO-037 的 shared ModelRoute / PromptPolicyDecision / StreamHandle admission 结论继续有效；在 LLM-FIX-002 / 003 已分别完成 production provider family 注册与 production observability / audit sink 后，后续只需在 llm owner 范围内推进 release evidence 与 boundary guard，而不是重开 shared admission。

### 17.23 LLM-TODO-037

1. 状态：Done；037 已完成 shared ModelRoute / PromptPolicyDecision / StreamHandle admission 评审，并给出 No-Go for shared 结论。`LLM-BLK-006` 在“无消费者矩阵/无迁移窗口/无 Go-No-Go 结论”层面解阻，但三个候选对象继续保持 module-local/deferred。
2. 设计交付：[docs/todos/llm/deliverables/LLM-TODO-037-shared-supporting-object-admission评审设计收敛.md](deliverables/LLM-TODO-037-shared-supporting-object-admission%E8%AF%84%E5%AE%A1%E8%AE%BE%E8%AE%A1%E6%94%B6%E6%95%9B.md)。
3. 设计回链：[docs/architecture/DASALL_llm子系统详细设计.md](../../architecture/DASALL_llm%E5%AD%90%E7%B3%BB%E7%BB%9F%E8%AF%A6%E7%BB%86%E8%AE%BE%E8%AE%A1.md) 新增 7.2.2，固化消费者矩阵、No-Go 结论和 Phase 0~3 迁移窗口。
4. 代码交付：无新增 shared 头文件；未修改 `contracts/include/**`；继续保持 [llm/include/route/ResolvedModelRoute.h](../../../llm/include/route/ResolvedModelRoute.h)、[llm/include/prompt/PromptPolicyDecision.h](../../../llm/include/prompt/PromptPolicyDecision.h)、[llm/include/stream/StreamSessionRef.h](../../../llm/include/stream/StreamSessionRef.h) 为 module-local supporting objects。
5. 消费者矩阵：`ResolvedModelRoute` 当前只由 ModelRouter/LLMManager 与 llm tests 直接消费；`PromptPolicyDecision` 完整对象只由 PromptPolicy/PromptPipeline/LLMManagerResult 与 tests 消费，runtime/cognition 只经 `LLMManagerResult.governance_disposition` 看到最小摘要；`StreamSessionRef` 继续保持 module-local supporting object，当前真实消费者已扩展到 `LLMManager::stream_generate()`、OpenAI-compatible adapter、`StreamSessionLifecycleTest` 与 `LLMStreamingIntegrationTest`，但仍没有跨模块 shared stream handle consumer 证据。
6. 验收目标：`dasall_contract_tests` 构建成功，全量 `ctest -L contract` 通过；现有 `LLMRequestResponseContractTest`、`PromptComposeRequestContractTest`、`PromptComposeRequestFieldContractTest`、`PromptComposeResultContractTest`、`PromptComposeResultFieldContractTest` 不回退。
7. 验收结果：`Build_CMakeTools(buildTargets=["dasall_contract_tests"])` 成功，实际执行 `/usr/bin/ctest --output-on-failure -L contract`，157/157 contract tests 通过。`RunCtest_CMakeTools` 定向执行 `LLMRequestResponseContractTest`、`PromptComposeRequestContractTest`、`PromptComposeRequestFieldContractTest`、`PromptComposeResultContractTest`、`PromptComposeResultFieldContractTest`，5 个 shared handoff 锚点全部通过。
8. 边界结论：037 不解除 shared admission No-Go。后续若任一候选对象需要进入 contracts，必须另起 contracts admission 任务，提交字段表、consumer matrix、source/wire/semantic compatibility 评估、side-by-side migration plan 与 contract tests。

### 17.24 LLM-FIX-004

1. 状态：Done；004 已把 llm owner 仍然缺失的 release evidence 统一收口为“历史 authoritative qemu L5 + 当前 installed local rerun L4 + release-runner contract fixed + provider failure-handling basis”，不再把 BC-07/BC-16 错写成“只有本机 L4 local”。
2. 设计交付：[docs/todos/llm/deliverables/LLM-FIX-004-L5-release-runner-provider长稳态证据收口.md](deliverables/LLM-FIX-004-L5-release-runner-provider%E9%95%BF%E7%A8%B3%E6%80%81%E8%AF%81%E6%8D%AE%E6%94%B6%E5%8F%A3.md)。
3. 代码交付：[docs/ssot/BusinessChainIntegrationMatrix.md](../../ssot/BusinessChainIntegrationMatrix.md)、[docs/todos/integration/DASALL_全量业务链集成验证专项TODO-2026-05-11.md](../integration/DASALL_%E5%85%A8%E9%87%8F%E4%B8%9A%E5%8A%A1%E9%93%BE%E9%9B%86%E6%88%90%E9%AA%8C%E8%AF%81%E4%B8%93%E9%A1%B9TODO-2026-05-11.md)、[docs/todos/DASALL_子系统查漏补缺专项记录.md](../DASALL_%E5%AD%90%E7%B3%BB%E7%BB%9F%E6%9F%A5%E6%BC%8F%E8%A1%A5%E7%BC%BA%E4%B8%93%E9%A1%B9%E8%AE%B0%E5%BD%95.md) 与 [docs/worklog/DASALL_开发执行记录.md](../../worklog/DASALL_%E5%BC%80%E5%8F%91%E6%89%A7%E8%A1%8C%E8%AE%B0%E5%BD%95.md)；release-runner contract 的 blocker-fix 已前置落盘于 [scripts/packaging/validate_gate_int_10_installed_package_qemu.sh](../../../scripts/packaging/validate_gate_int_10_installed_package_qemu.sh) 与 [.github/workflows/release-package-gate.yml](../../../.github/workflows/release-package-gate.yml)。
4. 验证结果：先定向构建 `dasall_model_router_stability_unit_test`、`dasall_llm_manager_timeout_policy_unit_test`、`dasall_llm_manager_retry_budget_unit_test`、`dasall_llm_manager_concurrency_guard_unit_test`、`dasall_deepseek_dual_mode_selection_integration_test`、`dasall_llm_fallback_integration_test`、`dasall_llm_profile_integration_test` 与 `dasall_llm_production_observability_integration_test`，随后执行 `ctest --test-dir build-ci --output-on-failure -R "(LLMFallbackIntegration|LLMProfileIntegration|DeepSeekDualModeSelectionIntegration|LLMProductionObservabilityIntegration|LLMManager(TimeoutPolicy|RetryBudget|ConcurrencyGuard)|ModelRouterStability)Test"`，结果为 `100% tests passed, 0 tests failed out of 8`。文档一致性检索继续命中 `LLM-FIX-004`、`BC-07`、`BC-16`、`FULLINT-BLK-001`、`DASALL-Release-Package-Gate` 与 `llm.origin=deepseek-prod`；少量旧 deliverable 仍保留当时的 blocker 表述，但这些属于历史快照，不再代表当前态。
5. gate 结论：004 不要求新增第二条 qemu smoke。PKG-TODO-018 的 authoritative qemu `pkg-smoke-local-control-plane PASS` 已经在 machine-isolation testbed 中断言 `llm.origin=deepseek-prod/` 且拒绝 `agent.dataset`，因此 BC-07 与 BC-16 当前都可记录为具备历史 L5 证据；`FULLINT-TODO-019` 则收窄为当前 release candidate 的 rerun / artifact archive owner，而不是继续充当 llm release evidence 的 blocker。
6. 边界结论：004 不把 focused timeout / retry / fallback / route stability / observability tests 误写成 L6 soak。当前只记录“provider failure-handling basis 已有证据”，不记录“production confidence 已完成”。
7. 后继任务：`LLM-FIX-006` 与 `LLM-FIX-005` 已分别收口当前态回写与 source boundary guard；剩余 owner 现拆分为 `FULLINT-TODO-019` 的 current release candidate rerun / artifact archive，以及 `LLM-FIX-007` 的 external provider 长稳态 / L6 soak 证据。

### 17.25 LLM-FIX-006

1. 状态：Done；006 已把 llm 详设中的 placeholder-only baseline、空测试目录与“无 Prompt/Provider loader / route / adapter”的旧表述收敛为历史语境，并把当前真实实现状态统一回写到详设、专项 TODO、总账与 worklog。
2. 设计交付：[docs/todos/llm/deliverables/LLM-FIX-006-llm详设当前状态追溯收敛.md](deliverables/LLM-FIX-006-llm%E8%AF%A6%E8%AE%BE%E5%BD%93%E5%89%8D%E7%8A%B6%E6%80%81%E8%BF%BD%E6%BA%AF%E6%94%B6%E6%95%9B.md)。
3. 代码交付：[docs/architecture/DASALL_llm子系统详细设计.md](../../architecture/DASALL_llm%E5%AD%90%E7%B3%BB%E7%BB%9F%E8%AF%A6%E7%BB%86%E8%AE%BE%E8%AE%A1.md)、[docs/todos/llm/DASALL_llm子系统专项TODO.md](DASALL_llm%E5%AD%90%E7%B3%BB%E7%BB%9F%E4%B8%93%E9%A1%B9TODO.md)、[docs/todos/DASALL_子系统查漏补缺专项记录.md](../DASALL_%E5%AD%90%E7%B3%BB%E7%BB%9F%E6%9F%A5%E6%BC%8F%E8%A1%A5%E7%BC%BA%E4%B8%93%E9%A1%B9%E8%AE%B0%E5%BD%95.md) 与 [docs/worklog/DASALL_开发执行记录.md](../../worklog/DASALL_%E5%BC%80%E5%8F%91%E6%89%A7%E8%A1%8C%E8%AE%B0%E5%BD%95.md)。
...
6. 边界结论：005 完成后，llm/source boundary 已有自动化守护；后续 llm 当前剩余开放项不再是 source boundary，而是 `FULLINT-TODO-019` 的 current release candidate rerun / artifact archive，以及 `LLM-FIX-007` 的 external provider 长稳态与 L6 soak 证据。

### 17.27 LLM-FIX-007

1. 状态：Done；007 已新增专属 deliverable / soak plan，把 provider jitter、network loss、secret rotate、retry budget exhaustion 与 observability trend 拆成可执行验收项；本任务完成的是执行矩阵冻结，不是宣称 L6 evidence 已执行完成。
2. 设计交付：[docs/todos/llm/deliverables/LLM-FIX-007-external-provider长稳态与L6-soak执行计划.md](deliverables/LLM-FIX-007-external-provider%E9%95%BF%E7%A8%B3%E6%80%81%E4%B8%8EL6-soak%E6%89%A7%E8%A1%8C%E8%AE%A1%E5%88%92.md)。
3. 代码交付：[docs/architecture/DASALL_llm子系统详细设计.md](../../architecture/DASALL_llm%E5%AD%90%E7%B3%BB%E7%BB%9F%E8%AF%A6%E7%BB%86%E8%AE%BE%E8%AE%A1.md)、[docs/todos/DASALL_子系统查漏补缺专项记录.md](../DASALL_%E5%AD%90%E7%B3%BB%E7%BB%9F%E6%9F%A5%E6%BC%8F%E8%A1%A5%E7%BC%BA%E4%B8%93%E9%A1%B9%E8%AE%B0%E5%BD%95.md)、[docs/worklog/DASALL_开发执行记录.md](../../worklog/DASALL_%E5%BC%80%E5%8F%91%E6%89%A7%E8%A1%8C%E8%AE%B0%E5%BD%95.md) 与本文档；本轮不新增 llm 产品代码。
4. 验证结果：`rg -n 'LLM-FIX-007|provider jitter|network loss|secret rotate|retry budget exhaustion|observability trend|SOAK-0' docs/todos/llm/DASALL_llm子系统专项TODO.md docs/todos/llm/deliverables/LLM-FIX-007-external-provider长稳态与L6-soak执行计划.md docs/todos/DASALL_子系统查漏补缺专项记录.md docs/architecture/DASALL_llm子系统详细设计.md docs/worklog/DASALL_开发执行记录.md` 与 `rg -n 'DASALL_AUTOPKGTEST_SETUP_COMMANDS_BOOT|DASALL_DEEPSEEK_API_KEY_FILE|config apply --from-file|LlmSecretPageTest|LLMManagerRetryBudgetTest|LLMProductionObservabilityIntegrationTest' docs/todos/llm/deliverables/LLM-FIX-007-external-provider长稳态与L6-soak执行计划.md scripts/packaging/validate_gate_int_10_installed_package_qemu.sh scripts/packaging/pkg_smoke_install.sh tests/unit/apps/cli/LlmSecretPageTest.cpp tests/unit/llm/LLMManagerRetryBudgetTest.cpp tests/integration/llm/LLMProductionObservabilityIntegrationTest.cpp .github/workflows/release-package-gate.yml` 应同时命中，确保计划中的命令都锚定到现有仓库入口。
5. 设计结论：007 不改变 llm owner 的技术边界，只把 external provider L6 执行面从模糊 residual phrase 收敛为可执行 slice。`FULLINT-TODO-019` 继续 owner 当前候选版 rerun / artifact archive，007 只负责 external provider soak 执行矩阵。
6. 边界结论：007 不把 focused timeout / retry / fallback / observability tests 写成 L6 已完成；完成后，llm 当前剩余开放项收敛为 `FULLINT-TODO-019` 的 current release candidate rerun，以及按 007 deliverable 执行的 external provider soak slices。

### 17.28 LLM-GAP-004 local installed blocker closeout

1. 状态：Done（历史 blocker ledger；2026-05-20 已由本机 installed positive evidence 解阻）。本轮按用户约束禁止 qemu/kvm 采集证据，改用本机 installed host 与 focused baseline。
2. 设计交付：[docs/todos/llm/deliverables/LLM-GAP-004-local-installed-soak-blocker-closeout.md](deliverables/LLM-GAP-004-local-installed-soak-blocker-closeout.md)。
3. 证据结果：`/tmp/dasall-llm-gap004-local-installed-1779204346` 记录 5 次 installed run 均为 `accepted_async` 且无 `llm.origin`；`/tmp/dasall-llm-gap004-sync-shape-1779204782` 记录 JSON positional run 为 `disposition=completed` / `exit_code=5` / `task_not_completed`，plain string / pseudo `--prompt` 形态进入 `accepted_async`。async 响应缺 ownership token / request id，因此无法形成 owner-authoritative status follow-up。
4. 验证边界：SOAK-00 focused baseline 继续证明 timeout / retry / fallback / routing / observability / secret bootstrap regression 未回退；这不外推为 installed external provider L6 soak success。
5. 当前态：本条的 installed positive evidence 已由 17.29 解阻；async ownership/status 链仍不在本轮同步 `run` 正向证据范围内，后续若要声明 async follow-up 能力需另行验证。

### 17.29 LLM-GAP-004 local installed positive evidence closeout

1. 状态：Done（本机 installed positive evidence 已解阻；不宣称 L6 external provider soak）。
2. 设计交付：[docs/todos/llm/deliverables/LLM-GAP-004-local-installed-positive-closeout.md](deliverables/LLM-GAP-004-local-installed-positive-closeout.md)。
3. 用户 smoke：`sudo dasall run '{"prompt":"告诉我现在几点了"}' --request-id llm-smoke-001 --session llm-session-001 --trace-id trace-llm-smoke-001 --json` 返回 `disposition=completed`、`exit_code=0`、`task_completed=true`，且 `response_text` 含 `llm.origin=deepseek-prod/deepseek-reasoner model=deepseek-v4-flash finish_reason=stop`。
4. 复验证据：artifact `/tmp/dasall-llm-gap004-positive-1779243811` 记录 5 次本机 installed run；`summary.txt` 为 `total_runs: 5`、`exit0_count: 5`、`completed_count: 5`、`task_completed_count: 5`、`llm_origin_count: 5`、`agent_dataset_count: 0`、`accepted_async_count: 0`、`task_not_completed_count: 0`。
5. focused baseline：`build-ci` 目录同一 SOAK-00 正则仅 4 个单测可运行、5 个集成 / CLI 测试因缺少可执行文件为 `Not Run`；fallback 到 `build/vscode-linux-ninja` 后同一 9 项测试 `100% tests passed, 0 tests failed out of 9`。
6. 边界结论：当前 CLI -> daemon -> runtime -> production LLM provider 同步 generation 路径已恢复正向 evidence；不得外推为 30 轮 SOAK-01、SOAK-02/04 qemu negative slice、async ownership/status 链或 release-runner artifact archive 已完成。
