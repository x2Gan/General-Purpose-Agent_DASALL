...
最近更新时间：2026-04-12（完成 LLM-TODO-001/002/003/004/005/006/007/008/009/010/011/012/013/014/015/016/017/018/019/039，阶段 B 公共接口冻结、阶段 C 测试夹具升级、阶段 D Prompt/Provider 资产基线与阶段 E PromptRegistry/TokenEstimator/TemplateRenderer/PromptComposer/PromptPolicy/PromptPipeline 已落盘）
...
5. 边界结论：本轮把 profile 中的抽象 route 名继续保留为输入 envelope，但 `ResolvedModelRoute` 的输出已经收敛到具体 `provider_id/model_id`；route owner 只消费 `LLMSubsystemConfig`、`ProviderCatalogSnapshot`、`ModelSelectionHint` 与 module-local health snapshot，不扩 public/shared contracts，也不在代码里写死除了测试资产外的厂商专有分支。
6. 设计收敛：020 已固定 `候选集装配 -> 硬过滤 -> 确定性评分 -> fallback chain 展开` 四步；其中硬过滤覆盖 context window、output hard limit、tools verified、reasoning capability、provider trusted source、summary verification_state 与 health blocked，评分再叠加 route order、tier match、reasoning/task 偏好、interactive SLA、hard-cap budget、visible reasoning 与 previous_route_failures。为满足详细设计 6.6.6 的“interactive / hard_cap 可降档”要求，本轮显式加入 `interactive_hard_cap_downgrade` 偏置，并使用 `score desc -> route_order asc -> route_id lexicographic asc` 做稳定 tie-break。
