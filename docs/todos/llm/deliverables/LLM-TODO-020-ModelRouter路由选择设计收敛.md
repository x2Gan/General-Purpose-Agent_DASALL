...
1. 020 在 [llm/src/route/ModelRouter.h](../../../../llm/src/route/ModelRouter.h) 中新增 module-local `ModelRouter` concrete owner，`init()` 只接受 `LLMSubsystemConfig` 投影视图，`resolve()` 则显式消费 `ModelSelectionHint`、immutable `ProviderCatalogSnapshot` 与注入式 `ModelRouterHealthSnapshot`。`ModelRouterResolveResult` 继续只向外暴露最终 `ResolvedModelRoute` 和 `selection_reason_codes`，同时新增 internal-only `audit_evidence` 用来记录候选为何被接受、降级或硬过滤。这样 020 已为 021 预留健康快照注入点，但不提前引入 AdapterRegistry，也不把审计结构推入 shared contracts。
...
6. 020 现在把“最终选择理由”和“候选被淘汰理由”拆成两层证据：module-local `ModelRouterResolveResult.selection_reason_codes` 只保留本次最终 primary route 为何成立，例如 `selected_primary_route`、`selected_from_fallback_chain`、`preferred_tier_match`、`tier_upgraded`、`tier_degraded`、`interactive_latency_bias`、`budget_low_cost`、`interactive_hard_cap_downgrade`、`requires_reasoning` 等；而 internal-only `audit_evidence` 则额外记录每个 concrete candidate 的 `route_preference`、`outcome` 与 reason codes，例如 `rejected_hard_filter + health_blocked`。这样在“成功 fallback”场景下，审计面可以同时解释“为什么 lan.general 赢了”和“为什么 cloud.reasoning 下的 deepseek 候选被淘汰”，而不需要把 rejection reason 混进 shared or public ABI。
...
9. [docs/architecture/DASALL_llm子系统详细设计.md](../../../architecture/DASALL_llm子系统详细设计.md) 的 6.15.1 把 `timeout_policy` 列进了 route owner 会读取的 profile 输入，但 [docs/architecture/DASALL_llm子系统详细设计.md](../../../architecture/DASALL_llm子系统详细设计.md) 的配置职责表又明确把 `timeout_ms`、`retry_budget` 与 `circuit_breaker_threshold` 的实际执行 owner 放在 `LLMManager` 调用治理。020 因此采用了更保守的边界：`ModelRouter` 只消费 route policy、degrade policy、trusted source、catalog metadata 与 health snapshot 做选路，不把 timeout/retry/circuit breaker 提前揉进评分；相关执行治理明确留给 040/024。

## 4. Design -> Build 映射

| Design 项 | Build 落点 |
|---|---|
| 落地 ModelRouter concrete owner、health snapshot 注入面、route resolve 结果对象与 internal-only audit evidence | [llm/src/route/ModelRouter.h](../../../../llm/src/route/ModelRouter.h)、[llm/src/route/ModelRouter.cpp](../../../../llm/src/route/ModelRouter.cpp) |
...
| 覆盖上下文/输出硬过滤、tools 验证降档、fallback 顺序、成功 fallback 的 rejection evidence、双模式切换与稳定性 | [tests/unit/llm/ModelRouterPolicyTest.cpp](../../../../tests/unit/llm/ModelRouterPolicyTest.cpp)、[tests/unit/llm/ModelRouterFallbackTest.cpp](../../../../tests/unit/llm/ModelRouterFallbackTest.cpp)、[tests/unit/llm/ModelRouterReasoningModeSelectionTest.cpp](../../../../tests/unit/llm/ModelRouterReasoningModeSelectionTest.cpp)、[tests/unit/llm/ModelRouterStabilityTest.cpp](../../../../tests/unit/llm/ModelRouterStabilityTest.cpp) |
...
1. 代码目标：实现 `ModelRouter`，完成抽象 route envelope 到具体 provider/model route 的解析，并落地 health/verification/context/output/tools/reasoning 的硬门禁、可解释评分，以及成功 fallback 下的 rejected candidate 审计证据。
2. 测试目标：
   - `ModelRouterPolicyTest`：覆盖 context window / output hard limit 双硬过滤，以及未验证 reasoning-tools 候选降档到 chat。
   - `ModelRouterFallbackTest`：覆盖 explicit `fallback_route` 优先于 degrade chain、`allow_model_failover=false` 时不展开 degrade chain，以及成功 fallback 时保留 primary rejection reason 的 audit evidence。
...
4. 020 当前保留了 `selection_reason_codes` 作为“最终 primary 为什么成立”的对外可观测面，并把成功 fallback 时的 rejection reason 下沉到 internal-only `audit_evidence`。这意味着 028/030 若要输出更细粒度的路由审计，可直接消费内部证据，而不需要回头修改 020 的 shared/public shape。