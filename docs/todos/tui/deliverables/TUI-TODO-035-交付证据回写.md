...
3. 本任务不会把 CJK/IME/resize manual gate 或命令迁移 readiness 误写成已通过。`TUI-PROTO-017` 已补齐采纳 / 延后 / 废弃清单，`TUI-PROTO-017-formal-sample-signoff.md` 已完成 Product / Engineering formal sign-off，`TUI-TODO-036` 已补齐 FTXUI installed-path packaging review；但 `BLK-TUI-006` 与 Gate-TUI-08 继续保持 Open/Blocked。
...
4. `TUI-PROTO-017` 已提供样品采纳 / 延后 / 废弃清单，`TUI-PROTO-017-formal-sample-signoff.md` 已完成 formal sign-off；`TUI-TODO-036` 已提供 FTXUI installed-path Debian source/binary strategy review。这些证据只消除样品评审、签署和 packaging review 缺口，不替代人工 terminal gate。
...
1. 延后 Gate-TUI-08 复检与 command release 实现；虽然 `TUI-PROTO-017` formal product/engineering sign-off 已完成，但真实终端人工质量门仍未闭合。
2. 延后 `BLK-TUI-006` 的 CJK/IME/resize manual gate；当前只拥有自动化状态机、snapshot、fake-only smoke 证据与可签署验收文档，不拥有真实终端人工验收通过证据。
...
2. 对原任务 `TUI-TODO-031` 的执行判断回退到 gate-driven 策略：即便 `TUI-TODO-030`、`TUI-PROTO-017`、`TUI-PROTO-017-formal-sample-signoff.md` 与 `TUI-TODO-036` 已 Done，只要 `BLK-TUI-006` 未闭合，就不得提前释放 `dasall-cli` 产物名。

## 5. Gate 回写结论

| Gate | 当前状态 | 正式命令 / 证据 | 长期交付物路径 | 当前状态说明 | 后继动作 | 残余风险 |
|---|---|---|---|---|---|---|
| Gate-TUI-05 | Blocked | `TUI-TODO-020` deliverable + `TUI-PROTO-017` 等价样品评审资产 + `TUI-PROTO-017-formal-sample-signoff.md` + `BLK-TUI-006-manual-terminal-evidence.md` | `docs/todos/tui/deliverables/TUI-PROTO-017-样品评审证据.md` | fake-only prototype、采纳/延后/废弃清单与 formal sign-off 已完成，但真实终端结果仍缺 | 人工填写并签署 `BLK-TUI-006` manual terminal evidence | 未完成人工 terminal gate 前，不得把当前 prototype 结论外推为正式 command release 准入 |
...
3. 修复后重新判断：`TUI-TODO-031` 仍未恢复可执行。原因已收敛：
   ...
   - FTXUI installed-path Debian source/binary strategy review 已由 `TUI-TODO-036` 闭合，`BLK-TUI-006-manual-terminal-evidence.md` 已落盘，但 `BLK-TUI-006` 的 CJK/IME/resize manual evidence 仍未完成人工结果填写，B.0 第 5 项仍不能转为 full Pass。
...
3. 最小解阻动作现已收敛为一条：人工填写并签署 `BLK-TUI-006-manual-terminal-evidence.md`；formal sample sign-off 与 FTXUI installed-path Debian source/binary strategy review 已不再是 031 的阻塞项。