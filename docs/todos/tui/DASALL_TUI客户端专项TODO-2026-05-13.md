...
| 代码现状 | `apps/CMakeLists.txt` 已接入 `apps/tui`，`apps/tui/CMakeLists.txt` 已落 `dasall_tui_prototype` 与 FTXUI private-link helper，`apps/tui/src/data/TuiProjectionTypes.h` 已提供 module-local projection DTO 基线，`apps/tui/src/model/TuiAction.h` 与 `apps/tui/src/model/TuiScreenModel.h` 已提供 MVU model/action 基线；`apps/cli/CMakeLists.txt` 中 `dasall-cli` 仍 `OUTPUT_NAME dasall`；`cmake/DASALLThirdParty.cmake` 已冻结 FTXUI 统一 resolver/pin；`tests/unit/tui/` 与 `tests/integration/tui/` 已物化并可发现 focused test 名；LLM internal streaming 已有实现基础但 TUI 端到端 streaming lifecycle 未闭环 |
...
| 推荐执行策略 | 先执行 TUI-TODO-010~012，形成 reducer/data source 底座；随后在 FTXUI resolver/pin 已冻结、prototype target、projection DTO 与 screen model/action 已就位的前提下执行 TUI-TODO-013~020，并把 full-screen renderer 继续受 BLK-TUI-006 的终端样品 gate 约束；再以 TUI-TODO-021~029 补 daemon/status/route 真链路；最后在 TUI-TODO-030 证据通过后执行命令迁移 |
| 当前专项 TODO 状态 | Build-ready subset：TUI-TODO-010~018；Renderer-ready subset：TUI-TODO-019~020，依赖 BLK-TUI-006 与各自代码前置；Design/Review-ready subset：TUI-TODO-021、027、030、035；Not-started after blocker closure：TUI-TODO-019、028~029 可继续按前置依赖串行推进；Blocked subset：TUI-TODO-023、026、030~034 中依赖 external owner 的 session/命令迁移任务 |
