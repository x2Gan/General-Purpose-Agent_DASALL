...
| 推荐执行策略 | 下一步执行 TUI-TODO-020，把已落盘的 terminal capability probe、design tokens/layout metrics、renderer snapshot baseline、status/transcript、slash command、composer 状态机、selector fake 交互与 fake data source 底座接成 fake-only `TuiApp` loop；随后在 FTXUI resolver/pin 已冻结、prototype target、projection DTO、screen model/action、reducer、`ITuiDataSource` seam、deterministic fake replay、slash parser、composer、selector、transcript、status panel、terminal probe 与 renderer baseline 已就位的前提下推进 `TUI-TODO-020`，并继续把 full-screen interactive loop 受 `BLK-TUI-006` 的终端样品 gate 约束；再以 `TUI-TODO-021~029` 补 daemon/status/route 真链路；最后在 `TUI-TODO-030` 证据通过后执行命令迁移 |
| 当前专项 TODO 状态 | Build-ready subset：TUI-TODO-018~019 已闭合；Prototype-app subset：TUI-TODO-020，依赖 BLK-TUI-006 与 019 renderer baseline；Design/Review-ready subset：TUI-TODO-021、027、030、035；Not-started after blocker closure：TUI-TODO-020、028~029 可继续按前置依赖串行推进；Blocked subset：TUI-TODO-023、026、030~034 中依赖 external owner 的 session/命令迁移任务 |
