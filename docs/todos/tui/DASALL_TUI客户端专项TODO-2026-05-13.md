...
| 推荐执行策略 | 下一步执行 TUI-TODO-021，把本轮已稳定的 fake-only `TuiApp` 调用上下文沉淀为 daemon projection request/response mapping 与 header 草案；随后在已闭合的 prototype target、projection DTO、screen model/action、reducer、`ITuiDataSource` seam、deterministic fake replay、slash parser、composer、selector、transcript、status panel、terminal probe、renderer baseline 与 fake-only app loop 基线上推进 `TUI-TODO-021~029`，并继续把 full-screen interactive promotion 受 `BLK-TUI-006` 的终端样品 gate 约束；最后在 `TUI-TODO-030` 证据通过后执行命令迁移 |
| 当前专项 TODO 状态 | Build-ready subset：TUI-TODO-018~020 已闭合；Prototype-app subset：TUI-TODO-020 已完成，remaining manual gate 为 `BLK-TUI-006`；Design/Review-ready subset：TUI-TODO-021、027、030、035；Not-started after blocker closure：TUI-TODO-021、028~029 可继续按前置依赖串行推进；Blocked subset：TUI-TODO-023、026、030~034 中依赖 external owner 的 session/命令迁移任务 |
