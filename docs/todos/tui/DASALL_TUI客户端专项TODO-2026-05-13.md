...
| 推荐执行策略 | 下一步优先执行 `TUI-TODO-025`，在本轮已闭合的 startup failure + daemon-backed attach 基线上接入 status/tool/recovery projection 刷新；`TUI-TODO-027` 也已具备并行推进条件；`BLK-TUI-007` 现只约束 `TUI-TODO-026` 的真实 session lifecycle integration，命令迁移仍后置到 `TUI-TODO-030` 证据通过之后 |
| 当前专项 TODO 状态 | Build-ready subset：TUI-TODO-018~024 已闭合；Prototype-app subset：TUI-TODO-020 已完成，remaining manual gate 为 `BLK-TUI-006`；Phase-3-ready subset：TUI-TODO-025、027、035；Next executable task：TUI-TODO-025；Blocked subset：TUI-TODO-026、030~034 中依赖 external owner 的 session/命令迁移任务 |
