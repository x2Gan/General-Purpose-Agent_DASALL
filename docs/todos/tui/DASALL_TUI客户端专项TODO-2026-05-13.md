...
| 证据对象 | 当前状态 | 结论 |
|---|---|---|
| `apps/tui/` | 已接入 non-installed `dasall_tui_prototype`；`apps/tui/src/data/` 已落 `TuiProjectionTypes.h`、`ITuiDataSource.h`、`FakeScenarioCatalog.h`、`FakeTuiDataSource.h/.cpp`；`apps/tui/src/model/` 已落 `TuiAction.h`、`TuiScreenModel.h`、`TuiReducer.h/.cpp` | prototype substrate、DTO、data source seam 与 deterministic fake replay 已就位，`TUI-TODO-013~020` 可继续在 no-daemon 边界内迭代 |
...
| 推荐执行策略 | 先执行 TUI-TODO-013，继续把 slash command/composer/selector/status/transcript/app loop 叠加到已完成的 fake data source 底座；随后在 FTXUI resolver/pin 已冻结、prototype target、projection DTO、screen model/action、reducer、`ITuiDataSource` seam 与 deterministic fake replay 已就位的前提下执行 TUI-TODO-014~020，并把 full-screen renderer 继续受 BLK-TUI-006 的终端样品 gate 约束；再以 TUI-TODO-021~029 补 daemon/status/route 真链路；最后在 TUI-TODO-030 证据通过后执行命令迁移 |
| 当前专项 TODO 状态 | Build-ready subset：TUI-TODO-013~018；Renderer-ready subset：TUI-TODO-019~020，依赖 BLK-TUI-006 与各自代码前置；Design/Review-ready subset：TUI-TODO-021、027、030、035；Not-started after blocker closure：TUI-TODO-019、028~029 可继续按前置依赖串行推进；Blocked subset：TUI-TODO-023、026、030~034 中依赖 external owner 的 session/命令迁移任务 |
