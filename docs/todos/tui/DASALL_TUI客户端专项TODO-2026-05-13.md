...
| 推荐执行策略 | 下一步执行 TUI-TODO-022，在本轮已冻结的 `TuiIpcController.h` payload/outcome/timeout 草案上实现 transport、serialization 与错误归一化；随后在已闭合的 prototype target、projection DTO、screen model/action、reducer、`ITuiDataSource` seam、deterministic fake replay、slash parser、composer、selector、transcript、status panel、terminal probe、renderer baseline、fake-only app loop 与 IPC mapping/header 草案基线上推进 `TUI-TODO-022~029`，并继续把 full-screen interactive promotion 受 `BLK-TUI-006` 的终端样品 gate 约束；最后在 `TUI-TODO-030` 证据通过后执行命令迁移 |
| 当前专项 TODO 状态 | Build-ready subset：TUI-TODO-018~021 已闭合；Prototype-app subset：TUI-TODO-020 已完成，remaining manual gate 为 `BLK-TUI-006`；Phase-3-ready subset：TUI-TODO-022、027、030、035；Not-started after blocker closure：TUI-TODO-022、028~029 可继续按前置依赖串行推进；Blocked subset：TUI-TODO-023、026、030~034 中依赖 external owner 的 session/命令迁移任务 |
