...
当前结论：`apps/runtime_support::compose_minimal_live_dependency_set()` 已把 daemon / gateway 的 runtime dependency owner 收敛为共享 helper，并已完成 install layout + SQLite memory baseline、production LLM manager、cognition/response ports、minimal ToolManager、typed multi-agent seam 和 optional knowledge seam 的最小 live baseline。当前剩余缺口集中在真实 services backend 注入、production observability/health sinks、knowledge optional degraded semantics 证据，以及 installed / qemu / release 层级验证。
...
| tool path | 仍使用 `execution_service = nullptr`、`data_service = nullptr` | current helper 仍不能证明 production tools -> services concrete backend |
| observability / health | helper 未统一注入 runtime/tools/services 所需 sinks | current helper 仍不能证明 production observability / health hot path |
...
| tool path 需要真实 services backend，而非 default service | tools / capability services 专项 TODO | 未完成 | RTSUP-TODO-005 |
| production observability / health sinks 需要 shared helper 注入 | infra / runtime / tools / services 专项 TODO | 未完成 | RTSUP-TODO-006 |
...
| production services backend | `RuntimeLiveDependencyComposition.cpp`、必要时 services factory | tool/services/runtime composition tests | 当前最关键的行为缺口 |
| production observability / health sinks | helper + infra/runtime/tools/services bridges | observability / health composition tests | 当前最关键的 production 证据缺口 |
...
| Task ID | 状态 | 任务标题 | 设计依据 | 精确范围 | 粒度 | 代码目标 | 目标函数/接口/数据结构 | 测试目标 | 验收命令 | 前置任务 | 关联阻塞项 | 解阻条件 | 交付物 | 完成判定 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| RTSUP-TODO-005 | NotStarted | 接入 runtime production services backend | tools / capability services 专项 TODO；`TOOL-GAP-001`、`CAPSRV-GAP-001` | `BuiltinExecutorLaneDependencies` 不再为空服务；tool path 走 concrete services backend | L2 | `apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp`、必要时 services composition helper / factory | `compose_runtime_tool_manager()`、`IExecutionService`、`IDataService`、`ServiceFacade` | `ToolServicesProductionBridgeIntegrationTest`、扩展 daemon/gateway composition tests | `RunCtest_CMakeTools(tests=["ToolServicesSmokeIntegrationTest","ToolServicesProductionBridgeIntegrationTest","DaemonRuntimeLiveDependencyCompositionTest","GatewayRuntimeLiveDependencyCompositionTest"])` | RTSUP-TODO-004 | RTSUP-BLK-001 | services facade / lanes 组合口径确定，或在 helper 内提供最小 production composition | 更新后的 helper source 与 focused tests | app live composition 的 tool path 不再回落 tools default service |
| RTSUP-TODO-006 | NotStarted | 接入 production observability 与 health sinks | infra / runtime / tools / capability services 专项 TODO；`TOOL-GAP-007`、`CAPSRV-GAP-006`、`INF-GAP-001` | logger / audit / metrics / trace / health provider 注入 shared helper | L2 | `apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp`、必要时 infra composition helper | runtime/tools/services observability bridge injection、health probe registration | `ToolProductionObservabilityIntegrationTest`、`RuntimeProductionHealthCompositionTest`、扩展 daemon/gateway composition tests | `RunCtest_CMakeTools(tests=["ToolObservabilityIntegrationTest","ToolProductionObservabilityIntegrationTest","RuntimeHealthMaintenanceIntegrationTest","DaemonRuntimeLiveDependencyCompositionTest"])` | RTSUP-TODO-004 | RTSUP-BLK-002 | infra provider / tracer / audit logger / health provider 注入口径收敛 | 更新后的 helper source 与 observability/health tests | shared helper 组合出的 live path 能发出真实 sink event，并把 health hot path 纳入 app composition |
...
|---|---|---|---|---|
| RTSUP-BLK-001 | runtime_support 尚未具备真实 services composition root，tool path 仍回落 default service | 005、008、009 | services facade / lanes / adapters 在 helper 侧具备最小 production composition 口径 | 继续把现状写成 focused baseline，不把 tool path 外推为 production backend ready |
| RTSUP-BLK-002 | infra provider / tracer / audit logger / health provider 的 shared injection 路径尚未统一 | 006、008、009 | runtime/tools/services/infra 就 helper 注入口径达成一致，或在 helper 中收口最小 provider seam | 局部 observability tests 继续保留，但不得写成 production hot path evidence |
