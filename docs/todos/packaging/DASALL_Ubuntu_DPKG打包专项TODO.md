...
当前结论：Ubuntu DPKG 打包设计已 Ready，但工程实现仍处于 Implementation Not Ready / Package Gate Not Started。当前仓库已具备 `dasall-cli`、`dasall-daemon`、`dasall_profiles`、LLM baseline 资产和一组可复用的 contract/integration tests，但 install surface、安装态路径收敛、`debian/` 元数据、installed-package QA gate 仍未落盘。
...
| `CMakeLists.txt` | 仅 `add_subdirectory(...)`，未出现 GNUInstallDirs / `install()` / 打包入口 | 顶层 install surface 缺失 |
| `profiles/CMakeLists.txt` | 只定义 `dasall_profiles` 静态库，无任何安装规则 | `dasall-common` 无法稳定消费 profile 资产安装面 |
| `apps/cli/CMakeLists.txt`、`apps/daemon/CMakeLists.txt` | 只定义 executable target，无 `OUTPUT_NAME` / `install(TARGETS ...)` | `dh_auto_install` 无法产出 CLI / daemon 安装面 |
...
| `llm/include/LLMSubsystemConfig.h` | Prompt / Provider baseline root 默认仍是 `llm/assets/prompts` 与 `llm/assets/providers` | LLM 安装态资产根仍是 repo-relative |
...
| `debian/` | 仓库不存在该目录 | source package / binary package 元数据完全缺失 |

### 2.2 总体判断

1. 设计层已经回答了“怎么打包”，但工程层还没有回答“如何生成可安装、可升级、可验证的 Debian 产物”。
2. 当前 package-ready 的 P0 缺口集中在三类：install surface 缺失、安装态路径未收敛、installed-package gate 缺失。
...
| PKG-TC013 | 打包设计 7.8 | Must | daemon 以 `dasall:dasall` 运行，socket 默认 `0660`，不放宽到 world-writable | service、postinst、autopkgtest 必须守住最小权限 |
...
|---|---|---|---|
| 顶层与子模块 install surface | 根/子模块 CMake 无 `install()` 与 GNUInstallDirs | PKG-TODO-005、006、007 | `debian/tmp` 可消费的标准安装面 |
...
| Debian source metadata | `debian/` 目录不存在 | PKG-TODO-001、008、009 | 可执行的 source package/binary package 骨架 |
| 服务/配置/首次部署 UX | 无 service、无 package-local payload tree、无 README/postinst | PKG-TODO-002、009、013、014 | daemon package payload 与 operator onboarding |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| PKG-TODO-001 | NotStarted | 实测并冻结 Build-Depends / Depends / Architecture 矩阵 | 7.2、7.9.2、7.10.4；Maint Guide control/install | `Build-Depends`、`Depends`、`Recommends`、`Architecture: any/all`、`adduser` 运行时依赖 | L2 | `debian/control`；`docs/todos/packaging/deliverables/PKG-TODO-001-依赖矩阵冻结.md` | `Source/Package stanza` 字段、Build-Depends 列表 | process：依赖矩阵一致性检查 | `rg -n "Build-Depends|Architecture: any|Architecture: all|adduser|dasall-common|dasall-daemon" docs/architecture/DASALL_Ubuntu平台DPKG打包方案设计.md docs/todos/packaging/DASALL_Ubuntu_DPKG打包专项TODO.md docs/todos/packaging/deliverables/PKG-TODO-001-依赖矩阵冻结.md` | 无 | PKG-BLK-04 | 完成目标 Ubuntu 版本上的依赖盘点 | `debian/control` 初稿；依赖矩阵 deliverable | 仅当四包关系、Build-Depends、Architecture 与运行时依赖全部形成唯一口径，且不再保留占位字段时完成 |
| PKG-TODO-002 | NotStarted | 冻结 package-mode profile 选择接缝与配置 ownership | 7.7.4、7.10.9；当前 `DaemonEntryConfigLoader` 仅支持 `daemon.*` 覆盖 | `/etc/default/dasall-daemon`、`/etc/dasall/daemon.json`、`--profile-id`、`EnvironmentFile` | L2 | `apps/daemon/src/main.cpp`；`apps/daemon/src/DaemonEntryConfigLoader.*`；`debian/dasall-daemon.service`；`debian/dasall-daemon/etc/default/dasall-daemon` | `--profile-id`、`--config-file`、package-mode 输入模型 | unit：`DaemonEntryConfigProjectionTest`；design/process：service/config 一致性 | `rg -n "/etc/default/dasall-daemon|--profile-id|daemon.json|EnvironmentFile" docs/architecture/DASALL_Ubuntu平台DPKG打包方案设计.md docs/todos/packaging/DASALL_Ubuntu_DPKG打包专项TODO.md` | 无 | PKG-BLK-03 | 明确 v1 是否继续采用 EnvironmentFile 基线 | 接缝冻结记录；service/config 统一口径 | 仅当 profile 选择入口、daemon.json 边界和 systemd ExecStart 参数形成唯一实现口径时完成 |
| PKG-TODO-003 | NotStarted | 冻结 v1 packaging QA gate 与 autopkgtest 运行策略 | 7.10.5、7.10.7、7.14；`autopkgtest(1)` | preflight 切片、install smoke、upgrade smoke、remove/purge、autopkgtest 虚拟化策略 | L2 | `docs/todos/packaging/deliverables/PKG-TODO-003-QA-Gate与autopkgtest策略冻结.md`；`scripts/packaging/README.md` | `autopkgtest` testbed strategy、`lintian`、fresh install/upgrade gate matrix | process：gate matrix 校验 | `rg -n "autopkgtest|lintian|install smoke|upgrade smoke|remove|purge|PKG-GATE" docs/architecture/DASALL_Ubuntu平台DPKG打包方案设计.md docs/todos/packaging/DASALL_Ubuntu_DPKG打包专项TODO.md docs/todos/packaging/deliverables/PKG-TODO-003-QA-Gate与autopkgtest策略冻结.md` | 无 | PKG-BLK-05 | 明确本地与 CI 两套最小 testbed 策略 | QA 策略 deliverable | 仅当 build-tree gate 与 installed-package gate 的边界、命令和环境前提全部冻结时完成 |
| PKG-TODO-004 | NotStarted | 冻结统一安装态布局 owner 与 path resolver 归属 | 3.2、7.4、7.5、7.10.8；当前路径分散在 access/apps/daemon/llm | `/usr/share/dasall`、`/etc/dasall`、`/run/dasall`、`/var/lib/dasall` 的代码 owner 与 API surface | L2 | `docs/architecture/DASALL_Ubuntu平台DPKG打包方案设计.md`；`docs/todos/packaging/deliverables/PKG-TODO-004-InstallLayout与PathResolver归属冻结.md`；后续共享头文件落位说明 | `InstallLayout` / asset resolver / daemon config root / socket root 归属 | process：设计与代码锚点一致性 | `rg -n "/usr/share/dasall|/etc/dasall|/run/dasall|/var/lib/dasall|WorkingDirectory" docs/architecture/DASALL_Ubuntu平台DPKG打包方案设计.md docs/todos/packaging/DASALL_Ubuntu_DPKG打包专项TODO.md docs/todos/packaging/deliverables/PKG-TODO-004-InstallLayout与PathResolver归属冻结.md` | 无 | PKG-BLK-01、PKG-BLK-02 | 明确共享安装态路径的唯一 owner | InstallLayout 归属 deliverable | 仅当 install layout 的 owner、导出位置、调用边界不再依赖口头约定时完成 |

### 6.2 Install Surface 与 debian 骨架任务

| Task ID | Status | 任务标题 | 设计依据 | 精确范围 | 粒度 | 代码目标 | 目标函数/接口/数据结构 | 测试目标 | 验收命令 | 前置任务 | 关联阻塞项 | 解阻条件 | 交付物 | 完成判定 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| PKG-TODO-005 | NotStarted | 引入 GNUInstallDirs 与顶层 install surface 骨架 | 7.1、7.10.5、7.10.8 | 顶层 CMake、公共安装变量、`dh_auto_install` 所需基础 install 面 | L2 | `CMakeLists.txt`；`apps/CMakeLists.txt`；`profiles/CMakeLists.txt`；`llm/CMakeLists.txt`；必要时 `access/CMakeLists.txt` | GNUInstallDirs、`install(TARGETS ...)` / `install(DIRECTORY ...)` 骨架 | build：stage install discoverability | `cmake -S . -B build-ci -G Ninja && cmake --build build-ci --target dasall-cli dasall-daemon dasall_profiles && rm -rf stage/pkg && DESTDIR="$PWD/stage/pkg" cmake --install build-ci && find stage/pkg -maxdepth 4 | sort` | PKG-TODO-004 | PKG-BLK-01 | 统一安装变量与 stage install 成立 | 更新后的 CMake install skeleton | 仅当 `cmake --install` 能产出非空 stage tree，且不再需要手工拷贝 build 产物时完成 |
| PKG-TODO-006 | NotStarted | 为 CLI / daemon 二进制补齐系统安装名与 install 规则 | 7.3、7.4、7.10.3 | `/usr/bin/dasall`、`/usr/sbin/dasall-daemon` 的 `OUTPUT_NAME` / install 规则 | L3 | `apps/cli/CMakeLists.txt`；`apps/daemon/CMakeLists.txt` | executable `OUTPUT_NAME`、`install(TARGETS ...)` | contract：`CliJsonOutputContractTest`、`CliExitCodeContractTest`；integration：`DaemonBinaryUnarySmokeTest` | `cmake -S . -B build-ci -G Ninja && cmake --build build-ci --target dasall-cli dasall-daemon dasall-cli_json_output_contract_test dasall-cli_exit_code_contract_test dasall_access_daemon_binary_unary_smoke_integration_test && ctest --test-dir build-ci -R "CliJsonOutputContractTest|CliExitCodeContractTest|DaemonBinaryUnarySmokeTest" --output-on-failure && rm -rf stage/pkg && DESTDIR="$PWD/stage/pkg" cmake --install build-ci && test -x stage/pkg/usr/bin/dasall && test -x stage/pkg/usr/sbin/dasall-daemon` | PKG-TODO-005 | PKG-BLK-01 | 顶层 install surface 已建立 | CLI/daemon 安装面与二进制命名 | 仅当 stage install 中实际出现 `/usr/bin/dasall` 与 `/usr/sbin/dasall-daemon`，且相关 contract/integration 继续通过时完成 |
| PKG-TODO-007 | NotStarted | 为 profiles / LLM baseline 资产补齐 `dasall-common` install surface | 7.2.1、7.4、7.6、7.10.3 | 五档 profile、Prompt baseline、Provider baseline 安装到 `/usr/share/dasall/` | L2 | `profiles/CMakeLists.txt`；`llm/CMakeLists.txt`；必要时资产目录 manifest | `install(DIRECTORY/FILES ...)` for profile / prompt / provider assets | unit：`ProfileCatalogTest`、`RuntimePolicyProviderTest`、`LLMSubsystemConfigProjectionTest` | `cmake -S . -B build-ci -G Ninja && cmake --build build-ci --target dasall_profile_catalog_unit_test dasall_runtime_policy_provider_unit_test dasall_llm_subsystem_config_projection_unit_test && ctest --test-dir build-ci -R "ProfileCatalogTest|RuntimePolicyProviderTest|LLMSubsystemConfigProjectionTest" --output-on-failure && rm -rf stage/pkg && DESTDIR="$PWD/stage/pkg" cmake --install build-ci && test -f stage/pkg/usr/share/dasall/profiles/desktop_full/runtime_policy.yaml && test -f stage/pkg/usr/share/dasall/llm/prompts/planner/default/manifest.yaml && test -f stage/pkg/usr/share/dasall/llm/providers/catalog.yaml` | PKG-TODO-004、005 | PKG-BLK-01、PKG-BLK-02 | asset install root 与 install 规则统一 | `dasall-common` 资产安装面 | 仅当 stage install 中出现完整 profile / prompt / provider 树，且现有 profile/LLM 单测保持绿色时完成 |
| PKG-TODO-008 | NotStarted | 新建 Debian source package 元数据骨架 | 7.1、7.9、7.10.1、7.10.4；Maint Guide 第 5 章 | `debian/control`、`debian/changelog`、`debian/rules`、`debian/source/format`、`debian/copyright` | L2 | `debian/control`；`debian/changelog`；`debian/rules`；`debian/source/format`；`debian/copyright` | source package metadata / debhelper 入口 | build：`dpkg-parsechangelog`、`dpkg-checkbuilddeps`、`debian/rules` preflight | `dpkg-parsechangelog && dpkg-checkbuilddeps && test -x debian/rules && sed -n '1,120p' debian/control` | PKG-TODO-001、005、006、007 | PKG-BLK-04 | 依赖矩阵与 install surface 已形成最小闭环 | source package skeleton | 仅当 `debian/` 必需 source files 全部落盘，且 changelog/control/rules/source format 语法通过最小检查时完成 |
| PKG-TODO-009 | NotStarted | 新建 binary package payload skeleton 与 service/config/doc 骨架 | 7.7、7.8、7.10.1、7.10.3、7.10.6、7.10.9 | `.install`、`dasall-daemon.service`、`postinst`、`README.Debian`、package-local payload tree、`debian/tests/control` 骨架 | L2 | `debian/dasall-cli.install`；`debian/dasall-daemon.install`；`debian/dasall-common.install`；`debian/dasall-daemon.service`；`debian/dasall-daemon.postinst`；`debian/dasall-daemon.README.Debian`；`debian/dasall-daemon/etc/default/dasall-daemon`；`debian/dasall-daemon/etc/dasall/daemon.json`；`debian/tests/control` | service/config/doc/package payload skeleton | build/package inspect：deb 内容与 control area 可检查 | `dpkg-buildpackage -us -uc -b && dpkg-deb -c ../dasall-daemon_*.deb | rg "etc/default/dasall-daemon|etc/dasall/daemon.json|lib/systemd/system/dasall-daemon.service|usr/share/doc/dasall-daemon/README.Debian" && dpkg-deb -c ../dasall-common_*.deb | rg "usr/share/dasall/"` | PKG-TODO-001、002、003、008 | PKG-BLK-01、PKG-BLK-03、PKG-BLK-04 | source metadata 与 install surface 已可驱动构包 | binary package skeleton | 仅当三类 binary skeleton 可被 `dpkg-buildpackage` 消费，且包内容与设计文档一致时完成 |
...
| PKG-TODO-013 | NotStarted | 落 package-mode service/config 语义与 daemon 首次部署输入面 | 7.7、7.7.4、7.8、7.10.6、7.10.9 | service `ExecStartPre/ExecStart`、EnvironmentFile、daemon.json 模板、最小 postinst 提示 | L2 | `debian/dasall-daemon.service`；`debian/dasall-daemon.postinst`；`debian/dasall-daemon/etc/default/dasall-daemon`；`debian/dasall-daemon/etc/dasall/daemon.json`；必要时 daemon config 相关源码 | `ExecStartPre`、`ExecStart`、`print_first_install_hint()`、profile/config 组合入口 | package inspect：daemon 包内容；installed-package：`pkg-smoke-local-control-plane` | `dpkg-buildpackage -us -uc -b && dpkg-deb -c ../dasall-daemon_*.deb | rg "etc/default/dasall-daemon|etc/dasall/daemon.json|lib/systemd/system/dasall-daemon.service" && dpkg-deb -e ../dasall-daemon_*.deb ../build-ci/pkg-inspect-daemon && rg -n "DASALL was installed successfully|enable --now dasall-daemon.service|#DEBHELPER#" ../build-ci/pkg-inspect-daemon/postinst` | PKG-TODO-002、009、011 | PKG-BLK-03 | package-mode profile/config seam 已冻结 | service/config/postinst 实现 | 仅当 daemon package 的 service、配置模板、EnvironmentFile、postinst hint 全部与设计文档一致，且不再依赖口头说明时完成 |
| PKG-TODO-014 | NotStarted | 补齐 operator docs、manpage 与首次部署文档安装面 | 7.7.4、7.10.1、7.10.2、7.10.9；`dh_installdocs(1)` | `README.Debian`、CLI manpage、必要时 `NEWS.Debian` | L2 | `debian/dasall-daemon.README.Debian`；`debian/dasall.1` 或 `debian/dasall-cli.manpages`；必要时 `debian/dasall-daemon.NEWS` | README / manpage / operator onboarding 文档 | package inspect：文档是否正确落到 doc/man 路径 | `dpkg-buildpackage -us -uc -b && dpkg-deb -c ../dasall-daemon_*.deb | rg "usr/share/doc/dasall-daemon/README.Debian" && dpkg-deb -c ../dasall-cli_*.deb | rg "usr/share/man/man1/dasall.1"` | PKG-TODO-009、013 | 无 | binary package skeleton 与 service/config 语义已落定 | operator docs / manpage | 仅当 README.Debian、manpage 与首次部署提示形成一致口径，且文档由正确 helper 自动安装时完成 |
...
|---|---|---|---|---|---|
| PKG-BLK-01 | 工程没有 install surface，`dh_auto_install` 无法产出可分包安装面 | PKG-TODO-004 ~ 009 | 顶层与关键子模块已有稳定 `cmake --install` 输出 | 完成 PKG-TODO-005 | 若 install 面仍不稳，暂不推进 `debian/`，只保留 stage install proof-of-concept |
| PKG-BLK-02 | daemon / LLM / profile 资产根仍是 repo-relative，安装后二进制无法独立运行 | PKG-TODO-004、007、010、011、012、013 | install-aware path resolver 落地且相关 tests 全绿 | 完成 PKG-TODO-004、010、011、012 | 若路径收敛暂时失败，不允许用 systemd `WorkingDirectory` 伪修复 |
| PKG-BLK-03 | package-mode profile 选择仍依赖 CLI 参数语义，`/etc/default` 与 daemon.json ownership 还未形成最终实现口径 | PKG-TODO-002、009、013 | 明确 v1 采用 `EnvironmentFile + --profile-id` 还是补新 schema | 完成 PKG-TODO-002 | 若短期无法扩 schema，维持 `/etc/default/dasall-daemon` 作为 v1 基线 |
| PKG-BLK-04 | `debian/control` 的 Build-Depends / Depends 仍有占位，尚未在目标 Ubuntu 版本实测 | PKG-TODO-001、008、009 | 目标 Ubuntu 版本上的依赖盘点完成 | 完成 PKG-TODO-001 | 在依赖未冻结前，禁止给出“可直接构包”的结论 |
| PKG-BLK-05 | installed-package QA 环境尚未具备：缺少 `lintian` 规则收口、autopkgtest testbed 约定、本地 rootful smoke harness | PKG-TODO-003、016、017、018 | QA 策略、testbed 与 smoke harness 全部落地 | 完成 PKG-TODO-003、016、017 | 若 testbed 暂缺，只能宣称“build-ready packaging skeleton”，不得宣称 package gate closed |
...
2. 最小可执行闭环已经清晰：先做 PKG-TODO-001 ~ 005，随后串联 PKG-TODO-006 ~ 013，最后以 PKG-TODO-015 ~ 017 建立 package gate。
