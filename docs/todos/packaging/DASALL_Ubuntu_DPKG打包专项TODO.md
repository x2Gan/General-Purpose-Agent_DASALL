...
当前结论：Ubuntu DPKG 打包设计已 Ready，但工程实现仍处于 Implementation Not Ready / Package Gate Not Started。当前仓库已具备 `dasall-cli`、`dasall-daemon`、`dasall_profiles`、LLM baseline 资产和一组可复用的 contract/integration tests；其中 CLI config `CLCFG-TODO-002` 已冻结安装态 canonical socket=`/run/dasall/daemon.sock`、P0 operator model=`0600 root/sudo-only` 与安装态公开命令名=`dasall config`。后续 package Build 仍需补 install surface、`debian/` 元数据与 installed-package QA gate，但不应再回退到 `0660 dasall group` 或安装态 `dasall-cli` 文案。
