...
| PKG-BLK-02 | repo-relative 路径实现尚未完全清除：PKG-TODO-011 已让 daemon profile 资产发现切到 `infra/config::InstallLayout`，但 LLM Prompt / Provider baseline root 仍保留 module-local repo-relative 默认值 | PKG-TODO-012 | install-aware path resolver 按 frozen owner 落地且相关 tests 全绿 | 完成 PKG-TODO-012 | 若路径收敛暂时失败，不允许用 systemd `WorkingDirectory` 伪修复 |
