...
| `debian/` | 已具 `control`、`changelog`、`rules`、`source/format`、`copyright` source skeleton；binary package payload/service/config/doc 文件仍待补齐 | source package 元数据已建立，binary package 元数据仍待 009 落盘 |
...
| Debian source metadata | source package skeleton 已建立，binary package payload skeleton 仍缺 | PKG-TODO-001、008、009 | 可执行的 source package/binary package 骨架 |
