...
| PKG-BLK-02 | repo-relative 路径实现仍未完全清除：PKG-TODO-010 已收敛 canonical socket 默认值，但 daemon asset root 与 LLM baseline root 仍保留 `cwd` / module-local 假设 | PKG-TODO-011、012、013 | install-aware path resolver 按 frozen owner 落地且相关 tests 全绿 | 完成 PKG-TODO-011、012 | 若路径收敛暂时失败，不允许用 systemd `WorkingDirectory` 伪修复 |
