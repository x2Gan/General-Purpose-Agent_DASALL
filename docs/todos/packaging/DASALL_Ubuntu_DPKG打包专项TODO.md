...
| PKG-BLK-02 | repo-relative 路径实现仍未清除：PKG-TODO-004 已冻结 owner=`infra/config::InstallLayout`，但 daemon / profiles / LLM 仍保留 module-local 默认值与 `cwd` 假设 | PKG-TODO-007、010、011、012、013 | install-aware path resolver 按 frozen owner 落地且相关 tests 全绿 | 完成 PKG-TODO-010、011、012 | 若路径收敛暂时失败，不允许用 systemd `WorkingDirectory` 伪修复 |
