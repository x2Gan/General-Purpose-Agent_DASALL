...
| `apps/cli/CMakeLists.txt`、`apps/daemon/CMakeLists.txt` | executable target 已由 `apps/CMakeLists.txt` 接入 install skeleton，但 `OUTPUT_NAME` 与正式安装名仍未收敛 | stage install 已可发现 CLI / daemon，最终 `/usr/bin/dasall` 与 `/usr/sbin/dasall-daemon` 继续由 PKG-TODO-006 完成 |
...
|---|---|---|---|
| 顶层与子模块 install surface | GNUInstallDirs + CLI/daemon install skeleton 已建立，剩余 binary naming 与 common assets install 仍缺 | PKG-TODO-006、007 | `debian/tmp` 可消费的标准安装面 |
