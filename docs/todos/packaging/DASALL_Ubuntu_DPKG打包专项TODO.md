...
| PKG-BLK-05 | installed-package QA 仍未完全闭环：PKG-TODO-003 已冻结 `lintian` / `autopkgtest` / qemu testbed 策略，PKG-TODO-016 已补齐 `debian/tests` metadata/scripts，但本地 rootful smoke harness 与最终 gate 收口仍缺 | PKG-TODO-017、018 | QA 策略已冻结，且 installed-package testbed 实现与 smoke harness 全部落地 | 完成 PKG-TODO-017 | 若 testbed 暂缺，只能宣称“build-ready packaging skeleton”，不得宣称 package gate closed |
