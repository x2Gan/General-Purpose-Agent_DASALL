...
4. `tests/integration/access/CMakeLists.txt`
...
```text
Build_CMakeTools(buildTargets=["dasall_access_daemon_unary_integration_test","dasall_access_daemon_reject_path_integration_test"])
RunCtest_CMakeTools(tests=["DaemonUnaryIntegrationTest","DaemonRejectPathIntegrationTest"])
```
...
3. `RunCtest_CMakeTools` stderr 仍打印仓库既有 `DartConfiguration.tcl` 缺失提示，但返回码为 0，按当前仓库基线计为有效 focused gate 证据。
