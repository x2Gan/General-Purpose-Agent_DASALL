...
当前结论：daemon 本地控制面专项 `DMD-TODO-001` ~ `DMD-TODO-035` 已形成首轮收敛基线并完成 focused gate / deliverable 回写，但 2026-05-02 的代码与部署评审新增 `DMD-TODO-036` ~ `DMD-TODO-040` 五项回归整改任务，专项状态从 fully-closed 调整为 reopen hardening。当前无新增外部 blocker；后续以默认 endpoint 一致性、真实 socket 权限与 stale restart、profile/config/reload 真接线，以及文档/测试/证据复验为主，并在整改完成后重跑 Gate-DMD-05 / Gate-DMD-07 / Gate-DMD-09 / Gate-DMD-10。
...
| DMD-TODO-036 | NotStarted | 收敛 daemon/CLI 默认 endpoint 与 CLI socket-path 覆盖 | 2026-05-02 评审发现 daemon 默认 path、CLI 默认 path 与 daemon socket policy 三者不一致；daemon 详设 6.7.3、6.10；DMD-TODO-031/035 follow-up | 6.7.3 socket_path；6.10 配置来源与 CLI 入口契约 | L2 | 抽取 daemon/CLI 共用 endpoint 默认值来源，更新 `apps/cli/src/main.cpp`、`apps/cli/src/CliCommandParser.*` 与必要的 shared endpoint defaults header；若保留 CLI 显式覆盖，必须支持 `--socket-path` 或等价受控参数，并同步 `docs/deploy/daemon/README.md` / `ACCEPTANCE_CHECKLIST.md` | `kDefaultDaemonSocketPath`、CLI endpoint override parsing、`CliCommandParser::parse()`、`main()` endpoint resolution | unit：扩展 `CliDaemonCommandParserTest`；integration：新增 `CliDaemonSocketPathIntegrationTest` 或等价 real smoke，验证 CLI 在默认值与显式 socket-path 两条路径下都能正向连到 daemon | `cmake -S . -B build-ci -G "Unix Makefiles" && cmake --build build-ci --target dasall_cli dasall_daemon dasall_access_daemon_ping_integration_test && ctest --test-dir build-ci -R "CliDaemonCommandParserTest|CliDaemonSocketPathIntegrationTest|DaemonPingIntegrationTest" --output-on-failure` | DMD-TODO-031、035 | 无 | 无 | `docs/todos/daemon/deliverables/DMD-TODO-036-daemon-cli-endpoint统一与覆盖收敛.md`；共享 endpoint defaults、CLI 参数与部署文档更新 | 仅当 CLI 与 daemon 默认 socket_path 来自同一处定义，CLI 可通过默认值和显式 socket-path 两条路径正向连通 daemon，且部署文档不再依赖“CLI 固定连接旧路径”的补充说明时完成 |
| DMD-TODO-037 | NotStarted | 收敛真实 UDS socket mode 与 stale restart 安全回归 | 2026-05-02 评审发现真实 daemon 创建的 socket mode 与 cleanup policy 不一致；daemon 详设 6.7.3、6.12；DMD-TODO-032/035 follow-up | 6.7.3 socket 文件权限；6.12 socket 地址冲突与 stale cleanup | L2 | 更新 `platform/src/linux/UnixIpcProvider.cpp`、`apps/daemon/src/DaemonSocketPolicy.*` 与必要的 `DaemonListenerHost` bind/close 路径，让真实 bind 后的 socket mode 收敛到 policy 要求，并补齐 daemon 创建 socket 的 stale cleanup / restart-safe 行为 | `UnixIpcProvider::listen()`、socket mode apply helper、`try_cleanup_stale_socket()`、restart-safe bind preflight | unit：扩展 `DaemonSocketPolicyTest`、`UnixIpcProviderTest`；integration：新增 `DaemonSocketModeIntegrationTest`、`DaemonStaleSocketRecoveryIntegrationTest` 或等价 restart smoke | `cmake -S . -B build-ci -G "Unix Makefiles" && cmake --build build-ci --target dasall_daemon dasall_daemon_config_validator_unit_test dasall_access_daemon_ping_integration_test && ctest --test-dir build-ci -R "DaemonSocketPolicyTest|UnixIpcProviderTest|DaemonSocketModeIntegrationTest|DaemonStaleSocketRecoveryIntegrationTest|DaemonPingIntegrationTest" --output-on-failure` | DMD-TODO-032、035 | 无 | 无 | `docs/todos/daemon/deliverables/DMD-TODO-037-UDS权限与stale-restart收敛.md`；socket mode / restart 回归代码与测试 | 仅当 daemon 实际创建的 socket mode 与 cleanup policy 对齐、daemon 异常退出后的 stale socket 可被安全回收、并且 restart smoke 能稳定通过时完成 |
| DMD-TODO-038 | NotStarted | 将 profile/config snapshot 真正接入 daemon 入口组合根 | 2026-05-02 评审发现 profile projection 仍主要停留在 helper / harness，main 仍硬编码 profile_id 与健康元数据；daemon 详设 6.6.1、6.10.1、6.10.3；DMD-TODO-003/004/009 follow-up | 6.6.1 启动主流程；6.10 ConfigCenter 四层模型 | L2 | 新增 `apps/daemon/src/DaemonEntryConfigLoader.{h,cpp}` 或等价 loader，更新 `apps/daemon/src/main.cpp` 与 `apps/daemon/CMakeLists.txt`，将 `profiles/include/DaemonProfileProjection.h` 与 deployment snapshot 接到真实入口；移除 `main.cpp` 中写死的 `daemon_profile_id` / `effective_profile_id` / 健康布尔常量 | `DaemonEntryConfigLoader::load()`、`resolve_bootstrap_config_from_profile()`、entry bootstrap wiring | unit：新增 `DaemonEntryConfigProjectionTest`；integration：扩展 `DaemonProfileCompatibilityTest` 或新增 entry-level profile smoke，验证真实入口能消费 profile 产物 | `cmake -S . -B build-ci -G "Unix Makefiles" && cmake --build build-ci --target dasall_daemon dasall_daemon_profile_projection_unit_test && ctest --test-dir build-ci -R "DaemonEntryConfigProjectionTest|DaemonProfileProjectionTest|DaemonProfileCompatibilityTest|DaemonBootstrapTest" --output-on-failure` | DMD-TODO-003、004、009、035 | 无 | 无 | `docs/todos/daemon/deliverables/DMD-TODO-038-daemon入口profile-config接线收敛.md`；入口 loader、focused tests 与文档回写 | 仅当 daemon 二进制入口不再依赖 `main.cpp` 中写死的 profile_id / readiness 常量，且 profile / deployment snapshot 能通过真实 main/bootstrap 路径进入 process context 时完成 |
| DMD-TODO-039 | NotStarted | 收敛 hot-reload 真实快照源与正向复验门 | 2026-05-02 评审发现 SIGHUP 目前主要重复 apply 初始 `parsed.config`，缺少 fresh snapshot source；daemon 详设 6.10.4、6.11；DMD-TODO-033 follow-up | 6.10.4 热重载矩阵；6.11 reload denied / applied 审计 | L2 | 在 `apps/daemon/src/DaemonConfigReloader.*` 与 `main.cpp` 上新增真实 reload candidate source，复用 `DMD-TODO-038` 的 loader / snapshot 解析路径；allowlisted key 变更必须能被进程实际观察到，restart-only key 仍拒绝并审计 | `load_reload_candidate()`、`DaemonConfigReloader::apply_reload_snapshot()`、reload audit projection | unit：扩展 `DaemonConfigReloadTest`；integration：新增 `DaemonHotReloadIntegrationTest`，验证 allowlisted key 正向生效、restart-only key 拒绝、审计字段稳定 | `cmake -S . -B build-ci -G "Unix Makefiles" && cmake --build build-ci --target dasall_daemon dasall_daemon_config_reload_unit_test && ctest --test-dir build-ci -R "DaemonConfigReloadTest|DaemonHotReloadIntegrationTest|DaemonSignalHandlerTest|DaemonObservabilityFieldSetTest" --output-on-failure` | DMD-TODO-033、038 | 无 | 无 | `docs/todos/daemon/deliverables/DMD-TODO-039-daemon热重载真实快照收敛.md`；reload source、integration test 与审计回写 | 仅当 SIGHUP 会读取 fresh snapshot 而不是重复初始 config，allowlisted key 的运行效果可观测、restart-only key 继续拒绝且有稳定审计事实时完成 |
| DMD-TODO-040 | NotStarted | 清理过期文档、陈旧测试并回写专项复验证据 | 2026-05-02 评审发现部署文档仍残留“CLI 不消费 readiness”口径，且存在陈旧 `CliDaemonPingIntegrationTest.cpp`；DMD-TODO-028/035 follow-up | 7 Gate / 9 验收 / 11 结论回写 | L2 | 更新 `docs/deploy/daemon/README.md`、`docs/deploy/daemon/ACCEPTANCE_CHECKLIST.md`、`docs/todos/daemon/DASALL_daemon本地控制面专项TODO.md`、相关 deliverables / worklog；删除、重写或正式注册 `tests/integration/access/CliDaemonPingIntegrationTest.cpp`，避免 send-only smoke 残留 | `Gate-DMD-05/07/09/10` 复验证据、陈旧 smoke 清理、文档一致性检查 | process + focused validation：负向检查过期口径已清除；回归 `DaemonPingIntegrationTest`、`CliDaemonCommandParserTest`、`DaemonConfigReloadTest`、`DaemonProfileCompatibilityTest` | `! rg -n "当前 CLI 尚未消费 readiness 响应|CliDaemonPingIntegrationTest|专项状态从 fully-closed" docs/deploy/daemon docs/todos/daemon tests/integration/access && ctest --test-dir build-ci -R "DaemonPingIntegrationTest|CliDaemonCommandParserTest|DaemonConfigReloadTest|DaemonProfileCompatibilityTest" --output-on-failure` | DMD-TODO-036、037、038、039 | 无 | 无 | `docs/todos/daemon/deliverables/DMD-TODO-040-daemon文档与证据复验收敛.md`；清理后的文档 / 测试 / worklog | 仅当部署文档、专项 TODO、deliverables 与测试拓扑不再残留过期口径，历史 send-only smoke 已被删除或改写为有效 gate，且 Gate-DMD-05 / 07 / 09 / 10 的复验证据已回写时完成 |
...
说明：以下 `Gate-DMD-01` ~ `Gate-DMD-09` 为 2026-05-02 回归评审前的首轮收敛基线。根据同日代码/部署评审，`DMD-TODO-036` ~ `DMD-TODO-040` 已将专项状态调整为 reopen hardening；后续需在本节补回 `Gate-DMD-10`，并重跑 `Gate-DMD-05` / `Gate-DMD-07` / `Gate-DMD-09` 的复验证据。
...
| daemon / CLI 默认 endpoint 与部署 smoke 口径仍存在回归整改项 | Reopened by 2026-05-02 review | CLI 正向互通、README / checklist 与默认 socket_path 的口径可能继续漂移 | 通过 `DMD-TODO-036` / `040` 统一默认值、补 CLI override 与文档复验；整改完成前不恢复 fully-closed 结论 |
| daemon 入口仍缺少 profile/config/reload 真接线的最终证据 | Reopened by 2026-05-02 review | `DMD-TODO-003` / `033` 的 helper 级能力尚未等价覆盖真实 binary 入口 | 通过 `DMD-TODO-038` / `039` 把 snapshot loader 接入 main，并补 process-level reload 正向 gate |

## 11. 可行性结论

### 11.1 当前执行结论

1. `DMD-TODO-001` ~ `DMD-TODO-035` 已形成首轮收敛基线，`Gate-DMD-01` ~ `Gate-DMD-09` 的历史 focused evidence 仍然可追溯，但 2026-05-02 回归评审已新增 `DMD-TODO-036` ~ `DMD-TODO-040`，专项状态转入 reopen hardening。
2. 当前可直接复用的稳定能力仍包括 local direct-bind UDS、unary + accepted_async、ping/status/cancel/只读 diag、readiness、CLI wire contract、并发/soak 与部署契约的首轮实现；但默认 endpoint 一致性、真实 socket mode / stale restart、profile/config/reload 真接线与 docs/test evidence 仍需补强。
3. 权威证据仍位于本文件 §9.4/§9.5、`docs/todos/daemon/deliverables/` 各交付物与 `docs/worklog/DASALL_开发执行记录.md`，但在 `DMD-TODO-036` ~ `DMD-TODO-040` 完成前，不再以“`DMD-TODO-001` ~ `DMD-TODO-035` 已全部完成”作为最终 closed 结论，需补跑 `Gate-DMD-05` / `Gate-DMD-07` / `Gate-DMD-09` / `Gate-DMD-10` 的复验证据。
