...
|---|---|---|---|---|---|
| DMD-BLK-001 | daemon profile 键资产未完成一致性验证 | DMD-TODO-003、027 | 详设列出 daemon 配置键；当前需逐 profile 验证 | 五档 profile 均有 daemon 运行键或明确安全默认投影 | 默认 diag/override/watchdog 关闭，socket_path 使用部署层保守默认 |
| DMD-BLK-002 | supervisor/watchdog 对外通知 surface 未冻结 | DMD-TODO-008 | 详设只给 READY/STOPPING/WATCHDOG 方向，未冻结 systemd 或 infra bridge 细节 | v1 冻结 no-op + infra watchdog event 最小接口；systemd 专用能力延后 | supervisor adapter 采用 no-op，不阻塞 ping/unary |
| DMD-BLK-003 | apps composition root 是否允许 include access concrete 需评审 | DMD-TODO-010 | 当前 `apps/daemon` 与 `apps/gateway` 均 include `access/src/AccessGateway.h`；蓝图强调接口优先 | access/apps 负责人确认保留现状或新增 public factory | 保留当前 concrete include，但不得扩张到其他上层实现 |
| DMD-BLK-004 | 当前 `UnixIpcProvider` 未形成真实双向 request/response loopback | DMD-TODO-006、009、019、024、025、026、027、029、031、034 | `connect/send` 与 listener `accept/receive` 当前不共享 payload；`CliDaemonPingIntegrationTest` 未启动 daemon | 完成 DMD-TODO-029，platform 提供真实 UDS 实现或测试专用 loopback fixture，且 CLI/client 可读取 daemon response | daemon 单测先用 fake IIPC；集成 gate 标 blocked，不冒充 E2E |
