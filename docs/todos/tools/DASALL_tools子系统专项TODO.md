...
适用范围：tools/  
当前结论：Tools 子系统已具备 L3/L2 混合粒度的专项拆分条件；公共接口/对象、治理链骨架、runtime caller fixture、builtin -> services 最小闭环、workflow/compensation 主链可直接进入 Build 规划，MCP loopback 夹具与 external skill 样本两项仍需以前置补设计/夹具任务解阻，generic MCP ready 与外部 Skill 兼容不得写成当前已可交付事实。
...
| ToolPolicyGate / ToolRouteSelector / ToolManager | 6.2、6.7、6.12.1 | L2 | fail-closed、route fallback、batch invoke、lane 选择与原因码规则明确；023 已冻结 tools-side caller fixture 口径 | reason code taxonomy 与完整执行/投影链尚未完全收口 | 先做组件骨架，再在 015b 中接通完整治理链 |
