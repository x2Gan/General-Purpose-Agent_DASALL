...
当前结论：`Gate-INT-08` 与 `Gate-INT-09` 已能证明 library/focused system integration 绿态，且 2026-05-09 连续两轮修复后 `dasall_packaging_preflight_tests` 已恢复 build-tree daemon app-binary / release-preflight 绿态，`apps/gateway` 也已接入 production runtime backend 并新增真实 binary unary smoke；当前剩余缺口收敛为 app entrypoint readiness 结构化语义与 daemon/gateway live dependency baseline，尚待 `INTFIX-TODO-007` / `008` / `009` / `010` / `011` 收口。本文档作为 5 月 6 日系统集成专项的接续规划，不改写旧 Gate 历史结论，而是新增一组面向真实二进制入口、release gate 与可诊断性的修复任务。
...
| Packaging preflight | `Build_CMakeTools(buildTargets=["dasall_access_daemon_binary_unary_smoke_integration_test","dasall_packaging_preflight_tests"])` 通过 | daemon app-binary / build-tree release-preflight 已恢复绿态；gateway release-preflight wiring 待 009/010 闭环 |
...
| gateway binary | `Build_CMakeTools(buildTargets=["dasall_gateway","dasall_access_gateway_submit_composition_test","dasall_access_gateway_binary_unary_smoke_integration_test"])` 通过 | gateway production submit pipeline owner 已回到 app composition root；缺 backend fail-closed regression 与 Gate-INT-10 wiring 待 009/010/011 收口 |
