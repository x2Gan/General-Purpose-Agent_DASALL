...
| multi_agent | 详设有主流程、状态机和投影规则；source full profiles 已校准为禁用态；installed package 旧资产仍需复核 | 设计链路清楚，当前实现仍只有 placeholder；未实现 coordinator 前不得启用 | 不得宣称 GA runtime-ready 或 profile-enabled production path |
...
| BC-17 | Multi-agent coordination | Runtime MultiAgentRequest | Runtime -> MultiAgentCoordinator -> Planner -> Registry/Lease/Scheduler -> WorkerBridge -> Merger -> Runtime fold Observation | MultiAgent §6.8、§6.9 | 详设清晰；source profile 已回落禁用态；multi_agent 模块仍只有 placeholder | 实现仍未闭合；需要 Null/Real coordinator、Observation 折叠与 Runtime owner 装配 Gate | FULLINT-TODO-004、018 |
...
| Task ID | Status | 任务标题 | 设计依据 | 精确范围 | 粒度 | 代码目标 | 目标函数/接口/数据结构 | 测试目标 | 验收命令 | 前置任务 | 关联阻塞项 | 解阻条件 | 交付物 | 完成判定 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| FULLINT-TODO-005 | Done | 新增全量业务链 discoverability verifier 清单 | `tests/CMakeLists.txt`；SystemIntegrationGateMatrix；`docs/ssot/BusinessChainIntegrationMatrix.md` | Gate discoverability 与 business-chain labels；BC-17 missing gate 显式标记 | L2 | `tests/VerifySystemGateDiscoverability.cmake`；`tests/CMakeLists.txt`；`docs/todos/integration/deliverables/FULLINT-TODO-005-全量业务链discoverability清单.md` | `DASALL_FULL_BUSINESS_CHAIN_DISCOVERABILITY_TEST_NAMES`、`EXPECTED_MISSING_GATES_CSV`、`BC-17:multi_agent_coordinator_runtime_gate_missing` | ctest discoverability：Access ingress、Gate-INT-03~10、knowledge/memory/tools/services/llm/profile/infra/package 代表测试；actual installed：`sudo -n dasall ping/readiness/status/cancel/diag` | `Build_CMakeTools(buildTargets=["dasall_full_business_chain_discoverability"])`；`rg -n "dasall_full_business_chain_discoverability|BC-17:multi_agent|FULLINT-TODO-005|status_missing|diag_disabled" tests docs/todos/integration docs/worklog/DASALL_开发执行记录.md` | 001、002 | 无 | 无 | `docs/todos/integration/deliverables/FULLINT-TODO-005-全量业务链discoverability清单.md` | BC-01~BC-16 均有当前 CTest 可发现代表入口；BC-17 被机器可读 missing gate 标记覆盖；安装态 rootful control-plane、status/cancel 缺失 receipt 与 diag disabled 已当轮记录，未外推为 qemu/production ready |
...
4. 第四轮后续执行：FULLINT-TODO-018，补 Null/Real coordinator、Observation 折叠与恢复/side-effect 链路。
5. Release 前执行：FULLINT-TODO-019、021、022，在 release runner 归档 qemu / lintian / LLM smoke / worklog 证据。

## 10. 当前本轮交付说明

截至 2026-05-11 本轮已完成并提交 `FULLINT-TODO-001` ~ `FULLINT-TODO-004`：业务链 SSOT、package 证据分层、runtime/cognition/memory/llm 主链证据包、profile/multi_agent 声明边界校准均已落盘。下一轮可从 `FULLINT-TODO-006`、`FULLINT-TODO-007` 或 `FULLINT-TODO-012` 开始进入 Build / Debug / Gate 实施。
