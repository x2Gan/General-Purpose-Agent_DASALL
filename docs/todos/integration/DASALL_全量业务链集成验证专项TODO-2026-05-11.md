...
| FULLINT-BLK-005 | 部分解阻（2026-05-11 / FULLINT-TODO-008）：gateway shutdown exit-code 已显式断言；startup failure stage 覆盖仍不足 | BC-02、BC-13 | 已补 gateway exit-code 断言；下一步由 FULLINT-TODO-009 补 runtime composition/init/listen/bind failure diagnostics binary regressions | release candidate 可继续编译，但 release confidence 仍受 startup diagnostics 缺口降级 |
