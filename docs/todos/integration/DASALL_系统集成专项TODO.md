...
|---|---|---|---|---|---|
| INT-BLK-01 | TODO 新增 | default unary required/optional product mode 未冻结 | 010、014、021 无法稳定判断 ready 与 degraded | 完成 001 并通过相关 owner 评审 | 未冻结前，只允许 fixture / docs 级验证，不宣布 default ready |
...
| INT-BLK-06 | Access 残项吸收 | Access v1 production path 仍存在 `AgentRequest` handoff 断裂、空 pipeline 可 Ready 与安全治理未闭合 | 025、027、028、030 无法宣称 ingress production ready | 完成 025、027、028，并使 focused Access Gate 转绿 | 在解阻前，daemon/gateway 只能显式使用 mock pipeline 作为测试 profile，不得冒充 production 证据 |
| INT-BLK-07 | health/watchdog 重定级 | health cadence / event publish boundary 未完全冻结；其中 timer seam 已存在，但 event publish 最小接口仍未明确 | 026、029 不能稳定形成统一 system cadence | 完成 026，并明确 029 采用 `ITimer` seam + event fallback 规则 | event publish 未冻结前，仅允许同步 evaluate、日志/指标 fallback 与局部缓存，不宣称 bus-ready |
...
|---|---|---|---|---|
| INT-BLK-01 | 2026-05-06 | 未解阻 | required/optional mode 相关实现与 gate | 来源于集成评审 GINT-02 |
...
| INT-BLK-06 | 2026-05-06 | 未解阻 | Access production path、安全治理与 Access Gate | 来源于 Access 专项 TODO 034 ~ 050 与代码核对 |
| INT-BLK-07 | 2026-05-06 | 部分重定级 | health cadence / event publish residual | `ITimer` / `PosixTimerProvider` 已存在，HLT-BLK-001 不再应作为纯前置抽象 blocker；真正残留的是 event publish 与 cadence 口径 |
...
| contract / compile surface | `ContextPacketContractTest`、`RetrievalEvidenceRefContractTest`、`MemoryEvidenceProjectionCompileTest`、`ServiceResultSemanticsContractTest` | additive fields 不破坏旧路径，semantic contract 可编译可回归 |
...
可以，但不建议直接从 Build 任务起步。最合理的起点是 001、002、003、004 四个设计冻结任务并行，然后立即接 005、006、007 把系统级 SSOT 补齐。
