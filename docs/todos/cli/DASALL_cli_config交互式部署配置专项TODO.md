...
当前结论：`dasall-cli config` 方案方向 Ready，但 P0 Build 前置冻结尚未完成，工程实现仍处于 Build Not Started。当前仓库已具备 CLI 本地命令骨架、daemon `validate-only`、file secret backend 与 SecretManager 读链、daemon/CLI 本地控制面基础测试；其中 CLCFG-TODO-001 已完成并冻结 config v1 命令 grammar、TTY/non-TTY 与 exit contract，后续仍待收敛 socket/operator model、InstallState/ActionPlan/事务语义、secret bootstrap-only 写入接缝与 ToolSkillPage capability 边界。建议按 P0/P1/P2 三阶段推进：P0 先完成 grammar、socket/operator、state/action plan、文件事务与测试拓扑冻结，再交付 profile/daemon/service 的本地配置闭环；P1 再补 LLM secret onboarding；P2 才开放 tools/skills operator surface。
...
| CLCFG-BLK-002 | `ISecretManager` 无 create/set，当前不存在 bootstrap 写入 seam | CLCFG-TODO-004、016、021 | secret owner 接受 `SecretBootstrapWriter` 或等价 internal seam，并冻结 file backend root 语义 | 先完成 CLCFG-TODO-004 设计冻结，再补 internal writer | P1 保持 Blocked，只展示 secret 缺口，不收集或落盘 LLM key |
| CLCFG-BLK-003 | tools/skills operator-facing deployment surface 未冻结 | CLCFG-TODO-005、017 | tools owner 明确 bundle/source/allowlist/撤销路径与 deployment overlay owner | 先完成 CLCFG-TODO-005，把 P0/P1 固定为隐藏或只读摘要 | ToolSkillPage 在 P0/P1 彻底隐藏，不在 UI 中暗示“可配置” |
