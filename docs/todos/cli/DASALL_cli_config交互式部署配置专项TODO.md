...
当前结论：`dasall-cli config` 方案方向 Ready，P0/P1 Build 与 closeout 已完成且 CLCFG-TODO-006/007/008/009/010/011/012/013/014/015/016/017/018/019/020/021/022 已完成。当前仓库已具备 CLI 本地命令骨架、daemon `validate-only`、file secret backend 与 SecretManager 读链、daemon/CLI 本地控制面基础测试、canonical daemon 配置文件的受控读写事务骨架、service/preflight 的稳定组件面、workflow/prompt/plan/summary 的单一 owner 骨架、config 专属的 unit/integration discoverability 拓扑，以及 `show/plan/validate/apply --from-file` 非交互工作流、交互式 P0 wizard 的 current-value reuse 闭环、shared apply helper 下的 service action 执行与 installed `dasall config` onboarding 文案对齐、P1 的 LLM secret bootstrap 写入接缝、`prompt/stdin/file:` 三种 secret 输入源与 redacted `auth_ref` summary、tools/skills 的 capability-gated summary surface 与 `ToolSkillPage` 三态投影，以及安装态 `dasall config` secret onboarding smoke、`pkg-smoke-local-control-plane` 的 config plan/apply gate、`validate_cli_config_v1.sh` 统一 validator 入口与 `scripts/packaging/validate_autopkgtest_metadata.py` 提供的 noble/autopkgtest 5.47 metadata compatibility shim；其中 CLCFG-TODO-001/002/003/004/005 已完成并冻结 config v1 命令 grammar、TTY/non-TTY、exit contract、安装态 canonical socket=`/run/dasall/daemon.sock`、`0600 root/sudo-only` operator model、安装态 `dasall config` 命令名、`InstallState` / `ConfigActionPlan` / `ConfigFileWriteTransaction` 的 P0 口径、bootstrap-only secret import seam，以及 `ToolSkillPageMode=hidden|summary_only|editable` 的 P0/P1/P2 capability 边界与未决问题处置；CLCFG-TODO-006 已把 `config/show/plan/validate/apply` grammar、usage 与 local-only dispatch skeleton 接入 CLI parser/main，并新增 `ConfigCommandParserTest` 作为 config parser 专属回归入口；CLCFG-TODO-007 已新增 `apps/cli/src/config/ConfigCommandTypes.*`，把 `InstallState`、`ConfigActionPlan`、`DesiredConfigSnapshot`、`ConfigApplyResult` 收敛为单一类型 owner；CLCFG-TODO-008 已新增 `InstallStateProbe` 与 `ConfigCapabilityResolver` 骨架，把 canonical/service/readiness facts 到六态 `InstallState` 的投影、以及 LLM secret/service/tool-skill capability gating 收敛到单一组件 owner；CLCFG-TODO-009 已新增 `DaemonConfigFileStore` 与 `ConfigFileWriteTransaction` helper，把 `/etc/default/dasall-daemon` 与 `/etc/dasall/daemon.json` 的读取、原子写回、权限保留、`.last-known-good` 备份和失败回滚收敛到单一 file-store owner，并允许在 drifted repair 路径中用原子覆盖修复既有无效 `daemon.json`；CLCFG-TODO-010 已新增 `PrivilegeProbe`、`ConfigPreflightChecker` 与 `ServiceManagerAdapter` 骨架，把 root/TTY 检测、canonical 文件可写性、`--validate-only` preflight 和 `systemctl` service action/degrade 语义收敛到稳定 owner；CLCFG-TODO-011 已新增 `CliConfigWorkflowCoordinator`、`InteractivePromptEngine`、`ConfigPlanFormatter` 与 `ConfigSummaryFormatter` 骨架，把交互默认值/掩码输入、plan human/JSON 投影、summary human/JSON 投影与后续 workflow owner 收敛到稳定组件面；CLCFG-TODO-012 已把 `main.cpp` local config dispatch、coordinator 的 `show/plan/validate` 子切片、只读 current-state plan、validate-only preflight 输出，以及 `ConfigShowWorkflowTest` / `ConfigPlanWorkflowTest` / `ConfigValidateWorkflowTest` / `ConfigShowValidateIntegrationTest` 接通为 focused 可回归出口；CLCFG-TODO-013 已新增 `ConfigDiffPlanner`、minimal desired-state fixture、`config plan --from-file` diff 投影与 `config apply --from-file` headless apply/rollback 流程，并把 preflight 可写性检测修正为向上查找最近已存在父目录；CLCFG-TODO-014 已把 interactive wizard 接到同一 plan/apply 管线，补齐 `WelcomeAndStatePage`/`ProfileSelectionPage`/`DaemonConfigPage`/`ReviewAndApplyPage`/`ServiceActionPage`/`SummaryPage` 的最小 prompt 流和当前值复用，并守住 P0 不重启 group-based operator path；CLCFG-TODO-015 已把 `ServiceManagerAdapter` 的 start/restart/enable 执行接入 shared apply helper，补齐 `ConfigModifyExistingWorkflowTest` 与 summary formatter 对 completed service actions 的 focused 证据，并确认 `debian/dasall-daemon.README.Debian`、`debian/dasall-daemon.postinst`、`debian/dasall.1` 与 `/usr/bin/dasall` 的安装态 onboarding 口径一致；CLCFG-TODO-016 已新增 `infra/src/secret/SecretBootstrapWriter.*` 与 `apps/cli/src/config/LlmSecretPage.*`，把 bootstrap-only secret import 写入、`auth_ref=secret://llm/providers/<provider_ref>` 投影、owner-only import file 校验，以及 `config apply --from-file` / wizard 对 secret bootstrap 的真实接线收敛到 focused workflow owner，并通过 `LlmSecretPageTest`、`ConfigDiffPlannerTest`、`ConfigApplyWorkflowTest`、`ConfigFreshInstallWorkflowTest`、`ConfigInteractiveWizardTest`、`ConfigModifyExistingWorkflowTest`、`SecretBootstrapWriterIntegrationTest`、`FileSecretBackendTest` 与 `SecretManagerFacadeTest` 形成 build-tree 自动化证据；CLCFG-TODO-017 已新增 `apps/cli/src/config/ToolSkillPage.*`，并把 `ConfigCapabilityResolver` 的 tools/skills mode 接到 `ConfigSummaryFormatter`、`CliConfigWorkflowCoordinator` 和 `config show` summary surface，通过 `ToolSkillPageTest`、`ConfigCapabilityResolverTest`、`ConfigSummaryFormatterTest`、`ConfigShowWorkflowTest` 与 `ConfigShowValidateIntegrationTest` 形成 hidden / summary_only / editable 三态投影的 focused build-tree 证据；CLCFG-TODO-018 已补齐 `tests/integration/apps/cli`、`unit;cli;config` / `integration;cli;config` 标签和 `ConfigFreshInstallWorkflowTest` discoverability 入口；CLCFG-TODO-019 已新增 `tests/contract/access/ConfigOutputContractTest.cpp` 并接入 access contract topology，把 `config plan` / `config show` / `config validate` 的 human+JSON stable schema、`config.validate` 的 deterministic exit family，以及 config focused unit/contract 矩阵收敛到同一 focused gate；CLCFG-TODO-020 已新增 `tests/integration/apps/cli/ConfigDriftRepairWorkflowTest.cpp`，并把 drifted repair、non-systemd degrade 与 service-action 分支收敛到独立 integration gate，同时修正 `DaemonConfigFileStore` 在 drifted repair 路径上对无效既有 `daemon.json` 的覆盖语义，使 `ConfigApplyWorkflowTest` 与新的 drift gate 可以共存回归；CLCFG-TODO-021 已扩展 `debian/tests/pkg-smoke-local-control-plane` 与 `scripts/packaging/pkg_smoke_install.sh` 到安装态 `dasall config show/validate/plan/apply --from-file` 主路径，补齐 file-backed secret import、redacted secret ref、service start/enable 与 socket assertions，并新增 `scripts/packaging/validate_cli_config_v1.sh` 串联 metadata validate、secret build-tree regressions 与本地 rootful package smoke；CLCFG-TODO-022 已把 noble 上失效的裸 `autopkgtest --validate .` 调用收敛为 `validate_autopkgtest_metadata.py` shim，并把 validator 的 build-tree 段收敛到 `SecretBootstrapWriterIntegrationTest`、`FileSecretBackendTest`、`SecretManagerFacadeTest` 三个 focused target，使 `validate_cli_config_v1.sh` 在当前具备 `autopkgtest`、passwordless sudo 与 fresh `.deb` 产物的环境中实跑通过。`CLCFG-BLK-002`、`CLCFG-BLK-003`、`CLCFG-BLK-004`、`CLCFG-BLK-005` 已全部解阻。当前 config 专项可宣称 close-ready：P0 profile/daemon/service 本地配置闭环、P1 secret onboarding，以及安装态 `dasall config` rootful lifecycle smoke 已完成统一验收；package-level authoritative qemu `autopkgtest` 仍由 PKG-TODO-018 独立跟踪，不计入 CLCFG 残余 blocker。建议后续只把 `validate_cli_config_v1.sh` 作为回归 gate 维护，不再继续扩写 CLCFG closeout 行。
...
| tools/skills capability 页 | 已补 `ToolSkillPage` capability 摘要与 page mode gating；真实 operator-facing editable list 仍待 tools owner 在后续 P2 冻结 owner-surface 后扩展 | CLCFG-TODO-017 | capability-gated ToolSkillPage summary surface 与 P2 editable 预留 |
...
| Task ID | Status | 任务标题 | 设计依据 | 精确范围 | 粒度 | 代码目标 | 目标函数/接口/数据结构 | 测试目标 | 验收命令 | 前置任务 | 关联阻塞项 | 解阻条件 | 交付物 | 完成判定 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| CLCFG-TODO-006 | Done | 扩展 CLI parser/main 以支持 config 子命令族 | config 设计 5.1；CLI 详设 parser/main contract | `config/show/plan/validate/apply` grammar、usage、local-only dispatch | L3 | `apps/cli/src/CliCommandParser.h`；`apps/cli/src/CliCommandParser.cpp`；`apps/cli/src/main.cpp`；`tests/unit/access/CliDaemonCommandParserTest.cpp`；新增 `tests/unit/apps/cli/ConfigCommandParserTest.cpp` | `CliCommandParser::parse()`；`CliCommandParser::usage_string()`；`main()` local config dispatch | unit：`ConfigCommandParserTest`；回归 `CliDaemonCommandParserTest` | `cmake --preset vscode-linux-ninja && cmake --build --preset vscode-linux-ninja --target dasall-cli dasall-cli_command_parser_unit_test dasall-config_command_parser_unit_test && ctest --preset vscode-linux-ninja -R "CliDaemonCommandParserTest|ConfigCommandParserTest" --output-on-failure` | CLCFG-TODO-001 | 无 | grammar 已冻结 | parser/main 代码与单测 | 仅当 `config` 子命令解析不影响既有 daemon-facing 命令，且 usage 文本稳定时完成 |
| CLCFG-TODO-007 | Done | 定义 config 类型面：InstallState、ActionPlan、DesiredConfig、ApplyResult | config 设计 6.1、7、9.3 | 状态枚举、计划对象、review diff、应用结果、manual followups | L2 | 新增 `apps/cli/src/config/ConfigCommandTypes.h`；`apps/cli/src/config/ConfigCommandTypes.cpp`；新增 `tests/unit/apps/cli/ConfigCommandTypesTest.cpp` | `InstallState`；`ConfigActionPlan`；`DesiredConfigSnapshot`；`ConfigApplyResult` | unit：`ConfigCommandTypesTest` 验证默认值、必填键与状态投影 | `cmake --build --preset vscode-linux-ninja --target dasall-config_command_types_unit_test && ctest --preset vscode-linux-ninja -R "^ConfigCommandTypesTest$" --output-on-failure` | CLCFG-TODO-003 | 无 | state/plan schema 已冻结 | config 类型头源与单测 | 仅当 P0 所需状态、计划和应用结果都具备单一类型 owner 时完成 |
| CLCFG-TODO-008 | Done | 新增 InstallStateProbe 与 ConfigCapabilityResolver 骨架 | config 设计 6.1、6.2.1、8.1 | canonical 文件、service 状态、secret 缺口、systemd 可用性与 capability 探测 | L2 | 新增 `apps/cli/src/config/InstallStateProbe.h`；`apps/cli/src/config/InstallStateProbe.cpp`；`apps/cli/src/config/ConfigCapabilityResolver.h`；`apps/cli/src/config/ConfigCapabilityResolver.cpp`；新增 `tests/unit/apps/cli/InstallStateProbeTest.cpp`；`tests/unit/apps/cli/ConfigCapabilityResolverTest.cpp` | `InstallStateProbe::probe()`；`ConfigCapabilityResolver::resolve()` | unit：state probe 正反例与 capability-gated 分支 | `cmake --build --preset vscode-linux-ninja --target dasall-install_state_probe_unit_test dasall-config_capability_resolver_unit_test && ctest --preset vscode-linux-ninja -R "^InstallStateProbeTest$|^ConfigCapabilityResolverTest$" --output-on-failure` | CLCFG-TODO-003、005 | CLCFG-BLK-001、CLCFG-BLK-003 | socket/operator/toolskill 边界已冻结 | probe/capability 组件与单测 | 仅当 `FreshInstall` 到 `Unsupported` 的状态投影可重复验证，且 capability 不再在页面代码中散落判断时完成 |
| CLCFG-TODO-009 | Done | 新增 DaemonConfigFileStore 与配置事务 helper | config 设计 6.2.3、7.1、8.1、9.3 | `/etc/default/dasall-daemon`、`/etc/dasall/daemon.json` 的读写、备份、回滚与权限保留 | L2 | 新增 `apps/cli/src/config/DaemonConfigFileStore.h`；`apps/cli/src/config/DaemonConfigFileStore.cpp`；新增 `tests/unit/apps/cli/DaemonConfigFileStoreTest.cpp` | `load_current()`；`write_desired()`；`rollback_last_write()`；`ConfigFileWriteTransaction` | unit：成功写入、部分失败回滚、权限保留、invalid JSON repair overwrite | `cmake --build --preset vscode-linux-ninja --target dasall-daemon_config_file_store_unit_test && ctest --preset vscode-linux-ninja -R "^DaemonConfigFileStoreTest$" --output-on-failure` | CLCFG-TODO-003 | 无 | transaction contract 已冻结 | file store 头源与单测 | 仅当 CLI 不再直接拼写 `/etc` 文件格式，且失败回滚有自动化覆盖时完成 |
| CLCFG-TODO-010 | Done | 新增 ServiceManagerAdapter、PrivilegeProbe 与 ConfigPreflightChecker | config 设计 4.4、6.2.8、8.3、10.2 | root 检测、TTY/systemd 检测、`systemctl` 编排、`validate-only` preflight、non-systemd degrade | L2 | 新增 `apps/cli/src/config/ServiceManagerAdapter.h`；`apps/cli/src/config/ServiceManagerAdapter.cpp`；`apps/cli/src/config/PrivilegeProbe.h`；`apps/cli/src/config/ConfigPreflightChecker.h`；对应 `*.cpp`；新增 `tests/unit/apps/cli/ServiceManagerAdapterTest.cpp`；`tests/unit/apps/cli/ConfigPreflightCheckerTest.cpp` | `ServiceManagerAdapter::plan_service_actions()`；`ServiceManagerAdapter::apply()`；`PrivilegeProbe::require_root_for_write()`；`ConfigPreflightChecker::run()` | unit：systemd/no-systemd、root/non-root、validate-only success/failure | `cmake --build --preset vscode-linux-ninja --target dasall-service_manager_adapter_unit_test dasall-config_preflight_checker_unit_test && ctest --preset vscode-linux-ninja -R "^ServiceManagerAdapterTest$|^ConfigPreflightCheckerTest$" --output-on-failure` | CLCFG-TODO-002、003 | CLCFG-BLK-001、CLCFG-BLK-005 | operator model 与 service action schema 已冻结 | service/preflight 组件与单测 | 仅当 service 行为不再靠页面直接调用 shell 命令，且 preflight 可以独立复验时完成 |
| CLCFG-TODO-011 | Done | 新增 WorkflowCoordinator、PromptEngine、PlanFormatter 与 SummaryFormatter 骨架 | config 设计 6.2、8.1、8.4、14 | coordinator 驱动、masked prompt、默认值复用、plan dry-run JSON/human 投影、summary JSON/human 投影 | L2 | 新增 `apps/cli/src/config/CliConfigWorkflowCoordinator.h`；`apps/cli/src/config/CliConfigWorkflowCoordinator.cpp`；`apps/cli/src/config/InteractivePromptEngine.h`；`apps/cli/src/config/InteractivePromptEngine.cpp`；`apps/cli/src/config/ConfigPlanFormatter.h`；`apps/cli/src/config/ConfigPlanFormatter.cpp`；`apps/cli/src/config/ConfigSummaryFormatter.h`；`apps/cli/src/config/ConfigSummaryFormatter.cpp`；新增 `tests/unit/apps/cli/InteractivePromptEngineTest.cpp`；`tests/unit/apps/cli/ConfigPlanFormatterTest.cpp`；`tests/unit/apps/cli/ConfigSummaryFormatterTest.cpp` | `CliConfigWorkflowCoordinator::run()`；`InteractivePromptEngine::prompt_*()`；`ConfigPlanFormatter::format_*()`；`ConfigSummaryFormatter::format_*()` | unit：plan/summary formatter 与 masked/default prompt 行为 | `cmake --build --preset vscode-linux-ninja --target dasall-cli dasall-interactive_prompt_engine_unit_test dasall-config_plan_formatter_unit_test dasall-config_summary_formatter_unit_test && ctest --preset vscode-linux-ninja -R "^InteractivePromptEngineTest$|^ConfigPlanFormatterTest$|^ConfigSummaryFormatterTest$" --output-on-failure` | CLCFG-TODO-001、007、008、010 | 无 | grammar/type/probe/service 骨架已存在 | workflow/prompt/plan/summary 骨架 | 仅当后续实现已有单一 coordinator owner，且 plan 与 summary 输出口径已集中时完成 |

### 6.3 配置/策略/组件实现任务

| Task ID | Status | 任务标题 | 设计依据 | 精确范围 | 粒度 | 代码目标 | 目标函数/接口/数据结构 | 测试目标 | 验收命令 | 前置任务 | 关联阻塞项 | 解阻条件 | 交付物 | 完成判定 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| CLCFG-TODO-012 | Done | 分步实现 `config show`、`config plan/dry-run`、`config validate` 非交互工作流 | config 设计 5.1、6.1、6.2.1、6.2.7、6.2.9 | 012.1 `show` 只读摘要；012.2 `plan`/`--dry-run` 输出 action plan；012.3 `validate` 复用 daemon `--validate-only` 且不修改系统状态 | L2 | 更新 `apps/cli/src/main.cpp`；`apps/cli/src/config/CliConfigWorkflowCoordinator.h`；`apps/cli/src/config/CliConfigWorkflowCoordinator.cpp`；`apps/cli/src/config/DaemonConfigFileStore.h`；`apps/cli/src/config/DaemonConfigFileStore.cpp`；`apps/cli/CMakeLists.txt`；新增 `tests/unit/apps/cli/ConfigShowWorkflowTest.cpp`；新增 `tests/unit/apps/cli/ConfigPlanWorkflowTest.cpp`；新增 `tests/unit/apps/cli/ConfigValidateWorkflowTest.cpp`；新增 `tests/integration/apps/cli/ConfigShowValidateIntegrationTest.cpp`；更新 `tests/unit/apps/cli/CMakeLists.txt`；更新 `tests/integration/apps/cli/CMakeLists.txt` | `run_show()`；`run_plan()`；`run_validate()`；只读 current-state plan；validate-only preflight 输出 | unit：`ConfigShowWorkflowTest`、`ConfigPlanWorkflowTest`、`ConfigValidateWorkflowTest`；integration：`ConfigShowValidateIntegrationTest` | `cmake --preset vscode-linux-ninja && cmake --build --preset vscode-linux-ninja --target dasall-cli dasall-config_show_workflow_unit_test dasall-config_plan_workflow_unit_test dasall-config_validate_workflow_unit_test dasall_cli_config_show_validate_integration_test && ctest --preset vscode-linux-ninja -R "ConfigShowWorkflowTest|ConfigPlanWorkflowTest|ConfigValidateWorkflowTest|ConfigShowValidateIntegrationTest" --output-on-failure` | CLCFG-TODO-006 ~ 011 | 无 | skeleton 与 preflight 组件已落盘 | 本地只读工作流代码与测试 | 仅当 `show`、`plan/dry-run`、`validate` 三个子切片各自有输出契约、测试和无副作用证明时完成 |
| CLCFG-TODO-013 | Done | 实现 ConfigDiffPlanner、apply executor 与 `config apply --from-file` | config 设计 5.1、7.5、8.1、9.2/9.3、10.1 | desired-state fixture schema、desired vs current diff、文件写入、manual followups、headless apply、rollback | L2 | 新增 `apps/cli/src/config/ConfigDiffPlanner.h`；`apps/cli/src/config/ConfigDiffPlanner.cpp`；新增 `tests/fixtures/apps/cli/config/desired_state_minimal.yaml`；更新 `apps/cli/src/config/CliConfigWorkflowCoordinator.h`；`apps/cli/src/config/CliConfigWorkflowCoordinator.cpp`；`apps/cli/src/config/ConfigPreflightChecker.cpp`；`apps/cli/CMakeLists.txt`；`tests/unit/apps/cli/CMakeLists.txt`；`tests/integration/apps/cli/CMakeLists.txt`；新增 `tests/unit/apps/cli/ConfigDiffPlannerTest.cpp`；更新 `tests/unit/apps/cli/ConfigPlanWorkflowTest.cpp`；新增 `tests/integration/apps/cli/ConfigApplyWorkflowTest.cpp` | `ConfigDiffPlanner::load_desired_from_file()`；`ConfigDiffPlanner::build_plan()`；`CliConfigWorkflowCoordinator::run()` apply/from-file 分支；preflight 可写性父目录探测 | unit：`ConfigDiffPlannerTest`、`ConfigPlanWorkflowTest`；integration：`ConfigApplyWorkflowTest` | `cmake --preset vscode-linux-ninja && cmake --build --preset vscode-linux-ninja --target dasall-cli dasall-config_diff_planner_unit_test dasall-config_plan_workflow_unit_test dasall_cli_config_apply_integration_test && ctest --preset vscode-linux-ninja -R "ConfigDiffPlannerTest|ConfigPlanWorkflowTest|ConfigApplyWorkflowTest" --output-on-failure` | CLCFG-TODO-007、009、010、012 | CLCFG-BLK-005 | action plan、desired-state fixture 与 file transaction 已冻结 | diff planner / apply executor 代码、fixture 与测试 | 仅当 `config plan/apply --from-file` 可以在无交互模式下复用同一 action plan 管线，并在写入失败时回滚时完成 |
| CLCFG-TODO-014 | Done | 实现交互式 P0 wizard 页面与当前值复用 | config 设计 6.2.1 ~ 6.2.9、14 | `WelcomeAndStatePage`、`ProfileSelectionPage`、`DaemonConfigPage`、`ReviewAndApplyPage`、`ServiceActionPage`、`SummaryPage`；回车保留当前值；P0 不展示加组主路径 | L2 | 更新 `apps/cli/src/config/CliConfigWorkflowCoordinator.h`；`apps/cli/src/config/CliConfigWorkflowCoordinator.cpp`；`apps/cli/src/main.cpp`；更新 `tests/integration/apps/cli/CMakeLists.txt`；更新 `tests/integration/apps/cli/ConfigFreshInstallWorkflowTest.cpp`；新增 `tests/integration/apps/cli/ConfigInteractiveWizardTest.cpp`；更新 `tests/unit/apps/cli/CMakeLists.txt` | `CliConfigWorkflowCoordinator::run()` wizard 分支；shared apply helper；TTY prompt/confirm handler 注入；current-value reuse | integration：`ConfigInteractiveWizardTest`；`ConfigFreshInstallWorkflowTest` | `cmake --preset vscode-linux-ninja && cmake --build --preset vscode-linux-ninja --target dasall-cli dasall_cli_config_fresh_install_workflow_integration_test dasall_cli_config_interactive_wizard_integration_test && ctest --preset vscode-linux-ninja -R "ConfigFreshInstallWorkflowTest|ConfigInteractiveWizardTest" --output-on-failure` | CLCFG-TODO-008、010、011、012、013 | CLCFG-BLK-001、CLCFG-BLK-005 | show/plan/apply 骨架已稳定；P0 root/sudo-only operator model 已冻结 | wizard 实现、页面测试与交付说明 | 仅当 fresh install 与 existing config 两条路径都能以当前值预填、生成正确 plan，且不会误导用户通过加组获取 P0 访问权限时完成 |
| CLCFG-TODO-015 | Done | 分切片接入 service action 编排与安装态 onboarding UX | config 设计 1.1、6.2.8、7.1、12.1；打包设计 7.7.4、7.10.1、7.10.9 | 015.1 service action apply/restart/enable；015.2 安装态 `dasall config` 命令、README.Debian、postinst next steps、manpage、installed-package explicit start/enable 流程 | L2 | `apps/cli/CMakeLists.txt`；新增或更新 `debian/dasall.1` 或等价 manpage；新增或更新 `debian/dasall-daemon.README.Debian`；新增或更新 `debian/dasall-daemon.postinst`；必要时 `scripts/packaging/*` | `ServiceManagerAdapter::apply()`；installed command alias；operator onboarding 文案 | integration：`ConfigModifyExistingWorkflowTest`；packaging smoke：`pkg-smoke-local-control-plane` 扩展 config 入口 | `cmake --preset vscode-linux-ninja && cmake --build --preset vscode-linux-ninja --target dasall-cli dasall-daemon dasall_integration_tests && ctest --preset vscode-linux-ninja -R "ConfigModifyExistingWorkflowTest|CliDaemonSocketPathIntegrationTest" && test -d debian && rg -n "dasall config|README.Debian|enable --now dasall-daemon.service" debian` | CLCFG-TODO-002、010、013、014 | 无 | service action 可在 build-tree 复验；packaging 安装态命令与 service/config payload 已可接线 | service action 代码；installed-package onboarding 代码/文档 | 仅当 service action 与 installed onboarding 两个子切片各自有独立证据，且 docs/postinst/manpage 口径一致时完成 |
| CLCFG-TODO-016 | Done | 实现 SecretBootstrapWriter 与 LLMSecretPage P1 onboarding | config 设计 6.2.4、8.4、12.2；secret 设计 6.7/6.8/6.9 | masked prompt、stdin/import file、file backend 导入、redacted summary、runtime verification pending | L2 | 新增 `infra/src/secret/SecretBootstrapWriter.h`；`infra/src/secret/SecretBootstrapWriter.cpp`；更新 `infra/CMakeLists.txt`；新增 `apps/cli/src/config/LlmSecretPage.h`；`apps/cli/src/config/LlmSecretPage.cpp`；更新 `apps/cli/src/config/ConfigCommandTypes.h`；`apps/cli/src/config/ConfigDiffPlanner.cpp`；`apps/cli/src/config/CliConfigWorkflowCoordinator.h`；`apps/cli/src/config/CliConfigWorkflowCoordinator.cpp`；`apps/cli/CMakeLists.txt`；`tests/unit/apps/cli/CMakeLists.txt`；`tests/integration/apps/cli/CMakeLists.txt`；新增 `tests/unit/apps/cli/LlmSecretPageTest.cpp`；更新 `tests/unit/apps/cli/ConfigDiffPlannerTest.cpp`；更新 `tests/integration/apps/cli/ConfigApplyWorkflowTest.cpp`；`tests/integration/apps/cli/ConfigFreshInstallWorkflowTest.cpp`；`tests/integration/apps/cli/ConfigInteractiveWizardTest.cpp`；`tests/integration/apps/cli/ConfigModifyExistingWorkflowTest.cpp`；新增 `tests/integration/infra/secret/SecretBootstrapWriterIntegrationTest.cpp` | `SecretBootstrapWriter::import_secret()`；`LlmSecretPage::collect_and_apply()`；`ConfigDiffPlanner::load_desired_from_file()` secret refs 分支；`CliConfigWorkflowCoordinator::execute_apply_workflow()`；`auth_ref` projection | unit：`LlmSecretPageTest`、`ConfigDiffPlannerTest`；integration：`ConfigApplyWorkflowTest`、`ConfigFreshInstallWorkflowTest`、`ConfigInteractiveWizardTest`、`ConfigModifyExistingWorkflowTest`、`SecretBootstrapWriterIntegrationTest`、`FileSecretBackendTest`、`SecretManagerFacadeTest` | `cmake --build --preset vscode-linux-ninja --target dasall-cli dasall-config_diff_planner_unit_test dasall-llm_secret_page_unit_test dasall_cli_config_apply_integration_test dasall_cli_config_fresh_install_workflow_integration_test dasall_cli_config_interactive_wizard_integration_test dasall_cli_config_modify_existing_workflow_integration_test dasall_file_secret_backend_unit_test dasall_secret_manager_facade_unit_test && ctest --test-dir build/vscode-linux-ninja -R '^(LlmSecretPageTest|ConfigDiffPlannerTest|ConfigApplyWorkflowTest|ConfigFreshInstallWorkflowTest|ConfigInteractiveWizardTest|ConfigModifyExistingWorkflowTest|SecretBootstrapWriterIntegrationTest|FileSecretBackendTest|SecretManagerFacadeTest)$' --output-on-failure` | CLCFG-TODO-004、011 | 无（`CLCFG-BLK-002` 已于 2026-05-08 解阻） | bootstrap-only secret seam 已冻结并接入 file backend root 策略 | secret bootstrap writer、LLM page、workflow 接线、focused tests 与交付证据 | 仅当 secret 不经普通 flag、不回写 `daemon.json`、summary 只显示 redacted ref，且 apply/wizard 两条路径都能通过 prompt/stdin/import file 完成 bootstrap 导入时完成 |
| CLCFG-TODO-017 | Done | 实现 ToolSkillPage capability 摘要与 P2 可编辑预留 | config 设计 6.2.5、12.3；tools 设计 6.5.4/6.12.5 | P0/P1 隐藏或只读摘要；P2 才允许 editable controls；bundle/source/allowlist/撤销提示 | L2 | 新增 `apps/cli/src/config/ToolSkillPage.h`；`apps/cli/src/config/ToolSkillPage.cpp`；更新 `apps/cli/src/config/ConfigSummaryFormatter.h`；`apps/cli/src/config/ConfigSummaryFormatter.cpp`；`apps/cli/src/config/CliConfigWorkflowCoordinator.cpp`；`apps/cli/CMakeLists.txt`；`tests/unit/apps/cli/CMakeLists.txt`；`tests/integration/apps/cli/CMakeLists.txt`；新增 `tests/unit/apps/cli/ToolSkillPageTest.cpp`；更新 `tests/unit/apps/cli/ConfigSummaryFormatterTest.cpp`；`tests/unit/apps/cli/ConfigShowWorkflowTest.cpp` | `ToolSkillPage::render()`；`ConfigSummaryView::tool_skill_page`；`ConfigSummaryFormatter::format_*()`；`build_summary()`；`build_apply_summary()` | unit/integration：`ToolSkillPageTest`、`ConfigCapabilityResolverTest`、`ConfigSummaryFormatterTest`、`ConfigShowWorkflowTest`、`ConfigShowValidateIntegrationTest` | `cmake --build --preset vscode-linux-ninja --target dasall-cli dasall-config_capability_resolver_unit_test dasall-tool_skill_page_unit_test dasall-config_summary_formatter_unit_test dasall-config_show_workflow_unit_test dasall_cli_config_show_validate_integration_test && ctest --test-dir build/vscode-linux-ninja -R '^(ConfigCapabilityResolverTest|ToolSkillPageTest|ConfigSummaryFormatterTest|ConfigShowWorkflowTest|ConfigShowValidateIntegrationTest)$' --output-on-failure` | CLCFG-TODO-005、011 | 无（`CLCFG-BLK-003` 已于 2026-05-08 解阻） | `ToolSkillPageMode` 契约与 summary surface owner 已冻结且可消费 | ToolSkillPage 代码、summary 接线、focused tests 与交付证据 | 仅当 P0/P1 不再误导用户“默认安装 skills/tools 可任意修改”，且 P2 预留条件清晰并有 focused build-tree 证据时完成 |

### 6.4 测试支撑 / 集成 / 门禁任务

| Task ID | Status | 任务标题 | 设计依据 | 精确范围 | 粒度 | 代码目标 | 目标函数/接口/数据结构 | 测试目标 | 验收命令 | 前置任务 | 关联阻塞项 | 解阻条件 | 交付物 | 完成判定 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| CLCFG-TODO-018 | Done | 优先建立 config unit / integration topology 与 discoverability | 工程规范 4.1；CLI 专项测试拓扑基线 | `tests/unit/apps/cli`、新增 `tests/integration/apps/cli`、必要的 `tests/integration/infra/secret` 注册与 `unit;cli;config` / `integration;cli;config` label | L2 | 更新 `tests/unit/apps/cli/CMakeLists.txt`；新增 `tests/integration/apps/CMakeLists.txt`；新增 `tests/integration/apps/cli/CMakeLists.txt`；新增 `tests/integration/apps/cli/ConfigFreshInstallWorkflowTest.cpp`；更新 `tests/integration/CMakeLists.txt` | config tests discoverability；`ctest -N` 与 `-L config` 入口；config 测试不再只借挂 `access` label | build/test：discoverability 验证 | `cmake --preset vscode-linux-ninja && cmake --build --preset vscode-linux-ninja --target dasall-config_command_parser_unit_test dasall-install_state_probe_unit_test dasall_cli_config_fresh_install_workflow_integration_test && ctest --preset vscode-linux-ninja -N | rg "ConfigCommandParserTest|InstallStateProbeTest|ConfigFreshInstallWorkflowTest"` | 无 | 无 | 测试拓扑目录、CMake 入口和 label 规范先落盘 | config 专属测试拓扑与 discoverability 证据 | 仅当 config 相关单测/集成测试有独立可发现拓扑，而不是继续借挂 access 测试时完成 |
| CLCFG-TODO-019 | Done | 补齐 config focused unit / contract 回归矩阵 | config 设计 13、14；TODO 基线 | parser、types、state probe、capability、file store、diff planner、plan formatter、summary formatter、exit contract | L2 | 更新 `tests/unit/apps/cli/ConfigCommandParserTest.cpp`、`ConfigCommandTypesTest.cpp`、`InstallStateProbeTest.cpp`、`ConfigCapabilityResolverTest.cpp`、`DaemonConfigFileStoreTest.cpp`、`ConfigDiffPlannerTest.cpp`、`ConfigPlanFormatterTest.cpp`、`ConfigSummaryFormatterTest.cpp`；新增 `tests/contract/access/ConfigOutputContractTest.cpp` 与 `tests/contract/access/CMakeLists.txt` 接线 | focused unit/contract matrix | `ctest` focused gate | `cmake --preset vscode-linux-ninja && cmake --build --preset vscode-linux-ninja --target dasall-config_command_parser_unit_test dasall-config_command_types_unit_test dasall-install_state_probe_unit_test dasall-config_capability_resolver_unit_test dasall-daemon_config_file_store_unit_test dasall-config_diff_planner_unit_test dasall-config_plan_formatter_unit_test dasall-config_summary_formatter_unit_test dasall-config_output_contract_test && ctest --test-dir build/vscode-linux-ninja -R '^(ConfigCommandParserTest|ConfigCommandTypesTest|InstallStateProbeTest|ConfigCapabilityResolverTest|DaemonConfigFileStoreTest|ConfigDiffPlannerTest|ConfigPlanFormatterTest|ConfigSummaryFormatterTest|ConfigOutputContractTest)$' --output-on-failure` | CLCFG-TODO-006 ~ 013、018 | 无 | config focused unit/contract gate 已通过 | focused tests 与 contract gate | 仅当 config 核心组件均有 focused 自动化出口，且 `--json` / human summary 口径可回归时完成 |
| CLCFG-TODO-020 | Done | 补齐 P0 workflow 集成与 drift repair 门 | config 设计 6.1、6.2、9、14；kubeadm 分阶段实践 | fresh install、configured running 修改、drifted repair、non-systemd degrade、service action 分支 | L2 | 更新 `apps/cli/src/config/DaemonConfigFileStore.cpp`；更新 `tests/unit/apps/cli/DaemonConfigFileStoreTest.cpp`；更新 `tests/integration/apps/cli/CMakeLists.txt`；新增 `tests/integration/apps/cli/ConfigDriftRepairWorkflowTest.cpp` | `DaemonConfigFileStore::write_desired()`；`config` interactive / plan / apply / service action integration | integration：fresh/modify/drift/non-systemd 四类路径；unit：invalid daemon.json drift repair overwrite | `cmake --build --preset vscode-linux-ninja --target dasall-daemon_config_file_store_unit_test dasall_cli_config_drift_repair_workflow_integration_test dasall_cli_config_apply_integration_test && ctest --test-dir build/vscode-linux-ninja -R '^(DaemonConfigFileStoreTest|ConfigDriftRepairWorkflowTest|ConfigApplyWorkflowTest)$' --output-on-failure` | CLCFG-TODO-012 ~ 015、018 | CLCFG-BLK-005 | workflow 与 service adapter 已可组合执行 | config workflow integration tests | 仅当 fresh install、后续修改与 drift repair 三条主路径都可自动化复验时完成 |
| CLCFG-TODO-021 | Done | 补齐 secret onboarding 与 installed-package smoke / autopkgtest 门 | config 设计 12.2；打包设计 7.10.7；secret 设计 9 | P1 secret import、installed-package `dasall config` smoke、autopkgtest metadata、rootful validator script | L2 | 更新 `debian/tests/pkg-smoke-local-control-plane`；新增 `scripts/packaging/validate_cli_config_v1.sh`；更新 `scripts/packaging/pkg_smoke_install.sh`；落盘 `scripts/packaging/pkg_smoke_upgrade.sh`、`scripts/packaging/pkg_smoke_remove_purge.sh`、`scripts/packaging/validate_ubuntu_dpkg_v1.sh`；复用 `tests/integration/infra/secret/SecretBootstrapWriterIntegrationTest.cpp` | secret/install-package 双层 gate | integration：secret onboarding；installed-package：`dasall config` smoke；metadata：`debian/tests/control` 语法与 discoverability | `python3 scripts/packaging/validate_autopkgtest_metadata.py && test -x debian/tests/pkg-smoke-local-control-plane && test -x scripts/packaging/validate_cli_config_v1.sh` | CLCFG-TODO-015、016、018、020 | CLCFG-BLK-002、CLCFG-BLK-004、CLCFG-BLK-005 | secret seam、packaging payload、validator script 全部就绪 | installed-package smoke、validator script 与相关集成测试 | 仅当 build-tree 与 installed-package 两层 gate 都覆盖 `config` 主路径时完成 |
| CLCFG-TODO-022 | Done | 回写 config Gate、交付证据与统一验收入口 | TODO 基线；config 设计 14；专项收口要求 | Gate 状态、deliverables、worklog、one-shot validator 与残余 blocker/risk/OQ 回写 | L2 | 更新 `docs/todos/cli/DASALL_cli_config交互式部署配置专项TODO.md`；新增 `docs/todos/cli/deliverables/CLCFG-TODO-022-config专项Gate与交付证据收口.md`；更新 `docs/worklog/DASALL_开发执行记录.md`；新增 `scripts/packaging/validate_autopkgtest_metadata.py`；更新 `scripts/packaging/validate_cli_config_v1.sh` | `CLCFG-GATE-*`；统一验收入口；残余 blocker/risk/OQ 状态 | process + validator：gate 一致性复验 | `test -f docs/todos/cli/deliverables/CLCFG-TODO-022-config专项Gate与交付证据收口.md && rg -n "CLCFG-TODO-|CLCFG-GATE-|CLCFG-BLK-|CLCFG-RISK-|CLCFG-OQ-" docs/todos/cli/DASALL_cli_config交互式部署配置专项TODO.md docs/todos/cli/deliverables/CLCFG-TODO-022-config专项Gate与交付证据收口.md docs/worklog/DASALL_开发执行记录.md && bash scripts/packaging/validate_cli_config_v1.sh` | CLCFG-TODO-018 ~ 021 | 无 | 统一 validator 与 gate 证据已具备，且 noble/autopkgtest 5.47 metadata 兼容路径已固定 | 收口 deliverable、worklog 与 one-shot validator | 仅当 config 专项每个 gate 都有正式命令、证据与残余风险说明，并具备 one-shot 验收入口时完成 |
...
| F P1 secret onboarding | CLCFG-TODO-016 -> 021 | 016 已完成后串行 | 先补 bootstrap seam，再做 installed-package/secret 双层 gate |
| G P2 tools/skills | CLCFG-TODO-017 | 已完成 | 已补 capability 摘要与 P2 editable 预留；真实 operator-facing editable controls 仍待 tools owner surface 后续增量实现 |
...
| CLCFG-BLK-004 | 已解阻（2026-05-09）：`pkg-smoke-local-control-plane` 已扩展到安装态 `dasall config` + secret onboarding 主路径，`validate_cli_config_v1.sh` 已落盘为正式 validator 入口 | CLCFG-TODO-022 | 统一 gate、deliverable 与 worklog 证据回写 | 在 CLCFG-TODO-022 收口 gate/evidence/documentation | installed-package smoke/validator 入口已具备，剩余工作转为 closeout 而非能力缺口 |
| CLCFG-BLK-005 | 已解阻（2026-05-09）：当前主机已具备 `autopkgtest`、passwordless sudo 与同源 `.deb` 产物，`validate_autopkgtest_metadata.py` 已替代 noble 上失效的裸 `autopkgtest --validate .` 调用，`validate_cli_config_v1.sh` 已实跑通过 | CLCFG-TODO-022 | 无；后续只需把 `validate_cli_config_v1.sh` 作为回归 gate 持续维护 | 证据回链到 `docs/todos/cli/deliverables/CLCFG-TODO-022-config专项Gate与交付证据收口.md` 与 `docs/worklog/DASALL_开发执行记录.md` | 若后续环境失去 rootful/autopkgtest 条件或 metadata shim 与 `debian/tests/control` 再次失配，则重新转回 Blocked |
...
| CLCFG-GATE-05 | `python3 scripts/packaging/validate_autopkgtest_metadata.py && cmake --build --preset vscode-linux-ninja --target dasall_secret_bootstrap_writer_integration_test dasall_file_secret_backend_unit_test dasall_secret_manager_facade_unit_test && ctest --test-dir build/vscode-linux-ninja -R '^(SecretBootstrapWriterIntegrationTest|FileSecretBackendTest|SecretManagerFacadeTest)$' --output-on-failure && bash scripts/packaging/validate_cli_config_v1.sh` | CLCFG-TODO-016、021 |
| CLCFG-GATE-06 | `test -f docs/todos/cli/deliverables/CLCFG-TODO-022-config专项Gate与交付证据收口.md && rg -n "CLCFG-GATE-|CLCFG-BLK-|CLCFG-RISK-|CLCFG-OQ-" docs/todos/cli/DASALL_cli_config交互式部署配置专项TODO.md docs/todos/cli/deliverables/CLCFG-TODO-022-config专项Gate与交付证据收口.md docs/worklog/DASALL_开发执行记录.md && bash scripts/packaging/validate_cli_config_v1.sh` | CLCFG-TODO-022 |
...
该脚本由 CLCFG-TODO-021/022 落盘并完成 noble metadata validate 兼容修正。统一验收前置条件是工作树旁存在与当前源码同源的 `.deb` / `.changes` 产物；通常由最近一次 `dpkg-buildpackage -us -uc -b` 提供。脚本当前至少覆盖以下步骤：

1. `python3 scripts/packaging/validate_autopkgtest_metadata.py`
2. `cmake --build --preset vscode-linux-ninja --target dasall_secret_bootstrap_writer_integration_test dasall_file_secret_backend_unit_test dasall_secret_manager_facade_unit_test`
3. `ctest --test-dir build/vscode-linux-ninja -R '^(SecretBootstrapWriterIntegrationTest|FileSecretBackendTest|SecretManagerFacadeTest)$' --output-on-failure`
4. installed-package / rootful smoke：`bash scripts/packaging/validate_ubuntu_dpkg_v1.sh`
5. `validate_cli_config_v1.sh` 作为 config 专项 one-shot gate 只负责 metadata + secret-focused build-tree regressions + installed-package lifecycle smoke 的组合验证，不再复用会隐式执行全量 integration 的聚合 target
