...
| CLCFG-BLK-005 | 已部分解阻（2026-05-09）：rootful/systemd smoke 与 one-shot validator 入口已落盘，但当前环境缺 `autopkgtest` 与 passwordless sudo，live gate 尚待正式 rootful/autopkgtest 环境复验 | CLCFG-TODO-022 | 在具备 autopkgtest + rootful 条件的环境执行 `validate_cli_config_v1.sh` 或等价 gate | 保留 021 的 smoke/validator 入口，待 022 收口环境侧执行证据 | 在无 rootful/autopkgtest 条件的机器上只宣称 build-tree ready，不宣称 installed-package gate 已实跑闭环 |
