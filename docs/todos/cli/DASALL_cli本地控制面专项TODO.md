...
当前结论：CLI 专项已完成两项补设计解阻任务，以及 `CLI-TODO-006` 的 `CliCommand` 扩展选项模型、`CLI-TODO-007` 的 parser/help/version 校验、`CLI-TODO-008` 的 request builder/request frame shaping、`CLI-TODO-009` 的 `CliExitDecision` 本地退出码决策、`CLI-TODO-010` 的 human/JSON 双格式输出、`CLI-TODO-012` 的 JSON/exit code contract 扩展场景，以及 `CLI-TODO-013` 的 binary async/显式 ID/stdout-stderr 集成门落盘，当前仓库已经完成 CLI 纯客户端依赖方向、CLI-daemon wire contract、shared endpoint 默认值、`CLI-TODO-001` 的命令参数 schema/usage 冻结，以及 `CLI-TODO-002` 的 CLI projection JSON envelope / `CliExitDecision` 冻结。剩余未完成面已收敛到 `CLI-TODO-014` 的最终证据收口与 platform peer identity 外部依赖状态复验；后续 Build 任务不再需要猜测公开参数、`CliCommand` 字段、help/version 行为、request frame owner、stdout/stderr 归属、CLI v1 退出码矩阵、JSON error object 的 access fact 投影口径，或 built `dasall_cli` 的 sync/async 显式 ID 表面行为。
...
CLI 专项完成态必须提供一条 one-shot 命令覆盖 build、discoverability、unit、contract 与 integration focused gate。当前推荐命令如下；`CliJsonOutputContractTest`、`CliExitCodeContractTest` 与 Gate-CLI-05 对应的三项 binary/integration tests 均已进入真实断言阶段，后续剩余范围仅为 `CLI-TODO-014` 的证据收口。
