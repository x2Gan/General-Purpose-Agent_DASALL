...
当前结论：CLI 专项已完成两项补设计解阻任务，以及 `CLI-TODO-006` 的 `CliCommand` 扩展选项模型与 `CLI-TODO-009` 的 `CliExitDecision` 本地退出码决策落盘，当前仓库已经完成 CLI 纯客户端依赖方向、CLI-daemon wire contract、shared endpoint 默认值、`CLI-TODO-001` 的命令参数 schema/usage 冻结，以及 `CLI-TODO-002` 的 CLI projection JSON envelope / `CliExitDecision` 冻结。剩余未完成面集中在 parser/help/version、request builder、human/JSON formatter、`CliJsonOutputContractTest` 与 binary 质量门；后续 Build 任务不再需要猜测公开参数、`CliCommand` 字段或 CLI v1 退出码矩阵。
...
| 命令参数口径 | 已补设计冻结，字段模型已落位 | `CLI-TODO-001` 已冻结 `run/status/cancel/help/version` 的 usage skeleton、`--socket-path` 稳定命名、selector 规则与 version local-only 边界；`CLI-TODO-006` 已将稳定 flags/selector 字段落到 `CliCommand`，Build 侧剩余 parser/help/version 与 request builder 对齐实现 |
