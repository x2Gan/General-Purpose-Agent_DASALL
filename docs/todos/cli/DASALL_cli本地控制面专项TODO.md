...
当前结论：CLI 专项已完成两项补设计解阻任务，以及 `CLI-TODO-006` 的 `CliCommand` 扩展选项模型、`CLI-TODO-007` 的 parser/help/version 校验、`CLI-TODO-008` 的 request builder/request frame shaping 和 `CLI-TODO-009` 的 `CliExitDecision` 本地退出码决策落盘，当前仓库已经完成 CLI 纯客户端依赖方向、CLI-daemon wire contract、shared endpoint 默认值、`CLI-TODO-001` 的命令参数 schema/usage 冻结，以及 `CLI-TODO-002` 的 CLI projection JSON envelope / `CliExitDecision` 冻结。剩余未完成面集中在 human/JSON formatter、`CliJsonOutputContractTest` 与 binary 质量门；后续 Build 任务不再需要猜测公开参数、`CliCommand` 字段、help/version 行为、request frame owner 或 CLI v1 退出码矩阵。
...
| JSON 输出与退出码矩阵 | 已补设计冻结，退出码实现已落位 | `CLI-TODO-002` 已冻结 `cli.output.v1` envelope、stdout/stderr 归属、`DaemonClientResponse` CLI projection 边界，以及 access error fact -> CLI `0/2/3/4/5/6/7` 投影矩阵；`CLI-TODO-009` 已将 `CliExitDecision`、`main()` 退出路径和 `CliExitCodeContractTest` 落盘，剩余 human/JSON formatter 与 `CliJsonOutputContractTest` |
| CLI 自有测试拓扑 | 已完成，exit code contract 已激活 | `CLI-TODO-011` 已建立 CLI 自有 unit/contract discoverability；`CliExitCodeContractTest` 已在 `CLI-TODO-009` 中替换 reserved entrypoint 为真实断言，`CliJsonOutputContractTest` 仍保留 fail-closed reserved 状态 |
...
| CLI-TODO-012 | NotStarted | 补齐 CLI JSON contract tests 与 exit code 扩展场景 | CLI 详设 7、9、12 未决问题 3；`CLI-TODO-011` 提供 contract topology | `CliJsonOutputContractTest`；`CliExitCodeContractTest` | L2 | `tests/contract/access/CliJsonOutputContractTest.cpp`；`tests/contract/access/CliExitCodeContractTest.cpp` | `CliJsonOutputContractTest`；扩展后的 `CliExitCodeContractTest` | `ctest --test-dir build-ci -R "CliJsonOutputContractTest|CliExitCodeContractTest" --output-on-failure` | `CLI-TODO-002`、`CLI-TODO-009`、`CLI-TODO-010`、`CLI-TODO-011` | 无 | `CLI-TODO-009` 已落基础 exit code contract；`CLI-TODO-010` 落 JSON formatter 后需补齐 JSON envelope 与 binary-facing exit contract 场景，并把 `CliJsonOutputContractTest` 从 reserved/fail-closed 替换为真实断言 | 新增/扩展 contract 测试用例 | 仅当 `ping`、`run completed`、`accepted_async`、`auth deny`、`daemon unavailable` 等场景的 JSON 主键与 CLI v1 退出码均可自动化锁定时完成 |
...
| Wave 3 | `CLI-TODO-007 -> CLI-TODO-008`；`CLI-TODO-010` | 007/008 已完成；010 待实施 | parser/help/version 与 request builder 已完成；当前只剩 human/JSON formatter 收口 |
