...
当前结论：CLI 专项已完成两项补设计解阻任务，以及 `CLI-TODO-006` 的 `CliCommand` 扩展选项模型、`CLI-TODO-007` 的 parser/help/version 校验、`CLI-TODO-008` 的 request builder/request frame shaping、`CLI-TODO-009` 的 `CliExitDecision` 本地退出码决策与 `CLI-TODO-010` 的 human/JSON 双格式输出落盘，当前仓库已经完成 CLI 纯客户端依赖方向、CLI-daemon wire contract、shared endpoint 默认值、`CLI-TODO-001` 的命令参数 schema/usage 冻结，以及 `CLI-TODO-002` 的 CLI projection JSON envelope / `CliExitDecision` 冻结。剩余未完成面集中在扩展 JSON/exit contract 场景与 binary 质量门；后续 Build 任务不再需要猜测公开参数、`CliCommand` 字段、help/version 行为、request frame owner、stdout/stderr 归属或 CLI v1 退出码矩阵。
...
| JSON 输出与退出码矩阵 | 已补设计冻结，formatter 与基础 contract 已落位 | `CLI-TODO-002` 已冻结 `cli.output.v1` envelope、stdout/stderr 归属、`DaemonClientResponse` CLI projection 边界，以及 access error fact -> CLI `0/2/3/4/5/6/7` 投影矩阵；`CLI-TODO-009/010` 已将 `CliExitDecision`、`CliOutputFormatter`、`main()` 路由与 `CliJsonOutputContractTest`/`CliExitCodeContractTest` 落盘，剩余 012 继续扩展场景矩阵 |
| CLI 自有测试拓扑 | 已完成，JSON/exit contract 已激活 | `CLI-TODO-011` 已建立 CLI 自有 unit/contract discoverability；`CliExitCodeContractTest` 已在 `CLI-TODO-009` 中替换 reserved entrypoint 为真实断言，`CliJsonOutputContractTest` 也已在 `CLI-TODO-010` 中进入真实断言 |
...
| CLI-TODO-012 | NotStarted | 补齐 CLI JSON contract tests 与 exit code 扩展场景 | CLI 详设 7、9、12 未决问题 3；`CLI-TODO-011` 提供 contract topology | `CliJsonOutputContractTest`；`CliExitCodeContractTest` | L2 | `tests/contract/access/CliJsonOutputContractTest.cpp`；`tests/contract/access/CliExitCodeContractTest.cpp` | 扩展后的 `CliJsonOutputContractTest`；扩展后的 `CliExitCodeContractTest` | `ctest --test-dir build-ci -R "CliJsonOutputContractTest|CliExitCodeContractTest" --output-on-failure` | `CLI-TODO-002`、`CLI-TODO-009`、`CLI-TODO-010`、`CLI-TODO-011` | 无 | `CLI-TODO-009/010` 已落基础 exit code contract 与真实 JSON contract；012 剩余目标收敛为补齐 `run completed`、`auth deny`、`daemon unavailable`、binary-facing exit/stream 组合等扩展场景矩阵 | 新增/扩展 contract 测试用例 | 仅当 `ping`、`run completed`、`accepted_async`、`auth deny`、`daemon unavailable` 等场景的 JSON 主键与 CLI v1 退出码均可自动化锁定时完成 |
...
| Unit | human/json formatter | 已落基础实现，后续补扩场景 | `CliDaemonOutputFormatterTest` |
| Contract | `--json` 主键与 envelope 稳定性 | 已落基础断言，后续补扩场景 | `CliJsonOutputContractTest` |
| Contract | exit code 稳定性 | 已落基础断言，后续补扩场景 | `CliExitCodeContractTest` |
...
CLI 专项完成态必须提供一条 one-shot 命令覆盖 build、discoverability、unit、contract 与 integration focused gate。当前推荐命令如下；`CliJsonOutputContractTest` 与 `CliExitCodeContractTest` 已进入真实断言阶段，`CLI-TODO-012` 的剩余范围是扩展场景矩阵，而不再是替换 reserved entrypoint。
