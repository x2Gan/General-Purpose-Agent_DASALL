...
当前结论：CLI 专项已完成两项补设计解阻任务，以及 `CLI-TODO-006` 的 `CliCommand` 扩展选项模型、`CLI-TODO-007` 的 parser/help/version 校验、`CLI-TODO-008` 的 request builder/request frame shaping、`CLI-TODO-009` 的 `CliExitDecision` 本地退出码决策、`CLI-TODO-010` 的 human/JSON 双格式输出与 `CLI-TODO-012` 的 JSON/exit code contract 扩展场景落盘，当前仓库已经完成 CLI 纯客户端依赖方向、CLI-daemon wire contract、shared endpoint 默认值、`CLI-TODO-001` 的命令参数 schema/usage 冻结，以及 `CLI-TODO-002` 的 CLI projection JSON envelope / `CliExitDecision` 冻结。剩余未完成面已收敛到 CLI binary 的 async/显式 ID/stdout-stderr 集成门与最终证据收口；后续 Build 任务不再需要猜测公开参数、`CliCommand` 字段、help/version 行为、request frame owner、stdout/stderr 归属、CLI v1 退出码矩阵或 JSON error object 的 access fact 投影口径。
...
| JSON 输出与退出码矩阵 | 已补设计冻结，formatter 与扩展 contract 已落位 | `CLI-TODO-002` 已冻结 `cli.output.v1` envelope、stdout/stderr 归属、`DaemonClientResponse` CLI projection 边界，以及 access error fact -> CLI `0/2/3/4/5/6/7` 投影矩阵；`CLI-TODO-009/010/012` 已将 `CliExitDecision`、`CliOutputFormatter`、`main()` 路由、access error JSON projection 与 `CliJsonOutputContractTest`/`CliExitCodeContractTest` 的扩展场景一并落盘，剩余工作转入 013 binary 集成门 |
...
| Wave 4 | `CLI-TODO-012 -> CLI-TODO-013` | 012 已完成，013 待执行 | JSON/exit code 契约已锁定；下一步只剩 binary/integration gate，避免脚本化行为只被集成测试隐式覆盖 |
...
CLI 专项完成态必须提供一条 one-shot 命令覆盖 build、discoverability、unit、contract 与 integration focused gate。当前推荐命令如下；`CliJsonOutputContractTest` 与 `CliExitCodeContractTest` 已进入扩展场景后的真实断言阶段，后续剩余范围仅为 `CLI-TODO-013` 的 binary/integration gate。
