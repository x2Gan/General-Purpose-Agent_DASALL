...
| WP05-T010-D | Done | 细化 LLMRequest/LLMResponse 职责边界 | deliverables/WP05-T010-LLMRequest-LLMResponse说明.md | ✅ 新增 LLMRequest/LLMResponse 说明文档，冻结 provider-neutral 请求面与语义输出响应面边界；显式给出 Design->Build 映射、Build 三件套与 D Gate=PASS（2026-03-20） |
| WP05-T010-B | Done | 新增 LLMRequest/LLMResponse 契约对象与守卫 | LLMRequest.h/LLMResponse.h/LLMBoundaryGuards.h + contract test | ✅ 新增 contracts/include/llm/LLMRequest.h、contracts/include/llm/LLMResponse.h、contracts/include/llm/LLMBoundaryGuards.h、tests/contract/llm/LLMRequestResponseContractTest.cpp，并接入 tests/contract/CMakeLists.txt；验证证据：cmake -S . -B build-ci -G Ninja 重新配置成功；ctest --test-dir build-ci -N -R LLMRequestResponseContractTest 发现 1 个测试；cmake --build build-ci --target dasall_contract_tests 通过且 70/70 contract tests passed；ctest --test-dir build-ci -R LLMRequestResponseContractTest --output-on-failure 1/1 通过（2026-03-20） |
| WP05-T011-D | Done | 识别进入 contracts 的接口候选 | deliverables/WP05-T011-接口候选清单.md | ✅ 新增 deliverables/WP05-T011-接口候选清单.md，基于阶段 5 准入原则、架构 7.4 模块依赖规则、Blueprint 接口文件分布与 ADR-006/008，锁定 10 个跨模块接口候选；明确剔除 platform/infra/protocol-internal 接口，并区分 ReviewReady 与 AwaitingSupportingContracts；D Gate=PASS（2026-03-21） |
...
| WP05-T014-D | Done | 定义错误码与枚举兼容测试矩阵 | deliverables/WP05-T014-错误码枚举测试矩阵.md | ✅ 新增 deliverables/WP05-T014-错误码枚举测试矩阵.md，锁定 ResultCode 编号/分段快照与 EnumLifecycle 兼容矩阵，覆盖未知值降级、deprecated 保留、缺失 Unspecified 哨兵阻断等场景；显式给出 Design->Build 映射、Build 三件套与 D Gate=PASS（2026-03-21） |
| WP05-T014-B | Done | 实现错误码/枚举兼容 contract tests | ErrorCodeEnumCompatibilityContractTest.cpp | ✅ 新增 tests/contract/error/ErrorCodeEnumCompatibilityContractTest.cpp 与 tests/contract/CMakeLists.txt 注册项；验证证据：构建前 `ctest --test-dir build-ci -N -R ErrorCodeEnumCompatibilityContractTest` 返回 Total Tests: 0；`cmake --build build-ci --target dasall_contract_tests` 通过且 74/74 contract tests passed；构建后 `ctest --test-dir build-ci -R ErrorCodeEnumCompatibilityContractTest --output-on-failure` 1/1 通过（2026-03-21） |
| WP05-T015-D | Done | 定义 EventEnvelope 兼容测试矩阵 | deliverables/WP05-T015-事件测试矩阵.md | ✅ 新增 deliverables/WP05-T015-事件测试矩阵.md，锁定 EventEnvelope 兼容矩阵，覆盖核心头部 round-trip 稳定、未知非头部字段忽略、payload 扩展放行、required 缺失/version 非法/header 白名单越权负例；显式给出 Design->Build 映射、Build 三件套与 D Gate=PASS（2026-03-21） |
| WP05-T015-B | Done | 实现 EventEnvelope 兼容 contract tests | EventEnvelopeCompatibilityContractTest.cpp | ✅ 新增 tests/contract/event/EventEnvelopeCompatibilityContractTest.cpp 与 tests/contract/CMakeLists.txt 注册项；验证证据：构建前 `ctest --test-dir build-ci -N -R EventEnvelopeCompatibilityContractTest` 返回 Total Tests: 0；`cmake --build build-ci --target dasall_contract_tests` 通过且 75/75 contract tests passed；构建后 `ctest --test-dir build-ci -R EventEnvelopeCompatibilityContractTest --output-on-failure` 1/1 通过（2026-03-21） |
| WP05-T016-D | Done | 定义 ADR 约束对象边界测试矩阵 | deliverables/WP05-T016-ADR边界测试矩阵.md | ✅ 新增 deliverables/WP05-T016-ADR边界测试矩阵.md，锁定 ADR-006/007/008 三对象（ContextPacket/ReflectionDecision/MultiAgentResult）边界回归矩阵，覆盖对象正例、禁区字段负例与 ADR 映射目录回归；显式给出 Design->Build 映射、Build 三件套与 D Gate=PASS（2026-03-22） |
| WP05-T016-B | Done | 实现 ADR 边界回归 contract tests | ADRBoundaryRegressionContractTest.cpp | ✅ 新增 tests/contract/smoke/ADRBoundaryRegressionContractTest.cpp 与 tests/contract/CMakeLists.txt 注册项；验证证据：构建前 `ctest --test-dir build-ci -N -R ADRBoundaryRegressionContractTest` 返回 Total Tests: 0；`cmake --build build-ci --target dasall_contract_tests` 通过且 76/76 contract tests passed；构建后 `ctest --test-dir build-ci -N -R ADRBoundaryRegressionContractTest` 发现 1 个测试；`ctest --test-dir build-ci -R ADRBoundaryRegressionContractTest --output-on-failure` 1/1 通过（2026-03-22） |
