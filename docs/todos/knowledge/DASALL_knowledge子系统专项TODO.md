...
| KNO-TODO-034-B | NotStarted | 落地 request-scoped hybrid query surface、CLI/access payload 反投影与最小 explain surface | `knowledge/include/KnowledgeTypes.h`；`knowledge/src/query/QueryNormalizer.cpp`；`apps/cli/src/CliCommandParser.cpp`；`apps/cli/src/CliRequestBuilder.h`；`apps/cli/src/CliRequestBuilder.cpp`；`access/src/AccessGatewayFactory.cpp`；`tests/integration/access/KnowledgeRuntimeQuerySurfaceIntegrationTest.cpp` | 仅当 `knowledge retrieve` 能显式透传 `preferred_mode`、`query_kind`、`allowed_corpora`、`domain_tags`、`required_tags`、`required_language`，并在 JSON 响应中返回 `mode`、`degraded`、reason codes、warning count 与 corpus summary，且 `contracts/` 无新增 retrieval supporting object 时完成 |
