...
| `knowledge/include/` | 目录已存在，并已落盘 `KnowledgeTypes.h`、`KnowledgeErrors.h`、`IKnowledgeService.h` 三个 root public headers | public interface 根目录与 file set 已建立，006 可直接在既有头文件上冻结真实 ABI |
| `tests/unit/knowledge/CMakeLists.txt` | 已注册 `dasall_knowledge_interface_surface_unit_test` | knowledge unit discoverability 已建立 |
