...
| KNO-TODO-020 | Done | 实现 IndexWriter shadow build 与 snapshot swap | 详设 6.13.3、10.4、11.1 KNO-B03 | `IndexWriter`；`swap_active_snapshot`；核心事务边界 | L2 | 新增 `docs/todos/knowledge/deliverables/KNO-TODO-020-IndexWriter设计收敛.md`、`knowledge/include/index/IndexWriter.h`、`knowledge/src/index/IndexWriter.cpp`，落盘 SQLite FTS5 shadow build、candidate 记录、active swap、last-known-good 回退、core/sidecar 分层语义与 `refresh_catalog` seam | `apply_update_batch`；`rebuild_all`；`build_shadow_index`；`swap_active_snapshot` | unit：`IndexWriterTest`、`IndexWriterSnapshotSwapTest`、`IndexWriterRecoveryTest` | `cmake -S . -B build-ci -G "Unix Makefiles" && cmake --build build-ci --target dasall_knowledge dasall_index_writer_unit_test dasall_index_writer_snapshot_swap_unit_test dasall_index_writer_recovery_unit_test && ctest --test-dir build-ci -R "IndexWriter(Test|SnapshotSwapTest|RecoveryTest)" --output-on-failure` | KNO-TODO-001、006、018、019、024 | 已解除：KNO-BLK-001 | 已完成 KNO-TODO-001、018、019、024 | `docs/todos/knowledge/deliverables/KNO-TODO-020-IndexWriter设计收敛.md`；`knowledge/include/index/IndexWriter.h`；`knowledge/src/index/IndexWriter.cpp`；`tests/unit/knowledge/IndexWriterTest.cpp`；`tests/unit/knowledge/IndexWriterSnapshotSwapTest.cpp`；`tests/unit/knowledge/IndexWriterRecoveryTest.cpp`；`knowledge/CMakeLists.txt`；`tests/unit/knowledge/CMakeLists.txt`；2026-04-21 已通过 Build_CMakeTools 定向构建与 `ListTests_CMakeTools` 发现；`RunCtest_CMakeTools` 工具态仍报 `生成失败` 后，已按仓库回退路径使用 build-ci `ctest` 3/3 验收通过 | 仅当 lexical snapshot 方案已冻结，且 `record_candidate -> swap -> mark_active -> refresh_catalog` 顺序可二值验证、失败时不污染 active snapshot 时完成 |
...
| KNO-TODO-022 | Done | 实现 Canonicalizer 文档规范化 | 详设 6.13.5、11.1 KNO-B04 | `Canonicalizer`；`CanonicalDocument` | L2 | 新增 `knowledge/include/ingest/Canonicalizer.h`、`knowledge/src/ingest/Canonicalizer.cpp`，落盘 markdown 展平、runtime policy yaml key-path flatten、typed metadata 提取、fallback 规则与 stable `document_id` 生成 | `Canonicalizer::canonicalize`；`normalize_markup`；`extract_metadata` | unit：`CanonicalizerTest`、`CanonicalizerMarkupNormalizeTest`、`CanonicalizerMetadataFallbackTest` | `cmake -S . -B build-ci -G "Unix Makefiles" && cmake --build build-ci --target dasall_knowledge dasall_canonicalizer_unit_test dasall_canonicalizer_markup_normalize_unit_test dasall_canonicalizer_metadata_fallback_unit_test && ctest --test-dir build-ci -R "Canonicalizer.*Test" --output-on-failure` | KNO-TODO-003、005、006 | 已解除：KNO-BLK-003 | 已完成 KNO-TODO-003 | `docs/todos/knowledge/deliverables/KNO-TODO-022-Canonicalizer设计收敛.md`；`knowledge/include/ingest/Canonicalizer.h`；`knowledge/src/ingest/Canonicalizer.cpp`；`tests/unit/knowledge/CanonicalizerTest.cpp`；`tests/unit/knowledge/CanonicalizerMarkupNormalizeTest.cpp`；`tests/unit/knowledge/CanonicalizerMetadataFallbackTest.cpp`；`tests/unit/knowledge/CMakeLists.txt`；`knowledge/CMakeLists.txt`；2026-04-21 已通过 Build_CMakeTools 定向构建；`RunCtest_CMakeTools` 工具态仍报 `生成失败` 后，已按仓库回退路径使用 build-ci `ctest` 3/3 验收通过 | 仅当 metadata baseline 已冻结，且 markup 规范化失败会显式 quarantine/ warning，而不是生成半有效文档时完成 |
| KNO-TODO-023 | Done | 实现 Chunker stable chunk 切分 | 详设 6.13.5、11.1 KNO-B04 | `Chunker`；`ChunkRecord`；stable chunk id | L2 | 新增 `knowledge/include/ingest/Chunker.h`、`knowledge/src/ingest/Chunker.cpp`，落盘段落/章节优先切分、overlap、token estimate、citation span、typed provenance 继承与 `chunk_id` 生成；**评审补充**：v1 默认 fixed-size 切分，接口预留 `ChunkStrategy`（fixed-size / semantic / document-aware）可配置扩展槽位 | `Chunker::chunk`；`split_into_spans`；`build_chunk_id` | unit：`ChunkerTest`、`ChunkerStableIdTest`、`ChunkerBoundaryFallbackTest` | `cmake -S . -B build-ci -G "Unix Makefiles" && cmake --build build-ci --target dasall_knowledge dasall_chunker_unit_test dasall_chunker_stable_id_unit_test dasall_chunker_boundary_fallback_unit_test && ctest --test-dir build-ci -R "Chunker.*Test" --output-on-failure` | KNO-TODO-003、005、006 | 已解除：KNO-BLK-003 | 已完成 KNO-TODO-003 | `docs/todos/knowledge/deliverables/KNO-TODO-023-Chunker设计收敛.md`；`knowledge/include/ingest/Chunker.h`；`knowledge/src/ingest/Chunker.cpp`；`tests/unit/knowledge/ChunkerTest.cpp`；`tests/unit/knowledge/ChunkerStableIdTest.cpp`；`tests/unit/knowledge/ChunkerBoundaryFallbackTest.cpp`；`tests/unit/knowledge/CMakeLists.txt`；`knowledge/CMakeLists.txt`；2026-04-21 已通过 Build_CMakeTools 定向构建；`RunCtest_CMakeTools` 工具态仍报 `生成失败` 后，已按仓库回退路径使用 build-ci `ctest` 3/3 验收通过 | 仅当 chunk policy 与 corpus baseline 已冻结，且 stable id、empty text、forced split 三类路径可二值验证时完成 |
| KNO-TODO-024 | Done | 实现 IngestionCoordinator update batch 编排 | 详设 6.9、6.13.3、7 KNO-D05、11.1 KNO-B04 | `IngestionCoordinator`；`CorpusChangeSet`；`IndexUpdateBatch` | L2 | 新增 `knowledge/include/ingest/IngestionCoordinator.h`、`knowledge/src/ingest/IngestionCoordinator.cpp`，落盘 scan/canonicalize/chunk 组合编排、warning 收敛、quarantine 透传、selective refresh 与 batch 装配；**实现收敛**：在 017 `VersionLedger` 未落盘前，使用 deterministic `document_lineage_id` 作为 batch replace/remove key | `build_update_batch`；`scan_and_canonicalize`；`build_chunk_records` | unit：`IngestionCoordinatorTest`、`IngestionCoordinatorSelectiveRefreshTest`、`IngestionCoordinatorBadSourceTest` | `cmake -S . -B build-ci -G "Unix Makefiles" && cmake --build build-ci --target dasall_knowledge dasall_ingestion_coordinator_unit_test dasall_ingestion_coordinator_selective_refresh_unit_test dasall_ingestion_coordinator_bad_source_unit_test && ctest --test-dir build-ci -R "IngestionCoordinator.*Test" --output-on-failure` | KNO-TODO-003、021、022、023 | 已解除：KNO-BLK-003 | 已完成 KNO-TODO-003 | `docs/todos/knowledge/deliverables/KNO-TODO-024-IngestionCoordinator设计收敛.md`；`knowledge/include/ingest/IngestionCoordinator.h`；`knowledge/src/ingest/IngestionCoordinator.cpp`；`tests/unit/knowledge/IngestionCoordinatorTest.cpp`；`tests/unit/knowledge/IngestionCoordinatorSelectiveRefreshTest.cpp`；`tests/unit/knowledge/IngestionCoordinatorBadSourceTest.cpp`；`tests/unit/knowledge/CMakeLists.txt`；`knowledge/CMakeLists.txt`；2026-04-21 已通过 Build_CMakeTools 定向构建；`RunCtest_CMakeTools` 工具态仍报 `生成失败` 后，已按仓库回退路径使用 build-ci `ctest` 3/3 验收通过 | 仅当 source baseline 已冻结，且单源失败 warning、selective refresh 与 batch 组装都可自动验证时完成 |
...
| KNO-TODO-030 | Done | 验证 retrieval quality regression gate | 详设 9.1、12.2 | `RetrievalQualityRegressionTest`；golden set | L2 | 新增 `docs/todos/knowledge/deliverables/KNO-TODO-030-retrieval-quality-regression-gate设计收敛.md`、`tests/integration/knowledge/RetrievalQualityRegressionTest.cpp` 与 `tests/integration/knowledge/golden/retrieval_quality_v1.yaml`，落盘固定 schema YAML parser、真实 lexical retrieval harness、`source_uri` 去重后的 `MRR@10` / `NDCG@10` / `Recall@5` / `Recall@10` 回归校验与 `hard_fail` case 阻断 | `RetrievalQualityRegressionTest`；golden set manifest schema | integration：golden set ≥30 条，aggregate 指标不低于基线阈值，`hard_fail` case 全通过；负向断言：人为打断 hard-fail case 时 gate 必须显式失败 | `cmake -S . -B build-ci -G "Unix Makefiles" && cmake --build build-ci --target dasall_integration_tests && ctest --test-dir build-ci -R RetrievalQualityRegressionTest --output-on-failure` | KNO-TODO-004、027、028 | 已解除：KNO-BLK-004 | 已完成 KNO-TODO-004、027、028 | `docs/todos/knowledge/deliverables/KNO-TODO-030-retrieval-quality-regression-gate设计收敛.md`；`tests/integration/knowledge/RetrievalQualityRegressionTest.cpp`；`tests/integration/knowledge/golden/retrieval_quality_v1.yaml`；`tests/integration/knowledge/CMakeLists.txt`；2026-04-22 已通过 Build_CMakeTools 定向构建并完成 `ctest --test-dir build-ci -R RetrievalQualityRegressionTest --output-on-failure` 1/1；聚合 `dasall_integration_tests` 中 `RetrievalQualityRegressionTest` 已 Passed，但 aggregate target 仍被仓库既有 `InfraDiagnosticsSmokeTest` / `InfraDiagnosticsIntegrationTest` 失败拖住，已记录为无关残余风险 | 仅当 golden set 资产、绝对阈值、relative regression 公式与 `hard_fail` case 都被自动验证，且质量退化会被自动阻断时完成 |
| KNO-TODO-031 | Done | 回写 knowledge 专项 Gate 与交付证据 | 阶段 H；详设 8.2 Phase K5、9.3；工程执行规范 | 8.2 Phase K5；9.3 质量门 | L2 | 新增 `docs/todos/knowledge/deliverables/KNO-TODO-031-knowledge专项Gate与交付证据收敛.md`，并更新 `docs/todos/knowledge/DASALL_knowledge子系统专项TODO.md`、`docs/worklog/DASALL_开发执行记录.md`，统一回写 build / unit / integration / quality / blocker / rollback 证据 | Gate 结果、命令、风险、blocker、后继动作 | process：所有 Gate 都有命令证据、结果摘要和残余风险回写；必须显式区分 knowledge 自身 PASS 与聚合 target 外部噪音 | `cmake -S . -B build-ci -G "Unix Makefiles" && cmake --build build-ci --target dasall_knowledge dasall_unit_tests dasall_integration_tests && ctest --test-dir build-ci -N && ctest --test-dir build-ci --output-on-failure -R "Knowledge|dasall_knowledge"` | KNO-TODO-027、028、029、030、032、033 | 031 自身无阻塞；残余 blocker 已回写：`FreshnessControllerTest.cpp` 语法破损阻塞 `dasall_unit_tests`，`InfraDiagnosticsSmokeTest` / `InfraDiagnosticsIntegrationTest` 污染 `dasall_integration_tests` aggregate | 无 | `docs/todos/knowledge/deliverables/KNO-TODO-031-knowledge专项Gate与交付证据收敛.md`；更新后的专项 TODO；更新后的 worklog；2026-04-22 已记录 `ctest -N` 总测试数 601、knowledge 命名范围 `ctest -R "Knowledge|dasall_knowledge"` 20/20 Passed、`RetrievalQualityRegressionTest` 1/1 Passed；并显式记录 `dasall_unit_tests` / `dasall_integration_tests` 的残余 blocker 状态 | 仅当每个 Gate 都存在可追溯证据、blocker 状态与后继动作回写，不再依赖口头结论时完成 |

### 6.7 评审补充任务

| ID | 状态 | 任务标题 | 来源依据 | 设计锚点 | 粒度等级 | 代码目标 | 目标函数/接口/数据结构 | 测试目标 | 验收命令 | 前置依赖 | 阻塞项 | 解阻条件 | 交付物 | 完成判定 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| KNO-TODO-032 | Done | 补全 KnowledgeServiceFacade 完整编排（hybrid/ingest/health） | 评审 D-1：Facade 依赖过重需拆分为骨架 + 完整两阶段；详设 6.6、6.10、6.13.1 | `KnowledgeServiceFacade`；`RecallCoordinator` 真实接入；`IngestionCoordinator` 真实 refresh 编排；`KnowledgeHealthProbe` 接入 | L2 | 更新 `docs/todos/knowledge/deliverables/KNO-TODO-032-KnowledgeServiceFacade完整编排设计收敛.md`、`knowledge/src/facade/KnowledgeService.h`、`knowledge/src/facade/KnowledgeService.cpp`，把 facade 升级为 real component owner + seam 共存的组合根；补齐 `RecallCoordinator` owner 默认接线、真实 `IngestionCoordinator -> IndexWriter` refresh 编排与 `KnowledgeHealthProbe` 聚合接线 | `KnowledgeServiceFacade::retrieve`（完整 hybrid 路径）；`request_refresh`（真实 ingest 编排）；`health_snapshot`（完整 probe 接入） | unit：`KnowledgeServiceFacadeHybridRecallTest`、`KnowledgeServiceFacadeRealRefreshTest`、`KnowledgeServiceFacadeFullHealthSnapshotTest`；兼容回归：`KnowledgeServiceFacadeFailurePathTest` | `cmake -S . -B build-ci -G "Unix Makefiles" && cmake --build build-ci --target dasall_knowledge dasall_knowledge_service_facade_hybrid_recall_unit_test dasall_knowledge_service_facade_real_refresh_unit_test dasall_knowledge_service_facade_full_health_snapshot_unit_test dasall_knowledge_service_facade_failure_path_unit_test && ctest --test-dir build-ci -R "KnowledgeServiceFacade(HybridRecallTest|RealRefreshTest|FullHealthSnapshotTest|FailurePathTest)" --output-on-failure` | KNO-TODO-012、013、014、015、019、020、024、026 | 已解除：KNO-BLK-001、KNO-BLK-002、KNO-BLK-003 | 已完成 KNO-TODO-001、002、003、012、013、014、015、019、020、024、026 | `docs/todos/knowledge/deliverables/KNO-TODO-032-KnowledgeServiceFacade完整编排设计收敛.md`；`knowledge/src/facade/KnowledgeService.h`；`knowledge/src/facade/KnowledgeService.cpp`；`tests/unit/knowledge/KnowledgeServiceFacadeHybridRecallTest.cpp`；`tests/unit/knowledge/KnowledgeServiceFacadeRealRefreshTest.cpp`；`tests/unit/knowledge/KnowledgeServiceFacadeFullHealthSnapshotTest.cpp`；`tests/unit/knowledge/KnowledgeServiceFacadeFailurePathTest.cpp`；`tests/unit/knowledge/CMakeLists.txt`；2026-04-22 已通过 Build_CMakeTools 定向构建，且已按仓库回退路径使用 build-ci `ctest` 4/4 验收通过 | 仅当 facade 在不提供 facade-level lambda seams 的情况下可跑通 hybrid retrieve、real refresh 与 full health 三条路径，且 012 既有 failure-path 兼容回归不被破坏时完成 |
| KNO-TODO-033 | Done | 验证 request_refresh → ingest → snapshot swap → retrieve 端到端集成闭环 | 评审 D-4：缺少 refresh 闭环集成测试；详设 6.6、6.9、10.4 | `request_refresh`；`IngestionCoordinator`；`IndexWriter.swap_active_snapshot`；`IndexReader`；retrieve 闭环 | L2 | 新增 `docs/todos/knowledge/deliverables/KNO-TODO-033-refresh-loop-integration设计收敛.md` 与 `tests/integration/knowledge/KnowledgeRefreshLoopTest.cpp`，在真实 refresh/retrieve 组合根上验证 `request_refresh(changes)` → ingest → shadow build → snapshot swap → `retrieve()` 闭环，同时覆盖 refresh busy reject 与 `mark_active` 失败 rollback 语义 | `KnowledgeRefreshLoopTest`；`dasall_knowledge_refresh_loop_integration_test` | integration：refresh → retrieve 闭环可通过；refresh busy reject 可验证；swap 失败时 active snapshot 不污染；更新文档不会以重复新增污染 active snapshot | `cmake -S . -B build-ci -G "Unix Makefiles" && cmake --build build-ci --target dasall_integration_tests && ctest --test-dir build-ci -R dasall_knowledge_refresh_loop_integration_test --output-on-failure` | KNO-TODO-020、024、032 | 已解除：KNO-BLK-001、KNO-BLK-003 | 已完成 KNO-TODO-020、024、032 | `docs/todos/knowledge/deliverables/KNO-TODO-033-refresh-loop-integration设计收敛.md`；`tests/integration/knowledge/KnowledgeRefreshLoopTest.cpp`；`tests/integration/knowledge/CMakeLists.txt`；2026-04-22 已通过 Build_CMakeTools 定向构建；`RunCtest_CMakeTools` 仍报仓库已知 `生成失败`，回退到 build-ci 后 `ctest --test-dir build-ci -R KnowledgeRefreshLoopTest --output-on-failure` 1/1 Passed；聚合 `dasall_integration_tests` 中 `KnowledgeRefreshLoopTest` 已 Passed，但 aggregate target 仍被无关 `InfraDiagnosticsSmokeTest` / `InfraDiagnosticsIntegrationTest` 失败拖住 | 仅当 refresh → ingest → swap → retrieve 端到端闭环可自动验证、refresh busy 拒绝可二值判定、swap 失败不丢失 last-known-good snapshot 时完成 |

### 6.8 向量能力第一阶段双轨任务包

说明：`KNO-TODO-034` 对应 2026-05-20 新增“先能用”阶段的第一项，负责把 production knowledge retrieve 从“只有 lexical-only 默认值”推进到“可以由请求显式发起 hybrid canary”。本节按 `-D + -B` 双轨模板细化该任务；`KNO-TODO-034-D` 的设计交付已落盘到 `docs/todos/knowledge/deliverables/KNO-TODO-034-request-scoped-hybrid-query-surface双轨任务包.md`，`KNO-TODO-034-B` 只在不扩 `contracts/`、不改变 default `LexicalOnly` baseline 的前提下推进。

#### 6.8.1 Design->Build 拆分总表

| 主任务 | Design 子任务（文档交付） | Build 子任务（代码交付） | 输入依据 | 代码目标 | 测试目标 | 验收命令 |
|---|---|---|---|---|---|---|
| KNO-TODO-034 | KNO-TODO-034-D：收敛 request-scoped hybrid query surface 设计与 D Gate | KNO-TODO-034-B：落地 request-scoped hybrid query surface 与 degrade explain payload | `KNO-GAP-013`、`KNO-GAP-018`；`KNO-C016`、`KNO-C017`；Azure Hybrid Search / OpenSearch Hybrid Search / Pinecone hybrid search | `knowledge/include/KnowledgeTypes.h`；`knowledge/src/query/QueryNormalizer.cpp`；`apps/cli/src/CliCommandParser.cpp`；`apps/cli/src/CliRequestBuilder.h`；`apps/cli/src/CliRequestBuilder.cpp`；`access/src/AccessGatewayFactory.cpp` | `dasall_knowledge_interface_surface_unit_test`；`CliDaemonCommandParserTest`；`DaemonAccessPipelineFactoryTest`；`KnowledgeRuntimeQuerySurfaceIntegrationTest` | `cmake --build build/vscode-linux-ninja --target dasall_knowledge_interface_surface_unit_test dasall-cli_command_parser_unit_test dasall-daemon_access_pipeline_factory_unit_test dasall_knowledge_runtime_query_surface_integration_test -j2`；`ctest --test-dir build/vscode-linux-ninja -R dasall_knowledge_interface_surface_unit_test --output-on-failure`；`ctest --test-dir build/vscode-linux-ninja -R CliDaemonCommandParserTest --output-on-failure`；`ctest --test-dir build/vscode-linux-ninja -R DaemonAccessPipelineFactoryTest --output-on-failure`；`ctest --test-dir build/vscode-linux-ninja -R KnowledgeRuntimeQuerySurfaceIntegrationTest --output-on-failure` |

#### 6.8.2 原子任务状态清单（按子任务）

| 子任务 ID | 状态 | 任务描述 | 交付物 | 完成判定 |
|---|---|---|---|---|
| KNO-TODO-034-D | Done | 收敛 request-scoped hybrid query surface 的边界、字段矩阵、响应解释面与 Design->Build 映射 | `docs/todos/knowledge/deliverables/KNO-TODO-034-request-scoped-hybrid-query-surface双轨任务包.md` | 已补齐本地证据、行业实践、Build 三件套、风险回退与 D Gate；明确保持 `contracts` 零扩张、default `LexicalOnly` baseline 不变，并锁定 `034-B` 的代码目标、测试目标与验收命令（2026-05-20） |
| KNO-TODO-034-B | Done | 落地 request-scoped hybrid query surface、CLI/access payload 反投影与最小 explain surface | `knowledge/include/KnowledgeTypes.h`；`knowledge/src/query/QueryNormalizer.cpp`；`knowledge/src/facade/KnowledgeService.cpp`；`knowledge/src/retrieve/SparseRetriever.cpp`；`knowledge/src/retrieve/VectorRetrieverBridge.cpp`；`apps/cli/src/CliCommandParser.cpp`；`apps/cli/src/CliRequestBuilder.h`；`access/src/AccessGatewayFactory.cpp`；`tests/integration/access/KnowledgeRuntimeQuerySurfaceIntegrationTest.cpp` | `knowledge retrieve` 现已显式透传 `preferred_mode`、`query_kind`、`allowed_corpora`、`domain_tags`、`required_tags`、`required_language`；daemon/access JSON 同步返回 `mode`、`degraded`、`reason_codes`、`warning_count` 与 `corpus_summary`，且 `contracts/` 无新增 retrieval supporting object（2026-05-26） |

#### 6.8.3 Build 完成证据（2026-05-26）

1. `knowledge/include/KnowledgeTypes.h`、`knowledge/src/query/QueryNormalizer.cpp`、`knowledge/src/facade/KnowledgeService.cpp`、`knowledge/src/retrieve/SparseRetriever.cpp` 与 `knowledge/src/retrieve/VectorRetrieverBridge.cpp` 已把 `preferred_mode`、`required_tags`、`required_language` 收口到 module-local query plane；`required_tags` 不再继续借用 `domain_tags`，`KnowledgeRetrieveResult` 现可回传 `reason_codes`、`warning_count` 与 `corpus_summary` explain 摘要。
2. `apps/cli/src/CliCommandParser.cpp`、`apps/cli/src/CliRequestBuilder.h` 与 `access/src/AccessGatewayFactory.cpp` 已把 `knowledge retrieve` 扩展为 request-scoped control surface：CLI 支持 `--preferred-mode`、`--query-kind`、`--allowed-corpus`、`--domain-tag`、`--required-tag`、`--required-language`，daemon payload 可将其映射到 `KnowledgeQuery`，并把 additive explain fields 反投影回 JSON payload。
3. 已新增 `tests/integration/access/KnowledgeRuntimeQuerySurfaceIntegrationTest.cpp` 并接入 `tests/integration/access/CMakeLists.txt`；同时扩展 `CliDaemonCommandParserTest`、`DaemonAccessPipelineFactoryTest`、`dasall_knowledge_interface_surface_unit_test`、`QueryNormalizerTest`、`QueryNormalizerBoundaryTest`、`SparseRetrieverFilterTest` 与 `VectorRetrieverBridgeTest`，覆盖默认 lexical-only 兼容路径、显式 hybrid canary query 映射和非法 query surface 拒绝路径。
4. 2026-05-26 已通过 `Build_CMakeTools(buildTargets=["dasall_knowledge_interface_surface_unit_test","dasall_query_normalizer_unit_test","dasall_query_normalizer_boundary_unit_test","dasall_sparse_retriever_filter_unit_test","dasall_vector_retriever_bridge_unit_test","dasall-cli_command_parser_unit_test","dasall-daemon_access_pipeline_factory_unit_test","dasall_knowledge_runtime_query_surface_integration_test"])`；`RunCtest_CMakeTools` 对本组测试仍返回仓库已知泛化 `生成失败`，因此按仓库回退口径直接执行对应 build-tree binaries，退出码均为 `0`。

### 6.9 runtime-owned hybrid canary seam 双轨任务包

说明：`KNO-TODO-035` 承接 `KNO-TODO-034-B`，负责把“请求面已经能显式表达 hybrid canary”进一步收口成“只有 runtime owner 明确放行的 profile + corpus 才真正接受 `preferred_mode=Hybrid` / `DenseOnly`”。本节按 `-D + -B` 双轨模板细化该任务；`KNO-TODO-035-D` 的设计交付已落盘到 `docs/todos/knowledge/deliverables/KNO-TODO-035-runtime-owned-hybrid-canary-seam双轨任务包.md`，`KNO-TODO-035-B` 只在不新增 profile schema v1 键、不扩 `contracts/`、不改变 production default `LexicalOnly` 的前提下推进。

#### 6.9.1 Design->Build 拆分总表

| 主任务 | Design 子任务（文档交付） | Build 子任务（代码交付） | 输入依据 | 代码目标 | 测试目标 | 验收命令 |
|---|---|---|---|---|---|---|
| KNO-TODO-035 | KNO-TODO-035-D：收敛 runtime-owned hybrid canary seam 设计与 D Gate | KNO-TODO-035-B：落地 runtime allowlist、factory admission seam 与 ready marker | `KNO-TODO-034-B`；ADR-006 / ADR-007 / ADR-008 owner boundary；Azure Hybrid Search；Google Cloud Deploy Canary | `knowledge/include/KnowledgeServiceFactory.h`；`knowledge/src/KnowledgeServiceFactory.cpp`；`apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp` | `RuntimeLiveCompositionFailureMatrixTest`；`RuntimeKnowledgeHybridCanaryIntegrationTest`；`KnowledgeInstalledAssetHybridProbeTest` | `cmake --build build/vscode-linux-ninja --target dasall_access_runtime_live_composition_failure_matrix_integration_test dasall_runtime_knowledge_hybrid_canary_integration_test dasall_knowledge_installed_asset_hybrid_probe_integration_test -j2`；`ctest --test-dir build/vscode-linux-ninja -R RuntimeLiveCompositionFailureMatrixTest --output-on-failure`；`ctest --test-dir build/vscode-linux-ninja -R RuntimeKnowledgeHybridCanaryIntegrationTest --output-on-failure`；`ctest --test-dir build/vscode-linux-ninja -R KnowledgeInstalledAssetHybridProbeTest --output-on-failure` |

#### 6.9.2 原子任务状态清单（按子任务）

| 子任务 ID | 状态 | 任务描述 | 交付物 | 完成判定 |
|---|---|---|---|---|
| KNO-TODO-035-D | Done | 收敛 runtime-owned hybrid canary seam 的 owner、allowlist、ready marker 与 Design->Build 映射 | `docs/todos/knowledge/deliverables/KNO-TODO-035-runtime-owned-hybrid-canary-seam双轨任务包.md` | 已补齐本地证据、外部实践、runtime/factory seam、Build 三件套、风险回退与 D Gate；明确 `035-B` 仅能在 runtime owner 下放行 allowlisted explicit hybrid canary，并保持 production default `LexicalOnly` 不变（2026-05-26） |
| KNO-TODO-035-B | Done | 落地 runtime allowlist、factory request admission seam、runtime ready marker 与 canary integration | `knowledge/include/KnowledgeServiceFactory.h`；`knowledge/src/KnowledgeServiceFactory.cpp`；`apps/runtime_support/include/RuntimeLiveDependencyComposition.h`；`apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp`；`tests/integration/access/RuntimeKnowledgeHybridCanaryIntegrationTest.cpp`；`tests/integration/knowledge/KnowledgeInstalledAssetHybridProbeIntegrationTest.cpp` | runtime 现通过 allowlisted corpus seam 控制 explicit hybrid canary 的放行：`desktop_full` / `cloud_full` / `edge_balanced` allowlisted query 可返回 `mode=Hybrid`，non-allowlisted corpus 与 runtime 未放行路径继续 `LexicalOnly`，并额外暴露 `knowledge-hybrid-canary-ready` marker（2026-05-26） |

#### 6.9.3 Build 完成证据（2026-05-26）

1. `knowledge/include/KnowledgeServiceFactory.h` 与 `knowledge/src/KnowledgeServiceFactory.cpp` 已新增 runtime canary allowlist seam，并把 explicit `preferred_mode=Hybrid` / `DenseOnly` 的 admission 收口到 factory `build_plan` wrapper：只有在 `dense_bridge->available()` 且 `allowed_corpora` 全部命中 runtime allowlist 时，才对当前请求临时提升 `retrieval_mode_default`；否则继续 `LexicalOnly`，并回传 `runtime_canary_not_admitted`、`runtime_canary_allowlist_miss`、`runtime_canary_backend_not_ready` 等 reason code。
2. `apps/runtime_support/include/RuntimeLiveDependencyComposition.h` 与 `apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp` 已把 runtime canary allowlist 和 `knowledge-hybrid-canary-ready` marker 收口到 live composition owner；allowlist 仅由既有 `effective_profile_id()` 派生到 `desktop_full`、`cloud_full`、`edge_balanced`，不新增 profile schema v1 键，也不把 rollout 权限下沉到 Knowledge。
3. 已扩展 `RuntimeLiveCompositionFailureMatrixTest` 并新增 `RuntimeKnowledgeHybridCanaryIntegrationTest`、`KnowledgeInstalledAssetHybridProbeIntegrationTest`；其中 runtime integration 使用 composition-local fake vector override seam 构造稳定的 vector-ready runtime path，专门验证 owner 逻辑，不改变 production 默认组合路径。
4. 2026-05-26 已通过 `cmake --build build/vscode-linux-ninja --target dasall_access_runtime_live_composition_failure_matrix_integration_test dasall_runtime_knowledge_hybrid_canary_integration_test dasall_knowledge_installed_asset_hybrid_probe_integration_test -j2`；`ctest --test-dir build/vscode-linux-ninja -N | rg "RuntimeKnowledgeHybridCanaryIntegrationTest|KnowledgeInstalledAssetHybridProbeTest"` 可发现新测试；`ctest --test-dir build/vscode-linux-ninja -R RuntimeLiveCompositionFailureMatrixTest --output-on-failure`、`ctest --test-dir build/vscode-linux-ninja -R RuntimeKnowledgeHybridCanaryIntegrationTest --output-on-failure` 与 `ctest --test-dir build/vscode-linux-ninja -R KnowledgeInstalledAssetHybridProbeTest --output-on-failure` 均通过。

### 6.10 production-grade embedding / query encoder seam 双轨任务包

说明：`KNO-TODO-036` 承接 `KNO-TODO-035-B`，负责把 runtime-owned canary seam 下的 live dense path 从“隐式 local hash fallback”收口成“显式 query encoder / embedding seam + lexical-only fail-safe”。本节按 `-D + -B` 双轨模板细化该任务；`KNO-TODO-036-D` 的设计交付已落盘到 `docs/todos/knowledge/deliverables/KNO-TODO-036-production-grade-embedding-query-encoder-seam双轨任务包.md`，`KNO-TODO-036-B` 只在不扩 `contracts/`、不改变 production default `LexicalOnly`、不回退 035 runtime-owned canary 主控权的前提下推进。

#### 6.10.1 Design->Build 拆分总表

| 主任务 | Design 子任务（文档交付） | Build 子任务（代码交付） | 输入依据 | 代码目标 | 测试目标 | 验收命令 |
|---|---|---|---|---|---|---|
| KNO-TODO-036 | KNO-TODO-036-D：收敛 production-grade embedding / query encoder seam 设计与 D Gate | KNO-TODO-036-B：落地 detached embedding search、query encoder injection 与 lexical-only fail-safe | `KNO-TODO-035-B`；ADR-006 / ADR-007 / ADR-008 owner boundary；Azure AI Search vector query；OpenSearch neural query | `memory/include/vector/DetachedVectorIndexFactory.h`；`memory/src/vector/DetachedVectorIndexFactory.cpp`；`memory/src/vector/SqliteVssVectorBackend.h`；`memory/src/vector/SqliteVssVectorBackend.cpp`；`memory/src/vector/SimpleLocalEmbeddingAdapter.cpp`；`knowledge/include/KnowledgeServiceFactory.h`；`knowledge/src/KnowledgeServiceFactory.cpp`；`apps/runtime_support/include/RuntimeLiveDependencyComposition.h`；`apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp`；`knowledge/src/retrieve/VectorRetrieverBridge.cpp` | `dasall_memory_simple_local_embedding_adapter_unit_test`；`VectorRetrieverBridgeTest`；`RuntimeLiveCompositionFailureMatrixTest`；`RuntimeKnowledgeHybridCanaryIntegrationTest`；`RuntimeKnowledgeQueryEncoderIntegrationTest` | `Build_CMakeTools(buildTargets=["dasall_apps_runtime_support","dasall_vector_retriever_bridge_unit_test","dasall_access_runtime_live_composition_failure_matrix_integration_test","dasall_runtime_knowledge_query_encoder_integration_test","dasall_runtime_knowledge_hybrid_canary_integration_test"])`；若 `RunCtest_CMakeTools` 继续命中仓库已知泛化 `生成失败`，回退直接执行 `./build/vscode-linux-ninja/tests/unit/memory/dasall_memory_simple_local_embedding_adapter_unit_test && ./build/vscode-linux-ninja/tests/unit/knowledge/dasall_vector_retriever_bridge_unit_test && ./build/vscode-linux-ninja/tests/integration/access/dasall_access_runtime_live_composition_failure_matrix_integration_test && ./build/vscode-linux-ninja/tests/integration/access/dasall_runtime_knowledge_hybrid_canary_integration_test && ./build/vscode-linux-ninja/tests/integration/access/dasall_runtime_knowledge_query_encoder_integration_test` |

#### 6.10.2 原子任务状态清单（按子任务）

| 子任务 ID | 状态 | 任务描述 | 交付物 | 完成判定 |
|---|---|---|---|---|
| KNO-TODO-036-D | Done | 收敛 production-grade query encoder seam、detached embedding search、local fallback gate 与 Design->Build 映射 | `docs/todos/knowledge/deliverables/KNO-TODO-036-production-grade-embedding-query-encoder-seam双轨任务包.md` | 已补齐本地证据、外部实践、Build 三件套、风险回退与 D Gate；明确 live dense path 不再默认依赖 `SimpleLocalEmbeddingAdapter` 的 local hash fallback（2026-05-26） |
| KNO-TODO-036-B | Done | 落地 detached embedding search、runtime/knowledge query encoder injection、runtime marker regression closure 与 encoder-missing lexical fallback | `memory/include/vector/DetachedVectorIndexFactory.h`；`memory/src/vector/DetachedVectorIndexFactory.cpp`；`memory/src/vector/SqliteVssVectorBackend.h`；`memory/src/vector/SqliteVssVectorBackend.cpp`；`memory/src/vector/SimpleLocalEmbeddingAdapter.cpp`；`knowledge/include/KnowledgeServiceFactory.h`；`knowledge/src/KnowledgeServiceFactory.cpp`；`apps/runtime_support/include/RuntimeLiveDependencyComposition.h`；`apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp`；`tests/integration/access/RuntimeKnowledgeQueryEncoderIntegrationTest.cpp` | live dense path 现通过 `IQueryEncoder` + detached `dense.sqlite` embedding search 驱动；runtime_support 会在 vector runtime configured 且无显式 override 时注入 env-gated default query encoder；encoder 缺失或返回空 embedding 时继续 lexical-only；`DASALL_DETACHED_VECTOR_LOCAL_FALLBACK=1` 下 allowlisted canary 可走 Hybrid 正例（2026-05-26 post-eval 整改） |

#### 6.10.3 Build 完成证据（2026-05-26）

1. `memory/include/vector/DetachedVectorIndexFactory.h`、`memory/src/vector/DetachedVectorIndexFactory.cpp`、`memory/src/vector/SqliteVssVectorBackend.h`、`memory/src/vector/SqliteVssVectorBackend.cpp` 与 `memory/src/vector/SimpleLocalEmbeddingAdapter.cpp` 已完成 detached embedding seam 收口：sqlite-vss backend 可直接按 embedding 检索，detached adapter 只在 `DASALL_DETACHED_VECTOR_LOCAL_FALLBACK` 显式开启时才自动创建 `SimpleLocalEmbeddingAdapter`，且 local hash 已改为稳定 FNV-1a token hash。
2. `knowledge/include/KnowledgeServiceFactory.h`、`knowledge/src/KnowledgeServiceFactory.cpp`、`knowledge/src/retrieve/VectorRetrieverBridge.cpp`、`apps/runtime_support/include/RuntimeLiveDependencyComposition.h` 与 `apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp` 已接入 query encoder seam：installed/live factory 现可创建 `IQueryEncoder`，runtime detached dense path 改为 `EmbeddingRequired` 并对 per-snapshot `dense.sqlite` 做 embedding search；post-eval 整改补齐默认 `RuntimeDetachedVectorQueryEncoder`，只有显式 `DASALL_DETACHED_VECTOR_LOCAL_FALLBACK=1` 时才输出本地 query embedding，encoder 缺失或空 embedding 时继续 `LexicalOnly` fail-safe。
3. `tests/integration/access/RuntimeLiveCompositionFailureMatrixTest.cpp` 与 `tests/integration/access/RuntimeKnowledgeHybridCanaryIntegrationTest.cpp` 已随 036 收口 marker probe 语义，`knowledge-hybrid-canary-ready` 现依赖真实 allowlisted hybrid canary probe，而不是间接 health 布尔值；新增并扩展 `tests/integration/access/RuntimeKnowledgeQueryEncoderIntegrationTest.cpp` 锁定 encoder-ready、encoder-missing lexical fallback 与 env-enabled default local fallback positive path。
...
| 主任务 | Design 子任务（文档交付） | Build 子任务（代码交付） | 输入依据 | 代码目标 | 测试目标 | 验收命令 |
|---|---|---|---|---|---|---|
| KNO-TODO-040 | KNO-TODO-040-D：收敛 installed local hybrid canary / soak artifact contract 与 D Gate | KNO-TODO-040-B：落地 proof / soak hybrid canary artifact、strict positive summary 与 release workflow 接线 | `KNO-TODO-039-B`；`docs/todos/knowledge/deliverables/KNO-TODO-040-installed-local-hybrid-canary-soak-evidence双轨任务包.md`；`scripts/packaging/README.md` release-runner contract；2026-05-26 post-eval整改结论 | `scripts/packaging/knowledge_local_installed_proof.sh`；`scripts/packaging/knowledge_refresh_retrieve_soak.sh`；`.github/workflows/release-package-gate.yml`；`scripts/packaging/README.md` | local proof artifact contract；10 轮 soak artifact contract；release local workflow wiring review；strict positive hybrid assertions | `dpkg-buildpackage -us -uc -b`；`bash scripts/packaging/knowledge_local_installed_proof.sh --artifact-dir /tmp/dasall-kno-todo-040-proof-strict --hybrid-canary`；`bash scripts/packaging/knowledge_refresh_retrieve_soak.sh --artifact-dir /tmp/dasall-kno-todo-040-soak-strict --iterations 10 --hybrid-canary` |

#### 6.14.2 原子任务状态清单（按子任务）

| 子任务 ID | 状态 | 任务描述 | 交付物 | 完成判定 |
|---|---|---|---|---|
| KNO-TODO-040-D | Done | 收敛 installed local proof / soak hybrid canary artifact contract、strict positive 语义与 D Gate | `docs/todos/knowledge/deliverables/KNO-TODO-040-installed-local-hybrid-canary-soak-evidence双轨任务包.md` | 已补齐本地证据、外部实践、Build 三件套、风险回退与 D Gate；post-eval 整改后 040 不再接受 lexical fallback 作为通过，只接受 strict hybrid positive artifact（2026-05-26） |
| KNO-TODO-040-B | Done | 落地 proof / soak hybrid canary raw artifact、strict positive summary 与 release workflow 默认接线 | `scripts/packaging/knowledge_local_installed_proof.sh`；`scripts/packaging/knowledge_refresh_retrieve_soak.sh`；`.github/workflows/release-package-gate.yml`；`scripts/packaging/README.md` | proof / soak / workflow 现默认固定 `--hybrid-canary`；strict gate 要求 `mode=hybrid`、`runtime_canary_admitted`、`vector_backend_ready=true` 且 `dense_hit_count>0`，历史 lexical fallback artifact 只作为问题发现证据保留（2026-05-26 post-eval 整改） |
...
4. 2026-05-26 post-eval 整改已把上述历史 installed `mode=lexical_only` / `runtime_canary_backend_not_ready` / `dense_hit_count=0` artifact 重新归类为问题发现证据；新的完成判定必须在重构包后通过 strict proof / soak，并生成 `mode=hybrid`、`runtime_canary_admitted`、`vector_backend_ready=true`、`dense_hit_count>0` 的当轮 artifact。本轮已通过 focused query encoder 集成测试与 `bash -n` 脚本语法门禁，real installed strict rerun 归入后续 package/release 环境复核。

### 6.15 向量能力生产级整改闭合追加任务（2026-05-26）

| Task ID | 状态 | Design 任务 | Build 任务 | 依赖 | 代码目标 | 测试目标 | 验收命令 |
|---|---|---|---|---|---|---|---|
| KNO-TODO-042 | Done | strict hybrid ready marker 与 vector-enabled manifest 语义对齐 | 已落地 `vector_enabled` refresh batch 传递、strict marker 判定与 dense-hit/empty-hit 回归 | KNO-TODO-039 / 040 post-eval 问题清单 | `knowledge/include/ingest/IngestionCoordinator.h`；`knowledge/src/facade/KnowledgeService.cpp`；`knowledge/src/index/IndexWriter.cpp`；`apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp`；`tests/integration/access/RuntimeLiveCompositionFailureMatrixTest.cpp`；`tests/integration/access/RuntimeKnowledgeHybridCanaryIntegrationTest.cpp` | marker 必须证明 `Hybrid + runtime_canary_admitted + vector_backend_ready + dense_hit_count>0`；首次 refresh manifest 不再默认 vector disabled；empty dense lane 不暴露 hybrid ready marker | `cmake --build build/vscode-linux-ninja --target dasall_access_runtime_live_composition_failure_matrix_integration_test dasall_runtime_knowledge_hybrid_canary_integration_test -j2`；`./build/vscode-linux-ninja/tests/integration/access/dasall_access_runtime_live_composition_failure_matrix_integration_test && ./build/vscode-linux-ninja/tests/integration/access/dasall_runtime_knowledge_hybrid_canary_integration_test && echo PASS` |
| KNO-TODO-043 | Todo | production embedding provider contract 收敛 | 替换 production path 对 env-gated local fallback 的依赖，补 provider-ready / provider-missing / invalid-secret fail-closed | KNO-TODO-042；provider asset / secret owner 口径 | runtime_support provider factory；memory-owned vector adapter health；telemetry/audit reason code | `RuntimeKnowledgeQueryEncoderIntegrationTest` 扩展生产 provider 正例；缺 provider / 空 embedding / invalid secret 均 lexical-only fail-safe | `cmake --build build/vscode-linux-ninja --target dasall_runtime_knowledge_query_encoder_integration_test dasall_knowledge_production_telemetry_integration_test -j2`；禁用 local fallback env 后执行 provider-ready 正例 |
| KNO-TODO-044 | Done | 新包 strict installed evidence 复核 | 已重构 Debian 包并执行 strict proof / soak；产出 raw hybrid artifact、summary 与 package evidence | KNO-TODO-042 后新包；local installed/systemd/root 环境已具备；`SimpleLocalEmbeddingAdapter` 已补 UTF-8 local/test/package fallback，避免中文 chunk 被误判 unembeddable | `dasall_memory_simple_local_embedding_adapter_unit_test`；`knowledge-proof.json` 记录 `mode=hybrid`、`runtime_canary_admitted`、`vector_backend_ready=true`、`dense_hit_count=3`；`knowledge-soak-summary.json` 记录 10/10 轮 strict hybrid positive、`min_hybrid_dense_hit_count=3` | `Build_CMakeTools(buildTargets=["dasall_memory_simple_local_embedding_adapter_unit_test"])`；直接执行 `./build/vscode-linux-ninja/tests/unit/memory/dasall_memory_simple_local_embedding_adapter_unit_test && echo PASS`；`dpkg-buildpackage -us -uc -b`；`bash scripts/packaging/knowledge_local_installed_proof.sh --artifact-dir /tmp/dasall-kno-todo-044-proof-strict --hybrid-canary`；`bash scripts/packaging/knowledge_refresh_retrieve_soak.sh --artifact-dir /tmp/dasall-kno-todo-044-soak-strict --iterations 10 --hybrid-canary` |
| KNO-TODO-045 | Done | 已完成 D/B：`docs/todos/knowledge/deliverables/KNO-TODO-045-runtime-owned-selective-refresh-automation口径闭合双轨任务包.md` 已补到 Build 完成态；runtime auto-refresh 现为 selective-first，041 periodic full-scan 被降级为 fallback 基线 | 已落地 runtime-owned refresh plan/provider seam，default provider 复用 `knowledge::ingest::SourceScanner` inventory diff 生成 selective delta，并将 source URI 归一为 installed descriptor root 对应的绝对路径；`refresh_in_flight` tick 直接 benign skip，provider 无 delta / baseline 失配 / quarantine 时回退 full-scan | KNO-TODO-041；ADR-006/007/008 owner boundary | `apps/runtime_support/include/RuntimeLiveDependencyComposition.h`；`apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp`；`tests/integration/knowledge/KnowledgeRuntimeAutoRefreshIntegrationTest.cpp`；`tests/integration/knowledge/KnowledgeRefreshLoopTest.cpp` | `KnowledgeRuntimeAutoRefreshIntegrationTest` 已覆盖 changed-asset selective、injected provider target-only selective、full-scan fallback 与 busy skip；`KnowledgeRefreshLoopTest` 已覆盖 multi-corpus target-only selective regression | `cmake --build build/vscode-linux-ninja --target dasall_knowledge_runtime_auto_refresh_integration_test dasall_knowledge_refresh_loop_integration_test -j2`；`RunCtest_CMakeTools` 仍命中仓库已知泛化 `生成失败` 后直接执行 `./build/vscode-linux-ninja/tests/integration/knowledge/dasall_knowledge_runtime_auto_refresh_integration_test && ./build/vscode-linux-ninja/tests/integration/knowledge/dasall_knowledge_refresh_loop_integration_test && printf "%s\n" PASS`，结果 `PASS` |
...
| K installed local hybrid canary / soak evidence | KNO-TODO-040-D、040-B | 040-D / 040-B 已完成 | 040 已把 proof / soak hybrid canary request、mode / reason / dense artifact presence 与 release workflow 默认接线收口为 strict positive gate；只有 `mode=hybrid`、`runtime_canary_admitted`、`vector_backend_ready=true` 且 `dense_hit_count>0` 才能通过；041 才进入 Runtime-owned selective refresh automation |
...
| Gate-L：installed local hybrid evidence ready | local proof / soak 与 release workflow 已默认固定 `--hybrid-canary`，`knowledge-proof.json` / `knowledge-soak-summary.json` 必须记录 `mode=hybrid`、`runtime_canary_admitted`、`vector_backend_ready=true` 与 `dense_hit_count>0`；lexical fallback 只能作为问题证据，不能作为 Gate-L 通过依据 | KNO-TODO-040-B | 禁止推进 041 的 Runtime-owned selective refresh automation |
...
## 12. 追加执行回写（2026-05-26）

### 12.1 KNO-TODO-041 Runtime-owned selective refresh automation
...
1. `platform/include/linux/PosixTimerProvider.h`、`platform/src/linux/PosixTimerProvider.cpp` 与 `tests/unit/platform/linux/PosixTimerProviderTest.cpp` 已把 Linux timer provider 从 handle-only skeleton 收口为真实 callback delivery + cancel 语义；focused unit gate 现能直接验证 periodic callback 至少触发一次且 cancel 后不再继续回调。
2. `apps/runtime_support/include/RuntimeLiveDependencyComposition.h`、`apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp` 与 `apps/runtime_support/CMakeLists.txt` 已新增 `platform::ITimer` 注入 seam，并把 installed knowledge service 包装成 runtime-owned periodic full-scan auto-refresh controller：timer tick 固定走 `request_refresh(CorpusChangeSet{})`，timer-ready path 写出 `runtime:<owner>:knowledge-refresh-automation-ready`，timer 缺失或 arm 失败时写出 `runtime:<owner>:knowledge-refresh-automation-fallback:*`，不引入 Knowledge-owned watcher/timer。
3. `apps/daemon/src/main.cpp`、`apps/gateway/src/main.cpp` 与 `apps/gateway/CMakeLists.txt` 已把 `PosixTimerProvider` 接到真实入口 wiring；`tests/integration/access/RuntimeLiveCompositionFailureMatrixTest.cpp` 随之锁定 ready/fallback marker 的 stratified 语义，区分“knowledge ready 但 automation fallback”与“knowledge degraded”。
4. `tests/integration/knowledge/KnowledgeRefreshLoopTest.cpp` 已新增 empty `CorpusChangeSet` full-scan fallback regression；新增 `tests/integration/knowledge/KnowledgeRuntimeAutoRefreshIntegrationTest.cpp` 并更新 `tests/integration/knowledge/CMakeLists.txt` 后，real knowledge refresh loop + recording timer 已能证明 source 更新后仅靠 runtime tick 即可完成 refresh / retrieve 闭环。

### 12.2 KNO-TODO-045 Runtime-owned selective refresh automation 完成证据

1. `apps/runtime_support/include/RuntimeLiveDependencyComposition.h` 与 `apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp` 已新增 `RuntimeKnowledgeRefreshPlanKind`、`RuntimeKnowledgeRefreshPlan` 与 `IRuntimeKnowledgeRefreshSourceProvider`；production path 默认接上 installed selective provider，focused integration path 可注入 recording provider。default provider 现复用 `knowledge::ingest::SourceScanner` inventory diff，并把相对 source URI 归一为 installed descriptor root 使用的绝对路径，避免 selective delta 因路径口径漂移漏选 corpus。
2. `tests/integration/knowledge/KnowledgeRuntimeAutoRefreshIntegrationTest.cpp` 已新增四类 focused case：default provider changed-asset selective 正例、注入 provider 的 target-only selective 正例、provider 显式 full-scan fallback 正例，以及通过扩展 ADR corpus 体量拉长窗口后的 busy skip 回归。2026-05-26 已通过 `cmake --build build/vscode-linux-ninja --target dasall_knowledge_runtime_auto_refresh_integration_test -j2` 与 direct binary `./build/vscode-linux-ninja/tests/integration/knowledge/dasall_knowledge_runtime_auto_refresh_integration_test`。
3. `tests/integration/knowledge/KnowledgeRefreshLoopTest.cpp` 已新增 multi-corpus selective regression：当 `updated_sources` 仅包含 ADR source 时，active snapshot 只引入 ADR 更新，SSOT 非目标更新不会被误刷新；既有 full-scan fallback、busy reject 与 activation rollback 回归保持不变。2026-05-26 已通过 `cmake --build build/vscode-linux-ninja --target dasall_knowledge_refresh_loop_integration_test -j2` 与 direct binary `./build/vscode-linux-ninja/tests/integration/knowledge/dasall_knowledge_refresh_loop_integration_test`。
4. 最终 focused 验收命令 `./build/vscode-linux-ninja/tests/integration/knowledge/dasall_knowledge_runtime_auto_refresh_integration_test && ./build/vscode-linux-ninja/tests/integration/knowledge/dasall_knowledge_refresh_loop_integration_test && printf "%s\n" PASS` 已返回 `PASS`。`RunCtest_CMakeTools` 在本仓继续只返回泛化 `生成失败`，因此本轮沿用仓库既有 direct-binary fallback 口径，不额外扩大验证面。
5. 045 完成后，runtime timer tick 已从“固定 empty `CorpusChangeSet` 的 periodic full-scan”收口为“selective-first，busy tick benign skip，provider 无 delta / baseline 失配 / quarantine 才 full-scan fallback”；Runtime 继续拥有触发与 delta 生成权，Knowledge 继续只消费 `CorpusChangeSet`，ADR-006 / ADR-007 / ADR-008 owner boundary 未回退。
