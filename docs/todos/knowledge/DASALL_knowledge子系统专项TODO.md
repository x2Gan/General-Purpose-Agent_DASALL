...
适用范围：knowledge/  
当前结论：Knowledge 子系统当前可直接生成 L3/L2 混合专项 TODO。`KnowledgeQuery`、`EvidenceSlice`、`EvidenceBundle`、`KnowledgeRetrieveResult`、`IKnowledgeService`、`KnowledgeConfigProjector`、`QueryNormalizer`、`CorpusRouter`、`Reranker`、`EvidenceAssembler`、`KnowledgeServiceFacade`、`FreshnessController`、`VersionLedger`、`KnowledgeTelemetry`、`KnowledgeHealthProbe` 已具备接口级或数据结构级拆分条件；`KNO-TODO-001` 已冻结 lexical 路线为 SQLite FTS5，`KNO-TODO-002` 已冻结 `VectorRetrieverBridge` 的 owner、注入方向与 module-local ports（`IQueryEncoder` / `IVectorRecallStore`），并解除 `KNO-BLK-001`、`KNO-BLK-002`。当前仍需继续完成首批 corpus 资产与 metadata/trust 规范、retrieval quality golden set 两项前置补设计，之后再把 ingest/quality 全链路推进到稳定 Build。
...
2. L2：`KnowledgeServiceFacade`、`RecallCoordinator`、`SparseRetriever`、`VectorRetrieverBridge`、`CorpusCatalog`、`VersionLedger`、`IndexReader`、`IndexWriter`、`KnowledgeTelemetry`、`KnowledgeHealthProbe`。
3. L2 但当前 Blocked：`SourceScanner`、`Canonicalizer`、`Chunker`、`IngestionCoordinator`、`RetrievalQualityRegression`。
...
2. L2：KnowledgeServiceFacade（骨架版，已拆分为 012 + 032 两阶段）、SparseRetriever、VectorRetrieverBridge、CorpusCatalog、VersionLedger、IndexReader、IndexWriter、KnowledgeTelemetry、KnowledgeHealthProbe。
3. L2 但当前 Blocked：SourceScanner、Canonicalizer、Chunker、IngestionCoordinator、RetrievalQualityRegression、KnowledgeServiceFacade 完整版（032）、refresh 闭环集成测试（033）。
...
1. `KNO-TODO-001`、`KNO-TODO-002` 已完成；继续串行推进 KNO-TODO-003 ~ 004 的补设计解阻。
2. 在 blocker 解锁前，优先推进 KNO-TODO-005 ~ 012、016 ~ 018、025 的骨架、ABI、纯计算和观测类任务（025/026 已前移）。
3. `KNO-BLK-001`、`KNO-BLK-002` 已解锁，可推进 lexical 主链、hybrid/profile 与 failure/degrade；在 KNO-BLK-003 解锁后推进 ingest/index；在 KNO-BLK-004 解锁后补 quality regression Gate。
