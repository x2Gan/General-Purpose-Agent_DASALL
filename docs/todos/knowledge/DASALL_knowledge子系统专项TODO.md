...
| KNO-TODO-014 | Done | 实现 RecallCoordinator lane 编排 | 详设 6.10、6.13.2、7 KNO-D03 | `RecallCoordinator`；`RecallCandidateSet` | L2 | 新增 `knowledge/include/retrieve/RecallCoordinator.h`、`knowledge/src/retrieve/RecallCoordinator.cpp`，落盘 sparse/dense lane 调度、**v1 默认串行执行**（不开放并行度控制，为 v2 hybrid 并发预留 lane executor 接口）、partial results 和 degraded 标记；补充 `RecallRequest` / `DenseRecallRequest` / `RecallCoordinatorResult`，显式表达双 lane 都失败但尚未进入 facade `ErrorInfo` 映射的状态 | `RecallCoordinator::recall`；`run_sparse_lane`；`run_dense_lane` | unit：`RecallCoordinatorTest`、`RecallCoordinatorDegradedTest`、**`RecallCoordinatorSerialExecutionTest`** | `cmake -S . -B build-ci -G "Unix Makefiles" && cmake --build build-ci --target dasall_knowledge dasall_recall_coordinator_unit_test dasall_recall_coordinator_degraded_unit_test dasall_recall_coordinator_serial_execution_unit_test && ctest --test-dir build-ci -R "RecallCoordinator.*Test" --output-on-failure` | KNO-TODO-007、013 | 无 | 013 至少具备可替身的 lane seam；dense lane 使用 stub | `docs/todos/knowledge/deliverables/KNO-TODO-014-RecallCoordinator设计收敛.md`；`knowledge/include/retrieve/RecallCoordinator.h`；`knowledge/src/retrieve/RecallCoordinator.cpp`；`tests/unit/knowledge/RecallCoordinatorTest.cpp`；`tests/unit/knowledge/RecallCoordinatorDegradedTest.cpp`；`tests/unit/knowledge/RecallCoordinatorSerialExecutionTest.cpp`；`tests/unit/knowledge/CMakeLists.txt`；`knowledge/CMakeLists.txt`；2026-04-21 已通过 Build_CMakeTools 定向构建；`RunCtest_CMakeTools` 工具态报 `生成失败` 后，已使用 build-ci 回退路径 `ctest` 3/3 验收通过 | 仅当双 lane 超时、单 lane 成功、双 lane 失败三类路径可自动验证，且 dense 失败不会拖垮 sparse 返回时完成 |
