...
| KNO-TODO-045 | Todo | D 包已落盘并通过 D Gate：`docs/todos/knowledge/deliverables/KNO-TODO-045-runtime-owned-selective-refresh-automation口径闭合双轨任务包.md`；Build 待落地 runtime-owned selective delta provider | 在 041 periodic full-scan 基础上补 runtime-owned `changed_sources` delta 输入；full-scan 仅保留 fallback | KNO-TODO-041；ADR-006/007/008 owner boundary | `apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp`；daemon/gateway refresh source provider；Knowledge service 继续只消费 `CorpusChangeSet` | `KnowledgeRuntimeAutoRefreshIntegrationTest` 与 `KnowledgeRefreshLoopTest` 新增 selective positive、busy fail-safe 与 full-scan fallback 回归 | `cmake --build build/vscode-linux-ninja --target dasall_knowledge_runtime_auto_refresh_integration_test dasall_knowledge_refresh_loop_integration_test -j2`；直接执行两个 binaries |
...
### 12.2 KNO-TODO-045 Runtime-owned selective refresh automation 口径闭合

1. `docs/todos/knowledge/deliverables/KNO-TODO-045-runtime-owned-selective-refresh-automation口径闭合双轨任务包.md` 已落盘，并完成 D Gate：当前缺口已收敛为 runtime timer callback 缺少 selective delta provider，而不是 Knowledge refresh worker 缺失。
2. D 包已锁定最小实现面：在 `apps/runtime_support` 内新增 runtime-owned refresh plan / provider seam，default provider 复用 `knowledge::ingest::SourceScanner` 的 inventory diff，为 installed asset corpus roots 生成 `Skip / Selective / FullScanFallback` 三态计划。
3. Build 三件套已锁定：代码目标聚焦 `apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp` 与相邻 header，测试目标聚焦 `KnowledgeRuntimeAutoRefreshIntegrationTest`、`KnowledgeRefreshLoopTest`，验收命令聚焦 `cmake --build build/vscode-linux-ninja --target dasall_knowledge_runtime_auto_refresh_integration_test dasall_knowledge_refresh_loop_integration_test -j2` 与 direct binary。
4. 本轮 D 结论明确保留 owner boundary：Runtime 负责生成 delta，Knowledge 继续只消费 `CorpusChangeSet`；manual `changed_source -> updated_sources` control-plane seam 继续 authoritative，full-scan 仅作为 fallback。
