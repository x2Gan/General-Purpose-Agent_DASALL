...
| DIA-TC013 | diagnostics 设计 3.1、11.1；infra 专项 TODO INF-BLK-08 | Must | 剩余阻塞项已收敛到 allowed_commands 参数 schema、脱敏矩阵、导出细则与桥接接口；CommandCatalog/ValidationResult、命令白名单与 integration 顶层拓扑已完成校准 | 必须显式输出剩余 Blocked 与解阻动作 |
...
5. 剩余证据缺口已收敛为 allowed_commands 参数 schema、metrics/audit 桥接接口签名，以及导出格式与远程目标白名单；SnapshotQuery、SnapshotExportRequest、DiagnosticsSnapshotResult、CommandCatalog、ValidationResult 已分别由 INF-BLK-08 与 DIA-TODO-008 补齐。
...
| C 接口冻结 | DIA-TODO-009、DIA-TODO-011 | 可并行，但依赖 A-B | PolicyGuard 可先行；IDiagnosticsCommandRegistry 已因 DIA-TODO-008 解阻，可与 009 并行推进 |
...
| DIA-BLK-002 | 已解阻（2026-04-07）：docs/architecture/DASALL_infra_diagnostics模块详细设计.md 6.5/6.5.1/6.6 已补齐 CommandCatalog、ValidationResult 与 arg_schema_ref/arg_schema_summary/field_paths 返回语义；docs/todos/infrastructure/deliverables/DIA-TODO-008-CommandRegistry目录与校验语义收敛.md 已记录设计结论 | DIA-TODO-011；DIA-TODO-013（部分，仅保留 DIA-BLK-003） | 无；后续仅需保持 diagnostics 详细设计、专项 TODO 与后续接口任务口径同步 | 证据回链到 diagnostics 详细设计、设计收敛记录与 DIA-TODO-008 完成记录 | 若目录对象或 validate 返回语义回退，则重新转为 Blocked |
...
1. 数据结构：函数/字段级可执行。
2. 接口：IDiagnosticsPolicyGuard、IDiagnosticsService 与 IDiagnosticsCommandRegistry 均已具备方法级冻结条件；其中 IDiagnosticsCommandRegistry 的实现仍需等待后续头文件任务落盘。
3. 实现：CommandPolicyGuard、CommandExecutor、EvidenceCollector、SnapshotAssembler、SnapshotStore 当前可到类级骨架。

### 11.4 若未达到函数级，还缺哪些设计信息

1. allowed_commands 参数 schema 与脱敏规则矩阵。
2. 导出格式、checksum 规则、远程目标白名单。
3. metrics/audit 最小桥接接口签名与 diagnostics integration 闭环用例设计。

### 11.5 下一步建议

1. 优先推进 DIA-TODO-009 与 DIA-TODO-011，完成 diagnostics 公开接口收口。
