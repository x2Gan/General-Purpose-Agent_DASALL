...
3. 实现：SnapshotStore 当前可到类级骨架；RedactionEngine/ExportManager/Bridge 仍需先解相邻阻塞。
...
1. 主链 D 阶段已完成；若继续按安全顺序推进，先解 DIA-BLK-004，再落 DIA-TODO-018 RedactionEngine。
2. 若保持当前对象与接口口径不变，可开始评估 DIA-TODO-019 SnapshotStore 的最小持久化骨架与 retention 约束。
