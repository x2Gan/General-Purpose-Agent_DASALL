...
| DIA-TODO-018 | Not Started | 实现 RedactionEngine 脱敏骨架 | diagnostics 设计 6.2/6.3/6.8/6.9；11.1 | 6.2 RedactionEngine；6.8 脱敏失败；11.1 D-BLK-02 | L2 | infra/src/diagnostics/RedactionEngine.cpp | RedactionEngine | unit：DiagnosticsRedactionTest；failure：DiagnosticsRedactionFailureTest | cmake -S . -B build-ci -G Ninja && cmake --build build-ci --target dasall_infra && ctest --test-dir build-ci -R "DiagnosticsRedactionTest|DiagnosticsRedactionFailureTest" --output-on-failure | DIA-TODO-017 | 无（2026-04-07 已由 DIA-BLK-004 解阻） | 无 | RedactionEngine.cpp、脱敏测试；待实现 strict/compat 双档脱敏与 failure gate | 仅当敏感字段不落盘、脱敏失败阻断导出且测试可验证时完成 |
...
3. 实现：RedactionEngine、SnapshotStore 当前可到类级骨架；ExportManager/Bridge 仍需先解相邻阻塞。
...
1. DIA-BLK-004 已解阻；继续按安全顺序推进时，可直接落 DIA-TODO-018 RedactionEngine。
2. 若 018 完成并通过 redaction gate，再进入 DIA-TODO-019 SnapshotStore，保持“先脱敏，再存储，再导出”的顺序。
