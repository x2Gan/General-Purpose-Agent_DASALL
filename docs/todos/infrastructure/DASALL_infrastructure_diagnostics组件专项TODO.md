...
| F 桥接与门禁 | DIA-TODO-021、DIA-TODO-022、DIA-TODO-023、DIA-TODO-024、DIA-TODO-025 | 串行推进更稳妥：023 已完成，继续 024/025 | 2026-04-07 已由 DIA-BLK-006 冻结 bridge sink contract，且 DIA-TODO-021、DIA-TODO-022 已完成 diagnostics metrics/audit bridge，DIA-TODO-023 已完成 diagnostics 源码构建图收口；剩余门禁集中在测试发现性 |
...
5. tests/integration 顶层已接线并可发现 InfraDiagnosticsSmokeTest，且 diagnostics 私有源码已全部进入 `dasall_infra` 构建图；剩余门禁收口集中在 024/025 的测试发现性。

### 11.3 当前最小可执行粒度

1. 数据结构：函数/字段级可执行。
2. 接口：IDiagnosticsPolicyGuard、IDiagnosticsService 与 IDiagnosticsCommandRegistry 均已完成方法级冻结。
3. 实现：DiagnosticsMetricsBridge、DiagnosticsAuditBridge 与 ExportManager 均已落盘最小骨架，且 diagnostics 私有源码已完成构建接线；下一步直接收口 unit/contract/integration 发现性门禁。

### 11.4 若未达到函数级，还缺哪些设计信息

1. diagnostics unit-contract / integration 发现性门禁的最终收口证据。

### 11.5 下一步建议

1. 直接进入 DIA-TODO-024，收口 diagnostics 的 unit / contract 测试发现性与注册证据。
2. 024 完成后推进 DIA-TODO-025，完成 diagnostics integration 发现性闭环。
