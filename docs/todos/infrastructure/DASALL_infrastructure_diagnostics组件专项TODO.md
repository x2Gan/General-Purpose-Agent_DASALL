...
3. 实现：SnapshotStore 当前可到类级骨架；ExportManager/Bridge 仍需先解相邻阻塞。
...
1. 直接推进 DIA-TODO-019，收口 diagnostics snapshot 的最小持久化与 retention 语义。
2. 019 完成后先解 DIA-BLK-005，再进入 DIA-TODO-020 ExportManager，继续保持“先脱敏，再存储，再导出”的顺序。
3. 在导出格式冻结后推进 DIA-TODO-020；在 metrics/audit 桥接接口冻结后再推进 DIA-TODO-021、DIA-TODO-022。
