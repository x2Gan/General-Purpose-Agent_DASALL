...
| F 桥接与门禁 | DIA-TODO-021、DIA-TODO-022、DIA-TODO-023、DIA-TODO-024、DIA-TODO-025 | 串行推进更稳妥：先 022，再 023/024/025 | 2026-04-07 已由 DIA-BLK-006 冻结 bridge sink contract，且 DIA-TODO-021 已完成 diagnostics metrics bridge；完整 integration 闭环仍以后置依赖为前提 |
...
4. RedactionEngine、ExportManager 与 DiagnosticsMetricsBridge 已完成最小骨架；DiagnosticsAuditBridge 的 sink contract 已由 6.10.1 冻结，因此 022 现可直接进入实现。
5. tests/integration 顶层已接线并可发现 InfraDiagnosticsSmokeTest，但完整 diagnostics audit bridge / integration 闭环仍需随 022/025 落盘。

### 11.3 当前最小可执行粒度

1. 数据结构：函数/字段级可执行。
2. 接口：IDiagnosticsPolicyGuard、IDiagnosticsService 与 IDiagnosticsCommandRegistry 均已完成方法级冻结。
3. 实现：DiagnosticsMetricsBridge 已落盘最小七指标骨架；DiagnosticsAuditBridge 已具备最小 sink contract，可直接推进 bridge 骨架；ExportManager 已落盘最小导出骨架。

### 11.4 若未达到函数级，还缺哪些设计信息

1. diagnostics audit bridge / integration 闭环用例设计，以及 bridge runtime wiring 的最终 reachability 证据。

### 11.5 下一步建议

1. 直接进入 DIA-TODO-022，把 remote export / command extension 的强制审计接到 `DiagnosticsAuditBridge`。
2. 022 完成后推进 023/024/025，收口 diagnostics 的构建接线、测试注册与 integration 发现性。
