...
| F 桥接与门禁 | DIA-TODO-021、DIA-TODO-022、DIA-TODO-023、DIA-TODO-024、DIA-TODO-025 | 串行推进更稳妥：024 已完成，继续 025 | 2026-04-07 已由 DIA-BLK-006 冻结 bridge sink contract，且 DIA-TODO-021~024 已完成 diagnostics bridge、构建接线与 unit/contract 发现性门禁；剩余门禁只在 integration |
...
5. tests/integration 顶层已接线并可发现 InfraDiagnosticsSmokeTest，且 diagnostics 私有源码与 unit/contract 发现性均已收口；剩余门禁只集中在 025 的 integration 发现性。

### 11.3 当前最小可执行粒度

1. 数据结构：函数/字段级可执行。
2. 接口：IDiagnosticsPolicyGuard、IDiagnosticsService 与 IDiagnosticsCommandRegistry 均已完成方法级冻结。
3. 实现：DiagnosticsMetricsBridge、DiagnosticsAuditBridge 与 ExportManager 均已落盘最小骨架，且 diagnostics 私有源码与 unit/contract 测试发现性均已完成；下一步只需收口 integration 发现性门禁。

### 11.4 若未达到函数级，还缺哪些设计信息

1. diagnostics integration 发现性门禁的最终收口证据。

### 11.5 下一步建议

1. 直接进入 DIA-TODO-025，完成 diagnostics integration 发现性闭环。
2. 025 完成后再进入 DIA-TODO-026，统一回写 diagnostics 质量门证据。
3. 保持现有 diagnostics 测试标签与注册入口稳定，避免在 025 之前回退 `unit` / `contract` 发现性。