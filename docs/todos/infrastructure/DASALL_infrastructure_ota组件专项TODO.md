...
| OTA-TODO-008 | Done | 实现 ArtifactCompatibilityEvaluator 骨架 | OTA 设计 6.2/6.8 | 6.2 ArtifactCompatibilityEvaluator | L3 | infra/src/ota/ArtifactCompatibilityEvaluator.cpp | evaluate(verified_manifest, capability, profile) | unit：hardware_selector/profile/dependency_refs 冲突拒绝 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci --target dasall_unit_tests && ctest --test-dir build-ci --output-on-failure -L unit | OTA-TODO-001、007 | 无 | 无 | 骨架实现、unit 测试、deliverable；2026-04-07 已落盘 infra/src/ota/ArtifactCompatibilityEvaluator.h、infra/src/ota/ArtifactCompatibilityEvaluator.cpp、tests/unit/infra/ota/ArtifactCompatibilityEvaluatorTest.cpp、docs/todos/infrastructure/deliverables/OTA-TODO-008-ArtifactCompatibilityEvaluator骨架收敛.md，并更新 infra/tests CMake 接线；已通过 `cmake -S . -B build-ci`、`cmake --build build-ci --target dasall_artifact_compatibility_evaluator_unit_test`、`ctest --test-dir build-ci -N -R "ArtifactCompatibilityEvaluatorTest"`、`ctest --test-dir build-ci --output-on-failure -R "ArtifactCompatibilityEvaluatorTest"`、`cmake --build build-ci --target dasall_unit_tests` 与 `ctest --test-dir build-ci --output-on-failure -L unit` | 仅当兼容性失败可阻断安装且返回原因可追踪时完成 |
| OTA-TODO-009 | Done | 实现 InstallExecutor 骨架 | OTA 设计 6.2/6.8 | 6.2 InstallExecutor；6.8 安装失败处理 | L3 | infra/src/ota/InstallExecutor.cpp | stage_artifact/activate_plan/revert_install | unit：repo_bound 与 slot_bound 分支可区分；failure：写入失败可清理 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci --target dasall_unit_tests && ctest --test-dir build-ci --output-on-failure -L unit | OTA-TODO-004、008 | 无 | 无 | 骨架实现、unit 测试、deliverable；2026-04-07 已落盘 infra/src/ota/InstallExecutor.h、infra/src/ota/InstallExecutor.cpp、tests/unit/infra/ota/InstallExecutorTest.cpp、docs/todos/infrastructure/deliverables/OTA-TODO-009-InstallExecutor骨架收敛.md，并更新 infra/tests CMake 接线；已通过 `cmake -S . -B build-ci`、`cmake --build build-ci --target dasall_install_executor_unit_test`、`ctest --test-dir build-ci -N -R "InstallExecutorTest"`、`ctest --test-dir build-ci --output-on-failure -R "InstallExecutorTest"`、`cmake --build build-ci --target dasall_unit_tests` 与 `ctest --test-dir build-ci --output-on-failure -L unit` | 仅当安装失败能触发清理路径且产出 InstallEvidence 时完成 |
| OTA-TODO-010 | Done | 实现 SlotSwitchCoordinator 骨架 | OTA 设计 6.2/6.8/6.9 | 6.2 SlotSwitchCoordinator | L3 | infra/src/ota/SlotSwitchCoordinator.cpp | build_slot_plan/select_inactive_slot/set_next_boot | unit：仅允许 inactive slot；failure：slot 不可用拒绝 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci --target dasall_unit_tests && ctest --test-dir build-ci --output-on-failure -L unit | OTA-TODO-005、009 | 无 | 无 | 骨架实现、unit 测试、deliverable；2026-04-07 已落盘 infra/src/ota/SlotSwitchCoordinator.h、infra/src/ota/SlotSwitchCoordinator.cpp、tests/unit/infra/ota/SlotSwitchCoordinatorTest.cpp、docs/todos/infrastructure/deliverables/OTA-TODO-010-SlotSwitchCoordinator骨架收敛.md，并更新 infra/tests CMake 接线；已通过 `cmake -S . -B build-ci`、`cmake --build build-ci --target dasall_slot_switch_coordinator_unit_test`、`ctest --test-dir build-ci -N -R "SlotSwitchCoordinatorTest"`、`ctest --test-dir build-ci --output-on-failure -R "SlotSwitchCoordinatorTest"`、`cmake --build build-ci --target dasall_unit_tests` 与 `ctest --test-dir build-ci --output-on-failure -L unit` | 仅当 inactive slot 约束可验证且切换前生成 rollback token 时完成 |
| OTA-TODO-011 | Done | 实现 BootConfirmationMonitor 骨架 | OTA 设计 6.2/6.8/12.1 | 6.2 BootConfirmationMonitor；12.1 问题 5 | L2 | infra/src/ota/BootConfirmationMonitor.cpp | await_confirm/handle_timeout/evaluate_self_check | unit：confirm 成功与超时失败；failure：未确认默认失败 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci --target dasall_unit_tests && ctest --test-dir build-ci --output-on-failure -L unit | OTA-TODO-005、010 | 无（2026-04-07 已由 OTA-TODO-020 解阻） | 无 | 骨架实现与测试；2026-04-07 已落盘 infra/src/ota/BootConfirmationMonitor.h、infra/src/ota/BootConfirmationMonitor.cpp、tests/unit/infra/ota/BootConfirmationMonitorTest.cpp、docs/todos/infrastructure/deliverables/OTA-TODO-011-BootConfirmationMonitor骨架收敛.md，并同步扩展 infra/include/InfraErrorCode.h、infra/src/InfraErrorCode.cpp 以及 InfraErrorCode unit/contract 回归；已通过 `cmake --build build-ci --target dasall_boot_confirmation_monitor_unit_test dasall_infra_error_code_unit_test dasall_contract_infra_error_code_boundary_test`、`ctest --test-dir build-ci --output-on-failure -R "BootConfirmationMonitorTest|InfraErrorCodeUnitTest|InfraErrorCodeMappingContractTest"`、`cmake --build build-ci --target dasall_unit_tests`，聚合门结果 170/170 passed | 仅当 success/pending/fail/timeout 路径均可二值判定时完成 |
| OTA-TODO-012 | Done | 实现 RollbackController 骨架 | OTA 设计 6.2/6.9 | 6.2 RollbackController；6.9 回滚失败处理 | L3 | infra/src/ota/RollbackController.cpp | rollback(token)/restore_boot_target/recover_repo_pointer | unit：rollback 成功/失败双路径；failure：rollback_fail 独立可观测 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci --target dasall_unit_tests && ctest --test-dir build-ci --output-on-failure -L unit | OTA-TODO-009、010 | 无（2026-04-07 已由 OTA-TODO-018 解阻） | 无 | 骨架实现、unit 测试、deliverable；2026-04-07 已落盘 infra/src/ota/RollbackController.h、infra/src/ota/RollbackController.cpp、tests/unit/infra/ota/RollbackControllerTest.cpp、docs/todos/infrastructure/deliverables/OTA-TODO-012-RollbackController骨架收敛.md，并更新 infra/tests CMake 接线；已通过 `cmake -S . -B build-ci`、`cmake --build build-ci --target dasall_rollback_controller_unit_test`、`ctest --test-dir build-ci -N -R "RollbackControllerTest"`、`ctest --test-dir build-ci --output-on-failure -R "RollbackControllerTest"`、`cmake --build build-ci --target dasall_unit_tests` 与 `ctest --test-dir build-ci --output-on-failure -L unit` | 仅当 rollback 结果与证据可二值判定且失败路径可观测时完成 |
| OTA-TODO-013 | Done | 实现 OTAAuditBridge 骨架 | OTA 设计 6.2/6.11 | 6.2 OTAAuditBridge；6.11 审计字段 | L3 | infra/src/ota/OTAAuditBridge.cpp | write_precheck_audit/write_apply_audit/write_rollback_audit | unit：高风险动作强制审计；failure：审计写失败可见 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci --target dasall_unit_tests && ctest --test-dir build-ci --output-on-failure -L unit | OTA-TODO-001、012 | 无 | 无 | 骨架实现、unit 测试、deliverable；2026-04-07 已落盘 infra/src/ota/OTAAuditBridge.h、infra/src/ota/OTAAuditBridge.cpp、tests/unit/infra/ota/OTAAuditBridgeTest.cpp、docs/todos/infrastructure/deliverables/OTA-TODO-013-OTAAuditBridge骨架收敛.md，并更新 infra/tests CMake 接线；已通过 `cmake -S . -B build-ci`、`cmake --build build-ci --target dasall_ota_audit_bridge_unit_test`、`ctest --test-dir build-ci -N -R "OTAAuditBridgeTest"`、`ctest --test-dir build-ci --output-on-failure -R "OTAAuditBridgeTest"`、`cmake --build build-ci --target dasall_unit_tests` 与 `ctest --test-dir build-ci --output-on-failure -L unit` | 仅当 ota.apply/ota.rollback 路径均产出审计且字段完整时完成 |
| OTA-TODO-014 | Done | 实现 OTAHealthProbe 骨架 | OTA 设计 6.2/6.11 | 6.2 OTAHealthProbe；6.11 健康信号 | L3 | infra/src/ota/OTAHealthProbe.cpp | probe(backlog,last_failure,pending_confirm) | unit：degraded 条件可判定；unit：pending_confirm 计数准确 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci --target dasall_unit_tests && ctest --test-dir build-ci --output-on-failure -L unit | OTA-TODO-001、011、012 | 无 | 无 | 骨架实现、unit 测试；2026-04-07 已落盘 infra/src/ota/OTAHealthProbe.h、infra/src/ota/OTAHealthProbe.cpp、tests/unit/infra/ota/OTAHealthProbeTest.cpp、docs/todos/infrastructure/deliverables/OTA-TODO-014-OTAHealthProbe骨架收敛.md，并更新 infra/tests CMake 接线；已通过 `cmake --build build-ci --target dasall_ota_health_probe_unit_test`、`ctest --test-dir build-ci -N -R "OTAHealthProbeTest"`、`ctest --test-dir build-ci --output-on-failure -R "OTAHealthProbeTest"`、`cmake --build build-ci --target dasall_unit_tests`，聚合门结果 171/171 passed | 仅当关键健康信号可查询且与失败路径一致时完成 |
| OTA-TODO-015 | Done | 接线 ota 到 infra CMake 构建入口 | OTA 设计 7/8.1；代码现状 | 8.1 文件落盘建议 | L2 | infra/CMakeLists.txt、infra/src/ota/ | ota 源文件纳入 dasall_infra | build：dasall_infra 可编译；test：后续 unit 目标可链接 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci --target dasall_infra | OTA-TODO-001~014（可分批） | 无 | 无 | CMake 改动、构建记录；2026-04-07 已在 infra/CMakeLists.txt 收敛 `DASALL_INFRA_OTA_PUBLIC_HEADERS`、`DASALL_INFRA_OTA_SOURCES` 与 `DASALL_INFRA_OTA_PRIVATE_HEADERS` 的统一入口，确保 OTA public/private 头文件与实现源统一挂接到 `dasall_infra` 的 `target_sources` / `PUBLIC_HEADER`；已落盘 docs/todos/infrastructure/deliverables/OTA-TODO-015-OTA构建入口接线收敛.md，并通过 `cmake -S . -B build-ci -G "Unix Makefiles"` 与 `cmake --build build-ci --target dasall_infra` | 仅当 placeholder 不再是唯一源码入口且 ota 文件入图时完成 |
| OTA-TODO-016 | Done | 注册 ota 的 unit 与 contract 测试入口 | OTA 设计 9.1/9.2；规范 3.7 | 9.1 测试矩阵；9.2 gate | L2 | tests/unit/CMakeLists.txt、tests/unit/infra/ota/、tests/contract/CMakeLists.txt | unit：precheck/verify/install/switch/rollback；contract：边界与错误映射 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci --target dasall_unit_tests dasall_contract_tests && ctest --test-dir build-ci -N && ctest --test-dir build-ci --output-on-failure -L unit && ctest --test-dir build-ci --output-on-failure -L contract | OTA-TODO-015 | 无 | 无 | 测试注册改动、执行记录；2026-04-07 已在 tests/unit/CMakeLists.txt 收敛 `DASALL_OTA_UNIT_TEST_EXECUTABLE_TARGETS`，并在 tests/unit/infra/CMakeLists.txt / tests/contract/CMakeLists.txt 新增 OTA 专属 helper 统一 unit/contract 注册与 `ota` 标签；已落盘 docs/todos/infrastructure/deliverables/OTA-TODO-016-OTA测试入口注册收敛.md，并通过 `cmake -S . -B build-ci -G "Unix Makefiles"`、`cmake --build build-ci --target dasall_unit_tests dasall_contract_tests`、`ctest --test-dir build-ci -N -L ota` 与 `ctest --test-dir build-ci --output-on-failure -L ota` | 仅当新增 ota 测试可被 ctest -N 发现并在 unit/contract 标签下执行时完成 |
| OTA-TODO-017 | Done | 注册 ota integration/failure 测试入口 | OTA 设计 9.1/9.2；代码现状 | 9.1 Integration/Failure 覆盖 | L0 | tests/CMakeLists.txt、tests/integration/infra/ota/、tests/stress/ | integration：apply->confirm->success；failure：verify_fail/confirm_timeout/rollback_fail | cmake -S . -B build-ci -G Ninja && cmake --build build-ci && ctest --test-dir build-ci -N | OTA-TODO-015、016 | 无（2026-03-30 已由 INF-BLK-06 integration 顶层拓扑校准解阻） | 无；tests/stress 当前未形成独立拓扑，failure injection 继续按仓库现行约定落在 tests/integration/infra/ota/ | 测试注册改动、执行记录；2026-04-07 已新增 tests/integration/infra/ota/CMakeLists.txt、OTAWorkflowTest.cpp、OTAFailureInjectionTest.cpp，并在 tests/integration/CMakeLists.txt / tests/integration/infra/CMakeLists.txt 接入 `dasall_ota_workflow_integration_test` 与 `dasall_ota_failure_injection_integration_test`；已落盘 docs/todos/infrastructure/deliverables/OTA-TODO-017-OTA集成与故障注入测试入口收敛.md，并通过 `cmake -S . -B build-ci -G "Unix Makefiles"`、`cmake --build build-ci --target dasall_integration_tests`、`ctest --test-dir build-ci -N -L ota` 与 `ctest --test-dir build-ci --output-on-failure -L ota` | 仅当 tests 顶层接线完成并可发现 ota integration 后可解阻 |
...
|---|---|---|---|---|
| OTA-BLK-01 | 已解阻（2026-04-07）：rollback token 单 active token 文件路径、TTL 下界与重启恢复矩阵已冻结 | OTA-TODO-012 | 无；后续按冻结边界实现 RollbackController 与 token state store | 证据回链到 OTA-TODO-018 与 docs/architecture/DASALL_infra_OTA模块详细设计.md |
