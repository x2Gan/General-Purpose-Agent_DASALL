...
| LOG-TODO-012 | Not Started | 实现 LoggingConfigAdapter 四层配置适配 | logging 设计 6.6、6.9；架构 8.6 | 6.9 配置项与覆盖层级 | L2 | infra/src/logging/LoggingConfigAdapter.cpp | ILogConfigurator::apply(config) | unit：层级优先级验证；contract：Profile 不绕过审计主链 | cmake -S . -B build-ci -G Ninja && cmake --build build-ci --target dasall_infra | LOG-TODO-001 | 无（2026-04-03 已由 LOG-BLK-001 通过 LoggingConfig 对象表、键名与 per-key 层级接受规则冻结解阻） | 无；已进入可执行状态，后续只需保持 design/header/test 键域一致 | 配置适配实现或阻塞记录 | 仅当按冻结键域读取四层 active config、非法 runtime/profile 输入被拒绝且不绕过 audit 主链时完成 |
