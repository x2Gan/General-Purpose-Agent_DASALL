...
| integration/failure/profile 测试注册 | 9.1/9.2；现状 | 测试与门禁 | WDG-TODO-022 | 顶层 integration 已解阻，022 已落 watchdog 具体 integration/failure/profile 用例与标签 |
...
| WDG-TODO-014 | Done | 实现 TimeoutEventPublisher 发布骨架 | watchdog 设计 6.2/6.8/6.10；11.1 | 6.2 TimeoutEventPublisher | L2 | infra/src/watchdog/TimeoutEventPublisher.cpp | publish_timeout(decision) | unit：仅超时触发发布；failure：发布失败计数 | cmake -S . -B build-ci -G "Unix Makefiles" && cmake --build build-ci --target dasall_timeout_event_publisher_unit_test && ctest --test-dir build-ci --output-on-failure -R TimeoutEventPublisherTest | WDG-TODO-006、013、015 | 无（2026-04-08 已由 WDG-BLK-02 解阻） | 无 | infra/src/watchdog/TimeoutEventPublisher.h；infra/src/watchdog/TimeoutEventPublisher.cpp；tests/unit/infra/watchdog/TimeoutEventPublisherTest.cpp；2026-04-08 通过 cmake -S . -B build-ci -G "Unix Makefiles"、cmake --build build-ci --target dasall_timeout_event_publisher_unit_test、ctest --test-dir build-ci -N -R TimeoutEventPublisherTest 与 ctest --test-dir build-ci --output-on-failure -R TimeoutEventPublisherTest | 仅当 critical/fatal timeout 可发布、warning 可跳过且 publish failure 可计数并写入 fallback ring-buffer 时完成 |
...
| WDG-TODO-017 | Done | 实现 RecoveryRequestEmitter 边界守卫骨架 | watchdog 设计 6.2/6.8；ADR-007；11.1 | 6.2 RecoveryRequestEmitter | L2 | infra/src/watchdog/RecoveryRequestEmitter.cpp | emit_recovery_hint(decision)/sanitize_payload() | contract：建议与执行分离；unit：evidence_ref 完整性 | cmake -S . -B build-ci -G "Unix Makefiles" && cmake --build build-ci --target dasall_recovery_request_emitter_unit_test dasall_contract_watchdog_recovery_hint_request_boundary_test && ctest --test-dir build-ci --output-on-failure -R "(RecoveryRequestEmitterTest|WatchdogRecoveryHintRequestBoundaryContractTest)" | WDG-TODO-008、013 | 无 | 无 | infra/src/watchdog/RecoveryRequestEmitter.h；infra/src/watchdog/RecoveryRequestEmitter.cpp；tests/unit/infra/watchdog/RecoveryRequestEmitterTest.cpp；2026-04-08 通过 cmake -S . -B build-ci -G "Unix Makefiles"、cmake --build build-ci --target dasall_recovery_request_emitter_unit_test dasall_contract_watchdog_recovery_hint_request_boundary_test、ctest --test-dir build-ci -N -R "(RecoveryRequestEmitterTest|WatchdogRecoveryHintRequestBoundaryContractTest)" 与 ctest --test-dir build-ci --output-on-failure -R "(RecoveryRequestEmitterTest|WatchdogRecoveryHintRequestBoundaryContractTest)" | 仅当 critical/fatal 级 timeout 只输出 advisory RecoveryHintRequest、warning 被拒绝且 contract 模板保持建议-执行分离时完成 |
| WDG-TODO-018 | Done | 定义 WatchdogConfigPolicy 配置模型与覆盖规则 | watchdog 设计 6.9/11.1；蓝图 5.1 | 6.9 配置项表 | L2 | infra/src/watchdog/WatchdogConfigPolicy.cpp | load_defaults()/merge_layers()/validate_limits() | unit：默认值、覆盖优先级、阈值合法性 | cmake -S . -B build-ci -G "Unix Makefiles" && cmake --build build-ci --target dasall_watchdog_config_policy_unit_test dasall_contract_profile_runtime_policy_schema_test && ctest --test-dir build-ci --output-on-failure -R "(WatchdogConfigPolicyTest|ProfileRuntimePolicySchemaContractTest)" | WDG-TODO-001、013 | 无（2026-04-08 已由 WDG-BLK-04 解阻） | 无 | infra/src/watchdog/WatchdogConfigPolicy.h；infra/src/watchdog/WatchdogConfigPolicy.cpp；tests/unit/infra/watchdog/WatchdogConfigPolicyTest.cpp；infra/src/config/ConfigLoader.cpp；profiles/cloud_full/runtime_policy.yaml；profiles/desktop_full/runtime_policy.yaml；profiles/edge_balanced/runtime_policy.yaml；profiles/edge_minimal/runtime_policy.yaml；profiles/factory_test/runtime_policy.yaml；tests/contract/smoke/ProfileRuntimePolicySchemaContractTest.cpp；2026-04-08 通过 cmake -S . -B build-ci -G "Unix Makefiles"、cmake --build build-ci --target dasall_watchdog_config_policy_unit_test dasall_contract_profile_runtime_policy_schema_test、ctest --test-dir build-ci -N -R "(WatchdogConfigPolicyTest|ProfileRuntimePolicySchemaContractTest)" 与 ctest --test-dir build-ci --output-on-failure -R "(WatchdogConfigPolicyTest|ProfileRuntimePolicySchemaContractTest)" | 仅当 watchdog 默认值、四层覆盖顺序、阈值合法性与 profile 键路径冻结都可重复验证时完成 |
...
| E 集成与兼容门禁 | WDG-TODO-022 | 串行 | 已于 2026-04-08 完成 watchdog integration/failure/profile 用例与标签落盘 |
...
| WDG-BLK-02 | 已解阻（2026-04-08）：TimeoutEventPublisher.h 已冻结进程内 `ITimeoutEventSink::publish_timeout_event(event)` 最小接口与 `INF_E_WATCHDOG_EVENT_PUBLISH_FAIL` outward 失败返回 | WDG-TODO-014 | 无；后续可直接复用 TimeoutEvent / ITimeoutEventSink / TimeoutEventPublishResult 边界 | 证据回链到 infra/src/watchdog/TimeoutEventPublisher.h、infra/src/watchdog/TimeoutEventPublisher.cpp、tests/unit/infra/watchdog/TimeoutEventPublisherTest.cpp |
...
| WDG-BLK-04 | 已解阻（2026-04-08）：ConfigLoader 默认层、五个 runtime_policy profile 与 ProfileRuntimePolicySchemaContractTest 已共同冻结 `infra.watchdog.*` 键名及其存在性 | WDG-TODO-018 | 无；后续可直接复用 WatchdogConfigPolicy 与 profile schema contract | 证据回链到 infra/src/watchdog/WatchdogConfigPolicy.h、infra/src/watchdog/WatchdogConfigPolicy.cpp、infra/src/config/ConfigLoader.cpp、profiles/*/runtime_policy.yaml、tests/unit/infra/watchdog/WatchdogConfigPolicyTest.cpp、tests/contract/smoke/ProfileRuntimePolicySchemaContractTest.cpp |
...
| 执行 watchdog integration 聚合目标 | cmake --build build-ci --target dasall_watchdog_integration_tests |
...
| 检查 watchdog integration/failure/profile 标签发现性 | ctest --test-dir build-ci -N -L watchdog-integration && ctest --test-dir build-ci -N -L watchdog-failure && ctest --test-dir build-ci -N -L watchdog-profile |
| 执行 tests 自定义聚合目标 | cmake --build build-ci --target dasall_unit_tests dasall_contract_tests |

说明：

1. watchdog integration/failure/profile 门禁已于 2026-04-08 通过 `dasall_watchdog_integration_tests` 与 `watchdog-integration` / `watchdog-failure` / `watchdog-profile` 标签落盘；仓库通用基线仍以 unit/contract 为默认全量门禁，组件级 integration 子集按需定向执行。
