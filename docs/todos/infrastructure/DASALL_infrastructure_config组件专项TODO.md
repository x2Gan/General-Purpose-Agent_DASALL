...
| ConfigAuditBridge | config 设计 6.2/6.10 | L2 | 审计字段清单、logger/audit sink 契约已明确 | metrics/tracing 仍为后续扩展 | 可进入 bridge/integration 落盘 |
| SecretRefResolver | config 设计 6.2/6.4/6.8 | L1 | `secret://` 解析职责明确，且 CLCFG-TODO-004 已冻结 bootstrap-only internal seam、`auth_ref` 命名与 consumer 读路径 | resolver 类与 focused tests 尚未落盘 | 可直接按已冻结 projection 契约补实现，不再需要先补 secret 接口模型 |
...
| 可观测与审计桥接 | config 设计 6.10 | 桥接/门禁 | CFG-BLK-002 | v1 sink 契约已冻结，可继续 bridge/integration 落盘 |
| secret 引用解析 | config 设计 6.2/6.4/6.8 | 适配器/门禁 | CFG-BLK-003（已解阻） | bootstrap/projection 契约已冻结，后续只需补 `SecretRefResolver` 实现与测试 |
...
| CFG-BLK-002 | 已解阻（2026-04-02）：ConfigAuditBridge v1 已冻结为 `logging::ILogger + audit::IAuditLogger` 必选 sink，日志 attrs 与审计字段最小集合已入设计 | CFG-TODO-016 | 无 | 已在 config 设计补充 6.10.1 sink 契约章节，并将 016 的 observability 前置条件恢复为可执行 | metrics/tracing 若暂未接入，保留为后续 bridge 扩展，不阻塞 016 |
| CFG-BLK-003 | 已解阻（2026-05-08）：CLCFG-TODO-004 已冻结 bootstrap-only internal seam、`auth_ref=secret://llm/providers/<provider_ref>` 命名、install-mode root `/var/lib/dasall/secrets`，且 consumer 读路径继续复用 `ISecretManager` / `SecretManagerFacade` / `FileSecretBackend` | 后续 SecretRefResolver 任务 | 无；后续只需按已冻结 projection contract 落实现与测试 | 证据回链到 docs/architecture/DASALL_cli_config交互式部署配置设计.md 6.2.4/8.4/11.2、docs/todos/infrastructure/DASALL_infrastructure_secret组件专项TODO.md 与 docs/todos/cli/deliverables/CLCFG-TODO-004-secret-bootstrap-seam冻结.md | 若后续把 bootstrap 写入并入 `ISecretManager` 公共 ABI、改写 `secret://` naming rule，或重新发明第二个 install-mode root，则重新转为 Blocked |
...
1. Blocker 变化：CFG-BLK-001、CFG-BLK-002、CFG-BLK-003、CFG-BLK-004 均已解阻并有独立执行记录；当前只剩 CFG-BLK-005（持久化后端）仍为后续设计/实现缺口，但不阻塞 001~017 当前 gate。
...
3. 当前剩余缺口：`secret://` projection contract 已由 CLCFG-TODO-004 冻结，下一步可直接补 `SecretRefResolver` 实现与测试；CFG-BLK-005 对应的持久化后端策略仍待设计；metrics/tracing 仍是 ConfigAuditBridge v1 之后的扩展项。
4. 当前最小稳定交付粒度：L2/L3 骨架 + config integration 拓扑 + gate 证据回写。
5. 下一步建议：
   - 若继续推进 config v2，优先解 CFG-BLK-005，并按已冻结 projection contract 直接补 `SecretRefResolver` 与持久化后端实现。
...
## 28. 本轮执行记录（2026-04-02 / CFG-BLK-002）

### 28.1 选中任务

1. 本轮任务：CFG-BLK-002。
2. 可执行性依据：CFG-TODO-016 的剩余缺口已收敛到 ConfigObservabilityIntegrationTest 缺少稳定 sink 契约，而 infra 侧 logger/audit/metrics/tracing 接口与 audit/log 事件对象均已落盘，可通过单轮设计冻结完成最小解阻。

### 28.2 研究与 Design 结论

本地证据：

1. `audit::IAuditLogger`、`logging::ILogger`、`metrics::IMetricsProvider`、`tracing::ITracerProvider` 与 `AuditEvent`/`AuditContext`/`LogEvent` 已在 infra 侧冻结，CFG-BLK-002 中“audit 写入接口未冻结”的原始阻塞描述已不再成立。
2. config 详细设计 6.10 已列出日志、指标、追踪、审计四类观测目标，但未明确 v1 哪些 sink 为必选、哪些属于后续扩展，因此 ConfigAuditBridge 的最小实现范围仍不稳定。
3. infra 子系统详设 6.10 已给出 profiles v1 可观测 sink 合同：仅 `ILogger` 与 `audit::IAuditLogger` 为跨子域稳定输出口，metrics/tracing 继续留在 infra 内部演进。

外部参考：

1. OpenTelemetry Logs Data Model 强调稳定 top-level 字段、显式 trace context 字段与 attributes 承载扩展上下文；本轮据此把 config 桥接收敛为“固定动作名 + 必选 log/audit sink + attrs 承载差异细节”的 v1 契约。

D 结论：

1. Design -> Build 映射：在 config 设计文档补充 6.10.1，冻结 ConfigAuditBridge v1 必选 sink 为 `logging::ILogger` 与 `audit::IAuditLogger`，并明确 metrics/tracing 为后续扩展点；同时将设计映射表与 blocker 台账同步更新。
2. Build 三件套：
   - 代码目标：无代码实现；本轮仅更新设计文档与 TODO blocker 台账。
   - 测试目标：文档证据校验，确认 6.10.1 sink 契约、CFG-BLK-002 解阻记录与 CFG-TODO-016 前置条件变化均已落盘。
   - 验收命令：`grep -nE "ConfigAuditBridge v1 可观测 sink 冻结|ILogger|IAuditLogger|CFG-BLK-002|CFG-TODO-016" docs/architecture/DASALL_infra_config模块详细设计方案.md docs/todos/infrastructure/DASALL_infrastructure_config组件专项TODO.md`。
3. D Gate：PASS；CFG-TODO-016 已达到进入实现轮条件。

### 28.3 Build 交付与证据

交付物：

1. docs/architecture/DASALL_infra_config模块详细设计方案.md：新增 6.10.1 `ConfigAuditBridge v1 可观测 sink 冻结` 章节，明确 logger/audit 必选 sink、log attrs 与 audit 字段最小集合、失败语义与 v1 验收边界。
2. docs/todos/infrastructure/DASALL_infrastructure_config组件专项TODO.md：将 CFG-BLK-002 标记为已解阻，并更新 CFG-TODO-016 的前置条件说明。

验收结果：

1. `grep -nE "ConfigAuditBridge v1 可观测 sink 冻结|ILogger|IAuditLogger|CFG-BLK-002|CFG-TODO-016" docs/architecture/DASALL_infra_config模块详细设计方案.md docs/todos/infrastructure/DASALL_infrastructure_config组件专项TODO.md`：预期应命中 6.10.1 冻结章节、blocker 解阻记录和 016 前置条件回写。

Build 合规复核：

1. 代码注释：本轮为 blocker 设计冻结，无代码改动。
2. 正负例覆盖：本轮为文档解阻，不适用代码正负例；通过显式冻结“日志/审计必选、metrics/tracing 后置、sink 失败可判定”的三条语义替代原有模糊口径。
3. 测试发现性：本轮不涉及测试注册。
4. TODO 证据回写：已回写 blocker 解阻证据和 016 前置条件变化。
5. 提交隔离：本轮提交范围限定为 config 设计文档与 TODO 阻塞台账，不混入 016 的代码与 integration 注册改动。

## 29. 本轮执行记录（2026-04-02 / CFG-TODO-016）

### 29.1 选中任务

1. 本轮任务：CFG-TODO-016。
2. 可执行性依据：CFG-TODO-015 已完成，CFG-BLK-002 已在上一轮解阻；当前只需把 config integration 用例收束到独立子目录并补齐 `ConfigObservabilityIntegrationTest`，即可满足 discoverable + executable 的完成判定。

### 29.2 研究与 Design 结论

本地证据：

1. docs/architecture/DASALL_infra_config模块详细设计方案.md 8.1 已建议 config integration 用例落盘到 tests/integration/infra/config。
2. tests/integration/CMakeLists.txt 与 tests/integration/infra/CMakeLists.txt 已具备顶层 integration 聚合能力，但在本轮前只有 `ConfigRuntimePatchIntegrationTest`，尚缺 `ConfigObservabilityIntegrationTest`。
3. CFG-BLK-002 已冻结 ConfigAuditBridge v1 必选 sink 为 `ILogger + IAuditLogger`，因此 `ConfigObservabilityIntegrationTest` 现在可以用 logger/audit 双 sink 作为稳定集成边界，而不等待 metrics/tracing bridge 完整落地。

D 结论：

1. Design -> Build 映射：新增 tests/integration/infra/config/CMakeLists.txt，将 config integration 从 infra 根 CMake 收束到子目录；新增 `ConfigObservabilityIntegrationTest.cpp`，并将顶层 integration 目标列表补齐到 `dasall_config_observability_integration_test`。
2. Build 三件套：
   - 代码目标：新增 tests/integration/infra/config/CMakeLists.txt、tests/integration/infra/config/ConfigObservabilityIntegrationTest.cpp；调整 tests/integration/infra/CMakeLists.txt 与 tests/integration/CMakeLists.txt。
   - 测试目标：保留 `ConfigRuntimePatchIntegrationTest`，新增 `ConfigObservabilityIntegrationTest` 覆盖 logger+audit 成功路径、invalid payload 无 side effect、audit sink 失败的部分成功可见性。
   - 验收命令：`cmake -S . -B build-ci -G "Unix Makefiles"`、`cmake --build build-ci --target dasall_config_runtime_patch_integration_test dasall_config_observability_integration_test`、`ctest --test-dir build-ci -N -R "ConfigRuntimePatchIntegrationTest|ConfigObservabilityIntegrationTest"`、`ctest --test-dir build-ci --output-on-failure -R "ConfigRuntimePatchIntegrationTest|ConfigObservabilityIntegrationTest"`。
3. D Gate：PASS。

边界说明：

1. 本轮不引入生产版 ConfigAuditBridge.cpp，只完成 integration 拓扑与 observability 合同的测试落盘。
2. metrics/tracing 仍作为后续 bridge 扩展，不进入本轮 integration 验收面。

### 29.3 Build 交付与证据

交付物：

1. tests/integration/infra/config/CMakeLists.txt：新增 config integration 子目录注册，统一承载 runtime patch 与 observability 两个 config integration 目标。
2. tests/integration/infra/config/ConfigObservabilityIntegrationTest.cpp：新增 logger+audit sink 成功、invalid payload 拒绝、audit sink 失败部分成功可见性的 integration 用例。
3. tests/integration/infra/CMakeLists.txt、tests/integration/CMakeLists.txt：完成 config integration 子目录与顶层 executable target 列表注册。
4. docs/todos/infrastructure/DASALL_infrastructure_config组件专项TODO.md：将 CFG-TODO-016 状态更新为 Done，并回写本轮 integration 发现性与验收记录。

验收结果：

1. `cmake -S . -B build-ci -G "Unix Makefiles"`：通过；刷新新增 config integration 子目录后的构建图。
2. `cmake --build build-ci --target dasall_config_runtime_patch_integration_test dasall_config_observability_integration_test`：通过；两个 config integration 可执行目标均编译成功。
3. `ctest --test-dir build-ci -N -R "ConfigRuntimePatchIntegrationTest|ConfigObservabilityIntegrationTest"`：通过，发现 2 个测试：ConfigRuntimePatchIntegrationTest、ConfigObservabilityIntegrationTest。
4. `ctest --test-dir build-ci --output-on-failure -R "ConfigRuntimePatchIntegrationTest|ConfigObservabilityIntegrationTest"`：通过，2/2 tests passed。

Build 合规复核：

1. 代码注释：observability integration 的本地 logger/audit harness 通过类型名和断言名即可自解释，无需额外注释。
2. 正负例覆盖：runtime patch integration 继续覆盖主链正例；observability integration 覆盖成功路径、invalid payload 负例、audit sink 失败部分成功负例。
...
2. 阻塞变化：CFG-BLK-001、CFG-BLK-002、CFG-BLK-003、CFG-BLK-004 已解除；CFG-BLK-005 仍保留为后续设计/实现缺口，但已从当前 gate 阻塞集中移除。
