...
4. timeout / cancellation 与 production observability / health hot path 已有 focused evidence；scheduler model、knowledge degraded semantics 与更高层 runtime full-path evidence 仍待继续收口。
...
Runtime 当前缺的不是控制面骨架，也不再是 path evidence 分层、durable checkpoint / resume gate 或 deadline / cancellation 主链，而是可观测健康、production degraded semantics、执行模型与更高层 full-path evidence 的剩余闭合。后续优先顺序为 `RT-GAP-005`，再推进 `RT-GAP-006` / `RT-GAP-007` / `RT-GAP-008`。
