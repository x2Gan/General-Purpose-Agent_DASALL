...
15. 上述结论只解除了 TUI 的设计 blocker、composer state-machine blocker、selector fake-interaction blocker、transcript-view baseline blocker、status-panel baseline blocker、terminal-probe baseline blocker 与 renderer-layout baseline blocker，不代表 bare `dasall` 命令迁移已可放行；TUI-TODO-020~030 及 031~034 仍需按后续 gate 单独验收。
...
4. `TUI-TODO-020` 仍受 CJK/IME/resize manual gate 与 installed path packaging review 约束；`TUI-TODO-019` 仅闭合 renderer baseline，不等于 full-screen interactive loop 或 release path 已 ready。
