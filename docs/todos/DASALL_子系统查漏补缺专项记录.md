...
4. SQLite / observability / concurrency：版本 gate 已收口；production sink 和 reader pool 并发防护仍待补。
5. 查漏补缺优先级：先把 `MEM-GAP-001` 从 focused seam 推进到 production/package 证据，再收口 `MEM-GAP-003` / `MEM-GAP-005`，最后通过 `MEM-GAP-004` / `MEM-GAP-006` / `MEM-GAP-007` 提升 release 与边界可信度。
