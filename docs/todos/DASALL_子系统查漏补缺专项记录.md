...
15. 上述结论只解除了 TUI 的设计 blocker、composer state-machine blocker、selector fake-interaction blocker、transcript-view baseline blocker、status-panel baseline blocker、terminal-probe baseline blocker、renderer-layout baseline blocker、fake-only app-loop blocker 与 IPC mapping/header blocker，不代表 bare `dasall` 命令迁移已可放行；TUI-TODO-022~030 及 031~034 仍需按后续 gate 单独验收。
...
2. `TUI-TODO-022~026` 尚未把已冻结的 daemon/access projection seam 继续落盘为 controller/data source，并补 session open/close/query 与 IPC 错误归一化；其中 `TUI-TODO-021` 已闭合 mapping/header 草案，`/clear` 的用户可见语义也已闭合，但 backend close/open 细节仍后置。
