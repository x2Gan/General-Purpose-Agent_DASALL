...
13. 上述结论只解除了 TUI 的设计 blocker、composer state-machine blocker 与 selector fake-interaction blocker，不代表 bare `dasall` 命令迁移已可放行；TUI-TODO-016~030 及 031~034 仍需按后续 gate 单独验收。
...
4. `TUI-TODO-016~018` 仍待沿着已完成的 fake replay、slash parser、composer 状态机与 selector fake 交互基线，继续把 transcript、status panel 与 terminal capability probe 落到 focused tests。
