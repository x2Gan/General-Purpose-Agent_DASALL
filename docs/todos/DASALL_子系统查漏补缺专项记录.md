...
| TUI client | 已记录 | 当前达到 L2 focused 集成可信：TUI-TODO-001~036 已闭合基线 owner gap，评审后 `TUI-TODO-037` 已冻结 true daemon-backed integration/协议收敛口径，`TUI-TODO-038` 已新增 daemon/access `tui_ipc.v1` server handler、bootstrap 分流与 focused unit evidence | formal submit UX、temp socket/headless seam、true daemon-backed E2E、installed smoke、binary purity 与 closeout evidence 仍待 039~044 闭合 |
...
17. 但 TUI 当前仍不能宣称 true daemon-backed ready 或 installed release-ready：`BLK-TUI-010` 继续保持 Open，`TUI-TODO-039~044` 仍需补 formal socket override、composer submit UX、真实 daemon-backed E2E、installed smoke、formal/prototype purity gate 与 closeout evidence。
...
1. `TUI-TODO-037`、`TUI-TODO-038` 已完成：协议收敛方向已冻结，daemon/access `tui_ipc.v1` server handler 与 focused server tests 已落盘，`BLK-TUI-009` 已关闭。
2. `BLK-TUI-010` 继续保持 Open；在 `TUI-TODO-039` 提供 formal socket override、`TUI-TODO-040` 接通 composer submit UX、`TUI-TODO-041` 跑通真实 daemon-backed E2E 之前，不得把 038 的 focused server evidence 外推为 true daemon-backed ready。
...
4. `TUI-PROTO-017`、`TUI-TODO-036`、`TUI-TODO-030/031/032/033/034/035/036`、`BLK-TUI-008-command-release-gate-recheck.md`、`TUI-PROTO-017-formal-sample-signoff.md` 与 `BLK-TUI-006-manual-terminal-evidence.md` 形成的 command release owner chain 仍然有效，但这条链不替代 039~044 的 true integration / release follow-up。
