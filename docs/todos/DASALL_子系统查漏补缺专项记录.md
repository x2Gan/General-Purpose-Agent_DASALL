...
| Tools -> Infra/plugin | 基本打通 | L2 | 已新增 `ToolPluginLifecycleBridge`，消费 `IPluginManager::list_active()`、`PluginLoadResult.handle_ref` 与 `PluginUnloadResult`，自动驱动 `PluginExtensionBridge` snapshot；`ToolPluginLifecycleBridgeIntegrationTest` 通过 | plugin snapshot 仍未自动分发到 `ToolRegistry` / `CapabilityDiscovery` / `SkillRegistry`，后续由 `TOOL-FIX-004` 收口 |
...
3. Plugin / MCP / Skill 扩展：plugin lifecycle 自动接线已补齐，但 snapshot 二次分发、generic MCP rollout、Runtime production skill 消费仍待补。
...
查漏补缺优先级：`TOOL-GAP-001` / `TOOL-GAP-002` / `TOOL-GAP-003` 已闭合；后续先修 `TOOL-GAP-004`，再收口 `TOOL-GAP-005` / `TOOL-GAP-006` / `TOOL-GAP-007` / `TOOL-GAP-008` / `TOOL-GAP-009`，最后通过 `TOOL-GAP-010` / `TOOL-GAP-011` / `TOOL-GAP-012` 提升 runtime production、installed 和 release 可信度。
