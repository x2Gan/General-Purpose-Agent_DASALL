...
| KNO-TODO-043 | Todo | production embedding provider 替代 env-gated local fallback | 在 runtime_support / memory owner 边界内新增 production query encoder provider 接线，明确 provider asset / secret / health contract；`SimpleLocalEmbeddingAdapter` 与 `DASALL_DETACHED_VECTOR_LOCAL_FALLBACK=1` 仅保留为 local/test/package fallback，不作为生产 semantic quality 通过依据 | 新增 provider-ready、provider-missing、provider-empty-embedding、invalid-secret fail-closed 集成测试；扩展 `RuntimeKnowledgeQueryEncoderIntegrationTest` 与 production telemetry gate，证明生产 provider ready 时 Hybrid/Dense 正例不依赖 local fallback env | `cmake --build build/vscode-linux-ninja --target dasall_runtime_knowledge_query_encoder_integration_test dasall_knowledge_production_telemetry_integration_test -j2`；禁用 `DASALL_DETACHED_VECTOR_LOCAL_FALLBACK` 后运行 provider-ready 正例；缺 provider / invalid secret 路径必须 lexical-only 且有明确 reason code | 不再把 local hash embedding 当作 production-grade embedding；生产 provider readiness、错误分类、审计字段与 fallback 行为均可验证 |
