...
19. 但 TUI 当前仍不能宣称 true daemon-backed ready 或 installed release-ready：`TUI-TODO-041~044` 仍需补真实 daemon-backed E2E、installed smoke、formal/prototype purity gate 与 closeout evidence。
...
1. `TUI-TODO-037`、`TUI-TODO-038`、`TUI-TODO-039`、`TUI-TODO-040` 已完成：协议收敛方向已冻结，daemon/access `tui_ipc.v1` server handler、formal socket override 与 formal submit UX 均已落盘，`BLK-TUI-009` 与 `BLK-TUI-010` 已关闭。
2. `TUI-TODO-041` 现已成为下一原子任务；在它跑通真实 daemon-backed E2E 之前，不得把 038 的 focused server evidence、039 的 socket override seam 或 040 的 scripted/headless submit integration 外推为 true daemon-backed ready。
...
4. `TUI-PROTO-017`、`TUI-TODO-036`、`TUI-TODO-030/031/032/033/034/035/036`、`BLK-TUI-008-command-release-gate-recheck.md`、`TUI-PROTO-017-formal-sample-signoff.md` 与 `BLK-TUI-006-manual-terminal-evidence.md` 形成的 command release owner chain 仍然有效，但这条链不替代 041~044 的 true integration / release follow-up。
