...
| knowledge | 已记录 | 主体实现 L2 基本闭合；Runtime -> Memory evidence 读链路已打通；refresh 异步与首启 prewarm 已收口，installed asset probe 首启回归已转绿 | concrete vector backend、lane timeout/parallel recall、首批 corpus baseline、持久化 ledger/catalog/migration、production telemetry sink、installed/qemu/soak 证据 |
...
6. installed asset factory 已接入 apps live composition，但其 build-tree / installed 证据仍需单独按 `KNO-GAP-009` 与更高层 package gate 收口；且 factory 实际只扫描 profiles 与 LLM provider YAML，不覆盖详设首批 architecture / ADR / SSOT / profile policy normative corpus baseline。
...
| Installed asset factory | 部分覆盖 | apps runtime support 调用 `create_installed_asset_knowledge_service` 并传 `service_instance_id`；factory 创建 lexical-only service，并在缺失 active snapshot 时执行同步 init prewarm | build-tree / installed 证据仍需由 `KNO-GAP-009` 与 package gate 单独收口；factory corpus 只含 profiles / LLM providers |
...
| apps runtime_support -> Knowledge factory | 部分打通 | L0 / L2 partial | production composition 传入 `service_instance_id` 并可注入 dependency set；但 build-tree / installed 证据仍需由 `KNO-GAP-009` 与 package gate 单独收口，不能外推 installed OK |
...
Knowledge 子系统的实现覆盖度已经很高，当前主要问题不是“缺少 Knowledge 模块本体”，而是“runtime write-side refresh trigger、vector/hybrid concrete backend、lane timeout/parallel recall、installed corpus baseline、ledger/catalog 持久化、production telemetry 与 installed/release 证据”尚未全部闭合。
...
2. 本轮复验：`KnowledgeRefreshAsyncLifecycleTest`、`KnowledgeRefreshLoopTest`、`KnowledgeServiceFacadeRealRefreshTest`、`KnowledgeInitPrewarmTest`、`KnowledgeInstalledAssetProbeIntegrationTest` 与 `KnowledgeRetrievalSmokeTest` 已建立 refresh + startup prewarm 聚焦证据；installed asset gate 仍由 `KNO-GAP-009` 与更高层 package evidence 单独跟踪，不由本轮结果外推为 installed package 绿色。
...
5. 查漏补缺优先级：先修 `KNO-GAP-009`，再收口 `KNO-GAP-003` / `KNO-GAP-004` / `KNO-GAP-005` / `KNO-GAP-007`，最后通过 `KNO-GAP-006` / `KNO-GAP-008` / `KNO-GAP-010` / `KNO-GAP-011` 提升 release 与边界可信度。
