...
| installed / production evidence | 已由 LLM-FIX-004 收口到 L5 / L6 partial | `LLMProductionFactory`、runtime direct path、PKG-TODO-018 qemu authoritative `pkg-smoke-local-control-plane PASS`、BC-07 `llm.origin` 断言、`LLM-FIX-004` 交付文档与 release-runner workflow/script contract | 当前已具历史 L5 qemu / release evidence 与仓库级 rerun contract；L6 soak / chaos 与当前 release candidate rerun 仍未证明 |
