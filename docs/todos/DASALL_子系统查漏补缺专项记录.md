...
5. `request_refresh()` 已收口为单槽位异步 refresh worker：调用面返回 `Accepted` / `Busy`，终态成功与失败由 `Completed` / `Failed` 和 `health_snapshot()` 中的 `refresh_in_flight` / `last_refresh_status` 暴露；`init()` 仍不触发首启 active snapshot build。
...
| Refresh lifecycle | 基本覆盖 | `KnowledgeRefreshAsyncLifecycleTest`、`KnowledgeRefreshLoopTest`、`KnowledgeServiceFacadeRealRefreshTest` 通过；`KnowledgeServiceFacade` 已具备单槽位 async refresh worker、`refresh_in_flight` 与 `last_refresh_status` 观测面，并保留 synchronous test helper | `init()` 不自动首建 active snapshot；Runtime 写侧触发链路未证明 |
...
| Installed asset factory | 部分覆盖 | apps runtime support 调用 `create_installed_asset_knowledge_service` 并传 `service_instance_id`；factory 创建 lexical-only service | build-tree / installed 证据仍需由 `KNO-GAP-009` 与 package gate 单独收口；factory corpus 只含 profiles / LLM providers；无自动 refresh |
...
2. 本轮复验：`KnowledgeRefreshAsyncLifecycleTest`、`KnowledgeRefreshLoopTest`、`KnowledgeServiceFacadeRealRefreshTest` 三条 refresh-focused 聚焦测试通过；installed asset gate 仍由 `KNO-GAP-009` 与更高层 package evidence 单独跟踪，不由本轮结果外推为绿色。
...
5. 查漏补缺优先级：先修 `KNO-GAP-002` / `KNO-GAP-009`，再收口 `KNO-GAP-003` / `KNO-GAP-004` / `KNO-GAP-005` / `KNO-GAP-007`，最后通过 `KNO-GAP-006` / `KNO-GAP-008` / `KNO-GAP-010` / `KNO-GAP-011` 提升 release 与边界可信度。
