...
3. Plugin / MCP / Skill 扩展：plugin lifecycle 自动接线、snapshot 二次分发、MCP v1 stdio-only transport 边界与 production observability sink 已补齐；剩余重点转为 builtin surface 扩展、Runtime production skill 消费与 installed evidence。
...
查漏补缺优先级：`TOOL-GAP-001` / `TOOL-GAP-002` / `TOOL-GAP-003` / `TOOL-GAP-004` / `TOOL-GAP-005` / `TOOL-GAP-006` / `TOOL-GAP-007` 已闭合；后续先修 `TOOL-GAP-008` / `TOOL-GAP-009`，最后通过 `TOOL-GAP-010` / `TOOL-GAP-011` / `TOOL-GAP-012` 提升 runtime production、installed 和 release 可信度。
