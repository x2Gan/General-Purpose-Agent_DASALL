...
当前事实已经推进到：`RuntimeDependencySet` 新增 `secret_manager` retention 面，`compose_minimal_live_dependency_set()` 现在会按 profile-selected `secret_backend_type` 与 install layout / `state_root` override 组合 `FileSecretBackend + SecretManagerFacade`，并在 daemon / gateway shared runtime composition 中保留 owner-level secret seam evidence；`SecretManagerLiveCompositionTest` 以及 daemon / gateway runtime composition tests 已覆盖 positive file-backed manager、missing root unavailable/provider-timeout 与 seam retention。当前剩余事实缺口转为 `INF-FIX-008`：daemon / gateway main 尚未把该 seam 注入 `DaemonAccessPipelineOptions` / `GatewayAccessPipelineOptions::ownership_secret_manager`，因此 Access accepted_async ownership HMAC 等真实 consumer 仍不能宣称 production-wired。
...
Infrastructure 当前主体接口和多数组件已完成，shared observability composition 也已从“局部组件可测”推进到“生产组合根可注入、跨子系统可观测”的 build-tree focused evidence。diagnostics retained snapshot gate、health/watchdog event publish、optional backend profile gate、local release/soak gate 与 SecretManager live composition seam 现已分层固化；`INF-FIX-007` 已将 SecretManager / ISecretManager 收口为 infra public builder、runtime dependency retention 与 daemon/gateway shared owner-level focused evidence。当前剩余关键转到 `INF-FIX-008` Access ownership seam 注入与 `INF-FIX-009` secret consumer matrix，避免 Access、LLM、OTA、Plugin 等消费者继续把 focused secret 读链外推为 production-wired 结论。更高层 qemu/autopkgtest guest-side rerun 继续留在 packaging / release 环境，不再作为 infra owner blocker。
