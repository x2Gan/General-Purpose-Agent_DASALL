...
3. `TUI-TODO-030/035/036`、`BLK-TUI-008-command-release-gate-recheck.md`、`TUI-PROTO-017-formal-sample-signoff.md` 与 `BLK-TUI-006-manual-terminal-evidence.md` 已完成 bare `dasall` 迁移门禁证据、旧入口 inventory、阶段 closeout、FTXUI packaging review、Gate-TUI-08 复检、formal sign-off、manual terminal 执行对象与 Full pass 签署；`TUI-TODO-031` 恢复为下一原子任务，`TUI-TODO-032~034` 仍保持前序依赖阻塞；本轮结论不能被误写成 command release implementation ready。
