...
| TUI client | 已记录 | 当前达到 L2 owner closeout：TUI-TODO-001~036 已闭合权限、clear、projection seam、FTXUI resolver、测试拓扑、prototype、DTO/model/reducer/data source、slash/composer/selector/transcript/status、terminal probe、renderer snapshot、daemon data source、startup/status/session lifecycle、route catalog consumption、next preference submit echo、formal target、Debian/script 命令迁移与 command routing smoke；TUI-PROTO-017、TUI-PROTO-017-formal-sample-signoff、BLK-TUI-006 与 BLK-TUI-008 的 gate evidence 均已回写 | higher-layer installed / release / qemu evidence 继续归 packaging / release 环境复核 |
...
当前已无新增 TUI owner 实现缺口；后续若继续推进，仅剩更高层 installed / release 环境复核：
...
3. `TUI-TODO-030/031/032/033/034/035/036`、`BLK-TUI-008-command-release-gate-recheck.md`、`TUI-PROTO-017-formal-sample-signoff.md` 与 `BLK-TUI-006-manual-terminal-evidence.md` 已完成 bare `dasall` 迁移门禁证据、旧入口 inventory、阶段 closeout、FTXUI packaging review、Gate-TUI-08 复检、formal sign-off、manual terminal 执行对象、Full pass 签署、CLI 产物名释放、正式 TUI target 落地、Debian/script 双命令迁移与 command routing smoke；当前 command release owner chain 已闭合，若后续要补 installed / qemu / release 证据，应转入 packaging / release 环境复核，不再作为本章 TUI owner blocker。
