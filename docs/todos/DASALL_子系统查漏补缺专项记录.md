...
| knowledge | 已记录 | 主体实现 L2 基本闭合；Runtime -> Memory evidence 读链路已打通；refresh 异步、首启 prewarm、lane timeout/parallel recall 与 installed normative corpus baseline 已收口 | concrete vector backend、持久化 ledger/catalog/migration、production telemetry sink、runtime write-side refresh trigger、installed/qemu/soak 证据 |
...
| MEM-GAP-004 | Medium | qemu / soak 隔离证据仍缺；release-runner local installed contract 已闭合 | 2026-05-18 `MEM-FIX-006` 已把 `.github/workflows/release-package-gate.yml` 的 local installed memory evidence、same-session marker recall 与 `memory-proof.json` 固定下来；但 qemu guest-side / soak 仍未在本轮重跑 | 不能宣称 Memory 已有当轮 L5 qemu rerun 或 L6 production-ready；但不影响 MEM-FIX-006 以 release-runner local installed authoritative 口径收口 | 后续在 packaging / release 环境复核 qemu / autopkgtest 与 soak；Memory 功能性 evidence owner 保持 `pkg_smoke_install.sh` 与 release workflow local step |
...
Memory 子系统的实现覆盖度已经很高，当前主要残余不是“缺少 Memory 模块本体”，而是 qemu / soak 隔离证据尚未在本轮回补；release-runner local installed、边界回归与 installed maintenance 正向证据已在当前树闭合。
...
5. 查漏补缺优先级：仅保留 `MEM-GAP-004` 的 qemu / soak 隔离复核；installed maintenance 正向证据已由 `MEM-FIX-007` 闭合。
...
| apps runtime_support -> Knowledge factory | 基本打通 | L2 | production composition 传入 `service_instance_id`，并已通过 direct installed probe + `RuntimeLiveCompositionFailureMatrixTest` 证明 ready marker 与 lexical-only retrieve 一致；但 installed package / qemu / soak 证据仍不可外推 |
...
| Runtime / write-side -> Knowledge refresh trigger | 未证明 | L0 partial | 未见 runtime 非测试路径触发 `request_refresh()`；但 installed factory init 已可在缺失 snapshot 时同步首建 |
...
| KNO-GAP-005 | 已闭合 | installed 首批 corpus baseline 现已与详设对齐 | 2026-05-19 `KnowledgeServiceFactory` 已补 `architecture_reference`、`adr_normative`、`ssot_normative`、`profile_policy_normative` descriptors，并通过 `knowledge/CMakeLists.txt` / `pkg_smoke_install.sh` 把 normative docs 安装到 `/usr/share/dasall/docs` 且纳入 local installed smoke | installed factory / package smoke 现可覆盖详设要求的 architecture、ADR、SSOT 与 profile policy 权威证据面 | 2026-05-19 由 `KNO-FIX-005` 收口；后续只保留 ledger/catalog、telemetry、refresh trigger 与更高层 installed/qemu/soak 证据 |
...
| KNO-FIX-005 | Done | 扩展 installed normative corpus baseline | 已更新 `KnowledgeServiceFactory`、`knowledge/CMakeLists.txt` 与 `pkg_smoke_install.sh`：installed factory 现补齐 architecture / ADR / SSOT / profile policy descriptors，并把 normative docs 安装进 `/usr/share/dasall/docs`；新增 `KnowledgeInstalledNormativeCorpusTest`，同时扩展 installed asset probe 校验 normative corpora 已入库 | `KnowledgeInstalledAssetProbeIntegrationTest`、`KnowledgeInstalledNormativeCorpusTest`、`RetrievalQualityRegressionTest`、local installed `pkg_smoke_install.sh --explicit-start-check` | `ctest --test-dir build/vscode-linux-ninja --output-on-failure -R "KnowledgeInstalledAssetProbeIntegrationTest|KnowledgeInstalledNormativeCorpusTest|RetrievalQualityRegressionTest"`；`bash scripts/packaging/pkg_smoke_install.sh --explicit-start-check` | installed factory 能索引并检索详设定义的首批 normative corpora，且本机安装态 smoke 可复验 |
...
2. `knowledge/CMakeLists.txt` 现已把 `docs/architecture`、`docs/adr`、`docs/ssot` 安装到 `${DASALL_INSTALL_DASALL_DATADIR}/docs`；`scripts/packaging/pkg_smoke_install.sh` 已同步新增 installed asset file checks 与 `BusinessChainIntegrationMatrix` normative retrieve smoke，从而把 package install layout 与 runtime retrieve 行为一起纳入本机 authoritative gate。
...
4. 2026-05-19 已通过 `ctest --test-dir build/vscode-linux-ninja --output-on-failure -R "KnowledgeInstalledAssetProbeIntegrationTest|KnowledgeInstalledNormativeCorpusTest|RetrievalQualityRegressionTest"`（3/3 Passed）；同日已在本机 real installed-package 环境成功执行 `dpkg-buildpackage -us -uc -b`、`sudo dpkg -i ../dasall-common_0.1.0-1_all.deb ../dasall-cli_0.1.0-1_amd64.deb ../dasall-daemon_0.1.0-1_amd64.deb ../dasall_0.1.0-1_all.deb` 与 `bash scripts/packaging/pkg_smoke_install.sh --explicit-start-check`，未使用 qemu / kvm。
...
Knowledge 子系统的实现覆盖度已经很高，当前主要问题不是“缺少 Knowledge 模块本体”，而是“runtime write-side refresh trigger、vector/hybrid concrete backend、installed corpus baseline、ledger/catalog 持久化、production telemetry 与 installed/release 证据”尚未全部闭合。
...
2. 本轮复验：`KnowledgeConfigProjectionTest`、`KnowledgeProfileCompatibilityTest`、`KnowledgeRefreshAsyncLifecycleTest`、`KnowledgeRefreshLoopTest`、`KnowledgeServiceFacadeRealRefreshTest`、`KnowledgeInitPrewarmTest`、`KnowledgeInstalledAssetProbeIntegrationTest`、`RuntimeLiveCompositionFailureMatrixTest`、`RecallCoordinatorTimeoutTest` 与 `KnowledgeRetrievalSmokeTest` 已建立 projector freeze + refresh + startup prewarm + lane timeout/parallel policy + installed ready marker 聚焦证据；installed package / qemu / soak 仍不可由此直接外推为绿色。
...
5. 查漏补缺优先级：先收口 `KNO-GAP-007`，最后通过 `KNO-GAP-006` / `KNO-GAP-008` / `KNO-GAP-010` / `KNO-GAP-011` 提升 release 与边界可信度。
