...
| TUI client | 已记录 | 当前达到 L1 设计可信，并补齐当前阶段的 L2 基线：TUI-TODO-001 已冻结 daemon-backed `root/sudo-only` operator backend、ordinary-user fail-closed 与 user-level daemon/socket future-only 口径；TUI-TODO-002 已冻结 `/clear` 的新前台 session 语义与 input history 保留边界；TUI-TODO-003 已冻结 access/daemon owner 的 `TuiIpcRequestEnvelope` / `TuiIpcResponseEnvelope`、五个 operation 与 stable reason code；TUI-TODO-004 已冻结 access/runtime owner 的 typed request-scope `NextTurnPreference` carrier、拒绝 `request_context` / `client_capabilities` / profile override，并固定 `Auto` / `PreferDepth` / `PinModel` 的失败语义；TUI-TODO-005 已冻结 FTXUI 的 default-off resolver、`v6.1.9` / `5cfed50702f52d51c1b189b5f97f8beaf5eaa2a6` pin 与 `apps/tui` private dependency 规则；TUI-TODO-006 已物化 `tests/unit/tui` / `tests/integration/tui` / `tests/fixtures/tui/golden`，并让 `TuiScreenModelTest` / `TuiReducerTransitionTest` / `TuiComposerTest` / `TuiPrototypeSmokeTest` 进入顶层 discoverability；TUI-TODO-007 已接入 non-installed `dasall_tui_prototype`、`apps/tui/src/main.cpp` 与 `TuiPrototypeBuildSmokeTest`，使 no-daemon prototype target 可显式构建且不触碰 bare `dasall`；TUI-TODO-008 已落 `apps/tui/src/data/TuiProjectionTypes.h` 与 `TuiProjectionTypesTest`，把 module-local projection DTO 基线固定到可编译、可发现、无私有依赖的形态；TUI-TODO-009 已落 `apps/tui/src/model/TuiAction.h`、`apps/tui/src/model/TuiScreenModel.h` 与 `TuiScreenModelTest`，把 typed MVU model/action 基线固定到可编译、可发现、无 FTXUI/I/O 依赖的形态；TUI-TODO-010 已落 `apps/tui/src/model/TuiReducer.h/.cpp`、`tests/unit/tui/TuiReducerTransitionTest.cpp` 与 focused CMake 接线，把 typed reducer 状态迁移基线固定到可编译、可发现、无 FTXUI/I/O/owner 私有依赖的形态；TUI-TODO-011 已落 `apps/tui/src/data/ITuiDataSource.h`、`tests/unit/tui/TuiDataSourceContractTest.cpp` 与 focused CMake 接线，把五个 operation、stable issue contract 与 no-private-include data source seam 固定到可编译、可发现的形态；TUI-TODO-012 已落 `apps/tui/src/data/FakeScenarioCatalog.h`、`apps/tui/src/data/FakeTuiDataSource.h/.cpp`、`tests/unit/tui/TuiFakeScenarioCatalogTest.cpp` 与 `tests/unit/tui/FakeTuiDataSourceTest.cpp`，把六个 deterministic fake scenario、machine-readable load failure 与 session/cursor replay 语义固定到可编译、可发现的形态；TUI-TODO-013 已落 `apps/tui/src/command/TuiSlashCommandParser.h/.cpp`、`tests/unit/tui/TuiSlashCommandParserTest.cpp` 与 focused CMake 接线，把六个最小 slash command、help metadata、projection query/local action 映射与 fail-closed 错误 banner 固定到可编译、可发现的形态；TUI-TODO-014 已落 `apps/tui/src/view/TuiComposer.h/.cpp`、`apps/tui/src/view/TuiInputHistory.h`、`tests/unit/tui/TuiComposerTest.cpp` 与 `TuiComposerHistoryTest.cpp`，把 multiline/submit/history-recall/reverse-search/external-editor/busy draft 的本地状态机固定到可编译、可发现、无 owner 私有依赖的形态；TUI-TODO-015 已落 `apps/tui/src/view/TuiModelSelector.h/.cpp`、`tests/unit/tui/TuiModelSelectorTest.cpp` 与 `TuiRouteCatalogFilterTest.cpp`，把 selector fake 交互、disabled reason 展示与 local draft/apply/cancel 语义固定到可编译、可发现、无 owner 私有依赖的形态；TUI-TODO-016 已落 `apps/tui/src/view/TuiTranscriptView.h/.cpp`、`tests/unit/tui/TuiTranscriptViewTest.cpp` 与 focused CMake 接线，把 transcript flatten/render、collapse toggle、scroll-to-bottom 与 unsafe marker redaction 固定到可编译、可发现的形态；TUI-TODO-017 已落 `apps/tui/src/view/TuiStatusPanel.h/.cpp`、`tests/unit/tui/TuiStatusPanelTest.cpp` 与 focused CMake 接线，把 stage badge、health summary、decision summary 派生与 unknown/degraded fail-closed 文本基线固定到可编译、可发现的形态；TUI-TODO-018 已落 `apps/tui/src/terminal/TuiTerminalCapabilityProbe.h/.cpp`、`tests/unit/tui/TuiTerminalCapabilityProbeTest.cpp` 与 focused CMake 接线，把 terminal-local startup mode taxonomy、stable startup issue 与 `FullScreen/Narrow/Line/FailClosed` capability probe 基线固定到可编译、可发现的形态；TUI-TODO-019 已落 `apps/tui/src/view/TuiDesignTokens.h`、`apps/tui/src/view/TuiLayoutMetrics.h`、`apps/tui/src/terminal/FtxuiRendererAdapter.h/.cpp`、`tests/unit/tui/TuiDesignTokensTest.cpp` 与 `TuiMainLayoutSnapshotTest.cpp`，把 tokens/layout metrics、canonical render frame、default-off deterministic snapshot backend 与 optional private FTXUI backend 开关固定到可编译、可发现、无 model/reducer/data-source 污染的形态；TUI-TODO-020 已落 fake-only prototype app loop，把 probe -> fake source -> reducer -> renderer loop -> exit 接成统一 startup/smoke path，并通过 `TuiAppStartupTest` / `TuiPrototypeSmokeTest` 守住 no-daemon prototype 行为；TUI-TODO-021 已落 `TuiIpcController` request/response mapping header、五个 operation 的 payload/outcome/timeout 形状与 focused mapping guard；TUI-TODO-022 已落 `TuiIpcController` transport、JSON envelope codec 与 stable reason code normalization；TUI-TODO-023 已落 controller-backed `DaemonTuiDataSource` thin adapter 与五个 operation contract-like guard；TUI-TODO-024 已把 narrow/non-TTY startup degradation 以及 daemon unavailable / permission denied / profile_missing startup failure 接到 `TuiApp`，并通过 daemon-backed startup failure integration test 守住 stable startup issue、default fake-only、不隐式提权、不自动启动 daemon 的边界；TUI-TODO-025 已通过 `TuiStatusProjectionContractTest` / `TuiStatusPanelIntegrationTest` 证明 daemon-backed poll 的 stage/tool/pending/budget/recovery/safe-mode projection 能刷新到 `screen_model.status` 与 status panel，同时保持 no-internal-dump render 边界；TUI-TODO-026 已通过 reducer reset、session lifecycle integration 与 discoverability evidence，把 `/clear` / `/exit` 接到现有 session seam 的 explicit close/rebind path，并保持 close failure 可观测；TUI-TODO-027 已冻结 route catalog projection 的 `verification_state` / `health` / `profile_allowlisted` 字段、补齐 fake fixture 与 IPC/data source seam roundtrip，并通过 focused unit/contract/discoverability evidence 守住 summary-only、no-secret/no-profile-dump 边界；TUI-TODO-028 已把这些显式 route projection 字段接入 `TuiModelSelector` 消费，通过 daemon-backed selector test 与相邻 filter/topology tests 证明 selector option 现可展示 `verified healthy depth=...` 摘要并把 disabled reason 渲染为稳定用户文案；命令迁移仍未放行 | submit echo、CJK/IME/resize 与 packaging gate、bare `dasall` release gate |
...
| Task ID | 状态 | 任务标题 | 代码目标 | 测试目标 | 建议验收命令 | 完成判定 |
|---|---|---|---|---|---|---|
| KNO-TODO-034 | Todo | 暴露 request-scoped hybrid query surface | 更新 `knowledge/include/KnowledgeTypes.h`、`knowledge/src/query/QueryNormalizer.cpp`、`apps/cli/src/CliCommandParser.cpp`、`apps/cli/src/CliRequestBuilder.h`、`access/src/AccessGatewayFactory.cpp`，为 `knowledge retrieve` 增加 `preferred_mode`、`query_kind`、`allowed_corpora`、`domain_tags`、`required_tags`、`required_language` 等 module-local 检索控制位，并把 retrieve payload 暴露 `degraded`、reason codes、warning count 与命中 corpus 摘要；不得把 retrieval supporting types 提升到 `contracts` | 扩展 `CliDaemonCommandParserTest`、`DaemonAccessPipelineFactoryTest`，新增 `KnowledgeRuntimeQuerySurfaceIntegrationTest`，验证 CLI -> daemon/access -> `KnowledgeQuery` 映射与 degraded payload 反投影 | `cmake --build build/vscode-linux-ninja --target dasall-cli_command_parser_unit_test dasall-daemon_access_pipeline_factory_unit_test dasall_knowledge_runtime_query_surface_integration_test -j2`；`ctest --test-dir build/vscode-linux-ninja -R CliDaemonCommandParserTest --output-on-failure`；`ctest --test-dir build/vscode-linux-ninja -R DaemonAccessPipelineFactoryTest --output-on-failure`；`ctest --test-dir build/vscode-linux-ninja -R KnowledgeRuntimeQuerySurfaceIntegrationTest --output-on-failure` | daemon/control-plane 已能显式发起 hybrid canary query，并在 JSON 响应中看到 `mode`、`degraded`、reason codes 与 corpus 摘要；`contracts` 无新增 retrieval supporting object |
| KNO-TODO-035 | Todo | 收口 runtime-owned hybrid canary seam | 更新 `knowledge/src/KnowledgeServiceFactory.cpp`、`apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp` 与相关组合根测试，只在 vector backend ready、profile 允许且 corpus 命中 canary allowlist 时接受 `preferred_mode=Hybrid` 或 `DenseOnly`；其余路径保持 `LexicalOnly` fail-safe，不新增 profile schema v1 键 | 扩展 `RuntimeLiveCompositionFailureMatrixTest`，新增 `RuntimeKnowledgeHybridCanaryIntegrationTest` 与 `KnowledgeInstalledAssetHybridProbeTest`，验证 explicit canary query 的 positive path、fallback path 与 ready marker | `cmake --build build/vscode-linux-ninja --target dasall_access_runtime_live_composition_failure_matrix_integration_test dasall_runtime_knowledge_hybrid_canary_integration_test dasall_knowledge_installed_asset_hybrid_probe_integration_test -j2`；`ctest --test-dir build/vscode-linux-ninja -R RuntimeLiveCompositionFailureMatrixTest --output-on-failure`；`ctest --test-dir build/vscode-linux-ninja -R RuntimeKnowledgeHybridCanaryIntegrationTest --output-on-failure`；`ctest --test-dir build/vscode-linux-ninja -R KnowledgeInstalledAssetHybridProbeTest --output-on-failure` | 在 `desktop_full`、`cloud_full`、`edge_balanced` 的 vector-ready runtime path 上，白名单 corpus 的 explicit hybrid 请求可返回 `mode=Hybrid`；default baseline 仍 lexical-only |

##### 第二阶段：再好用

| Task ID | 状态 | 任务标题 | 代码目标 | 测试目标 | 建议验收命令 | 完成判定 |
|---|---|---|---|---|---|---|
| KNO-TODO-036 | Todo | 接入 production-grade embedding / query encoder seam | 更新 `memory/src/vector/DetachedVectorIndexFactory.cpp`、`memory/src/vector/SimpleLocalEmbeddingAdapter.cpp`、`apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp`、`knowledge/src/retrieve/VectorRetrieverBridge.cpp`，把当前 hash embedding 降为 test/local fallback，并接入 versioned query encoder 或 embedding provider seam；live dense path 需要支持 provider 不可用时的 lexical-only fail-safe | 扩展 `VectorRetrieverBridgeTest`、`RuntimeLiveCompositionFailureMatrixTest`，新增 `RuntimeKnowledgeQueryEncoderIntegrationTest`，验证 encoder ready、encoder unavailable fallback 与 owner boundary 不回退 | `cmake --build build/vscode-linux-ninja --target dasall_memory dasall_apps_runtime_support dasall_runtime_knowledge_query_encoder_integration_test -j2`；`ctest --test-dir build/vscode-linux-ninja -R VectorRetrieverBridgeTest --output-on-failure`；`ctest --test-dir build/vscode-linux-ninja -R RuntimeKnowledgeQueryEncoderIntegrationTest --output-on-failure`；`ctest --test-dir build/vscode-linux-ninja -R RuntimeLiveCompositionFailureMatrixTest --output-on-failure` | live dense path 不再默认依赖 `SimpleLocalEmbeddingAdapter` 的 hash 结果；encoder 或 provider 缺失时仍稳定退回 lexical-only |
| KNO-TODO-037 | Todo | 扩展 hybrid / dense retrieval quality gate | 更新 `tests/integration/knowledge/RetrievalQualityRegressionTest.cpp` 与 `tests/integration/knowledge/golden/retrieval_quality_v1.yaml`，为 architecture、ADR、SSOT 语料补 `Hybrid`、`DenseOnly` case、mode-specific threshold、hard-fail 规则与 dense fixture materialization，不再让 hybrid case 全量 skip | 扩展 `RetrievalQualityRegressionTest`，并保留 `RecallCoordinatorDenseBridgeTest`、`VectorRetrieverBridgeTest` 作为 lane-level 回归辅助门禁 | `ctest --test-dir build/vscode-linux-ninja -R RetrievalQualityRegressionTest --output-on-failure`；`ctest --test-dir build/vscode-linux-ninja -R RecallCoordinatorDenseBridgeTest --output-on-failure`；`ctest --test-dir build/vscode-linux-ninja -R VectorRetrieverBridgeTest --output-on-failure` | hybrid / dense case 已具独立阈值和 hard-fail gate，不再只证明 lexical baseline；任一关键 case 回归时 gate 会显式失败 |
| KNO-TODO-038 | Todo | 优化 mixed-corpus hybrid 路由语义 | 更新 `knowledge/src/query/CorpusRouter.cpp`、`knowledge/src/KnowledgeServiceFactory.cpp` 与对应 tests，使 mixed catalog 下的 hybrid 路由能够基于 dense-capable subset、explicit canary corpus policy 或等价 route narrowing 选路，而不是因 `all_dense_capable(candidates)` 被全局压回 lexical-only | 新增 `CorpusRouterMixedCapabilityTest`、`KnowledgeMixedCorpusHybridRoutingIntegrationTest`，覆盖带或不带 `allowed_corpora` 的 mixed corpus 查询 | `cmake --build build/vscode-linux-ninja --target dasall_corpus_router_unit_test dasall_knowledge_mixed_corpus_hybrid_routing_integration_test -j2`；`ctest --test-dir build/vscode-linux-ninja -R CorpusRouterMixedCapabilityTest --output-on-failure`；`ctest --test-dir build/vscode-linux-ninja -R KnowledgeMixedCorpusHybridRoutingIntegrationTest --output-on-failure` | mixed catalog 下的 generic query 不会仅因 `profile_policy_normative`、`dasall_llm_providers` 混入候选集就被永久压回 lexical-only；lexical-only corpus 也不会被误标 dense-capable |

##### 第三阶段：最后可放量

| Task ID | 状态 | 任务标题 | 代码目标 | 测试目标 | 建议验收命令 | 完成判定 |
|---|---|---|---|---|---|---|
| KNO-TODO-039 | Todo | 接入 vector observability / explain surface | 更新 `knowledge/src/health/KnowledgeTelemetry.cpp`、`knowledge/include/health/KnowledgeTelemetry.h`、`access/src/AccessGatewayFactory.cpp`、`apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp`，补齐 dense/sparse lane 命中、selected corpora、degraded reasons、warning summary、backend ready 等 vector-specific 可观测字段与 CLI/daemon 返回面 | 扩展 `KnowledgeProductionTelemetryIntegrationTest`、`RuntimeKnowledgeEvidenceIntegrationTest`，新增 `AccessKnowledgeRetrievePayloadTest`，验证 production-composed path 的 telemetry 与 JSON payload 可解释性 | `cmake --build build/vscode-linux-ninja --target dasall_knowledge_production_telemetry_integration_test dasall_runtime_knowledge_evidence_integration_test dasall_access_knowledge_retrieve_payload_unit_test -j2`；`ctest --test-dir build/vscode-linux-ninja -R KnowledgeProductionTelemetryIntegrationTest --output-on-failure`；`ctest --test-dir build/vscode-linux-ninja -R RuntimeKnowledgeEvidenceIntegrationTest --output-on-failure`；`ctest --test-dir build/vscode-linux-ninja -R AccessKnowledgeRetrievePayloadTest --output-on-failure` | production-composed path 已能判断 dense lane 是否参与、为何 degraded、命中了哪些 corpus，且不需要读内部日志才能解释结果 |
| KNO-TODO-040 | Todo | 固化 installed local hybrid canary / soak 证据 | 更新 `scripts/packaging/knowledge_local_installed_proof.sh`、`scripts/packaging/knowledge_refresh_retrieve_soak.sh`、`.github/workflows/release-package-gate.yml`，固定 hybrid canary request、返回 mode、dense artifact presence、degraded reasons 与 corpus hit summary 的 artifact，不把 qemu 作为本任务完成前提 | 扩展 local installed proof 与 soak artifact contract，必要时补 `debian/tests/pkg-smoke-local-control-plane` 断言 hybrid proof 已落盘 | `dpkg-buildpackage -us -uc -b`；`bash scripts/packaging/knowledge_local_installed_proof.sh --artifact-dir /tmp/dasall-kno-todo-040-proof --hybrid-canary`；`bash scripts/packaging/knowledge_refresh_retrieve_soak.sh --artifact-dir /tmp/dasall-kno-todo-040-soak --iterations 10 --hybrid-canary` | local installed / release-runner local artifact 能稳定记录 hybrid canary 结果与 dense artifact 证据，release gate 不再只有 lexical-only positive proof |
| KNO-TODO-041 | Todo | 建立 Runtime-owned selective refresh automation | 更新 `apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp`、`access/src/AccessGatewayFactory.cpp` 与相关 control-plane 组件，在不把 watcher/timer owner 下沉到 Knowledge 的前提下，基于既有 `request_refresh(updated_sources)` seam 增加 Runtime-owned 的 scheduled refresh 或 changed-source automation | 扩展 `KnowledgeRefreshLoopTest`、新增 `KnowledgeRuntimeAutoRefreshIntegrationTest` 与 `RuntimeLiveCompositionFailureMatrixTest` 子用例，验证自动 refresh 不会破坏 lexical-only fallback、busy reject 与 owner boundary | `cmake --build build/vscode-linux-ninja --target dasall_knowledge_refresh_loop_integration_test dasall_knowledge_runtime_auto_refresh_integration_test dasall_access_runtime_live_composition_failure_matrix_integration_test -j2`；`ctest --test-dir build/vscode-linux-ninja -R KnowledgeRefreshLoopTest --output-on-failure`；`ctest --test-dir build/vscode-linux-ninja -R KnowledgeRuntimeAutoRefreshIntegrationTest --output-on-failure`；`ctest --test-dir build/vscode-linux-ninja -R RuntimeLiveCompositionFailureMatrixTest --output-on-failure` | dense snapshot freshness 不再长期依赖手工 CLI refresh；自动 refresh 仍由 Runtime / Access 主控，ADR-008 不回退 |
...
截至当前已闭合的 TUI owner 缺口见 17.3 表；权限/clear 语义、projection seam、fake-only prototype、daemon-backed startup/status/session lifecycle、route catalog projection 字段基线、selector daemon consumption 与 focused topology baseline 均已闭合，剩余缺口集中在 submit echo、terminal manual gate 与 bare `dasall` release gate。
...
11. `TuiComposer` 与 `TuiInputHistory` 已冻结为 TUI local state machine：`Enter` / `Alt+Enter` / `Ctrl+J` / `Up` / `Down` / `Ctrl+R` / busy draft / external-editor failure 均有 focused unit evidence，且 `BLK-TUI-006` 仅继续保留在 IME/CJK/resize 的人工 gate。
12. `TuiModelSelector` 现已从 fake-only selector helper 前移到 daemon-backed projection consumption 基线：支持 Auto / PreferDepth / PinModel 三模式、depth tier 聚合、local draft/apply/cancel、disabled route/depth fail-closed，并已把 route catalog 的 `verification_state` / `health` / `depth_tier` 摘要接入 option label、把已冻结 reason code 渲染为稳定用户文案；submit echo 与 effective route 回显仍后置到 `TUI-TODO-029`。
...
15. 上述结论只解除了 TUI 的设计 blocker、composer state-machine blocker、selector fake-interaction blocker、transcript-view baseline blocker、status-panel baseline blocker、terminal-probe baseline blocker、renderer-layout baseline blocker、fake-only app-loop blocker、IPC mapping/header blocker、IPC error-normalization blocker、daemon data source attach blocker、startup failure path blocker、status projection refresh blocker、session lifecycle 接线 blocker、route catalog projection blocker 与 selector daemon consumption blocker，不代表 bare `dasall` 命令迁移已可放行；TUI-TODO-029~030 及 031~034 仍需按后续 gate 单独验收。
...
1. `TUI-TODO-029` 尚未把已冻结的 `NextTurnPreference` carrier 继续落盘为 submit echo 与 effective route 回显；027/028 已分别补 route catalog projection 字段基线与 selector daemon consumption。
2. `TUI-TODO-020` 已闭合 prototype app loop，但仍受 CJK/IME/resize manual gate 与 installed path packaging review 约束；本轮结果不等于 full-screen interactive promotion、route submit echo ready 或 release path 已 ready。
3. `TUI-TODO-030~034` 的 bare `dasall` 迁移仍保持 Blocked；本轮结论不能被误写成 command release ready。
