...
| KNO-TODO-045 | Todo | Runtime-owned selective refresh automation 实装 | 已完成 D 包并通过 D Gate：`docs/todos/knowledge/deliverables/KNO-TODO-045-runtime-owned-selective-refresh-automation口径闭合双轨任务包.md` 已锁定 runtime-owned selective delta provider 方案；Build 将在 041 periodic full-scan 基础上补真正 selective source 变更输入，full-scan 仅作为 fallback | 扩展 `KnowledgeRuntimeAutoRefreshIntegrationTest` 与 refresh loop gate，新增“单文件 changed_source 只刷新目标 corpus / stale busy fail-safe / full-scan fallback”断言 | `cmake --build build/vscode-linux-ninja --target dasall_knowledge_runtime_auto_refresh_integration_test dasall_knowledge_refresh_loop_integration_test -j2`；直接执行两个 binaries；新增 selective artifact 验证 active snapshot 变化与非目标 corpus 不误刷新 | `KNO-TODO-041` 的名称与实现口径闭合：自动化不再只是 periodic full-scan，而具备 Runtime-owned selective refresh 正例与 fallback 分层证据 |

#### KNO-TODO-045 设计完成证据（2026-05-26）

1. `docs/todos/knowledge/deliverables/KNO-TODO-045-runtime-owned-selective-refresh-automation口径闭合双轨任务包.md` 已落盘，明确当前缺口位于 `apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp` 的 timer callback 仍固定提交 empty `CorpusChangeSet`，而非 Knowledge refresh worker 缺失。
2. D 包已锁定 Build 切口：runtime owner 复用 `knowledge::ingest::SourceScanner` 的 inventory diff 能力生成 `Skip / Selective / FullScanFallback` 计划，manual `changed_source -> updated_sources` control-plane seam 保持 authoritative，`contracts/` 与 ADR-006 / ADR-007 / ADR-008 owner boundary 均不改。
3. Focused Build 三件套已固定为 `apps/runtime_support` selective provider 实装、`KnowledgeRuntimeAutoRefreshIntegrationTest` selective/busy/fallback 回归，以及 `KnowledgeRefreshLoopTest` target-only selective / full-scan fallback 回归。
