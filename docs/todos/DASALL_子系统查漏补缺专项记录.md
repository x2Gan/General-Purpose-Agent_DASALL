...
15. 上述结论只解除了 TUI 的设计 blocker、composer state-machine blocker、selector fake-interaction blocker、transcript-view baseline blocker、status-panel baseline blocker、terminal-probe baseline blocker、renderer-layout baseline blocker、fake-only app-loop blocker、IPC mapping/header blocker、IPC error-normalization blocker、daemon data source attach blocker与 startup failure path blocker，不代表 bare `dasall` 命令迁移已可放行；TUI-TODO-025~030 及 031~034 仍需按后续 gate 单独验收。
...
1. `TUI-TODO-025~026` 尚未把已冻结的 daemon/access projection seam 继续落盘为 status/tool/recovery 真消费与 foreground session close/query 真链路；其中 `TUI-TODO-021~024` 已闭合 mapping/header、controller normalization、daemon data source attach 与 startup failure path baseline，`/clear` 的用户可见语义也已闭合，但 backend close/open 细节仍后置。
