...
| infrastructure | 已记录 | logging/audit/config/secret/policy/diagnostics/health/plugin 等主体接口与多数组件已落盘；diagnostics/readiness 有 L3/L4 partial 证据 | production sinks across subsystems、diagnostics retained snapshot 分层、plugin-to-tools 自动接线、watchdog/event publish、KMS/OTLP 等 optional backend、release runner / soak |
...
| Gap ID | 严重级别 | 缺口 | 证据 | 影响 | 补缺方向 |
|---|---|---|---|---|---|
| MEM-GAP-001 | High | `sqlite-vss` focused internal seam 已完成，但真实 extension / packaging / production 接线未闭合 | `memory/src/vector/SqliteVssVectorBackend.*`、`tests/unit/memory/SqliteVssVectorBackendTest.cpp` 与 `VectorMemoryAdapterTest` 已证明 happy/failure path；专项 TODO 记录当前 SQLite 构建禁用 loadable extension，真实 third_party 接入后置 | VectorMemory 不能继续写成“无 concrete backend”，但也不能宣称语义向量召回、写回 sidecar、rebuild 和 installed profile 已 production-ready | 独立推进 sqlite-vss extension 依赖、package asset、factory wiring、installed/qemu 证据；或在 production profile 明确保持 lexical/no-vector fallback |
...
| Task ID | 状态 | 任务标题 | 代码目标 | 测试目标 | 建议验收命令 | 完成判定 |
|---|---|---|---|---|---|---|
| MEM-FIX-001 | Todo | 收口 sqlite-vss production 接线与证据 | 保留已完成的 `SqliteVssVectorBackend` internal seam；新增真实 sqlite-vss extension / third_party 依赖接入、`MemoryManagerFactory` production wiring、packaging asset 和 profile fail-closed 策略 | 复跑 `SqliteVssVectorBackendTest`、`VectorMemoryAdapterTest`，新增 extension-available / extension-missing / installed vector smoke；保持 `MemoryWritebackIntegrationTest` 与 `MemoryProfileCompatibilityTest` 不回退 | `RunCtest_CMakeTools(tests=["SqliteVssVectorBackendTest","VectorMemoryAdapterTest","MemoryWritebackIntegrationTest","MemoryProfileCompatibilityTest"])`；package gate 另按 `validate_gate_int_10_installed_package_qemu.sh` 固化 | internal seam、真实 extension、production factory、profile 策略和 installed/qemu 证据分层一致；不能再把 focused seam 外推为 production semantic vector recall |
...
3. VectorMemory：当前已能证明 sqlite-vss internal seam 与 disabled / unavailable fallback；不能宣称真实 sqlite-vss extension、production factory 或 installed semantic vector recall 已完成。
4. SQLite / observability / concurrency：仍需补版本 gate、production sink 和 reader pool 并发防护。
5. 查漏补缺优先级：先把 `MEM-GAP-001` 从 focused seam 推进到 production/package 证据，同时修 `MEM-GAP-002`；再收口 `MEM-GAP-003` / `MEM-GAP-005`，最后通过 `MEM-GAP-004` / `MEM-GAP-006` / `MEM-GAP-007` 提升 release 与边界可信度。
...
| RT-GAP-002 | High | terminal cognition decision 到 Runtime Responding 映射不完整 | cognition 章节已记录 `DirectResponse` / `ConvergeSafe` 第一跳缺口；Runtime live path 主要围绕 `ExecuteAction` 主链 | cognition terminal decision 可能被误判为失败或降级 | 与 `COG-FIX-001` 合并实现 Runtime 映射与 interaction contract |
...
| RT-FIX-002 | Todo | 补齐 terminal cognition decision 映射 | 更新 `runtime/src/AgentOrchestrator.cpp`，处理 `DirectResponse` / `ConvergeSafe` / `NoDecision`，调用 `IResponseBuilder.build()` 并保持 recovery owner 边界 | 扩展 `CognitionRuntimeInteractionContractTest` 和 runtime unary positive/negative fixtures | `RunCtest_CMakeTools(tests=["CognitionRuntimeInteractionContractTest","RuntimeUnaryIntegrationTest"])` | 五类 decision kind 均有第一跳断言，失败原因可审计 |
...
检查范围：`infra/include/*`、`infra/src/*`、`docs/todos/infrastructure/*`、`docs/architecture/DASALL_infrastructure子系统详细设计.md`、`apps/daemon/src/main.cpp`、`apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp`、`tests/unit/infra/*`、`tests/integration/infra/*`、`docs/ssot/SystemIntegrationGateMatrix.md`。
...
Infrastructure 当前主体接口和多数组件已完成，剩余关键是把 infra 能力从“局部组件可测”推进到“生产组合根可注入、跨子系统可观测、release runner 可复验”。
...
| P1 | Multi-Agent enabled gate、Infra observability/diagnostics、Profiles consumer matrix | 防止 profile/feature enablement 与实际 wiring 漂移 | `MA-FIX-001`、`INF-FIX-001`、`INF-FIX-002`、`PRF-FIX-001` |
