...
| MEM-GAP-007 | Low | installed maintenance 正向证据可加强 | 2026-05-18 `MEM-FIX-006` 已用 same-session 双轮 smoke 与 `memory-proof.json` 闭合 installed 多轮 context reuse；剩余未证的是 retention / checkpoint / quarantine cleanup 等 maintenance 正向路径 | installed 证据对维护任务生产运行覆盖仍不足 | 增加 installed maintenance smoke，记录 checkpoint / retention / quarantine 结果 |
...
| MEM-FIX-007 | Todo | 增加 installed maintenance 正向证据 | 不一定改产品代码；可增加 maintenance CLI / diag hook 或 test helper，记录 checkpoint / retention / quarantine cleanup | `MemoryMaintenanceInstalledSmokeTest` 或等价 package smoke | `RunCtest_CMakeTools(tests=["MemoryMaintenanceIntegrationTest"])`；installed 命令在任务 deliverable 中固定 | 能证明 checkpoint / retention / quarantine cleanup 在安装态可观测 |
...
5. 查漏补缺优先级：转向 `MEM-GAP-004` / `MEM-GAP-007` 的残余项，分别补 qemu / soak 隔离复核与 installed maintenance 正向证据。
