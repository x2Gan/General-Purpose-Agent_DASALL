...
3. 源码缺陷：`DataQueryLane` cache hit result triad、subscription public ABI trace 与 production adapter registry / backend handler 覆盖都已由 `CAPSRV-FIX-002` / `CAPSRV-FIX-003` / `CAPSRV-FIX-004` 闭合；当前残余重点不再是 self-owned triad/trace/backend handler seam 断裂，而是更高层 dynamic snapshot、observability 与 installed/release 证据。
4. Backend / production：build-tree production adapter registry / handler composition 已闭合，但 `CAPSRV-GAP-005` 的 dynamic capability snapshot/provider、`CAPSRV-GAP-006` 的 production observability/health sinks、`CAPSRV-GAP-007` 的 caller-domain owner 与 `CAPSRV-GAP-008` 的 installed/qemu/soak 证据仍需补。
5. 查漏补缺优先级：从 `CAPSRV-GAP-005` 开始，依次收口 `CAPSRV-GAP-006` / `CAPSRV-GAP-007`，最后通过 `CAPSRV-GAP-008` 提升 installed、release 与长稳态可信度。
