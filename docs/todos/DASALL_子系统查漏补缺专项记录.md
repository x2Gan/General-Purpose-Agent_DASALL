...
3. 源码缺陷：`DataQueryLane` cache hit result triad 与 subscription public ABI trace 覆盖都已由 `CAPSRV-FIX-002` / `CAPSRV-FIX-003` 闭合；当前残余重点不再是 self-owned trace/triad 断裂，而是更高层 backend / production 证据。
...
5. 查漏补缺优先级：从 `CAPSRV-GAP-004` 开始，依次收口 `CAPSRV-GAP-005` / `CAPSRV-GAP-006` / `CAPSRV-GAP-007`，最后通过 `CAPSRV-GAP-008` 提升 installed、release 与长稳态可信度。
