...
| runtime_support / app live composition | 已记录 | daemon/gateway 共享 app-level runtime 组合根已具 L2 focused、L3 release-preflight 与 L4 local installed authoritative evidence | qemu / release-runner 继续归 packaging / release 环境复核，不再是 runtime_support self-owned blocker |
| access / apps ingress | 已记录 | Access v1 unary focused ingress 已通过 Gate-INT-08；CLI/daemon/gateway app-binary、installed local control-plane、installed async receipt 与 installed HTTP gateway unary local package evidence 已形成 L3/L4 分层；`ACC-GATE-11` 已把 `StreamGateway / WS / MQTT` 固定为 feature flag default-off + async/poll fallback 的延后边界；`ACC-FIX-005` 已把 Access owner 当前安全口径冻结为 Local installed security matrix | 更高层 machine-isolation / release 环境复核继续归 packaging / release owner，不再是 Access owner blocker |
...
#### 向量能力生产级整改闭合追加任务（2026-05-26）

| Task ID | 状态 | 任务标题 | 代码目标 | 测试目标 | 建议验收命令 | 完成判定 |
|---|---|---|---|---|---|---|
| KNO-TODO-042 | Done | strict hybrid ready marker 与 vector-enabled manifest 闭合 | 已更新 `knowledge/include/ingest/IngestionCoordinator.h`、`knowledge/src/facade/KnowledgeService.cpp`、`knowledge/src/index/IndexWriter.cpp`、`apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp`、`tests/integration/access/RuntimeLiveCompositionFailureMatrixTest.cpp` 与 `tests/integration/access/RuntimeKnowledgeHybridCanaryIntegrationTest.cpp`；refresh batch 现显式携带 `config_.vector_enabled`，首次 refresh 可生成 vector-enabled manifest；`knowledge-hybrid-canary-ready` marker 现必须同时满足 `mode=Hybrid`、`runtime_canary_admitted`、`vector_backend_ready=true` 与 `dense_hit_count>0` | `RuntimeLiveCompositionFailureMatrixTest` 已新增 dense empty negative marker gate，`RuntimeKnowledgeHybridCanaryIntegrationTest` 已断言 ready backend 与 dense hit；并回归 query surface、query encoder、quality、mixed routing、telemetry、access payload、auto refresh、refresh loop 与 IndexWriter 基础测试 | `cmake --build build/vscode-linux-ninja --target dasall_access_runtime_live_composition_failure_matrix_integration_test dasall_runtime_knowledge_hybrid_canary_integration_test -j2`；`./build/vscode-linux-ninja/tests/integration/access/dasall_access_runtime_live_composition_failure_matrix_integration_test && ./build/vscode-linux-ninja/tests/integration/access/dasall_runtime_knowledge_hybrid_canary_integration_test && echo PASS`；扩展回归：`cmake --build build/vscode-linux-ninja --target dasall_knowledge_runtime_query_surface_integration_test dasall_runtime_knowledge_query_encoder_integration_test dasall_knowledge_retrieval_quality_regression_integration_test dasall_knowledge_mixed_corpus_hybrid_routing_integration_test dasall_knowledge_production_telemetry_integration_test dasall_access_knowledge_retrieve_payload_unit_test dasall_knowledge_runtime_auto_refresh_integration_test dasall_knowledge_refresh_loop_integration_test dasall_index_writer_unit_test dasall_index_writer_snapshot_swap_unit_test -j2` 后直接执行对应 binaries | runtime ready marker 不再只证明 admission；只有 dense lane 实际贡献且 health 证明 vector backend ready 时才暴露 `knowledge-hybrid-canary-ready`；首次 installed/local refresh 不再因 manifest 默认 vector disabled 造成 strict proof 假阴性 |
| KNO-TODO-043 | Done | production embedding provider 替代 env-gated local fallback | 已在 runtime_support / memory owner 边界内新增 provider-backed production query encoder、runtime-local health cache 与 explicit canary admission seam；`SimpleLocalEmbeddingAdapter` 与 `DASALL_DETACHED_VECTOR_LOCAL_FALLBACK=1` 现仅保留为 local/test/package fallback，不再作为生产 semantic quality 通过依据 | `RuntimeKnowledgeQueryEncoderIntegrationTest` 已新增 provider-ready、provider-missing、provider-empty-embedding、invalid-secret、local-fallback 五类 focused case；`KnowledgeProductionTelemetryIntegrationTest` 已新增 provider-ready Hybrid 与 invalid-secret lexical-only shared sink token gate，证明生产 provider ready 正例不依赖 local fallback env | `Build_CMakeTools(buildTargets=["dasall_runtime_knowledge_query_encoder_integration_test","dasall_knowledge_production_telemetry_integration_test"])`；`RunCtest_CMakeTools(tests=["KnowledgeProductionTelemetryIntegrationTest"])` 继续命中仓库已知泛化 `生成失败` 后直接执行 `./build/vscode-linux-ninja/tests/integration/access/dasall_runtime_knowledge_query_encoder_integration_test && echo PASS` 与 `./build/vscode-linux-ninja/tests/integration/knowledge/dasall_knowledge_production_telemetry_integration_test && echo PASS`，结果均 `PASS` | provider-ready path 现可在关闭 local fallback env 后返回 Hybrid/Dense 正例；provider-missing / empty-embedding / invalid-secret 都会在 explicit canary admission 前 lexical-only fail-closed，并保留明确 `runtime_canary_*` / `query_encoder_*` reason code |
| KNO-TODO-044 | Done | 新包 strict installed proof / soak 复跑闭合 | 已补 `memory/src/vector/SimpleLocalEmbeddingAdapter.cpp` UTF-8 tokenization fallback 与 `tests/unit/memory/SimpleLocalEmbeddingAdapterTest.cpp` 中文输入回归；已基于当前代码重建 Debian 包并执行 fresh install local proof 与 10 轮 soak，artifact 固定在 `/tmp/dasall-kno-todo-044-proof-strict` 与 `/tmp/dasall-kno-todo-044-soak-strict` | focused fallback 单测已通过；`knowledge-proof.json` 记录 `hybrid_canary_mode=hybrid`、`runtime_canary_admitted`、`hybrid_canary_vector_backend_ready=true`、`hybrid_canary_dense_hit_count=3`；`knowledge-soak-summary.json` 记录 `iterations_completed=10`、`hybrid_mode_values=["hybrid"]`、`all_hybrid_iterations_admitted=true`、`all_hybrid_iterations_vector_backend_ready=true`、`min_hybrid_dense_hit_count=3` | `Build_CMakeTools(buildTargets=["dasall_memory_simple_local_embedding_adapter_unit_test"])`；`RunCtest_CMakeTools(tests=["SimpleLocalEmbeddingAdapterTest"])` 命中仓库已知泛化 `生成失败` 后直接执行 `./build/vscode-linux-ninja/tests/unit/memory/dasall_memory_simple_local_embedding_adapter_unit_test && echo PASS`；`dpkg-buildpackage -us -uc -b`；`bash scripts/packaging/knowledge_local_installed_proof.sh --artifact-dir /tmp/dasall-kno-todo-044-proof-strict --hybrid-canary`；`bash scripts/packaging/knowledge_refresh_retrieve_soak.sh --artifact-dir /tmp/dasall-kno-todo-044-soak-strict --iterations 10 --hybrid-canary` | 当前代码对应的新包 strict installed proof / soak 已全绿，KNO-TODO-040 从脚本合同 ready 提升为当轮 local L4 positive installed evidence；不外推 qemu / release-runner / production provider SLA |
| KNO-TODO-045 | Done | Runtime-owned selective refresh automation 实装 | 已更新 `apps/runtime_support/include/RuntimeLiveDependencyComposition.h`、`apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp`、`tests/integration/knowledge/KnowledgeRuntimeAutoRefreshIntegrationTest.cpp` 与 `tests/integration/knowledge/KnowledgeRefreshLoopTest.cpp`；runtime auto-refresh 现新增 refresh plan/provider seam，default provider 复用 `knowledge::ingest::SourceScanner` inventory diff 生成 selective delta，并把 source URI 归一为 installed descriptor root 对应的绝对路径；`refresh_in_flight` tick 直接 benign skip，provider 无 delta / baseline 失配 / quarantine 时回退 full-scan | `KnowledgeRuntimeAutoRefreshIntegrationTest` 已覆盖 changed-asset selective、注入 provider 的 target-only selective、provider full-scan fallback 与 busy skip；`KnowledgeRefreshLoopTest` 已覆盖 multi-corpus target-only selective regression 与既有 full-scan fallback | `cmake --build build/vscode-linux-ninja --target dasall_knowledge_runtime_auto_refresh_integration_test dasall_knowledge_refresh_loop_integration_test -j2`；`RunCtest_CMakeTools` 继续命中仓库已知泛化 `生成失败` 后直接执行 `./build/vscode-linux-ninja/tests/integration/knowledge/dasall_knowledge_runtime_auto_refresh_integration_test && ./build/vscode-linux-ninja/tests/integration/knowledge/dasall_knowledge_refresh_loop_integration_test && printf "%s\n" PASS`，结果 `PASS` | `KNO-TODO-041` 的口径现已真正闭合：runtime-owned automation 不再只是 periodic full-scan，而具备 selective-first、busy benign skip 与 fallback 分层证据；manual `changed_source -> updated_sources` seam 继续 authoritative，ADR-006 / ADR-007 / ADR-008 owner boundary 未回退 |

#### KNO-TODO-043 完成证据（2026-05-26）

1. `apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp` 已把默认 query encoder 从 env-gated local hash fallback 收口为 provider-backed production path：runtime_support 负责 provider asset 选择、secret materialize、HTTP embedding transport 与 runtime-local health cache，Knowledge 继续只消费 `IQueryEncoder` inbound port，不接管 provider asset / secret / transport concrete owner。
2. `knowledge/include/KnowledgeServiceFactory.h` 与 `knowledge/src/KnowledgeServiceFactory.cpp` 已把 passive health 与 active canary readiness 分离：普通 lexical retrieve 继续只读本地 snapshot，不因为 telemetry/health 路径被动联网；只有 explicit hybrid canary admission 才调用 `runtime_canary_backend_ready(query)`。因此 provider-missing、provider-empty-embedding、invalid-secret 三条负例现在都会在 admission 前 lexical-only fail-closed，而不是先误入 dense lane。
3. `llm/assets/providers/deepseek/models.yaml` 已新增受治理的 `deepseek-embedding` metadata，`tests/integration/access/RuntimeKnowledgeQueryEncoderIntegrationTest.cpp` 已覆盖 provider-ready、provider-missing、provider-empty-embedding、invalid-secret、local-fallback 五类 focused case；provider-ready 正例在关闭 `DASALL_DETACHED_VECTOR_LOCAL_FALLBACK` 后仍返回 Hybrid，并复用 startup probe 预热的 cached embedding。
4. `tests/integration/knowledge/KnowledgeProductionTelemetryIntegrationTest.cpp` 已补 provider-backed telemetry gate：provider-ready 路径会把 `runtime_canary_admitted`、`retrieval_mode=hybrid`、`vector_backend_ready=true` 等 token 写入 shared audit/metrics/trace provider；invalid-secret 路径会把 `runtime_canary_backend_not_ready`、`query_encoder_invalid_secret`、`retrieval_mode=lexical_only` 与 `vector_backend_ready=false` 写入 shared sink。2026-05-26 已通过 `Build_CMakeTools(buildTargets=["dasall_runtime_knowledge_query_encoder_integration_test","dasall_knowledge_production_telemetry_integration_test"])`；`RunCtest_CMakeTools` 对 telemetry gate 继续命中仓库已知泛化 `生成失败`，因此沿用 direct-binary fallback，执行两个 focused binaries 均返回 `PASS`。

#### KNO-TODO-045 完成证据（2026-05-26）

1. `docs/todos/knowledge/deliverables/KNO-TODO-045-runtime-owned-selective-refresh-automation口径闭合双轨任务包.md` 已补到 Build 完成态，明确 045 的最终实现面就是 runtime-owned selective refresh plan/provider seam，而不是继续扩 Knowledge-owned watcher / scheduler。default provider 现复用 `knowledge::ingest::SourceScanner` inventory diff，并将相对 source URI 归一为 installed descriptor root 使用的绝对路径。
2. `apps/runtime_support/include/RuntimeLiveDependencyComposition.h` 与 `apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp` 已把 timer callback 从“固定 empty `CorpusChangeSet`”收口为“selective-first plan 驱动”：`refresh_in_flight=true` 时直接 benign skip；有 delta 时只提交目标 source URI；provider 无 delta / baseline 失配 / quarantine 时才回退 `request_refresh(CorpusChangeSet{})` full-scan。
3. `tests/integration/knowledge/KnowledgeRuntimeAutoRefreshIntegrationTest.cpp` 与 `tests/integration/knowledge/KnowledgeRefreshLoopTest.cpp` 已完成 focused 回归：前者锁定 changed-asset selective、target-only selective、full-scan fallback 与 busy skip，后者锁定 multi-corpus target-only selective 与既有 full-scan fallback/busy reject。2026-05-26 已通过 `cmake --build build/vscode-linux-ninja --target dasall_knowledge_runtime_auto_refresh_integration_test dasall_knowledge_refresh_loop_integration_test -j2`。
4. 最终 direct-binary focused 验收 `./build/vscode-linux-ninja/tests/integration/knowledge/dasall_knowledge_runtime_auto_refresh_integration_test && ./build/vscode-linux-ninja/tests/integration/knowledge/dasall_knowledge_refresh_loop_integration_test && printf "%s\n" PASS` 已返回 `PASS`。`RunCtest_CMakeTools` 在本仓仍只给出泛化 `生成失败`，因此本轮沿用仓库既有 direct-binary fallback 口径。
...
| RTSUP-GAP-002 | 已闭合 | production observability / health sinks 已注入 shared helper | `infra::compose_live_observability()`、`apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp`、`services/include/ServiceLiveComposition.h` 与 `runtime/include/RuntimeDependencySet.h` 已统一注入 shared logger / audit / metrics / trace / health provider，并保留 `:production-observability-health` marker 与 shared health monitor/probes | build-tree focused production composition 已可二值断言 shared sink emit、health aggregate 与 app retention；当前结论不外推为 installed / qemu / release 证据 | 2026-05-27 已由 `RTSUP-FIX-002` 收口；closeout 见 `docs/todos/runtime/deliverables/RTSUP-FIX-002-production-observability-health-closeout.md` |
| RTSUP-GAP-003 | 已闭合 | knowledge optional degraded semantics 与 installed positive evidence 已闭合 | `apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp` 现以 installed positive probe 的 terminal success 决定 `knowledge-installed-assets-ready`，失败则显式写出 `knowledge-degraded:<reason>`；`KnowledgeInstalledAssetProbeIntegrationTest` 与 daemon/gateway composition tests 已共同锁定 ready / degraded retained behavior | build-tree focused evidence 已可二值断言 knowledge optional port 的 marker 与 readiness 语义；当前结论不外推为 installed / qemu / release 证据 | 2026-05-27 已由 `RTSUP-FIX-003` 收口；closeout 见 `docs/todos/runtime/deliverables/RTSUP-FIX-003-knowledge-degraded-installed-probe-closeout.md` |
| RTSUP-GAP-004 | 已闭合 | owner/fail-closed/evidence marker regression matrix 已建立 | `RuntimeLiveCompositionFailureMatrixTest`、`DaemonRuntimeLiveDependencyCompositionTest`、`GatewayRuntimeLiveDependencyCompositionTest` 与 `MultiAgentDisabledByProfileIntegrationTest` 已共同锁定 required-missing fail-closed、daemon/gateway symmetry、knowledge degraded marker stratification 与 multi-agent disabled seam；本轮同时把 failure matrix 从后续 knowledge hybrid/automation 子面收回到 004 原始 contract | 修改 helper 后，不会再把 owner 边界、optional/required 口径或 diagnostics marker 写回历史状态；当前结论不外推为 installed / qemu / release 证据 | 2026-05-27 已由 `RTSUP-FIX-004` 收口；closeout 见 `docs/todos/runtime/deliverables/RTSUP-FIX-004-failure-matrix-closeout.md` |
| RTSUP-GAP-005 | 已闭合 | installed local / release-preflight composition gate 已闭合，qemu handoff 已与 runtime_support owner 解耦 | build-tree `dasall_packaging_preflight_tests` / `dasall_gate_int_10` 已恢复通过；2026-05-27 fresh rebuilt `.deb` 在 `/tmp/rtsup-fix-005-installed-smoke` 下重新产出 `runtime-installed-proof.json`、`runtime-proof.json`、`access-installed-gateway-http-proof.json` 与 `access-installed-async-receipt-proof.json` | runtime_support 自有 L3/L4 证据不再受 qemu 环境拖挂，且 local installed 结果不再与 qemu / release-runner 混写 | 2026-05-27 已由 `RTSUP-FIX-005` 收口；closeout 见 `docs/todos/runtime/deliverables/RTSUP-FIX-005-installed-release-preflight-composition-closeout.md` |
...
| RTSUP-FIX-002 | Done | 接入 production observability 与 health sinks | 已通过 `infra/include/ObservabilityLiveComposition.h`、`services/include/ServiceLiveComposition.h`、`runtime/include/RuntimeDependencySet.h` 与 `apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp` 统一注入 runtime/tools/services 所需 logger、audit、metrics、trace、health provider；closeout 见 `docs/todos/runtime/deliverables/RTSUP-FIX-002-production-observability-health-closeout.md` | `ToolObservabilityIntegrationTest`、`ToolProductionObservabilityIntegrationTest`、`RuntimeProductionHealthCompositionTest`、`DaemonRuntimeLiveDependencyCompositionTest`、`GatewayRuntimeLiveDependencyCompositionTest` 已覆盖 shared sink emit、health aggregate 与 app retention | `RunCtest_CMakeTools(tests=["ToolObservabilityIntegrationTest","ToolProductionObservabilityIntegrationTest","RuntimeProductionHealthCompositionTest","DaemonRuntimeLiveDependencyCompositionTest","GatewayRuntimeLiveDependencyCompositionTest"])` 命中仓库已知泛化 `生成失败` 时，fallback 执行 `build/vscode-linux-ninja/tests/integration/{tools,access}/` 下对应 direct binaries | shared helper 组合出的 live path 已可发出真实 sink event 与 health snapshot，且总账 / 专项 TODO / worklog 口径一致 |
| RTSUP-FIX-003 | Done | 收口 knowledge optional degraded semantics 与 installed positive probe | `apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp` 已按 installed positive probe 的 terminal success 收口 helper 对 knowledge unavailable / degraded / positive 的 marker 与 readiness 语义；closeout 见 `docs/todos/runtime/deliverables/RTSUP-FIX-003-knowledge-degraded-installed-probe-closeout.md` | `KnowledgeInstalledAssetProbeIntegrationTest`、`DaemonRuntimeLiveDependencyCompositionTest`、`GatewayRuntimeLiveDependencyCompositionTest` 已覆盖 direct installed probe 与 app composition ready/degraded retained behavior | `RunCtest_CMakeTools(tests=["KnowledgeInstalledAssetProbeIntegrationTest","DaemonRuntimeLiveDependencyCompositionTest","GatewayRuntimeLiveDependencyCompositionTest"])` 命中仓库已知泛化 `生成失败` 时，fallback 执行 `build/vscode-linux-ninja/tests/integration/{knowledge,access}/` 下对应 direct binaries | knowledge optional port 已不再在 fatal / ready / degraded 三种语义之间漂移，且总账 / 专项 TODO / worklog 口径一致 |
| RTSUP-FIX-004 | Done | 建立 owner / fail-closed / marker regression matrix | `tests/integration/access/RuntimeLiveCompositionFailureMatrixTest.cpp` 已与 daemon/gateway composition tests、`MultiAgentDisabledByProfileIntegrationTest` 共同收口 required ports missing、daemon/gateway symmetry、knowledge unavailable/degraded marker 与 multi-agent disabled seam；本轮同时把 failure matrix 收回 004 原始 scope，避免后续 hybrid/automation 子面误绑 | `DaemonRuntimeLiveDependencyCompositionTest`、`GatewayRuntimeLiveDependencyCompositionTest`、`MultiAgentDisabledByProfileIntegrationTest`、`RuntimeLiveCompositionFailureMatrixTest` 已覆盖 004 所需 focused contract | `RunCtest_CMakeTools(tests=["DaemonRuntimeLiveDependencyCompositionTest","GatewayRuntimeLiveDependencyCompositionTest","MultiAgentDisabledByProfileIntegrationTest","RuntimeLiveCompositionFailureMatrixTest"])` 命中仓库已知泛化 `生成失败` 时，fallback 执行 `build/vscode-linux-ninja/tests/integration/{access,multi_agent}/` 下对应 direct binaries；必要时先 `Build_CMakeTools(buildTargets=["dasall_access_runtime_live_composition_failure_matrix_integration_test"])` | 后续回归不会再把 helper 写回空 composition、owner 不清或 marker 混写，且 matrix gate 已重新与 004 的任务边界对齐 |
| RTSUP-FIX-005 | Done | 建立 installed / release-preflight composition gate（qemu 继续归 packaging） | 通过 fresh rebuilt `.deb` + local package smoke 重新验证 daemon / gateway shared helper 的 release-preflight / installed 分层，并把 qemu handoff 固定回 packaging / release owner | release-preflight matrix、runtime/access installed artifact family | `cmake --build build-ci --target dasall_packaging_preflight_tests dasall_gate_int_10 -j2`；`DASALL_PACKAGE_SMOKE_ARTIFACT_DIR=/tmp/rtsup-fix-005-installed-smoke bash scripts/packaging/pkg_smoke_install.sh --explicit-start-check` | shared helper 的证据层级已从 L2/L3 partial 推进到 L3 release-preflight + L4 local installed，且不再把 qemu / `autopkgtest` 记作当前 runtime_support blocker |
...
`apps/runtime_support` 当前已经解决“谁来在 daemon / gateway 装配 runtime live dependencies”这个 owner 问题，而且 `RTSUP-GAP-001` ~ `RTSUP-GAP-005` 的 runtime_support 自有缺口已全部闭合：services backend、observability/health、knowledge optional semantics、owner/fail-closed regression 与 local installed / release-preflight composition gate 均已落盘。后续更高层 qemu / release-runner 复核继续归 packaging / release 环境，不再回流为 runtime_support owner blocker。
...
| ACC-GAP-003 | 已收口 / v1 single-daemon authority frozen | receipt authority / multi-instance 口径已明确 | 2026-05-27 新增 `AccessReceiptAuthorityIntegrationTest`，验证 sibling daemon 与 restarted daemon 即使复用同一 ownership secret，对 pre-existing receipt 仍返回 `status_missing`；`docs/architecture/DASALL_access子系统详细设计.md` 同步冻结 `AsyncTaskRegistry` 为单 daemon / in-memory authority | Access owner 不再把本地 `AsyncTaskRegistry` 当成跨实例全局事实；installed async evidence 只代表 current daemon authoritative scope | 若未来需要 restart / multi-instance shared authority，再新开 receipt store / provider seam、TTL cleanup 与 cross-process replay tests |
| ACC-GAP-004 | 已收口 / `ACC-GATE-11` 延后边界冻结 | streaming lifecycle 与 shared admission 延后口径已固化 | 2026-05-27 新增 `AccessStreamingDeferralWordingGuardIntegrationTest`，并将 `SystemIntegrationGateMatrix.md` 的 `Gate-INT-08` 明确改写为 Access v1 unary production ingress gate；Access TODO 与 gateway source 注释继续把 `StreamGateway / WS / MQTT` 保持在 `ACC-GATE-11` + feature flag default-off + async receipt/poll fallback | `Gate-INT-08` 不再被误读为 streaming ready，stream disabled regression 已有自动化守卫 | 若未来要进入 stream-ready，需在 runtime / llm / contracts 共同冻结 shared lifecycle 后另开 listener / replay / auth Build 任务 |
| ACC-GAP-005 | 已收口 / Local installed security matrix | access release security matrix 已改由本机 installed proof + focused release-preflight tests 持有 | 2026-05-27 将 `pkg_smoke_install.sh --explicit-start-check`、`DaemonSocketModeIntegrationTest`、`AccessPolicyBackendUnavailableIntegrationTest` 与 Access owner 文档统一收敛为一套 Local installed security matrix；owner mismatch、missing-backend、diag disabled 与 socket/policy fail-closed 不再等待 qemu/autopkgtest 才可陈述 | Access owner 不再把 qemu / autopkgtest 当当前验收前置；installed negative semantics 与 focused release-preflight security seams 已进入统一矩阵 | machine isolation 继续由 packaging / release 环境承接，不回流为 Access owner blocker；不以 qemu / kvm 作为 Access owner 当前验收前置 |
...
| ACC-FIX-003 | Done | 收口 receipt authority / multi-instance 口径 | 已新增 `tests/integration/access/AccessReceiptAuthorityIntegrationTest.cpp` 与详设 / Access TODO closeout，明确 v1 receipt authority 仅属于 issuing daemon 的 in-memory registry；TTL、ownership、cancel 继续保持本地 registry 语义 | `AccessReceiptAuthorityIntegrationTest`、`DaemonReceiptTtlCleanupIntegrationTest`、`DaemonReceiptFlowIntegrationTest` | `Build_CMakeTools(buildTargets=["dasall_access_receipt_authority_integration_test"])`；`RunCtest_CMakeTools(tests=["AccessReceiptAuthorityIntegrationTest","DaemonReceiptTtlCleanupIntegrationTest","DaemonReceiptFlowIntegrationTest"])` 命中仓库已知泛化 `生成失败` 时，fallback 先 `Build_CMakeTools(buildTargets=["dasall_access_daemon_receipt_flow_integration_test"])`，再执行 `./build/vscode-linux-ninja/tests/integration/access/dasall_access_receipt_authority_integration_test && ./build/vscode-linux-ninja/tests/integration/access/dasall_access_daemon_receipt_ttl_cleanup_integration_test && ./build/vscode-linux-ninja/tests/integration/access/dasall_access_daemon_receipt_flow_integration_test` | sibling daemon / restart 边界已被明确，不能再产生“本地 registry 当全局事实”的歧义 |
| ACC-FIX-004 | Done | 固化 streaming 延后与 feature gate | 已新增 `tests/integration/access/AccessStreamingDeferralWordingGuardIntegrationTest.cpp` 并更新 Access TODO / `SystemIntegrationGateMatrix.md` / 顶层总账，明确 `Gate-INT-08` 仅代表 unary v1 focused ingress；`StreamGateway / WS / MQTT` 继续由 `ACC-GATE-11` 管理，stream flag 默认关闭并统一回到 async receipt + poll fallback | `AccessStreamingDeferralWordingGuardIntegrationTest`、`HttpProtocolAdapterTest` | `Build_CMakeTools(buildTargets=["dasall_access_streaming_deferral_wording_guard_integration_test","dasall_access_http_protocol_adapter_unit_test"])`；`RunCtest_CMakeTools(tests=["AccessStreamingDeferralWordingGuardIntegrationTest","HttpProtocolAdapterTest"])` 命中仓库已知泛化“生成失败”时，fallback 执行 `./build/vscode-linux-ninja/tests/integration/access/dasall_access_streaming_deferral_wording_guard_integration_test && ./build/vscode-linux-ninja/tests/unit/access/dasall_access_http_protocol_adapter_unit_test` | `Gate-INT-08` 不再被误读为 streaming-ready，stream disabled/not ready 与 feature gate default-off 有静态守卫和单测回归 |
| ACC-FIX-005 | Done | 建立 access release security matrix | 已把 `pkg_smoke_install.sh --explicit-start-check`、`access-installed-async-receipt-proof.json`、`access-installed-gateway-http-proof.json`、`DaemonSocketModeIntegrationTest` 与 `AccessPolicyBackendUnavailableIntegrationTest` 收敛为 Access owner 的 Local installed security matrix，并明确不以 qemu / kvm 作为 Access owner 当前验收前置 | `AccessInstalledSecurityMatrixWordingGuardIntegrationTest`、`DaemonSocketModeIntegrationTest`、`AccessPolicyBackendUnavailableIntegrationTest` | `Build_CMakeTools(buildTargets=["dasall_access_installed_security_matrix_wording_guard_integration_test","dasall_access_daemon_socket_mode_integration_test","dasall_access_observability_main_chain_integration_test"])`；`RunCtest_CMakeTools(tests=["AccessInstalledSecurityMatrixWordingGuardIntegrationTest","DaemonSocketModeIntegrationTest","AccessPolicyBackendUnavailableIntegrationTest"])` 命中仓库已知泛化“生成失败”时，fallback 执行 `./build/vscode-linux-ninja/tests/integration/access/dasall_access_installed_security_matrix_wording_guard_integration_test && ./build/vscode-linux-ninja/tests/integration/access/dasall_access_daemon_socket_mode_integration_test && ./build/vscode-linux-ninja/tests/integration/access/dasall_access_observability_main_chain_integration_test` | Access owner 当前安全矩阵已由本机 installed proof + focused release-preflight tests 收口；machine isolation 继续归 packaging / release 环境，而不是 Access owner blocker |

### 11.5 当前章节结论

Access / apps ingress 当前主体已具备 focused integration、local installed async receipt positive evidence 与 local installed HTTP gateway unary evidence；`ACC-FIX-003` 已把 receipt authority 明确冻结为 v1 single-daemon / in-memory scope，`ACC-FIX-004` 已把 `Gate-INT-08` 与 `ACC-GATE-11` 的边界统一冻结为 unary-not-stream-ready / feature flag default-off，`ACC-FIX-005` 又把当前安全验收口径收敛为 Local installed security matrix。Access owner 当前已无开放 blocker；后续更高层 machine-isolation / release 环境复核继续由 packaging / release owner 承接，不再把“installed 无 gateway binary / 无 HTTP request”“本地 registry 等于共享 authority”“unary Gate=streaming ready”或“必须先有 qemu / kvm 才能陈述 Access 当前安全矩阵”继续记成 Access owner blocker。
