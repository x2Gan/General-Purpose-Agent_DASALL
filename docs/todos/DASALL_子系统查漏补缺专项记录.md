...
| Gap ID | 严重级别 | 缺口 | 证据 | 影响 | 补缺方向 |
|---|---|---|---|---|---|
| MEM-GAP-001 | High | `sqlite-vss` source-tree / local-staging production 接线已闭合，但 installed/qemu authoritative 证据仍未闭合 | `memory/src/vector/SqliteVssVectorBackend.*` 已改为 loadable extension driver，`MemoryManagerFactory` / `RuntimeLiveDependencyComposition` / packaging asset / `V002__vector_sidecar.sql` 已接通；`SqliteVssVectorBackendTest` real-extension 正向、`VectorMemoryAdapterTest`、`MemoryWritebackIntegrationTest`、`MemoryProfileCompatibilityTest` 与 runtime_support 组合测试均已通过；`/tmp/dasall-mem-fix-001-stage` 已验证带出 `lib/dasall/sqlite-vss/*.so` 与 `share/dasall/sql/memory/V002__vector_sidecar.sql` | 当前可以明确说明 sqlite-vss 已不是“只有 internal seam”的状态，但还不能把 local/staging 证据外推为 installed package qemu / release-ready semantic vector recall | 继续执行 installed package / qemu authoritative gate，并把 asset、migration、profile fail-closed 与 real search hit 的证据回写到 release / SSOT 文档 |
...
| Task ID | 状态 | 任务标题 | 代码目标 | 测试目标 | 建议验收命令 | 完成判定 |
|---|---|---|---|---|---|---|
| MEM-FIX-001 | Blocked | 收口 sqlite-vss production 接线与证据 | 已完成真实 sqlite-vss loadable extension / third_party 依赖接入、`MemoryManagerFactory` production wiring、`SimpleLocalEmbeddingAdapter`、packaging asset / install layout、`V002__vector_sidecar.sql` migration 与 profile fail-closed 策略；剩余 installed package qemu authoritative evidence | 已通过 `SqliteVssVectorBackendTest` real-extension positive / missing、`SimpleLocalEmbeddingAdapterTest`、`VectorMemoryAdapterTest`、`SchemaMigrationTest`、`MemoryWritebackIntegrationTest`、`MemoryProfileCompatibilityTest`、`GatewayRuntimeLiveDependencyCompositionTest`、`RuntimeProductionHealthCompositionTest`；`cmake --install build-ci --prefix /tmp/dasall-mem-fix-001-stage` 已验证 sqlite-vss 资产与 migration | `ctest --test-dir build-ci --output-on-failure -R '^(SimpleLocalEmbeddingAdapterTest|VectorMemoryAdapterTest|SqliteVssVectorBackendTest|SchemaMigrationTest|MemoryWritebackIntegrationTest|MemoryProfileCompatibilityTest|GatewayRuntimeLiveDependencyCompositionTest|RuntimeProductionHealthCompositionTest)$'`；`cmake --install build-ci --prefix /tmp/dasall-mem-fix-001-stage`；package gate 另按 `validate_gate_int_10_installed_package_qemu.sh` 固化 | 仅当 installed package qemu gate 真正执行并回写 sqlite-vss asset、migration、profile fail-closed 与 real search hit 证据后，才能把 local/staging closure 升级为 Done；在此之前不得把 focused/local 结果外推为 release-ready semantic vector recall |
