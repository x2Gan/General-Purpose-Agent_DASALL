...
| access / apps ingress | 已记录 | Access v1 unary focused ingress 已通过 Gate-INT-08；CLI/daemon/gateway app-binary、installed local control-plane、installed async receipt 与 installed HTTP gateway unary local package evidence 已形成 L3/L4 分层；`ACC-GATE-11` 已把 `StreamGateway / WS / MQTT` 固定为 feature flag default-off + async/poll fallback 的延后边界 | installed/release security hardening 证据 |
...
Access / apps ingress 当前主体已具备 focused integration、local installed async receipt positive evidence 与 local installed HTTP gateway unary evidence；`ACC-FIX-003` 已把 receipt authority 明确冻结为 v1 single-daemon / in-memory scope，`ACC-FIX-004` 又把 `Gate-INT-08` 与 `ACC-GATE-11` 的边界统一冻结为 unary-not-stream-ready / feature flag default-off。剩余重点现已收敛到 installed / release security hardening 分账，不再把“installed 无 gateway binary / 无 HTTP request”“本地 registry 等于共享 authority”或“unary Gate=streaming ready”继续记成 Access owner blocker。
