...
| knowledge | 已记录 | 主体实现 L2 基本闭合；Runtime -> Memory evidence 读链路、refresh 异步、首启 prewarm、lane timeout/parallel recall、installed normative corpus baseline、ledger/catalog startup recovery、local installed failure injection、boundary regression 自动化与 production telemetry 已收口 | concrete vector backend |
...
5. 查漏补缺优先级：当前只保留 concrete vector backend 与更高层 guest-side / environment follow-up；不再保留 Knowledge owner 的 local installed 或 boundary regression 缺口。
