...
| CAPSRV-GAP-008 | Low / Medium | installed / release-runner local / soak 证据缺失 | 当前只执行 build-tree focused tests；未执行本机 installed package smoke、release-runner local artifact 固化、真实 external backend、long-running subscription/adapter soak | 不能从 services L2 fixture 全绿外推为生产可用性、长期稳定性或 release-runner local evidence 已闭合 | 增加 installed tools->services positive probe、本机 package-smoke / tools-installed-proof、release-runner local artifact 固化，以及 adapter timeout/overflow soak 与 chaos gate |
...
| CAPSRV-FIX-008 | Todo | 建立 installed / release services 证据 | 增加本机 installed `pkg_smoke_install` / tools-installed-proof 正向路径，显式证明 tools -> services -> adapter/backend；增加 release-runner local artifact 固化与 subscription/adapter soak gate | package smoke 与 local artifact 目录记录 service trace / payload / confirmation-gate evidence；long-running subscription overflow / remote timeout 稳定可复验 | `bash scripts/packaging/validate_ubuntu_dpkg_v1.sh`；`bash scripts/packaging/pkg_smoke_install.sh --explicit-start-check`；installed services-positive artifact 路径需在任务中固化 | services 从 L2 fixture 提升到 L4 local installed / release-runner local / L6 soak evidence，不再仅停留在 build tree |
...
| 优先级 | 范围 | 原因 | 首批任务 |
|---|---|---|---|
| P0 | Memory 口径同步、Runtime path 分层、Access installed async / gateway | 直接影响总账事实正确性与默认主链证据边界 | `MEM-FIX-001`、`RT-FIX-001`、`ACC-FIX-001` |
