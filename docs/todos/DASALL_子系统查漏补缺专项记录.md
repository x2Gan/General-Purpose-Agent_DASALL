...
3. Plugin / MCP / Skill 扩展：plugin lifecycle 自动接线、snapshot 二次分发、MCP v1 stdio-only transport 边界、production observability sink、builtin wrapper 布局、runtime production visible tools surface 与 local installed/release evidence 已补齐；剩余重点转为 Runtime production skill 消费、payload golden regression 与 qemu/soak 证据。
...
查漏补缺优先级：`TOOL-GAP-001` / `TOOL-GAP-002` / `TOOL-GAP-003` / `TOOL-GAP-004` / `TOOL-GAP-005` / `TOOL-GAP-006` / `TOOL-GAP-007` / `TOOL-GAP-008` / `TOOL-GAP-009` 已闭合，`TOOL-FIX-009` / `TOOL-FIX-010` / `TOOL-FIX-011` 已分别完成 local installed / release evidence、Knowledge / Tools boundary 与 runtime production visible tools surface 收口；后续通过 `TOOL-GAP-010` / `TOOL-GAP-011` / `TOOL-GAP-012` 提升 Runtime production skill 消费、payload regression、machine-isolated release 与 soak 可信度。
