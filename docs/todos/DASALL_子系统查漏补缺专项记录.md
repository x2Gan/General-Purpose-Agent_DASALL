...
4. Compensation / bulkhead：受控补偿入口与 lane bulkhead 执行语义已闭合；剩余重点转为 production observability sink、generic MCP 与 production/installed 证据。
...
查漏补缺优先级：`TOOL-GAP-001` / `TOOL-GAP-002` / `TOOL-GAP-003` / `TOOL-GAP-004` / `TOOL-GAP-005` 已闭合；后续先修 `TOOL-GAP-006` / `TOOL-GAP-007` / `TOOL-GAP-008` / `TOOL-GAP-009`，最后通过 `TOOL-GAP-010` / `TOOL-GAP-011` / `TOOL-GAP-012` 提升 runtime production、installed 和 release 可信度。
