...
15. 上述结论只解除了 TUI 的设计 blocker、composer state-machine blocker、selector fake-interaction blocker、transcript-view baseline blocker、status-panel baseline blocker、terminal-probe baseline blocker、renderer-layout baseline blocker 与 fake-only app-loop blocker，不代表 bare `dasall` 命令迁移已可放行；TUI-TODO-021~030 及 031~034 仍需按后续 gate 单独验收。
