...
| knowledge | 已记录 | 主体实现 L2 基本闭合；Runtime -> Memory evidence 读链路已打通；refresh 异步、首启 prewarm、lane timeout/parallel recall、installed normative corpus baseline、ledger/catalog startup recovery、local installed failure injection 与 production telemetry 已收口 | concrete vector backend、boundary regression 自动化 |
