...
Runtime 当前缺的不是控制面骨架，也不再是 path evidence 分层或 durable checkpoint / resume gate，而是取消超时、可观测健康、production degraded semantics、执行模型与更高层 full-path evidence 的剩余闭合。后续优先顺序为 `RT-GAP-004`，再推进 `RT-GAP-005` / `RT-GAP-006` / `RT-GAP-007` / `RT-GAP-008`。
