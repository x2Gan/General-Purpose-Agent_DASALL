...
截至当前已闭合的 TUI owner 缺口见 17.3 表；其中设计决策类已闭合五项，Build 基线已闭合十四项，本轮新增收口 `TUI-TODO-019` 的 renderer adapter / design tokens / main-layout snapshot 基线。
...
5. `/clear` 保留 input history，清空当前 composer draft；下一次 submit 绑定新的 `session_id`，但即时 daemon close/open 继续后置到 session seam 设计，不在本轮越权承诺。
...
12. `TuiModelSelector` 已冻结为 fake route catalog 下的纯本地 selector helper：支持 Auto / PreferDepth / PinModel 三模式、depth tier 聚合、disabled reason 展示、local draft/apply/cancel，以及 disabled route/depth 的 fail-closed 行为；它仍不接 real carrier、daemon route catalog 或 submit echo。
...
15. 上述结论只解除了 TUI 的设计 blocker、composer state-machine blocker、selector fake-interaction blocker、transcript-view baseline blocker、status-panel baseline blocker、terminal-probe baseline blocker、renderer-layout baseline blocker、fake-only app-loop blocker、IPC mapping/header blocker、IPC error-normalization blocker、daemon data source attach blocker、startup failure path blocker与 status projection refresh blocker，不代表 bare `dasall` 命令迁移已可放行；TUI-TODO-026~030 及 031~034 仍需按后续 gate 单独验收。
...
1. `TUI-TODO-026` 尚未把已冻结的 daemon/access projection seam 继续落盘为 foreground session close/query 真链路；其中 `TUI-TODO-021~025` 已闭合 mapping/header、controller normalization、daemon data source attach、startup failure path baseline 与 status/tool/recovery projection refresh evidence，`/clear` 的用户可见语义也已闭合，但 backend close/open 细节仍后置。
2. `TUI-TODO-027~029` 尚未把已冻结的 `NextTurnPreference` carrier 继续落盘为 route catalog projection、daemon selector consumption 与 submit echo。
3. `TUI-TODO-020` 已闭合 fake-only app loop，但仍受 CJK/IME/resize manual gate 与 installed path packaging review 约束；本轮结果不等于 full-screen interactive promotion、daemon/session/route 真链路 ready 或 release path 已 ready。
4. `TUI-TODO-030~034` 的 bare `dasall` 迁移仍保持 Blocked；本轮结论不能被误写成 command release ready。
