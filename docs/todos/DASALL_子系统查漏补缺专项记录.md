...
Runtime 当前缺的不是控制面骨架，也不再是 path evidence 分层、durable checkpoint / resume gate、deadline / cancellation 主链、production observability / health、optional degraded semantics 或 L3/L4 full-path evidence，而是 scheduler / background worker 执行模型与更高层 release-runner / soak 证据的剩余闭合。后续优先顺序收敛为 `RT-GAP-007`，再推进 packaging / release 环境中的更高层 runtime evidence。
