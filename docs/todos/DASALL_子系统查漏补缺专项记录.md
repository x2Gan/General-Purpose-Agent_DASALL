...
15. 上述结论只解除了 TUI 的设计 blocker、composer state-machine blocker、selector fake-interaction blocker、transcript-view baseline blocker、status-panel baseline blocker、terminal-probe baseline blocker、renderer-layout baseline blocker、fake-only app-loop blocker、IPC mapping/header blocker 与 IPC error-normalization blocker，不代表 bare `dasall` 命令迁移已可放行；TUI-TODO-023~030 及 031~034 仍需按后续 gate 单独验收。
...
2. `TUI-TODO-023~026` 尚未把已冻结的 daemon/access projection seam 继续落盘为 data source、startup failure 与 session open/close/query 真链路；其中 `TUI-TODO-021~022` 已闭合 mapping/header 草案与 controller normalization baseline，`/clear` 的用户可见语义也已闭合，但 backend close/open 细节仍后置。
