...
结论：TUI 目前达到 L1 设计可信，并已补齐十九项 L2 设计/Build 基线，但仍不能外推为 app-binary ready，更不能外推为 installed end-user ready。

截至当前已闭合的 TUI owner 缺口见 17.3 表；权限/clear 语义、projection seam、fake-only prototype、daemon-backed startup/status/session lifecycle、route catalog projection 字段基线与 focused topology baseline 均已闭合，剩余缺口集中在 daemon selector consumption、submit echo、terminal manual gate 与 bare `dasall` release gate。
...
12. `TuiModelSelector` 已冻结为 fake route catalog 下的纯本地 selector helper：支持 Auto / PreferDepth / PinModel 三模式、depth tier 聚合、disabled reason 展示、local draft/apply/cancel，以及 disabled route/depth 的 fail-closed 行为；027 已补齐 route catalog projection 字段基线，但 selector 本体仍不接 real carrier、daemon route catalog consumption 或 submit echo。
...
15. 上述结论只解除了 TUI 的设计 blocker、composer state-machine blocker、selector fake-interaction blocker、transcript-view baseline blocker、status-panel baseline blocker、terminal-probe baseline blocker、renderer-layout baseline blocker、fake-only app-loop blocker、IPC mapping/header blocker、IPC error-normalization blocker、daemon data source attach blocker、startup failure path blocker、status projection refresh blocker、session lifecycle 接线 blocker 与 route catalog projection blocker，不代表 bare `dasall` 命令迁移已可放行；TUI-TODO-028~030 及 031~034 仍需按后续 gate 单独验收。
...
1. `TUI-TODO-028~029` 尚未把已冻结的 `NextTurnPreference` carrier 继续落盘为 daemon selector consumption 与 submit echo；027 已补 route catalog projection 字段基线。
