...
Runtime 当前缺的不是控制面骨架，也不再是 path evidence 分层，而是生产路径、完整设计路径、恢复可靠性、取消超时、可观测健康和 release evidence 的剩余闭合。后续优先顺序为 `RT-GAP-003` / `RT-GAP-004`，再推进 `RT-GAP-005` / `RT-GAP-006` / `RT-GAP-007` / `RT-GAP-008`。
