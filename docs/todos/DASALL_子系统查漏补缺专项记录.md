...
3. 源码缺陷：`DataQueryLane` cache hit result triad、subscription public ABI trace、production adapter registry / backend handler、dynamic capability snapshot/provider 与 production observability/health sinks 都已由 `CAPSRV-FIX-002` / `CAPSRV-FIX-003` / `CAPSRV-FIX-004` / `CAPSRV-FIX-005` / `CAPSRV-FIX-006` 闭合；当前残余重点已上移到 caller-domain owner 与 installed/release 证据。
4. Backend / production：build-tree production adapter registry / handler composition、dynamic route view 与 shared observability/health sinks 均已闭合，但 `CAPSRV-GAP-007` 的 caller-domain owner 与 `CAPSRV-GAP-008` 的 installed/qemu/soak 证据仍需补。
5. 查漏补缺优先级：从 `CAPSRV-GAP-007` 开始，随后通过 `CAPSRV-GAP-008` 提升 installed、release 与长稳态可信度。
