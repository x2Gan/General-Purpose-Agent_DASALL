...
3. 源码缺陷：`DataQueryLane` cache hit result triad 已由 `CAPSRV-FIX-002` 闭合；当前剩余显性源码缺口主要是 subscription public ABI trace 覆盖不完整。
...
5. 查漏补缺优先级：先修 `CAPSRV-GAP-003`，再收口 `CAPSRV-GAP-004` / `CAPSRV-GAP-005` / `CAPSRV-GAP-006` / `CAPSRV-GAP-007`，最后通过 `CAPSRV-GAP-008` 提升 installed、release 与长稳态可信度。
