...
结论：TUI 目前达到 L1 设计可信，并已补齐十七项 L2 设计/Build 基线，但仍不能外推为 app-binary ready，更不能外推为 installed end-user ready。

截至当前已闭合的 TUI owner 缺口见 17.3 表；其中设计决策类已闭合五项，Build 基线已闭合十二项，本轮新增收口 `TUI-TODO-017` 的 status panel fake 展示基线。
...
13. 上述结论只解除了 TUI 的设计 blocker、composer state-machine blocker、selector fake-interaction blocker、transcript-view baseline blocker 与 status-panel baseline blocker，不代表 bare `dasall` 命令迁移已可放行；TUI-TODO-018~030 及 031~034 仍需按后续 gate 单独验收。
...
4. `TUI-TODO-018` 仍待沿着已完成的 fake replay、slash parser、composer 状态机、selector fake 交互、transcript view 与 status panel 基线，继续把 terminal capability probe 落到 focused tests。
5. `TUI-TODO-019~020` 仍受 CJK/IME/resize manual gate 与 installed path packaging review 约束；FTXUI third-party 接入策略已由 `TUI-TODO-005` 收口。
6. `TUI-TODO-030~034` 的 bare `dasall` 迁移仍保持 Blocked；本轮结论不能被误写成 command release ready。
