...
最近更新时间：2026-04-09（CAP-TODO-018 已完成，ExecutionSubscriptionHub 订阅骨架已落盘）
阶段：Detailed Design -> Special TODO  
适用范围：services/  
当前结论：D1 已完成；CAP-TODO-015/016/017/018 已为 D2 落盘 low-risk `ExecutionCommandLane`、static `CompensationCatalog`、query-only `ExecutionQueryLane` 与 internal `ExecutionSubscriptionHub`，Execution 子域现已具备幂等键缓存、关键动作串行化、补偿提示目录、strict/allow_stale freshness 分支，以及 cursor/batch 订阅骨架、`drop_oldest` overflow、`resync_required` / `dropped_count` 可观测基础；高风险 action 仍按 CAP-GATE-08 fail-closed，下一直接执行入口转为 CAP-TODO-019、020、022、023、028，剩余前置门禁主要是 query/diagnose-only integration smoke、ServiceAuditBridge、loopback fixture 与 shared-contract admission。
