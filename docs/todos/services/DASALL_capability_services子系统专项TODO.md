...
最近更新时间：2026-04-09（CAP-TODO-012 已完成，C 补设计解阻推进中）  
...
4. 但 AdapterSelection、AdapterReceipt、ResultMapper、ServicePolicyView 中部分 internal-only supporting objects 未形成完整字段表；integration fixture 仍停留在职责或建议层。
...
| ExecutionCommandLane | 6.2、6.3、6.7、6.8、7.1 | L2 | 命令路径、幂等键、串行化、错误分类、补偿提示明确 | AdapterSelection、AdapterReceipt 映射缺口仍在 | 先补设计，再推进实现 |
...
| CAP-TODO-034 | Blocked | 回写 services 专项 Gate 与交付证据 | 详细设计 9.4、10.2、11、12.2；本专项 TODO | 9.4 Gate；11 风险与阻塞；12.2 后续任务 | L2 | docs/todos/services/DASALL_capability_services子系统专项TODO.md | Gate、阻塞、验收、回退记录回写 | process：命令证据、阻塞变更、风险残留全部回写 | `cmake -S . -B build-ci -G "Unix Makefiles" && cmake --build build-ci --target dasall_services dasall_unit_tests dasall_contract_tests dasall_integration_tests && ctest --test-dir build-ci -N && ctest --test-dir build-ci --output-on-failure -L unit && ctest --test-dir build-ci --output-on-failure -L contract && ctest --test-dir build-ci --output-on-failure -L integration` | CAP-TODO-011、029、030、031、032、033 | CAP-BLK-002、CAP-BLK-003、CAP-BLK-004、CAP-BLK-005 | 相关 blocker 全部关闭且 Gate 已实际执行 | 更新后的专项 TODO 与执行证据 | 仅当每个 Gate 都有通过/失败结论、命令证据、阻塞状态和后续动作回写时完成 |
...
| 工程可实现性 | 部分直接可执行 | A-B-C 阶段可直接进入执行；D1（Adapter Build）与 D2 阶段需先关闭 CAP-BLK-002~004 |
...
2. CAP-TODO-013~014：剩余两个前置补设计任务。
...
3. Execution / Query / Diagnose / Subscription / Adapter / Observability / Integration 深链路：当前只能落到 L2，且若依赖 CAP-BLK-002~004 未关闭，则必须保持 Blocked。
...
2. 高风险 action taxonomy 与 require_confirmation 对齐规则已在 CAP-TODO-012 冻结，但 AdapterSelection / AdapterReceipt 仍未成表，命令路径暂不能安全落盘到函数级。
