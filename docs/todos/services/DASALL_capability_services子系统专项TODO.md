...
最近更新时间：2026-04-09（CAP-TODO-020 已完成，DataProjectionCache 缓存骨架已落盘）
阶段：Detailed Design -> Special TODO  
适用范围：services/  
当前结论：D1 已完成；CAP-TODO-015/016/017/018/019/020 已为 D2 落盘 low-risk `ExecutionCommandLane`、static `CompensationCatalog`、query-only `ExecutionQueryLane`、internal `ExecutionSubscriptionHub`、diagnose-only `ExecutionDiagnoseService` 与 `DataProjectionCache`，Execution/Data 子域现已具备幂等键缓存、关键动作串行化、补偿提示目录、strict/allow_stale freshness 分支、cursor/batch 订阅骨架、`drop_oldest` overflow、`resync_required` / `dropped_count` 可观测基础，以及 `target_reachable` / `report_json` 只读诊断语义和 TTL/stale/from_cache 缓存骨架；高风险 action 仍按 CAP-GATE-08 fail-closed，下一直接执行入口转为 CAP-TODO-021、022、023、028，剩余前置门禁主要是 query/diagnose-only integration smoke、ServiceAuditBridge、loopback fixture 与 shared-contract admission。
...
| CAP-TODO-021 | Todo | 实现 DataQueryLane 查询车道 | 详细设计 6.2、6.3、6.7、7.1 | 7.1 DataQueryLane.cpp；9.1 data query test | L2 | services/src/data/DataQueryLane.cpp | query()、list_capabilities()、projection/filter 路径 | unit：stale read / cache hit-miss / projection 构建可测 | `cmake -S . -B build-ci -G "Unix Makefiles" && cmake --build build-ci --target dasall_services dasall_unit_tests && ctest --test-dir build-ci --output-on-failure -L unit` | CAP-TODO-010、011、020 | 无 | 完成 CAP-TODO-020 | DataQueryLane.cpp 与数据车道 unit 用例 | 仅当 DataQueryLane 维持 query-only 语义、显式标记 from_cache 且不掩盖 stale 事实时完成 |
...
2. CAP-TODO-018、019、020、022、023、028：订阅 / 诊断车道，DataProjectionCache，SystemSnapshotLane，ServiceConfigAdapter 与 loopback fixture 方案。
