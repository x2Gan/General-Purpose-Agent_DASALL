...
最近更新时间：2026-04-09（CAP-TODO-036 已完成，D1 Adapter / ResultMapper Build 推进中）  
阶段：Detailed Design -> Special TODO  
适用范围：services/  
当前结论：可继续推进 D1；当前最细可安全落到 L3（公共 supporting objects / 公共接口级），内部 lane / adapter / bridge 以 L2 为主；高风险动作、route 输入与 receipt mapping 三项补设计已收敛，AdapterRouter 与 AdapterBridge 已落盘并通过 unit 验证，下一直接执行入口为 LocalPlatformAdapter / LocalServiceAdapter / RemoteServiceAdapter / ResultMapper，剩余前置门禁主要是 loopback fixture 与 shared-contract admission。
...
4. loopback integration fixture 与 shared-contract admission 仍停留在职责或评审门层；D1 Adapter / ResultMapper Build 尚未落盘为实际代码。
5. 因此可以直接拆公共 ABI、D1 Adapter / ResultMapper Build 和部分 internal skeleton 任务，但不能提前承诺 integration/admission 的函数级落盘。
...
| ExecutionCommandLane | 6.2、6.3、6.7、6.8、7.1 | L2 | 命令路径、幂等键、串行化、错误分类、补偿提示明确 | D1 Adapter / ResultMapper 代码路径尚未落盘 | 先完成 D1，再推进实现 |
...
| DataQueryLane | 6.2、6.3、6.7、7.1 | L2 | 查询职责、projection/filter 输入、测试槽位明确 | D1 Adapter / ResultMapper 代码路径尚未落盘 | 依赖 D1 完成后实现 |
...
| AdapterRouter / AdapterBridge / LocalPlatformAdapter / LocalServiceAdapter / RemoteServiceAdapter | 6.2、6.3、6.4、7.1、8.3 | L1→L2 | 路由输入输出、三类 adapter 槽位、测试槽位明确 | AdapterBridge / 三类 adapter 尚未落盘 | 先推进 CAP-TODO-035，再按 CAP-TODO-036~039 分步落盘 AdapterBridge 和三类 Adapter |
| ResultMapper | 6.2、6.3、6.8；7.1 | L1→L2 | 错误分类表（ServiceErrorClass→ErrorInfo mapping）、side_effects / compensation_hints / evidence 约束已冻结 | AdapterBridge 尚未提供稳定 receipt fixture | 先完成 CAP-TODO-036，再通过 CAP-TODO-040 落盘 ResultMapper 实现 |
...
| CAP-TODO-017 | Blocked | 实现 ExecutionQueryLane 只读查询车道 | 详细设计 6.2、6.3、6.7、7.1 | 7.1 ExecutionQueryLane.cpp；6.8 DataStale / AdapterUnavailable | L2 | services/src/execution/ExecutionQueryLane.cpp | query_state 路径、freshness 处理、只读错误映射 | unit：只读查询不产生 side_effects | `cmake -S . -B build-ci -G "Unix Makefiles" && cmake --build build-ci --target dasall_services dasall_unit_tests && ctest --test-dir build-ci --output-on-failure -L unit` | CAP-TODO-008、009、010、011 | 无 | 完成 CAP-TODO-035~040 | ExecutionQueryLane.cpp 与查询车道 unit 用例 | 仅当查询路径不触发隐式写入、可区分 strict / allow_stale 语义且错误映射稳定时完成 |
| CAP-TODO-018 | Blocked | 实现 ExecutionSubscriptionHub 订阅骨架 | 详细设计 6.2、6.3、6.6.1、6.9.2、9.3 | 6.6.1 subscribe 收口；6.9.2 `drop_oldest` | L2 | services/src/execution/ExecutionSubscriptionHub.cpp | subscribe、cursor/batch、`resync_required`、`dropped_count` | unit：overflow 后必须置 `resync_required`；process：回链 InfraConcurrencyPolicy | `cmake -S . -B build-ci -G "Unix Makefiles" && cmake --build build-ci --target dasall_services dasall_unit_tests && ctest --test-dir build-ci --output-on-failure -L unit && rg -n "drop_oldest|resync_required|InfraConcurrencyPolicy" docs/architecture/DASALL_capability_services子系统详细设计.md docs/ssot/InfraConcurrencyPolicy.md` | CAP-TODO-008、010、011 | 无 | 完成 CAP-TODO-035~040 | ExecutionSubscriptionHub.cpp 与订阅 unit 用例 | 仅当公共 ABI 只暴露 cursor/batch 语义、overflow 可观测且 internal buffer 实现未泄漏时完成 |
| CAP-TODO-019 | Blocked | 实现 ExecutionDiagnoseService 诊断路径 | 详细设计 6.2、6.3、6.7、7.1 | 6.3 ExecutionDiagnoseService；7.1 execution diagnose scope | L2 | services/src/execution/ | diagnose 路径、`target_reachable`、`report_json` | unit：诊断路径只读、不可改变目标状态 | `cmake -S . -B build-ci -G "Unix Makefiles" && cmake --build build-ci --target dasall_services dasall_unit_tests && ctest --test-dir build-ci --output-on-failure -L unit` | CAP-TODO-008、010、011 | 无 | 完成 CAP-TODO-035~040 | execution/ 诊断组件与 unit 用例 | 仅当诊断只返回 reachability / report 事实，不替代 infra diagnostics 导出时完成 |
...
| CAP-TODO-021 | Blocked | 实现 DataQueryLane 查询车道 | 详细设计 6.2、6.3、6.7、7.1 | 7.1 DataQueryLane.cpp；9.1 data query test | L2 | services/src/data/DataQueryLane.cpp | query()、list_capabilities()、projection/filter 路径 | unit：stale read / cache hit-miss / projection 构建可测 | `cmake -S . -B build-ci -G "Unix Makefiles" && cmake --build build-ci --target dasall_services dasall_unit_tests && ctest --test-dir build-ci --output-on-failure -L unit` | CAP-TODO-010、011、020 | 无 | 完成 CAP-TODO-035~040 | DataQueryLane.cpp 与数据车道 unit 用例 | 仅当 DataQueryLane 维持 query-only 语义、显式标记 from_cache 且不掩盖 stale 事实时完成 |
...
| 工程可实现性 | 部分直接可执行 | A-B-C 阶段已完成；CAP-TODO-020、022、023、028、037~040 可直接进入执行；其余 D2 / integration 阶段需按 `037~040 -> 015~021` 顺序推进，并继续受 CAP-BLK-004~005 约束 |
...
2. CAP-TODO-020、022、023、028、037~040：DataProjectionCache、SystemSnapshotLane、ServiceConfigAdapter、loopback fixture 方案、三类 Adapter 与 ResultMapper。
...
3. Execution / Query / Diagnose / Subscription / Adapter / Observability / Integration 深链路：当前只能落到 L2；其中 D1 Adapter / ResultMapper 可按 `037~040` 推进，D2 与 integration 仍需等待 D1 完成并继续受 CAP-BLK-004~005 约束。

### 11.3 阻止进一步细化到全量函数级的证据缺口

1. D1 Adapter / ResultMapper 代码路径已开始落盘，但 lane 的全量函数级实现仍依赖 037~040 继续完成。
...
2. CAP-TODO-035 与 CAP-TODO-036 已完成，当前按 `037 -> 038 -> 039 -> 040` 的顺序推进；CAP-TODO-040 应以 CAP-TODO-036 产出的 AdapterReceipt fixture 作为稳定输入。
...
5. CAP-TODO-024~027 继续依赖 D2 结果面与 CAP-TODO-023，不应跳过车道实现直接落 observability 终态代码。
