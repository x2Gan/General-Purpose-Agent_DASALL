...
最近更新时间：2026-04-09（CAP-TODO-025 已完成，ServiceMetricsBridge 已接入命令/查询/缓存/overflow 指标链路）
阶段：Detailed Design -> Special TODO  
适用范围：services/  
当前结论：D1 已完成；CAP-TODO-015~025 已为 D2 / Phase 4 落盘 low-risk `ExecutionCommandLane`、static `CompensationCatalog`、query-only `ExecutionQueryLane`、internal `ExecutionSubscriptionHub`、diagnose-only `ExecutionDiagnoseService`、`DataProjectionCache`、`DataQueryLane`、internal-only `SystemSnapshotLane`，以及 `RuntimePolicySnapshot` / `BuildProfileManifest` -> internal `ServicePolicyView` 的统一派生入口 `ServiceConfigAdapter`、接入 `ExecutionCommandLane` 的 internal `ServiceAuditBridge` 和接入命令/查询/缓存/overflow 链路的 internal `ServiceMetricsBridge`。Execution/Data/System 子域现已具备幂等键缓存、关键动作串行化、补偿提示目录、strict/allow_stale freshness 分支、cursor/batch 订阅骨架、`drop_oldest` overflow、`resync_required` / `dropped_count` 可观测基础、`target_reachable` / `report_json` 只读诊断语义、TTL/stale/from_cache 缓存骨架、统一数据 query/catalog 只读入口、供 health/internal orchestration 使用的系统快照骨架，以及 lane worker、timeout/circuit、TTL/stale-read、safe_mode/high-risk/audit/caller domain、platform/observability 开关的统一内部 policy 基线、高风险/补偿/fallback_blocked 审计发射能力，以及请求量/时延分布/route-unavailable 熔断代理、缓存 hit/miss、订阅 overflow 与补偿提示的指标发射能力；高风险 action 仍按 CAP-GATE-08 fail-closed，015~025 串行链已闭合，下一直接执行入口转为 CAP-TODO-026、028，剩余前置门禁主要是 query/diagnose-only integration smoke、ServiceTraceBridge、ServiceHealthProbe、loopback fixture 与 shared-contract admission。
