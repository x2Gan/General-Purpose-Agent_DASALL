...
3. gate 证据只统计真实 `add_test(NAME ... COMMAND <executable>)` 项；不得再用 `cmake -E true` placeholder 冒充 discoverability 或通过率证据。

## 2. 评审结论

1. 旧实现为满足早期 TODO 正则，引入了 `CognitionProfileCompatibilityIntegrationTest` placeholder alias，底层命令是 `cmake -E true`。
...
2. 输出面：
   ...
   - gate 文档不再把 `CognitionProfileCompatibilityIntegrationTest` 视为通过项。
3. 保持不变：
...
2. `CognitionProfileCompatibilityIntegrationTest` 已不再出现在 `ctest -N` 输出中。
3. cognition integration CMake 中已无 `cmake -E true` placeholder 注册。
...
2. `ctest -N` 中不存在由 `cmake -E true` 驱动的 cognition gate。
