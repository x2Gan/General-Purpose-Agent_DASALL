...
6. 后续边界：BLD-010 只冻结 structured telemetry / diagnostics 字段与 redaction 边界，不提前绑定 production telemetry sink，也不提前把 schema drift / safety negative matrix 混入同轮；production sink 继续由 `COG-FIX-005` 收口，`Gate-COG-FIX004A-04` 仍待 `COG-FIX-004A-BLD-011` 完成后统一转 Pass。
...
5. 不可外推范围：本轮只统一回写 cognition L1 / L2 structured projection 主链证据，不把 installed / qemu / soak evidence 或 production telemetry sink 外推为已完成；这些仍分别留在总账 L 层级与 `COG-FIX-005` 中继续收口。
