...
| Gate-COG-12 | Changes Requested / Pending | COG-TODO-039 ~ 044；2026-05-15 在 `build-ci-cog042` / `build-ci-cog044` 复验后确认：cognition scope 的 build 聚合、response/reflection 语义、详设追溯与 warning hygiene 已完成，但统一 gate 仍被 runtime / llm / tools / infra / platform / profiles / apps-daemon 非 cognition blocker 阻断 | 在 repo-wide blocker 清零前，不应宣称 cognition 具备完整可合并验收证据 |
...
| Gate-COG-FIX004A-04 | Pass | COG-FIX-004A-BLD-008 ~ 011 | runtime interaction、integration regression、telemetry fields 与 safety negative matrix 已具 focused evidence |
| Gate-COG-FIX004A-05 | Pass | COG-FIX-004A-BLD-012；总账附录 A / worklog 最终回写完成 | `COG-FIX-004` 完成证据、统一验收命令与不可外推范围已跨文档一致 |
...
| Risk ID | 当前状态 | 关闭/缓解证据 | 残余说明 |
|---|---|---|---|
| COG-R19 | Closed | COG-TODO-039 + COG-TODO-042；`RuntimeCognitionLoopSmokeTest` 已在 clean `build-ci-cog042` focused regex 中通过 | 统一 gate 当前 blocker 已转移到非 cognition repo-wide 残余 |
| COG-R20 | Closed | COG-TODO-039；042 复验未再出现 runtime canonical stage route / init fixture 失败信号 | 无新增 runtime fixture 风险 |
| COG-R21 | Closed | COG-TODO-040；042 复验 remaining blocker 为 executable 缺失，不是 response bridge / fallback 口径冲突 | 若后续修改 response 语义，仍需单独回归 integration 套件 |
| COG-R22 | Closed | COG-TODO-041 + COG-TODO-042；`CognitionFacadeFlowTest` 在 clean `build-ci-cog042` focused regex 中通过 | active PlanGraph façade 映射未见回退 |
| COG-R23 | Closed | COG-TODO-043、044 已收口详设当前状态与 cognition warning hygiene；`build-ci-cog044` 聚焦验收 3/3 通过且未再出现 `policy_snapshot` missing initializer warning | cognition scope 内与 Gate-COG-12 相关的 docs / warning hygiene 残余已关闭；统一 gate 继续受非 cognition blocker 影响 |
...
### 13.34 COG-FIX-004A-BLD-009：Runtime structured ActionDecision contract 收敛（2026-05-15）

1. 任务选择：在 BLD-008 已证明 structured payload 能贯通 snapshot-backed cognition `decide()` 主链后，最小下一跳是把证据延伸到 Runtime interaction contract，证明 Runtime 消费的是 projection 后的 module-public `ActionDecision`，而不是 provider JSON；同时把 `ExecuteAction`、`DirectResponse`、`ConvergeSafe` 三类 decision_kind 的 first hop / terminal handoff 全部补齐 structured case。
2. 代码交付物：更新 `tests/mocks/include/MockCognitionFixture.h`，补充 `StructuredExecutionPayloadScenario::ValidConvergeSafe`；更新 `tests/integration/cognition/CognitionRuntimeInteractionContractTest.cpp`，新增一个只代理 real structured `decide()` 的 cognition wrapper、一个 `ContractProbeToolManager`，并基于 `MockCognitionFixture` + `create_cognition_engine(snapshot, dependencies)` 扩展 structured ExecuteAction、DirectResponse、ConvergeSafe 三类 runtime contract case。
3. 测试对齐：structured ExecuteAction case 断言 Runtime 把 projected `tool_intent_hint.tool_name` / `argument_hints` 映射为 `ToolRequest.tool_name` / `arguments_payload`，并把 projected `selected_node_id` 与 tool-round `latest_observation` 传给 response builder；structured DirectResponse / ConvergeSafe cases 断言 Runtime 跳过 tool dispatch，直接把 projected terminal `ActionDecision` 与 `response_outline.summary` 传给 response builder。这样 Runtime contract 证明锚点保持在 module-public `ActionDecision`，而不是 raw provider payload。
4. 验收命令：
   - `cmake --build build-ci --target dasall_cognition_runtime_interaction_contract_integration_test`
   - `ctest --test-dir build-ci --output-on-failure -R "CognitionRuntimeInteractionContractTest"`
5. 验收结论：PASS；`CognitionRuntimeInteractionContractTest` 通过，新增 structured ExecuteAction / DirectResponse / ConvergeSafe cases 已证明 Runtime first hop、response builder 与 tool intent handoff 消费的是 projected `ActionDecision`。其中 ExecuteAction structured case 还验证了 tool dispatch 后会把 projected `latest_observation` 回送给 response builder。
6. 后续边界：BLD-009 只收 Runtime interaction contract 证据，不提前把 telemetry structured fields 或 schema drift / safety negative matrix 纳入同轮；这些继续由 `COG-FIX-004A-BLD-010` ~ `011` 收口，`Gate-COG-FIX004A-04` 仍保持 Pending。

### 13.35 COG-FIX-004A-BLD-010：structured telemetry / diagnostics fields 收敛（2026-05-15）

1. 任务选择：在 BLD-008 / 009 已分别补齐 structured integration 与 Runtime interaction focused evidence后，最小下一跳是把附录 A.7 冻结的 structured projection 字段真正接到 telemetry owner 与 façade diagnostics 上，避免 projection source、schema version、failure code 与 projected counts 仍只停留在设计文档而没有可执行证据。
2. 代码交付物：更新 `cognition/src/observability/CognitionTelemetry.h` / `.cpp`，为 `StageTelemetryContext` 新增 `StructuredProjectionTelemetry` 载体，并把 `structured_projection_enabled`、`structured_projection_required`、`structured_schema_version`、`structured_projection_source`、`structured_projection_failure_code`、`projected_node_count`、`projected_candidate_count` 接到 log / metric / trace / audit 事件字段。更新 `cognition/src/CognitionFacade.cpp`，为 planning / execution structured bridge path补齐 `structured_projection.enabled|required|schema_version|source|failure_code|projected_*_count` diagnostics，并在 `decide()` 出口把这些 diagnostics 汇总为 telemetry context，保证 stage 结果事件与 diagnostics taxonomy 同源。
3. 测试对齐：更新 `tests/unit/cognition/CognitionTelemetryFieldsTest.cpp`，断言 completed / failed event 能携带 structured projection 字段；更新 `tests/unit/cognition/CognitionFacadeStructuredPlanOutputTest.cpp` 与 `tests/unit/cognition/CognitionFacadeStructuredActionOutputTest.cpp`，分别覆盖 planning success / schema fallback 与 execution success / invariant fail-closed 场景下的 schema version、authoritative source、failure code 与 projected count diagnostics。
4. 验收命令：
   - `cmake --build build-ci --target dasall_cognition_telemetry_fields_unit_test dasall_cognition_facade_structured_plan_output_unit_test dasall_cognition_facade_structured_action_output_unit_test`
   - `ctest --test-dir build-ci --output-on-failure -R "CognitionTelemetryFieldsTest"`
   - `ctest --test-dir build-ci --output-on-failure -R "CognitionFacadeStructuredPlanOutputTest"`
   - `ctest --test-dir build-ci --output-on-failure -R "CognitionFacadeStructuredActionOutputTest"`
5. 验收结论：PASS；`CognitionTelemetryFieldsTest`、`CognitionFacadeStructuredPlanOutputTest` 与 `CognitionFacadeStructuredActionOutputTest` 全部通过。现已证明 structured projection success / failure 在 owner 级 telemetry 字段和 façade diagnostics 两侧都可观测，同时未记录 raw payload、provider-private fields 或未裁剪上下文内容。
6. 后续边界：BLD-010 只冻结 structured telemetry / diagnostics 字段与 redaction 边界，不在同轮混入 schema drift / safety negative matrix；production telemetry sink 已于 2026-05-16 由 `COG-FIX-005` 收口，但这不改写 BLD-010 的原始完成范围与验收结论。

2026-05-15 BLD-011 回写结果：`cognition/src/projection/PlanGraphStructuredProjector.cpp` 与 `cognition/src/projection/ActionDecisionStructuredProjector.cpp` 已补齐 `schema_version` 校验以及 top-level / nested unexpected-field fail-closed guard，避免 `provider_payload`、`delegate_hint`、`tool_intent_hint.arguments_payload` 这类 schema drift / safety overreach 被静默吞掉；`tests/unit/cognition/StageOutputValidatorSchemaTest.cpp`、`tests/unit/cognition/PlanGraphStructuredProjectionTest.cpp`、`tests/unit/cognition/ActionDecisionStructuredProjectionTest.cpp` 与 `tests/integration/cognition/CognitionStructuredOutputIntegrationTest.cpp` 已分别覆盖 schema version mismatch、provider-private leakage、tool arg overreach、delegate hint disabled，以及 planning fallback / execution fail-fast 的 integration regression。验收命令 `cmake --build build-ci --target dasall_stage_output_validator_schema_unit_test dasall_plan_graph_structured_projection_unit_test dasall_action_decision_structured_projection_unit_test dasall_cognition_structured_output_integration_test && ctest --test-dir build-ci --output-on-failure -R "^StageOutputValidatorSchemaTest$" && ctest --test-dir build-ci --output-on-failure -R "StructuredProjectionTest" && ctest --test-dir build-ci --output-on-failure -R "^CognitionStructuredOutputIntegrationTest$"` 通过，因此 `COG-FIX-004A-BLD-011` 完成，`Gate-COG-FIX004A-04` 转为 Pass；本轮只补 negative matrix 与 fail-closed guard，不提前回写 `COG-FIX-004` 总完成证据，该项继续由 `COG-FIX-004A-BLD-012` 收口。

### 13.36 COG-FIX-004A-BLD-012：COG-FIX-004 完成证据与 Gate 结果收口（2026-05-15）

1. 任务选择：在 `Gate-COG-FIX004A-04` 已于 BLD-011 转为 Pass 后，最小下一跳就是统一回写 `COG-FIX-004` 的最终状态、统一验收命令与不可外推范围，避免总账、专项 TODO 与 worklog 对同一专项出现“子任务全 Done 但总任务仍 Todo”的口径漂移。
2. 文档交付物：更新总账中的 `COG-FIX-004` 总任务行与附录 A，把 `COG-FIX-004A-BLD-012` 置为 Done，并将 `Gate-COG-FIX004A-05` 由 Pending 转为 Pass；同步更新本专项 TODO 的对应任务行、当前 Gate 状态与本节回写记录；更新 `docs/worklog/DASALL_开发执行记录.md`，补一条只面向证据收口的执行记录。
3. 一致性与验收命令：统一使用 `rg -n "COG-FIX-004" docs/todos/DASALL_子系统查漏补缺专项记录.md docs/todos/cognition/DASALL_cognition子系统专项TODO.md docs/worklog/DASALL_开发执行记录.md && rg -n "Gate-COG-FIX004A" docs/todos/DASALL_子系统查漏补缺专项记录.md docs/todos/cognition/DASALL_cognition子系统专项TODO.md docs/worklog/DASALL_开发执行记录.md` 做 docs consistency，再使用 `ctest --test-dir build-ci --output-on-failure -R "StageSchemaRegistryTest|StageOutputValidatorSchemaTest|PlanGraphStructuredProjectionTest|ActionDecisionStructuredProjectionTest|CognitionFacadeStructuredPlanOutputTest|CognitionFacadeStructuredActionOutputTest|CognitionStructuredOutputIntegrationTest|CognitionRuntimeInteractionContractTest|CognitionTelemetryFieldsTest"` 复核最终 focused evidence。
4. 验收结论：PASS；`COG-FIX-004` 已从 Todo 改为 Done，`Gate-COG-FIX004A-05` 已转为 Pass，且 A.13 的完成判定 1 ~ 8 已全部满足。
5. 不可外推范围：本轮只统一回写 cognition L1 / L2 structured projection 主链证据，不把 installed / qemu / soak evidence 外推为已完成；2026-05-16 的 production telemetry sink 接线已由 `COG-FIX-005` 单独收口，但仍不能替代 L4 installed / qemu / soak 证据。

2026-05-16 追加回写：`cognition/include/CognitionDependencies.h` 已扩展 live audit/metrics/tracing provider 依赖，`cognition/src/observability/CognitionTelemetry.cpp` 已把 live provider 适配为 production sink，`cognition/src/CognitionFacade.cpp` 与 `cognition/src/response/ResponseBuilder.cpp` 已分别接入 stage result 与 degraded response emit，`runtime/src/AgentFacade.cpp` / `apps/runtime_support/src/RuntimeLiveDependencyComposition.cpp` 也已透传 shared observability providers。新增 `tests/integration/cognition/CognitionProductionTelemetryIntegrationTest.cpp` 后，`CognitionTelemetryFieldsTest`、`CognitionTelemetryRedactionTest`、`CognitionTelemetryFailureIsolationTest` 与 `CognitionProductionTelemetryIntegrationTest` 在 `build` 目录下均已通过，因此 `COG-FIX-005` 已完成；同轮总账、详设与 worklog 的当前状态也已统一回写，`COG-FIX-006` 一并完成。
6. 2026-05-15 widened acceptance 补证：authority/fallback 收口后，`StagePolicyResolver.cpp`、`CognitionFacade.cpp`、`tests/fixtures/runtime/CognitionRuntimeIntegrationFixture.h` 与 `CognitionProfileCompatibilityTest.cpp` 已同步收口 profile-aware fallback、config-only degraded path 与 structured runtime fixture。`RunCtest_CMakeTools` 复核 `CognitionFacadeStageTimeoutTest`、`CognitionFacadeDegradedModeTest`、`CognitionProfileCompatibilityTest`、`CognitionRuntimePolicyProjectionIntegrationTest`、`StagePolicyResolverTest`、`StagePolicyResolverProfileDiffTest` 六个 blocker / owner tests 全绿，并进一步把当前 49 个 cognition unit / integration tests 全量复跑为绿，因此 `Gate-COG-FIX004A-05` 现已具 wider acceptance 补充证据。
7. 宽语义边界：依据 `COG-TODO-032` / `COG-TODO-040` 的既有冻结口径，`edge_minimal` 的 terminal status 与 reflection emission 仍不在当前固定 contract 内；`CognitionProfileCompatibilityTest` 现只冻结该 profile 的“显式终态必须存在”，以及“若发出 reflection bridge，则 route / timeout 必须来自真实 snapshot 投影”，不把 non-factory response bridge 或更宽 terminal semantics 提前宣称为已冻结。
