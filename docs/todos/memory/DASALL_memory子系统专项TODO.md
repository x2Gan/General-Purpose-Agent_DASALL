...
最近更新时间：2026-04-20（完成 MEM-TODO-001/002/003/004/005/006/007/008A/008B/009/009A/010/011/012/013/014/015/016/017/018/022/025）  
阶段：Detailed Design -> Special TODO  
适用范围：memory/  
当前结论：Memory 子系统已具备 L3/L2 混合粒度的专项拆分条件；`memory/include` 根与 `config/context/error/vector/working/writeback` 稳定子目录、`IMemoryManager.h`、`IContextOrchestrator.h`、`IMemoryStore.h`、`IStoreTransaction.h`、`ISummarizer.h`、`MaintenanceRequest.h`、`MaintenanceReport.h`、`working/IWorkingMemoryBoard.h`、`working/WorkingMemorySnapshot.h`、`working/WorkingMemoryExportRequest.h`、`working/WorkingMemoryExportResult.h`、`vector/IEmbeddingAdapter.h`、`vector/VectorMemoryIndexAdapter.h`、`config/MemoryConfig.h`、`context/MemoryContextRequest.h`、`context/ContextAssemblyResult.h`、`error/MemoryError.h`、`writeback/MemoryWritebackRequest.h`、`writeback/WritebackResult.h`、`writeback/SummaryGenerationRequest.h`、`writeback/SummaryGenerationResult.h`、`writeback/SummaryProjection.h`、`MemoryBuildSkeleton.cpp`、`src/MemoryManager.cpp`、`src/MemoryManagerFactory.cpp`、`src/working/WorkingMemoryBoard.cpp`、`src/store/sqlite/SqliteSchemaMigrator.cpp`、`src/store/sqlite/SqliteMemoryStore.cpp`、`src/store/sqlite/RowMappers.cpp`、`src/vector/UnavailableVectorMemoryIndexAdapter.cpp`、`src/context/CandidateCollector.h`、`src/context/CandidateCollector.cpp`、`src/context/BudgetAllocator.h`、`src/context/BudgetAllocator.cpp`、`src/writeback/CompressionCoordinator.h`、`src/writeback/CompressionCoordinator.cpp`、`sql/memory/V001__initial_schema.sql`、`MemoryInterfaceCompileTest`、`MemoryManagerLifecycleTest`、`MemoryManagerSmokeTest`、`WorkingMemoryBoardTest`、`WorkingMemorySnapshotTest`、`WorkingMemoryBoardConcurrencyTest`、`SchemaMigrationTest`、`SqliteTransactionTest`、`SqliteMemoryStoreTest`、`CandidateCollectorTest`、`CandidateCollectorVectorOffTest`、`BudgetAllocatorTest`、`ContextOrchestratorBudgetTest`、`CompressionCoordinatorTest`、`CompressionCoordinatorSummarizerTest`、`VectorMemoryAdapterTest`、`tests/mocks/include/FakeMemoryStore.h`、`tests/integration/memory/` 以及 `MemoryIntegrationTopologySmokeTest` 已落盘，memory 模块的 unit / integration 测试拓扑已进入顶层聚合且可被发现；public manager/orchestrator/store/summarizer/vector/working 接口、context / writeback request/result ABI、working export / maintenance supporting objects、summary/store/vector supporting objects、`MemoryConfig` 配置模型与 `MemoryError` 错误映射面已冻结第一批 supporting types，`MemoryManager` 生命周期骨架、`WorkingMemoryBoard` 核心、SQLite schema migration baseline、`SqliteMemoryStore` 的 Session/Turn/Summary/Fact/Experience/maintenance 基线、`CandidateCollector` 的多源候选收集路径、`BudgetAllocator` 的 slot budget / trim action 预算基线以及 `CompressionCoordinator` 的模板压缩 / 增量合并 / summarizer fallback 路径均已可执行，`011 ~ 018` 已完成，后续可继续进入 `ContextOrchestrator` 与 `WritebackCoordinator` 收口主链，仅剩 concrete vector backend 选型仍是独立评审 blocker。
...
| D Context 组装链 | MEM-TODO-016 ~ 019 | 016/017/018 已完成；019 串联收口 | `CandidateCollector`、`BudgetAllocator` 与 `CompressionCoordinator` 已稳定多源收集、slot budget、模板压缩、增量合并与 summarizer fallback 基线，下一步进入 `ContextOrchestrator` 完成 10 槽位映射；022 已前移，不再形成阶段循环 |
...
1. MEM-TODO-001 ~ 007、008A、008B、009、009A、010、011、012、013、014、015、016、017、018、022、025 已完成；公共 ABI、测试拓扑、vector unavailable baseline、manager lifecycle skeleton、working-board 核心、schema migration baseline、sqlite store 全路径以及 Context 组装链中的候选收集 / 预算 / 压缩基线均已收口。
2. 再进入 MEM-TODO-019 ~ 021 的主链实现，按 Context -> Writeback 的顺序收口。
