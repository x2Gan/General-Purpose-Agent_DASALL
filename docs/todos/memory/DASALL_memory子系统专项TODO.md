...
最近更新时间：2026-04-18（完成 MEM-TODO-001/002/003/004/005/006/007/008A/008B/009/009A/010/011/012/013/022/025）  
阶段：Detailed Design -> Special TODO  
适用范围：memory/  
当前结论：Memory 子系统已具备 L3/L2 混合粒度的专项拆分条件；`memory/include` 根与 `config/context/error/vector/working/writeback` 稳定子目录、`IMemoryManager.h`、`IContextOrchestrator.h`、`IMemoryStore.h`、`IStoreTransaction.h`、`ISummarizer.h`、`MaintenanceRequest.h`、`MaintenanceReport.h`、`working/IWorkingMemoryBoard.h`、`working/WorkingMemorySnapshot.h`、`working/WorkingMemoryExportRequest.h`、`working/WorkingMemoryExportResult.h`、`vector/IEmbeddingAdapter.h`、`vector/VectorMemoryIndexAdapter.h`、`config/MemoryConfig.h`、`context/MemoryContextRequest.h`、`context/ContextAssemblyResult.h`、`error/MemoryError.h`、`writeback/MemoryWritebackRequest.h`、`writeback/WritebackResult.h`、`writeback/SummaryGenerationRequest.h`、`writeback/SummaryGenerationResult.h`、`writeback/SummaryProjection.h`、`MemoryBuildSkeleton.cpp`、`src/MemoryManager.cpp`、`src/MemoryManagerFactory.cpp`、`src/working/WorkingMemoryBoard.cpp`、`src/store/sqlite/SqliteSchemaMigrator.cpp`、`src/vector/UnavailableVectorMemoryIndexAdapter.cpp`、`sql/memory/V001__initial_schema.sql`、`MemoryInterfaceCompileTest`、`MemoryManagerLifecycleTest`、`MemoryManagerSmokeTest`、`WorkingMemoryBoardTest`、`WorkingMemorySnapshotTest`、`WorkingMemoryBoardConcurrencyTest`、`SchemaMigrationTest`、`SqliteTransactionTest`、`VectorMemoryAdapterTest`、`tests/mocks/include/FakeMemoryStore.h`、`tests/integration/memory/` 以及 `MemoryIntegrationTopologySmokeTest` 已落盘，memory 模块的 unit / integration 测试拓扑已进入顶层聚合且可被发现；public manager/orchestrator/store/summarizer/vector/working 接口、context / writeback request/result ABI、working export / maintenance supporting objects、summary/store/vector supporting objects、`MemoryConfig` 配置模型与 `MemoryError` 错误映射面已冻结第一批 supporting types，`MemoryManager` 生命周期骨架、`WorkingMemoryBoard` 核心与 SQLite schema migration baseline 均已可执行，后续主链可以直接进入 `SqliteMemoryStore` 的 Session/Turn/Summary 与 Fact/Experience 路径，仅剩 concrete vector backend 选型仍是独立评审 blocker。
...
| C 生命周期与主存储基线 | MEM-TODO-011 ~ 015 | 011/012/013 已完成；014 依赖 013；015 依赖 014 | 已落 `MemoryManager` 生命周期骨架、`WorkingMemoryBoard` 核心与 SQLite schema migration baseline；下一步进入 store 主路径，再打开 Fact/Experience/maintenance 支持 |
...
1. MEM-TODO-001 ~ 007、008A、008B、009、009A、010、011、012、013、022、025 已完成；公共 ABI、测试拓扑、vector unavailable baseline、manager lifecycle skeleton、working-board 核心与 schema migration baseline 均已收口。
2. 再进入 MEM-TODO-014 ~ 021 的主链实现，按 Store -> Context -> Writeback 的顺序收口。
