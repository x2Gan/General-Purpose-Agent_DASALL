...
| INF-LOG-FIX-008 | audit split、audit ref schema 与 owner handoff | `AuditLinkAdapter`、`LoggingFacade`、audit route wiring | audit route/persistence/correlation tests | L2 build-tree audit route/persistence/correlation evidence |
| INF-LOG-FIX-009 | query artifact、retention、cleanup policy | `LogQueryService`、`FileLogReader`、`LogRetentionPolicy` | `LogQueryServicePersistedReaderTest`、`LoggingDiagnosticsArtifactIntegrationTest`、`LogRetentionPolicyTest` | L3 query artifact evidence |
...
7. owner-safe allowlist 当前固定为 `request_id`、`session_id`、`trace_id`、`task_id`、`parent_task_id`、`lease_id`、`event_name`、`event_kind`、`evidence_ref`、`evidence_kind`、`audit_ref_pending`、`audit_trace_id`、`audit_task_id`、`schema_version`、`correlation_id`、`idempotency_key`；这些字段允许保留，但其 value 一旦命中文本模式，仍必须被 redaction。
...
### 6.7 BLK-INF-LOG-007 audit route classifier / audit ref schema / privacy split freeze

1. `INF-LOG-FIX-008` 的 v1 high-risk classifier 现在冻结为三条 owner-safe 路径：`LogEvent.category() == "audit"`、`LogLevel::Fatal`，以及 attrs 显式声明 `event_kind=high_risk`。普通 `LogLevel::Error` 若没有这个显式 high-risk marker，仍留在 ordinary log route，不得因为“错误级别较高”就静默升级成 audit route。
2. `AuditLinkAdapter` 只接受完整 `AuditRef`：`evidence_ref.kind/ref`、`trace_id`、`task_id` 必须同时存在且不使用 unknown 默认值。高风险事件一旦缺失完整 ref，必须 fail-closed 返回 `ValidationFieldMissing`，并且不得留下部分 `audit_*` attrs 进入后续 route。
3. ordinary log 允许保留的 audit correlation attrs 固定为 `audit_ref_pending`、`evidence_ref`、`evidence_kind`、`audit_trace_id`、`audit_task_id`；其中 `audit_ref_pending` 只承担 route hint，`evidence_kind` 只允许序列化 `tool_result`、`recovery_outcome`、`worker_task` 三个 frozen enum 名称，`audit_trace_id` / `audit_task_id` 只作为 correlation anchor，不扩写新的 top-level log contract。
4. privacy split 在本轮正式冻结：ordinary log 继续只写 redacted message 与上述 audit anchor attrs；audit owner persistence 才拥有 `actor`、`action`、`target`、`outcome`、`side_effects` 等完整 audit payload。`AuditLinkAdapter`、`SinkDispatcher` 与后续 query/export 均不得把这些 audit payload 字段混入普通日志，也不得开放对 audit 主存储的 join/export 旁路。
5. 与 `docs/architecture/DASALL_infra_audit模块详细设计.md` 对齐，`target`、`evidence_ref.ref`、`side_effects` 在 v1 只承接结构化标识或 effect 名称，不得承载 access token、password、session id、原始文件路径或其他高敏感原文；若 owner-safe attrs 的值命中 redaction 文本模式，仍必须继续脱敏，而不是因为它属于 allowlist 就豁免。
6. `INF-LOG-GATE-005` 在 `INF-LOG-FIX-008` 的 focused 证据现固定为 `AuditLinkAdapterBoundaryContractTest`、`LoggingAuditRouteIntegrationTest`、`AuditLinkAdapterPersistenceTest` 与 `AuditLogCorrelationContractTest`：前两者守住 audit anchor attrs 与 route freeze，后两者闭合 audit owner handoff、privacy split 与 correlation contract。当前结论只到 L2 build-tree route/persistence/correlation evidence，不把它外推成 installed authoritative evidence。

### 6.8 BLK-INF-LOG-008 query artifact retention / admin boundary / redaction-at-query freeze

1. `INF-LOG-FIX-009` 的 query artifact surface 现在冻结为 default-disabled/admin-only：daemon / CLI / installed command surface 默认不得开放 log query artifact；只有 `infra.logging.export.enable_diag_pull == true` 且调用方携带 `PolicyDecision::Allow` 的完整 allow proof 时，`LogQueryService` 才允许 materialize 本地 artifact。`RequireConfirmation` / `Deny` 与未启用 surface 都继续 fail-closed，不在 logging 内部做二次授权或提权。
2. `diag://infra/logging/query/<query_id>` 继续只表示 diagnostics local artifact 引用；其 persisted artifact 与 index 只能落在本地 diagnostics artifact root，不得变成 primary runtime log owner、remote export target 或任意全文搜索入口。query index 只允许保存 `artifact_ref/query_id/selector_kind/selector_value/checksum/match_count/truncated/created_at` 这类 owner-safe metadata，不额外保存 raw record body 或未脱敏 attrs 副本。
3. redaction-at-query 在本轮正式冻结：即使源日志已经过 `RedactionFilter`，query artifact materialization 与 index write 仍必须对 matched record 再走一次 redaction，保证 `message`、attrs，以及 `evidence_ref` / `audit_*` allowlist 字段中若命中 secret/token/password/auth 文本模式，仍保持 redacted，而不是因为它们来自 persisted log 或 allowlist 就绕过脱敏。
4. retention cleanup 只允许作用于 query artifact / index 自身：按 `created_at` retention window 与 max artifact count 清理过期 artifact 与陈旧 index entry，但不得删除或截断 primary runtime log、rotation family、audit owner persistence 或其他 diagnostics retained snapshot artifact。
5. `INF-LOG-GATE-005` 在 `INF-LOG-FIX-009` 的 blocker freeze 证据当前固定为 `LoggingProductionAcceptanceContractTest`；待 build-track 实现后，再由 `LogQueryServicePersistedReaderTest`、`LoggingDiagnosticsArtifactIntegrationTest` 与 `LogRetentionPolicyTest` 闭合 persisted reader/index/materialization/cleanup 的 L2/L3 证据。本轮结论仍只到 L1 design / SSOT freeze，不把 default-disabled/admin-only 口径外推成 query artifact 已实现。
...
5. `INF-LOG-GATE-005` 在 `INF-LOG-FIX-009` 的 focused 证据现固定为 `LogQueryServicePersistedReaderTest`、`LoggingDiagnosticsArtifactIntegrationTest` 与 `LogRetentionPolicyTest`；相邻回归继续保留 `LogQueryServiceTest` 与 `LogQueryIntegrationTest`，证明既有 query summary 契约在引入 artifact/index/cleanup 后没有回退。`RunCtest_CMakeTools` 仍命中仓库既有泛化 `生成失败`，因此 authoritative evidence 继续采用 `Build_CMakeTools` + direct-binary fallback。
