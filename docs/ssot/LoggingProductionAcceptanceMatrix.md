...
6. `INF-LOG-GATE-005` 在 `INF-LOG-FIX-008` 的 focused 证据固定为 `AuditLinkAdapterBoundaryContractTest`、`LoggingAuditLinkIntegrationTest` 与后续 `AuditLogCorrelationContractTest`：前两者守住 audit anchor attrs 与 route freeze，后者再补 audit owner handoff/persistence correlation。当前 blocker 只闭合 L1 design / SSOT freeze，不把 build-tree route fixture 外推成 installed authoritative evidence。
