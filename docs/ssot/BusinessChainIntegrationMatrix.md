...
| BC-04 | Async receipt / query / cancel / replay | L3/L4 partial | 新增 `FullIntAsyncRecoveryCausalityTest` 覆盖 `AsyncTaskRegistry` ownership、`ResultReplayCache` trace/ref、owner-matched `RuntimeBridge::cancel()` forwarding；`AccessAsyncReceiptQueryCancelIntegrationTest` 同轮通过；installed package `run --async` 返回 completed/no receipt，missing receipt `status_missing` / `cancel_missing` fail-closed | build-tree async causality 已有独立 cross-chain 证据；installed package 当前未产真实 receipt，不能宣称 package async ready | FULLINT-TODO-013 |
