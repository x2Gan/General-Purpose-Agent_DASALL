...
3. `bash scripts/packaging/pkg_smoke_install.sh --explicit-start-check` 完成 fresh reinstall、validate-only、explicit start、control-plane、same-session 双轮 LLM、status/cancel/diag 与 assets 断言，退出码为 `0`；2026-05-18 同轮 artifact 已固定 `run-first.json`、`run-second.json`、`memory-proof.json`。
...
| BC-16 | Packaging / installed-package / release handoff | L5 | `FULLINT-TODO-013` 重新构包、fresh reinstall、explicit start、package smoke 与人工 LLM/control-plane 矩阵均通过；PKG-TODO-018 已记录 qemu `autopkgtest` authoritative PASS 与 `lintian RC=0`；2026-05-18 `.github/workflows/release-package-gate.yml` 已在 qemu gate 前固定 local installed memory evidence，`pkg_smoke_install.sh` 可产出 `run-first.json`、`run-second.json`、`memory-proof.json` | BC-16 已具 package handoff 历史 L5 锚点，且本轮 release-runner contract 已补本机 installed memory authoritative owner | 当前 release candidate 仍需 FULLINT-TODO-019 或后续 packaging / release qemu 复核任务重跑并归档当轮 artifact；不外推为 L6 soak / production confidence |
...
| installed-package 证据必须分 local、LLM origin、memory-proof、qemu、lintian | `pkg_smoke_install.sh`、`sudo dasall run`、`validate_gate_int_10_installed_package_qemu.sh`、`.github/workflows/release-package-gate.yml` | FULLINT-TODO-002、013、019、MEM-FIX-006 |
