...
| BC-08 | Knowledge retrieve / evidence projection | L3/L4 contract partial | `make_knowledge_query()` 仍保持 Runtime default lexical-only；`knowledge retrieve/refresh/health` 已具 CLI/daemon 控制面、request-scoped `preferred_mode` / corpus / tag / language 输入、vector explain payload 与 focused gates；runtime composition 现补齐 env-gated local query encoder fallback，installed proof / soak 脚本的 `--hybrid-canary` 已升级为 strict positive gate（必须 `mode=hybrid`、`runtime_canary_admitted`、`vector_backend_ready=true`、`dense_hit_count>0`） | 显式 CLI/daemon canary 链路已有 build-tree true-integration 与 installed local gate contract；默认 AgentOrchestrator 主链仍为 lexical-only，只有显式 allowlisted canary 请求才允许 Hybrid；本轮 strict installed proof / soak 尚需在新包上重跑后才能提升为当轮 L4 positive evidence，不外推 qemu / release-runner / soak | FULLINT-TODO-014；KNO-TODO-034~041；post-eval strict installed rerun |
