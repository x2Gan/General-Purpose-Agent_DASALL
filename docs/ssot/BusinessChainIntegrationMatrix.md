...
| BC-02 | HTTP gateway unary ingress | L4 | `GatewayBinaryUnarySmokeTest`、`GatewayBinaryMissingBackendRegressionTest` 与 `HttpGatewaySubmitIntegrationTest` 保持 build-tree app-binary / composition 证据；2026-05-22 rebuilt `dasall-daemon` package 已携带 `/usr/sbin/dasall-gateway`，`pkg_smoke_install.sh --explicit-start-check` 新增 `access-installed-gateway-http-proof.json`，固定 `READY runtime_readiness=default-ready`、`submit.status=200` 与 missing-backend no-listener fail-closed | build-tree gateway unary / missing-backend 与 local installed package positive HTTP unary evidence 已闭合到 single-instance gateway owner；仍不外推 policy deny release matrix、streaming、multi-instance 或 qemu/release-runner 结论 | ACC-FIX-003、ACC-FIX-004、ACC-FIX-005 或后续 release boundary work |
...
| BC-04 | Async receipt / query / cancel / replay | L4 | `FullIntAsyncRecoveryCausalityTest` 覆盖 `AsyncTaskRegistry` ownership、`ResultReplayCache` trace/ref、owner-matched `RuntimeBridge::cancel()` forwarding；`AccessAsyncReceiptQueryCancelIntegrationTest` 同轮通过；2026-05-22 `pkg_smoke_install.sh --explicit-start-check` 新增 `access-installed-async-receipt-proof.json`，固定 `accepted_async` submit/replay、`status_owner_mismatch` / `cancel_owner_mismatch` exit 4 与 owner-matched `cancelled` terminal status | build-tree async causality 与 local installed package positive receipt evidence 已闭合到 daemon owner；仍不外推 multi-instance authority、HTTP gateway 或 qemu/release-runner 结论 | ACC-FIX-002、ACC-FIX-003、ACC-FIX-005 或后续 release boundary work |
...
| BC-08 | Knowledge retrieve / evidence projection | L3/L4 local positive partial | `make_knowledge_query()` 仍保持 Runtime default lexical-only；`knowledge retrieve/refresh/health` 已具 CLI/daemon 控制面、request-scoped `preferred_mode` / corpus / tag / language 输入、vector explain payload 与 focused gates；runtime composition 现补齐 env-gated local query encoder fallback，installed proof / soak 脚本的 `--hybrid-canary` 已升级并兑现为 strict positive gate；2026-05-26 KNO-TODO-044 重构包后 `/tmp/dasall-kno-todo-044-proof-strict/knowledge-proof.json` 记录 `mode=hybrid`、`runtime_canary_admitted`、`vector_backend_ready=true`、`dense_hit_count=3`，10 轮 `/tmp/dasall-kno-todo-044-soak-strict/knowledge-soak-summary.json` 记录 `hybrid_mode_values=["hybrid"]` 与 `min_hybrid_dense_hit_count=3` | 显式 CLI/daemon canary 链路已有 build-tree true-integration 与 local installed positive evidence；默认 AgentOrchestrator 主链仍为 lexical-only，只有显式 allowlisted canary 请求才允许 Hybrid；本轮只闭合 local installed L4，不外推 qemu / release-runner / production provider SLA | FULLINT-TODO-014；KNO-TODO-034~044；KNO-TODO-043 / 045 后续闭环 |
...
| BC-16 | Packaging / installed-package / release handoff | L5 | `FULLINT-TODO-013` 重新构包、fresh reinstall、explicit start、package smoke 与人工 LLM/control-plane 矩阵均通过；PKG-TODO-018 已记录 qemu `autopkgtest` authoritative PASS 与 `lintian RC=0`；2026-05-21 `pkg_smoke_install.sh` 已产出 `run-first.json`、`run-second.json`、`memory-proof.json`、`runtime-installed-proof.json`、`runtime-proof.json`；2026-05-27 `RTSUP-FIX-005` 又在 fresh rebuilt packages 上重跑 `pkg_smoke_install.sh --explicit-start-check`，重新生成 `access-installed-gateway-http-proof.json`、`access-installed-async-receipt-proof.json`、`runtime-installed-proof.json` 与 `runtime-proof.json` | BC-16 已具 package handoff 历史 L5 锚点，且本轮再次确认 local installed authoritative artifact family 可在 fresh rebuilt packages 上复验；qemu / release-runner 仍保持更高层 packaging / release owner 边界 | 当前 release candidate 仍需 FULLINT-TODO-019 或后续 packaging / release qemu 复核任务重跑并归档当轮 artifact；不外推为 L6 soak / production confidence |
