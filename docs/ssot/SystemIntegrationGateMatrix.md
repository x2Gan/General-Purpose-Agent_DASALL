...
| `Gate-INT-05` | true integration | diagnostics retained snapshot gate | `InfraDiagnosticsSmokeTest`、`InfraDiagnosticsIntegrationTest` focused command | retained snapshot round-trip 被系统 Gate 稳定守住；installed / daemon `diag_disabled` 仅属 admin boundary，不计入本 Gate | integration TODO、worklog | 若仅 diagnostics 局部对象单测通过，或只看到 installed / daemon `diag_disabled`，仍不得宣称系统 diagnostics ready |
...
| `Gate-INT-08` | true integration | Access v1 unary production ingress gate | Access focused integration / contract command | Access production path、mock/test profile 与 production 证据分层清晰；`StreamGateway / WS / MQTT` 继续由 `ACC-GATE-11` 持有延后 Gate，feature flag default-off，`Gate-INT-08` 不得被误读为 streaming-ready | access TODO、integration TODO、worklog | mock pipeline、ping liveness、局部 envelope 字段不得冒充通过 |
