...
namespace dasall::tui::app {
    namespace {
        ...
        void append_attr(std::vector<std::pair<std::string, std::string>>& attrs, std::string key,
                         std::string value) {
        ...
        void append_optional_attr(std::vector<std::pair<std::string, std::string>>& attrs,
        ...
        void insert_log_attr(dasall::infra::logging::LogEvent::AttributeMap& attrs, std::string key,
                             std::string value) {
            if (key.empty() || value.empty()) {
                return;
            }
        ...
        void handle_interactive_resize_signal(int) {
            interactive_resize_requested = 1;
        }
        ...
        [[nodiscard]] bool same_interactive_size(const InteractiveTerminalSize& left,
        ...
        [[nodiscard]] bool escape_sequence_complete(std::string_view sequence) noexcept {
            ...
            if (prefix == '[') {
                for (std::size_t index = 2U; index < sequence.size(); ++index) {
                    if (is_escape_sequence_final_byte(
                            static_cast<unsigned char>(sequence[index]))) {
                        return true;
                    }
                }
                return false;
        ...
        void insert_printable_text(std::string& draft, std::size_t& cursor_offset,
        ...
        /**
         * @brief Applies a reduced action to the screen model.
         * 
         * @param screen_model The screen model to be updated.
        ...
        transcript_max_scroll_offset(const std::size_t total_line_count,
        ...
        [[nodiscard]] std::string session_modal_body(const data::TuiSessionView& session,
        ...
        [[nodiscard]] std::string status_summary_message(const data::TuiStatusProjection& status) {
            ...
            message << "stage=" << (status.stage.empty() ? "unknown" : trim_copy(status.stage))
                    ...
                    << ", health="
                    << (status.health_summary.empty() ? "unknown"
                                                      : trim_copy(status.health_summary));
            return message.str();
        }

        /**
         * @brief Creates a TUI action to display a banner.
         * 
         * @param level The severity level of the banner.
         * @param title The title of the banner.
         * @param message The message content of the banner.
         * @param reason_code An optional reason code for the banner.
         * @param debug_reason A debug reason for the banner.
         * @param sticky Whether the banner should be sticky.
         * @return model::TuiAction 
         */
        [[nodiscard]] model::TuiAction make_banner_action(const model::TuiBannerLevel level,
        ...
        [[nodiscard]] model::TuiAction make_modal_action(model::TuiModalState modal,
                                                         std::string debug_reason) {
            TuiAction action;
        ...
        [[nodiscard]] model::TuiAction make_route_action(data::TuiModelRouteProjection route,
                                                         std::string debug_reason) {
            TuiAction action;
        ...
        [[nodiscard]] model::TuiAction
        make_transcript_message_action(std::string role, std::string content, std::string timestamp,
                                       std::vector<std::string> badges, std::string debug_reason) {
            TuiAction action;
        ...
        [[nodiscard]] model::TuiAction
        make_foreground_session_reset_action(std::string debug_reason) {
            TuiAction action;
        ...
        [[nodiscard]] data::TuiStatusProjection make_baseline_status(std::string startup_mode) {
        ...
        [[nodiscard]] bool
        startup_issue_means_daemon_unavailable(const data::TuiDataSourceIssue& issue) {
        ...
        [[nodiscard]] bool
        composer_waiting_for_response(const model::TuiComposerState& composer) noexcept {
    ...
    TuiApp::TuiApp(terminal::TuiTerminalCapabilityProbe probe) : probe_(std::move(probe)) {}

    int TuiApp::run(TuiAppOptions options) {
        initialize_components(options);

        if (require_logger_ && logger_ == nullptr) {
            last_error_ = "TUI startup blocked: client logging unavailable for production mode.";
        ...
        if (data_source_ == nullptr) {
            log_tui_event(dasall::infra::logging::LogLevel::Error, "tui.startup",
                          "TUI startup failed before data source initialization",
                          {{"outcome", "failure"}, {"reason_code", "data_source_missing"}});
            flush_tui_logger();
        ...
        if (startup_mode_ == terminal::TuiStartupMode::FailClosed) {
            ...
            log_tui_event(
                dasall::infra::logging::LogLevel::Error, "tui.startup",
                "TUI startup failed terminal capability checks",
                {{"outcome", "fail_closed"}, {"reason_code", "terminal_capability_failed"}});
            flush_tui_logger();
            if (output_stream_ != nullptr && !last_error_.empty()) {
                *output_stream_ << last_error_ << '\n';
            }
        ...
        log_tui_event(dasall::infra::logging::LogLevel::Info, "tui.startup",
                      "TUI startup mode selected", {{"outcome", "accepted"}});
    ...
    /**
     * @brief Runs the interactive session of the TUI application.
     *
     * @return int Exit code of the interactive session.
     */
            ...
            for (std::size_t offset = 0; offset < static_cast<std::size_t>(bytes_read); ++offset) {
                ...
                if (byte == 0x1BU) {
                    ...
                    if (offset + 1U < static_cast<std::size_t>(bytes_read)) {
                        const std::size_t sequence_length = escape_sequence_length(std::string_view(
                            input_buffer + offset, static_cast<std::size_t>(bytes_read) - offset));
                    ...
                    if (is_cursor_up_sequence(sequence)) {
                        static_cast<void>(composer_.handle_key(
                            view::TuiComposerKeyEvent{.key = view::TuiComposerKey::Up,
                                                      .text = {},
                        ...
                        static_cast<void>(composer_.handle_key(view::TuiComposerKeyEvent{
                            .key = view::TuiComposerKey::Down, .text = {}}));
                        sync_composer_state();
                    } else if (is_cursor_left_sequence(sequence)) {
                        static_cast<void>(composer_.handle_key(view::TuiComposerKeyEvent{
                            .key = view::TuiComposerKey::Left, .text = {}}));
                        sync_composer_state();
                    } else if (is_cursor_right_sequence(sequence)) {
                        static_cast<void>(composer_.handle_key(view::TuiComposerKeyEvent{
                            .key = view::TuiComposerKey::Right, .text = {}}));
                        sync_composer_state();
                    } else if (is_delete_sequence(sequence)) {
                        static_cast<void>(composer_.handle_key(view::TuiComposerKeyEvent{
                            .key = view::TuiComposerKey::Delete, .text = {}}));
                        sync_composer_state();
                        ...
                        static_cast<void>(composer_.handle_key(view::TuiComposerKeyEvent{
                            .key = view::TuiComposerKey::AltEnter, .text = {}}));
                        sync_composer_state();
                ...
                if (byte == 0x02U || byte == 0x06U) {
                    sync_pending_draft(draft, cursor_offset,
                ...
                if (byte == 0x12U) {
                    sync_pending_draft(draft, cursor_offset,
                                       "interactive_text_before_reverse_search");
                    static_cast<void>(composer_.handle_key(
                ...
                insert_printable_text(draft, cursor_offset,
    ...
    /**
     * @brief Dispatches a TUI action and updates the screen model accordingly.
     * 
     * @param action The TUI action to be dispatched.
     */
    void TuiApp::dispatch_action(const model::TuiAction& action) {
        ...
        case TuiActionType::StatusQueryRequested:
            apply_reduced_action(screen_model_,
                                 make_banner_action(model::TuiBannerLevel::Info, "Status summary",
                                                    status_summary_message(screen_model_.status),
                                                    "status_query", "status_query_requested"));
            break;
        ...
        case TuiActionType::SessionQueryRequested: {
            ...
            apply_reduced_action(screen_model_,
                                 make_modal_action(std::move(modal), "session_query_requested"));
            break;
        }
    ...
    bool TuiApp::tick() {
        ...
        if (!result.ok()) {
            if (result.issue.has_value()) {
                ...
                log_tui_event(dasall::infra::logging::LogLevel::Error, "tui.events.poll",
                              "TUI event poll failed", std::move(attrs));
        ...
        log_tui_event(dasall::infra::logging::LogLevel::Debug, "tui.events.poll",
    ...
    int TuiApp::shutdown() {
        ...
        if (!result.ok()) {
            if (result.issue.has_value()) {
                std::vector<std::pair<std::string, std::string>> attrs{
                    {"operation", "close_session"},         {"outcome", "failure"},
                    {"request_id", request.request_id},     {"trace_id", request.trace_id},
                    {"close_reason", request.close_reason},
                };
                append_issue_attrs(attrs, *result.issue);
                log_tui_event(dasall::infra::logging::LogLevel::Error, "tui.session.close",
                              "TUI session close failed", std::move(attrs));
                last_error_ = result.issue->message;
        ...
        log_tui_event(dasall::infra::logging::LogLevel::Info, "tui.session.close",
    ...
    /**
     * @brief Clears the composer draft while preserving the history.
     * 
     */
    ...
    void TuiApp::set_composer_text(std::string text, const std::optional<std::size_t> cursor_offset,
                                   std::string debug_reason) {
        const view::TuiComposerKeyEvent event{
            .key = view::TuiComposerKey::TextChanged,
            .text = std::move(text),
    ...
    void TuiApp::sync_pending_draft(const std::string& draft, const std::size_t cursor_offset,
                                    std::string debug_reason) {
        const std::size_t clamped_cursor =
            view::clamp_to_terminal_text_offset(draft, cursor_offset);
        if (draft == composer_.state().text && clamped_cursor == composer_.state().cursor_offset) {
            return;
        }
    ...
    void TuiApp::restore_composer_draft(std::string text, const std::size_t cursor_offset) {
        static_cast<void>(composer_.set_busy(false));
        const view::TuiComposerKeyEvent event{
            .key = view::TuiComposerKey::TextChanged,
            .text = std::move(text),
    ...
    /**
     * @brief Handle the submission of an interactive command.
     * 
     */
    void TuiApp::dispatch_composer_submit() {
        if (!session_open_ || session_id_.empty() || data_source_ == nullptr) {
            apply_reduced_action(
                screen_model_,
                make_banner_action(model::TuiBannerLevel::Error, "Turn submit unavailable",
                                   "No active session is attached to the formal submit path.",
                                   "submit_unavailable", "turn_submit_unavailable", true));
            return;
        }
        ...
        if (!result.ok()) {
            ...
            if (result.issue.has_value()) {
                std::vector<std::pair<std::string, std::string>> attrs{
                    ...
                    {"request_id", request.request_id},
                    {"trace_id", request.trace_id},
                    {"next_preference_mode",
                     route_preference_mode_to_string(request.next_preference.mode)},
                };
                append_optional_attr(attrs, "preferred_depth_tier",
                                     request.next_preference.preferred_depth_tier);
                append_optional_attr(attrs, "pinned_provider_id",
                                     request.next_preference.pinned_provider_id);
                append_optional_attr(attrs, "pinned_model_id",
                                     request.next_preference.pinned_model_id);
                append_issue_attrs(attrs, *result.issue);
                log_tui_event(dasall::infra::logging::LogLevel::Error, "tui.turn.submit",
                              "TUI turn submit failed", std::move(attrs));
                last_error_ = result.issue->message;
                append_issue_banner(*result.issue, result.issue->reason_code == "validation_failed"
        ...
        if (!result.receipt.has_value()) {
            ...
            apply_reduced_action(
                screen_model_,
                make_banner_action(model::TuiBannerLevel::Error, "Turn submit failed",
                                   "submit_turn succeeded without a receipt payload.",
                                   "missing_submit_receipt", "turn_submit_missing_receipt", true));
            return;
        }

        last_error_.clear();
        log_tui_event(dasall::infra::logging::LogLevel::Info, "tui.turn.submit",
        ...
        apply_reduced_action(
            screen_model_,
            make_transcript_message_action("user", update.action.text, result.receipt->submitted_at,
                                           {"submitted"}, "submit_turn_user_message"));
        apply_reduced_action(screen_model_,
                             make_banner_action(model::TuiBannerLevel::Info, "Turn submitted",
    ...
    void TuiApp::handle_interactive_submit() {
        ...
        /// If the input is not a slash command, treat it as a normal turn submission.
        if (!parse_result.is_slash_command) {
            dispatch_composer_submit();
            return;
        }

        /// If the input is a slash command but failed to parse, show an error banner.
        ...
        /// If the input is a slash command that was successfully parsed 
        /// but is of an unsupported kind, show a warning banner.
        if (parse_result.command.kind == command::TuiSlashCommandKind::Editor) {
            apply_reduced_action(
                screen_model_,
                make_banner_action(model::TuiBannerLevel::Warning, "External editor unavailable",
                                   "Formal TUI interactive editor wiring is not available yet.",
                                   "external_editor_unavailable", "interactive_editor_unavailable",
                                   true));
            return;
        }
    ...
    void TuiApp::handle_foreground_session_clear() {
        ...
        log_tui_event(dasall::infra::logging::LogLevel::Info, "tui.session.clear",
        ...
        if (session_open_ && !session_id_.empty() && data_source_ != nullptr) {
            ...
            if (!close_result.ok() && close_result.issue.has_value()) {
                ...
                log_tui_event(dasall::infra::logging::LogLevel::Error, "tui.session.close",
                              "TUI foreground session clear close failed", std::move(attrs));
            } else {
                log_tui_event(dasall::infra::logging::LogLevel::Info, "tui.session.close",
        ...
        clear_composer_draft_preserving_history();
        apply_reduced_action(
            screen_model_, make_foreground_session_reset_action("foreground_session_clear_reset"));
        sync_composer_state();
        ...
        log_tui_event(
            close_issue.has_value()
                ? dasall::infra::logging::LogLevel::Warn
                : (session_reopened && route_reloaded ? dasall::infra::logging::LogLevel::Info
                                                      : dasall::infra::logging::LogLevel::Error),
            "tui.session.clear", "TUI foreground session clear completed",
            {{"operation", "foreground_session_clear"},
             {"outcome", close_issue.has_value()
                             ? "degraded"
                             : (session_reopened && route_reloaded ? "success" : "failure")},
             {"previous_session_id", previous_session_id},
        ...
        if (close_issue.has_value()) {
            apply_reduced_action(screen_model_,
                                 make_banner_action(model::TuiBannerLevel::Warning,
                                                    ...
                                                    close_issue->message, close_issue->reason_code,
                                                    "foreground_session_clear_close_issue", true));
        }
    ...
    void TuiApp::initialize_components(TuiAppOptions& options) {
        const bool has_initial_draft =
            options.initial_draft.has_value() && !options.initial_draft->empty();
    ...
    bool TuiApp::open_session() {
        ...
        if (!result.ok()) {
            if (result.issue.has_value()) {
                ...
                append_issue_attrs(attrs, *result.issue);
                log_tui_event(dasall::infra::logging::LogLevel::Error, "tui.session.open",
                              "TUI session open failed", std::move(attrs));
        ...
        log_tui_event(dasall::infra::logging::LogLevel::Info, "tui.session.open",
    ...
    bool TuiApp::load_route_catalog() {
        const data::TuiRouteCatalogRequest request{
            .session_id =
                session_id_.empty() ? std::nullopt : std::optional<std::string>{session_id_},
            .profile_id = profile_id_,
        ...
        if (!result.ok()) {
            if (result.issue.has_value()) {
                ...
                append_optional_attr(attrs, "profile_id", request.profile_id);
                append_issue_attrs(attrs, *result.issue);
                log_tui_event(dasall::infra::logging::LogLevel::Error, "tui.route_catalog",
                              "TUI route catalog load failed", std::move(attrs));
                last_error_ = format_startup_issue_message(*result.issue);
        ...
        log_tui_event(
            dasall::infra::logging::LogLevel::Info, "tui.route_catalog", "TUI route catalog loaded",
            {{"operation", "route_catalog"},
             {"outcome", "success"},
             {"request_id", request.request_id},
             {"trace_id", request.trace_id},
        ...
        apply_reduced_action(screen_model_, make_route_action(result.route_catalog->current_route,
                                                              "route_catalog_loaded"));
        return true;
    }

    /**
     * @brief Synchronizes the composer state with the screen model.
     * 
     */
    ...
    /**
     * @brief Sets the processing indicator for the composer.
     * 
     */
    ...
    /**
     * @brief Requests an interactive redraw of the screen.
     * 
     */
    ...
    void TuiApp::scroll_transcript_page(const int direction) {
        const view::TuiLayoutMetrics metrics =
            renderer_.apply_layout_metrics(effective_terminal_width(), effective_terminal_height());
        ...
        const auto rendered_transcript =
            transcript_view.render_transcript(transcript_height, transcript_wrap_width);
        const std::size_t max_offset =
            transcript_max_scroll_offset(rendered_transcript.total_line_count, transcript_height);
        if (max_offset == 0U) {
        ...
        if (direction < 0) {
            screen_model_.transcript_scroll_offset =
                current_offset > page_step ? current_offset - page_step : 0U;
            screen_model_.transcript_follow_tail = false;
        } else {
            screen_model_.transcript_scroll_offset =
                std::min(max_offset, current_offset + page_step);
    ...
    void TuiApp::show_selector_preview(const data::TuiRoutePreferenceMode mode) {
        ...
        apply_reduced_action(screen_model_,
                             make_route_action(std::move(route), "selector_preview_applied"));
        ...
        modal.selected_action_index = 0;
        apply_reduced_action(screen_model_,
                             make_modal_action(std::move(modal), "selector_preview_modal"));
    }

    /**
     * @brief Renders the current screen to the output stream.
     * 
     * @param flush_to_output Whether to flush the rendered screen to the output stream.
     */
    void TuiApp::render_current_screen(const bool flush_to_output) {
        last_rendered_screen_ = renderer_.render_to_screen(
            screen_model_, effective_terminal_width(), effective_terminal_height());
        rendered_frames_.push_back(last_rendered_screen_);
        log_tui_event(dasall::infra::logging::LogLevel::Debug, "tui.render.frame",
    ...
    std::string
    TuiApp::selector_modal_body(const std::vector<view::TuiModelSelectorOption>& options) const {
        if (options.empty()) {
        ...
        for (const auto& option : options) {
            ...
            if (!option.selectable) {
                body << " [disabled: " << selector_.render_disabled_reason(option.disabled_reasons)
                     << ']';
            }
            body << '\n';
        }
    ...
    std::string TuiApp::format_startup_issue_message(const data::TuiDataSourceIssue& issue) {
        const std::string detail = trim_copy(issue.message);

        if (issue.reason_code == "permission_denied") {
            std::string message = "TUI startup blocked: permission denied for the current "
                                  "root/sudo-only operator path.";
            if (!detail.empty()) {
                message += ' ';
                message += detail;
            }
    ...
    std::string TuiApp::startup_mode_to_string(const terminal::TuiStartupMode startup_mode) {
        switch (startup_mode) {
        case terminal::TuiStartupMode::FullScreen:
            return "full";
        case terminal::TuiStartupMode::Narrow:
    ...
    std::string TuiApp::next_request_id(std::string_view prefix) {
        ++request_sequence_;
        return std::string(prefix) + "-" + scenario_id_ + "-" + std::to_string(request_sequence_);
    }
    ...
    void TuiApp::append_issue_banner(const data::TuiDataSourceIssue& issue, std::string title) {
        ...
        log_tui_event(dasall::infra::logging::LogLevel::Error, "tui.issue",
                      "TUI data source issue surfaced", std::move(attrs));
        apply_reduced_action(screen_model_,
                             make_banner_action(model::TuiBannerLevel::Error, std::move(title),
                                                issue.message, issue.reason_code,
                                                "data_source_issue:" + issue.reason_code, true));
    }

    void TuiApp::note_logging_failure(std::string_view stage,
        ...
        apply_reduced_action(screen_model_,
                             make_banner_action(model::TuiBannerLevel::Warning,
                                                "Client logging degraded", std::move(message),
                                                "client_logging_degraded",
                                                "client_logging_degraded", true));
    }

    void TuiApp::log_tui_event(const dasall::infra::logging::LogLevel level,
                               const std::string_view event_name, std::string message,
                               std::vector<std::pair<std::string, std::string>> attrs) {
        if (logger_ == nullptr) {
            return;
        }
        ...
        insert_log_attr(event.attrs, "terminal_height",
                        std::to_string(effective_terminal_height()));
        insert_log_attr(event.attrs, "terminal_stdin_tty",
                        bool_to_string(terminal_capabilities_.stdin_is_tty));
        insert_log_attr(event.attrs, "terminal_stdout_tty",
                        bool_to_string(terminal_capabilities_.stdout_is_tty));
        insert_log_attr(event.attrs, "terminal_utf8",
                        bool_to_string(terminal_capabilities_.utf8_enabled));
    ...
    void TuiApp::flush_tui_logger() {
        ...
        const auto result =
            logger_->flush(dasall::infra::logging::LogFlushDeadline{.timeout_ms = 500});
        if (!result.ok) {
