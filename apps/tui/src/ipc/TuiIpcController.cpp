...
namespace dasall::tui::ipc {
    ...
    namespace {
          ...
          private:
            ...
            [[nodiscard]] bool parse_integer(int& out) {
                ...
                const auto parse_result = std::from_chars(
                    integer_text.data(), integer_text.data() + integer_text.size(), out);
        ...
        [[nodiscard]] dasall::platform::IpcPayload to_ipc_payload(std::string_view payload_text) {
        ...
        [[nodiscard]] std::string from_ipc_payload(const dasall::platform::IpcPayload& payload) {
            if (payload.empty()) {
                return {};
            }
        ...
        void append_string_member(std::string& out, std::string_view key, std::string_view value,
                                  bool& first) {
        ...
        void append_integer_member(std::string& out, std::string_view key, const int value,
                                   bool& first) {
            append_member_prefix(out, first);
            append_escaped_string(out, key);
            out.push_back(':');
        ...
        void append_bool_member(std::string& out, std::string_view key, const bool value,
                                bool& first) {
            append_member_prefix(out, first);
            append_escaped_string(out, key);
            out.push_back(':');
        ...
        void append_raw_member(std::string& out, std::string_view key, std::string_view raw_json,
                               bool& first) {
            append_member_prefix(out, first);
            append_escaped_string(out, key);
            out.push_back(':');
        ...
        void append_optional_string_member(std::string& out, std::string_view key,
        ...
        void append_optional_bool_member(std::string& out, std::string_view key,
                                         const std::optional<bool>& value, bool& first) {
            if (!value.has_value()) {
                return;
            }
        ...
        void append_optional_payload_member(std::string& out, std::string_view key,
                                            const std::optional<TuiIpcResponsePayload>& value,
                                            std::string_view operation_json, bool& first);

        [[nodiscard]] std::string
        route_preference_mode_to_string(const data::TuiRoutePreferenceMode mode) {
            switch (mode) {
        ...
        [[nodiscard]] bool parse_operation(std::string_view encoded, TuiIpcOperation* out) {
        ...
        [[nodiscard]] bool parse_outcome(std::string_view encoded, TuiIpcOutcome* out) {
            if (encoded == "success") {
        ...
        [[nodiscard]] bool read_required_string(const JsonObject& object, std::string_view key,
                                                std::string* out) {
        ...
        [[nodiscard]] bool read_optional_string(const JsonObject& object, std::string_view key,
        ...
        [[nodiscard]] bool read_required_bool(const JsonObject& object, std::string_view key,
                                              bool* out) {
            const JsonValue* value = find_member(object, key);
        ...
        [[nodiscard]] bool read_optional_bool(const JsonObject& object, std::string_view key,
        ...
        [[nodiscard]] bool read_required_int(const JsonObject& object, std::string_view key,
                                             int* out) {
            const JsonValue* value = find_member(object, key);
        ...
        [[nodiscard]] std::string encode_string_array(const std::vector<std::string>& values) {
            std::string json = "[";
            bool first = true;
        ...
        [[nodiscard]] std::string
        encode_metadata(const std::vector<std::pair<std::string, std::string>>& metadata) {
            std::string json = "[";
            bool first = true;
        ...
        [[nodiscard]] std::string
        encode_next_turn_preference(const data::NextTurnPreference& preference) {
            std::string json = "{";
            bool first = true;
            append_string_member(json, "mode", route_preference_mode_to_string(preference.mode),
                                 first);
            append_optional_string_member(json, "preferred_depth_tier",
                                          preference.preferred_depth_tier, first);
            append_optional_string_member(json, "pinned_provider_id", preference.pinned_provider_id,
                                          first);
            append_optional_string_member(json, "pinned_model_id", preference.pinned_model_id,
                                          first);
            append_string_member(json, "user_visible_summary", preference.user_visible_summary,
                                 first);
            append_string_member(json, "source", preference.source, first);
            append_bool_member(json, "applies_to_next_turn_only",
                               preference.applies_to_next_turn_only, first);
            json.push_back('}');
            return json;
        }

        [[nodiscard]] std::string encode_session_view(const data::TuiSessionView& session) {
            std::string json = "{";
            bool first = true;
        ...
        [[nodiscard]] std::string encode_turn_receipt(const data::TuiTurnReceipt& receipt) {
            std::string json = "{";
            bool first = true;
        ...
        [[nodiscard]] std::string
        encode_status_projection(const data::TuiStatusProjection& status) {
            std::string json = "{";
            bool first = true;
        ...
        [[nodiscard]] std::string encode_tool_summary(const data::TuiToolSummaryView& summary) {
            std::string json = "{";
            bool first = true;
        ...
        [[nodiscard]] std::string
        encode_route_catalog_entry(const data::TuiRouteCatalogEntry& entry) {
            std::string json = "{";
            bool first = true;
            ...
            append_raw_member(json, "disabled_reasons", encode_string_array(entry.disabled_reasons),
                              first);
            json.push_back('}');
            return json;
        }
        ...
        [[nodiscard]] std::string
        encode_model_route_projection(const data::TuiModelRouteProjection& route) {
            std::string json = "{";
            bool first = true;
            ...
            append_raw_member(json, "disabled_reasons", encode_string_array(route.disabled_reasons),
                              first);
            append_raw_member(json, "next_preference",
                              encode_next_turn_preference(route.next_preference), first);
            json.push_back('}');
            return json;
        }

        [[nodiscard]] std::string
        encode_route_catalog_view(const data::TuiRouteCatalogView& route_catalog) {
            std::string json = "{";
            bool first = true;
            append_raw_member(json, "current_route",
                              encode_model_route_projection(route_catalog.current_route), first);
            ...
            append_raw_member(json, "disabled_reasons",
                              encode_string_array(route_catalog.disabled_reasons), first);
            json.push_back('}');
            return json;
        }

        [[nodiscard]] std::string encode_event_projection(const data::TuiEventProjection& event) {
            std::string json = "{";
            bool first = true;
            ...
            if (event.status_delta.has_value()) {
                append_raw_member(json, "status_delta",
                                  encode_status_projection(*event.status_delta), first);
            }
            if (event.turn_receipt.has_value()) {
                append_raw_member(json, "turn_receipt", encode_turn_receipt(*event.turn_receipt),
                                  first);
            }
            if (event.tool_summary.has_value()) {
                append_raw_member(json, "tool_summary", encode_tool_summary(*event.tool_summary),
                                  first);
            }
        ...
        [[nodiscard]] std::string encode_request_payload(const TuiIpcRequestPayload& payload) {
            return std::visit(
                [](const auto& current) -> std::string {
                    ...
                    if constexpr (std::is_same_v<Payload, TuiIpcOpenSessionPayload>) {
                        append_optional_string_member(json, "profile_id", current.profile_id,
                                                      first);
                        append_optional_string_member(json, "startup_mode_hint",
                                                      current.startup_mode_hint, first);
                        ...
                        append_optional_string_member(json, "event_cursor", current.event_cursor,
                                                      first);
                    } else if constexpr (std::is_same_v<Payload, TuiIpcRouteCatalogPayload>) {
                        append_optional_string_member(json, "profile_id", current.profile_id,
                                                      first);
                        append_optional_string_member(json, "selector_mode", current.selector_mode,
                                                      first);
        ...
        [[nodiscard]] std::string encode_response_payload(const TuiIpcResponsePayload& payload,
            ...
            return std::visit(
                [operation](const auto& current) -> std::string {
                    ...
                    if constexpr (std::is_same_v<Payload, data::TuiSessionView>) {
                        ...
                        append_optional_string_member(json, "next_cursor", current.next_cursor,
                                                      first);
                        json.push_back('}');
                        return json;
                    ...
                    std::string fallback = "{";
                    bool first = true;
                    append_string_member(fallback, "operation", std::string(to_string(operation)),
                                         first);
                    fallback.push_back('}');
                    return fallback;
                },
        ...
        [[nodiscard]] std::string encode_request_envelope(const TuiIpcRequestEnvelope& envelope) {
            std::string json = "{";
            bool first = true;
            append_string_member(json, "schema_version", envelope.schema_version, first);
            append_string_member(json, "operation", std::string(to_string(envelope.operation)),
                                 first);
            ...
            append_integer_member(json, "deadline_ms", static_cast<int>(envelope.deadline_ms),
                                  first);
        ...
        [[nodiscard]] std::string encode_response_envelope(const TuiIpcResponseEnvelope& envelope) {
            std::string json = "{";
            bool first = true;
            append_string_member(json, "schema_version", envelope.schema_version, first);
            append_string_member(json, "operation", std::string(to_string(envelope.operation)),
            ...
            append_optional_payload_member(json, "payload", envelope.payload,
                                           to_string(envelope.operation), first);
        ...
        [[nodiscard]] bool
        decode_metadata_array(const JsonArray& metadata_json,
        ...
        [[nodiscard]] bool decode_next_turn_preference(const JsonObject& object,
            ...
            if (!read_required_string(object, "mode", &encoded_mode) ||
                ...
                !read_optional_string(object, "preferred_depth_tier",
                                      &preference->preferred_depth_tier) ||
                !read_optional_string(object, "pinned_provider_id",
                                      &preference->pinned_provider_id) ||
                !read_optional_string(object, "pinned_model_id", &preference->pinned_model_id) ||
                !read_required_string(object, "user_visible_summary",
                                      &preference->user_visible_summary) ||
                !read_required_string(object, "source", &preference->source) ||
                !read_required_bool(object, "applies_to_next_turn_only",
        ...
        [[nodiscard]] bool decode_session_view(const JsonObject& object,
            ...
            return read_required_string(object, "session_id", &session->session_id) &&
                   ...
                   read_required_string(object, "daemon_readiness", &session->daemon_readiness) &&
        ...
        [[nodiscard]] bool decode_status_projection(const JsonObject& object,
            ...
            return read_required_string(object, "stage", &status->stage) &&
                   ...
                   read_required_string(object, "pending_interaction",
                                        &status->pending_interaction) &&
                   read_required_string(object, "budget_summary", &status->budget_summary) &&
                   read_required_string(object, "recovery_summary", &status->recovery_summary) &&
                   read_required_string(object, "health_summary", &status->health_summary) &&
                   read_required_string(object, "safe_mode_summary", &status->safe_mode_summary);
        }

        [[nodiscard]] bool decode_tool_summary(const JsonObject& object,
            ...
            if (!read_required_string(object, "tool_name", &summary->tool_name) ||
                ...
                !read_required_string(object, "observation_summary",
        ...
        [[nodiscard]] bool decode_route_catalog_entry(const JsonObject& object,
            ...
            if (!read_required_string(object, "provider_id", &entry->provider_id) ||
                ...
                !read_required_string(object, "verification_state", &entry->verification_state) ||
                !read_required_string(object, "health", &entry->health) ||
                !read_required_bool(object, "profile_allowlisted", &entry->profile_allowlisted) ||
            ...
            return disabled != nullptr && decode_string_array(*disabled, &entry->disabled_reasons);
        }

        [[nodiscard]] bool decode_model_route_projection(const JsonObject& object,
                                                         data::TuiModelRouteProjection* route) {
            if (!read_required_string(object, "current_provider_id", &route->current_provider_id) ||
                !read_required_string(object, "current_model_id", &route->current_model_id) ||
                !read_required_string(object, "current_depth_tier", &route->current_depth_tier) ||
                !read_required_string(object, "verification_state", &route->verification_state) ||
                !read_required_string(object, "health", &route->health) ||
                !read_required_bool(object, "profile_allowlisted", &route->profile_allowlisted)) {
                return false;
            }

            const JsonArray* disabled = read_required_array(object, "disabled_reasons");
            const JsonObject* next_preference = read_required_object(object, "next_preference");
        ...
        [[nodiscard]] bool decode_route_catalog_view(const JsonObject& object,
            ...
            if (current_route == nullptr || candidates == nullptr || disabled == nullptr ||
                !decode_model_route_projection(*current_route, &route_catalog->current_route) ||
        ...
        [[nodiscard]] bool decode_event_projection(const JsonObject& object,
            ...
            if (const JsonObject* status_delta = read_optional_object(object, "status_delta");
        ...
        [[nodiscard]] bool decode_request_payload(const JsonObject& object,
            ...
            case TuiIpcOperation::OpenSession: {
                ...
                return read_optional_string(object, "profile_id", &decoded.profile_id) &&
                       read_optional_string(object, "startup_mode_hint",
                                            &decoded.startup_mode_hint) &&
            ...
            case TuiIpcOperation::SubmitTurn: {
                ...
                return read_required_string(object, "user_input", &decoded.user_input) &&
                       ...
                       decode_next_turn_preference(*next_preference, &decoded.next_preference) &&
                       ((*payload = std::move(decoded)), true);
            }
        ...
        decode_request_envelope(std::string_view payload_text) {
            JsonObject root;
            ...
            if (!read_required_string(root, "schema_version", &envelope.schema_version) ||
                ...
                !read_required_int(root, "deadline_ms", &deadline_ms) || deadline_ms < 0 ||
                payload == nullptr ||
        ...
        [[nodiscard]] DecodedResponseEnvelope
        decode_response_envelope(std::string_view payload_text) {
            JsonObject root;
            JsonReader reader(payload_text);
            if (!reader.parse_root_object(root)) {
            ...
            if (const JsonArray* metadata = read_optional_array(root, "metadata");
                metadata != nullptr && !decode_metadata_array(*metadata, &envelope.metadata)) {
                return DecodedResponseEnvelope{
                    .envelope = std::nullopt,
                    .error = EnvelopeDecodeError::Malformed,
                    .actual_schema_version = std::nullopt,
        ...
        [[nodiscard]] data::TuiDataSourceIssue
        make_request_issue(std::string_view reason_code, std::string message,
                           const TuiIpcRequestEnvelope& request, std::string_view socket_path) {
        ...
        [[nodiscard]] data::TuiDataSourceIssue
        make_transport_issue(std::string_view reason_code, std::string message,
                             const bool retryable, const TuiIpcRequestEnvelope& request,
                             std::string_view socket_path) {
            data::TuiDataSourceIssue issue;
        ...
        [[nodiscard]] data::TuiDataSourceIssue
        make_protocol_issue(std::string_view reason_code, std::string message,
                            const TuiIpcRequestEnvelope& request, std::string_view socket_path) {
            data::TuiDataSourceIssue issue;
        ...
        [[nodiscard]] data::TuiDataSourceIssue
        make_daemon_issue(std::string_view reason_code, std::string message, const bool retryable,
                          const TuiIpcRequestEnvelope& request, std::string_view socket_path) {
            data::TuiDataSourceIssue issue;
        ...
        [[nodiscard]] data::TuiDataSourceIssue
        make_issue_from_platform_error(const dasall::platform::PlatformError& error,
                                       const TuiIpcRequestEnvelope& request,
                                       std::string_view socket_path) {
            data::TuiDataSourceIssue issue;
            switch (error.code) {
            ...
            case dasall::platform::PlatformErrorCode::PayloadTooLarge:
                issue = make_request_issue("validation_failed", error.detail, request, socket_path);
                break;
            case dasall::platform::PlatformErrorCode::PermissionDenied:
                issue = make_transport_issue("permission_denied", error.detail, false, request,
                                             socket_path);
                break;
            case dasall::platform::PlatformErrorCode::Timeout:
                issue = make_transport_issue("timeout", error.detail, true, request, socket_path);
                break;
            case dasall::platform::PlatformErrorCode::PeerClosed:
            case dasall::platform::PlatformErrorCode::Disconnected:
                issue = make_transport_issue("peer_closed", error.detail, error.retryable_hint,
                                             request, socket_path);
                break;
            case dasall::platform::PlatformErrorCode::NotFound:
                issue = make_transport_issue("socket_missing", error.detail, false, request,
                                             socket_path);
                break;
            ...
            case dasall::platform::PlatformErrorCode::QueueClosed:
                issue = make_daemon_issue("daemon_unavailable", error.detail, error.retryable_hint,
                                          request, socket_path);
                break;
            }
            ...
            if (error.errno_value.has_value()) {
                append_unique_metadata(issue.metadata, "errno", std::to_string(*error.errno_value));
            }
            return issue;
        }

        [[nodiscard]] bool has_required_context(const std::string& request_id,
                                                const std::string& trace_id) {
            return !request_id.empty() && !trace_id.empty();
        }

        [[nodiscard]] std::optional<data::TuiDataSourceIssue>
        validate_request(const data::TuiOpenSessionRequest& request, std::string_view socket_path,
                         const TuiIpcRequestEnvelope& envelope) {
            if (!has_required_context(request.request_id, request.trace_id)) {
                return make_request_issue("validation_failed",
        ...
        [[nodiscard]] std::optional<data::TuiDataSourceIssue>
        validate_request(const data::TuiSubmitTurnRequest& request, std::string_view socket_path,
                         const TuiIpcRequestEnvelope& envelope) {
            if (!has_required_context(request.request_id, request.trace_id) ||
        ...
        [[nodiscard]] std::optional<data::TuiDataSourceIssue>
        validate_request(const data::TuiPollEventsRequest& request, std::string_view socket_path,
                         const TuiIpcRequestEnvelope& envelope) {
            if (!has_required_context(request.request_id, request.trace_id) ||
                request.session_id.empty()) {
                return make_request_issue(
                    "validation_failed",
                    "poll_events requires session_id, request_id, and trace_id", envelope,
                    socket_path);
            }
            return std::nullopt;
        }
        ...
        [[nodiscard]] std::optional<data::TuiDataSourceIssue>
        validate_request(const data::TuiRouteCatalogRequest& request, std::string_view socket_path,
                         const TuiIpcRequestEnvelope& envelope) {
            if (!has_required_context(request.request_id, request.trace_id)) {
                return make_request_issue(
                    "validation_failed",
                    "query_route_catalog requires non-empty request_id and trace_id", envelope,
                    socket_path);
            }
            return std::nullopt;
        }
        ...
        [[nodiscard]] std::optional<data::TuiDataSourceIssue>
        validate_request(const data::TuiCloseSessionRequest& request, std::string_view socket_path,
                         const TuiIpcRequestEnvelope& envelope) {
            if (!has_required_context(request.request_id, request.trace_id) ||
        ...
        [[nodiscard]] std::uint32_t resolve_deadline(const TuiIpcOperation operation,
        ...
        [[nodiscard]] RoundTripResult perform_roundtrip(const TuiIpcRequestEnvelope& request,
                                                        const TuiIpcControllerOptions& options) {
            if (options.socket_path.empty()) {
                return RoundTripResult{
                    .response = std::nullopt,
                    .issue = make_transport_issue("socket_missing", "tui ipc socket path is empty",
                                                  false, request, options.socket_path),
                };
            }
            ...
            if (!ipc) {
                return RoundTripResult{
                    .response = std::nullopt,
                    .issue =
                        make_daemon_issue("daemon_unavailable", "tui ipc provider is unavailable",
                                          false, request, options.socket_path),
                };
            }
            ...
            if (!channel.ok() || !channel.value.has_value()) {
                ...
                return RoundTripResult{
                    .response = std::nullopt,
                    .issue = make_issue_from_platform_error(error, request, options.socket_path),
                };
            }
            ...
            if (received.value->peer_closed) {
                return RoundTripResult{
                    ...
                    .issue = make_transport_issue(
                        "peer_closed", "daemon closed channel before returning a response", false,
                        request, options.socket_path),
                };
            }
            ...
            if (response_payload.empty()) {
                return RoundTripResult{
                    .response = std::nullopt,
                    .issue = make_protocol_issue("malformed_response",
            ...
            if (!decoded.envelope.has_value()) {
                data::TuiDataSourceIssue issue =
                    decoded.error == EnvelopeDecodeError::SchemaMismatch
                        ? make_protocol_issue(
                              "schema_mismatch",
                              "daemon returned an incompatible tui ipc schema_version", request,
                              options.socket_path)
                        : make_protocol_issue(
                              "malformed_response",
                              "daemon returned an invalid tui ipc response envelope", request,
                              options.socket_path);
                if (decoded.actual_schema_version.has_value()) {
                    append_unique_metadata(issue.metadata, "actual_schema_version",
                                           *decoded.actual_schema_version);
                }
                append_unique_metadata(issue.metadata, "expected_schema_version",
                                       std::string(kTuiIpcSchemaVersion));
                return RoundTripResult{
                    .response = std::nullopt,
            ...
            if (decoded.envelope->operation != request.operation ||
                ...
                data::TuiDataSourceIssue issue = make_protocol_issue(
                    ...
                    request, options.socket_path);
                append_unique_metadata(issue.metadata, "response_operation",
                                       std::string(to_string(decoded.envelope->operation)));
                append_unique_metadata(issue.metadata, "response_request_id",
                                       decoded.envelope->request_id);
                append_unique_metadata(issue.metadata, "response_trace_id",
                                       decoded.envelope->trace_id);
                return RoundTripResult{
                    .response = std::nullopt,
                    .issue = std::move(issue),
                };
        ...
        [[nodiscard]] data::TuiDataSourceIssue
        make_issue_from_response(const TuiIpcResponseEnvelope& response,
                                 const TuiIpcRequestEnvelope& request,
                                 std::string_view socket_path) {
            data::TuiDataSourceIssue issue;
        ...
        [[nodiscard]] data::TuiPollEventsResult
        poll_result_from_issue(data::TuiDataSourceIssue issue) {
    ...
    data::TuiOpenSessionResult
    TuiIpcController::open_session(const data::TuiOpenSessionRequest& request) {
        const TuiIpcRequestEnvelope envelope = make_request_envelope(
            request, resolve_deadline(TuiIpcOperation::OpenSession, options_.timeout_policy));
        if (const auto validation_issue = validate_request(request, options_.socket_path, envelope);
        ...
        if (session == nullptr) {
            return result_from_issue<data::TuiOpenSessionResult>(
                make_protocol_issue("malformed_response", "open_session expected a session payload",
                                    envelope, options_.socket_path));
        }
    ...
    /**
     * @brief Submits a turn to the daemon and waits for the turn receipt response. 
     * The turn receipt contains information about the accepted turn, 
     * such as the turn ID and any actions that should be taken by the TUI in response to the turn submission.
     * 
     * @param request The request containing the turn information.
     * @return data::TuiSubmitTurnResult The result of the turn submission.
     */
    data::TuiSubmitTurnResult
    TuiIpcController::submit_turn(const data::TuiSubmitTurnRequest& request) {
        const TuiIpcRequestEnvelope envelope = make_request_envelope(
            request, resolve_deadline(TuiIpcOperation::SubmitTurn, options_.timeout_policy));
        if (const auto validation_issue = validate_request(request, options_.socket_path, envelope);
            validation_issue.has_value()) {
        ...
        if (receipt == nullptr) {
            return result_from_issue<data::TuiSubmitTurnResult>(
                make_protocol_issue("malformed_response", "submit_turn expected a receipt payload",
                                    envelope, options_.socket_path));
        }
    ...
    data::TuiPollEventsResult
    TuiIpcController::poll_events(const data::TuiPollEventsRequest& request) {
        const TuiIpcRequestEnvelope envelope = make_request_envelope(
            request, resolve_deadline(TuiIpcOperation::PollEvents, options_.timeout_policy));
        if (const auto validation_issue = validate_request(request, options_.socket_path, envelope);
            validation_issue.has_value()) {
        ...
        if (batch == nullptr) {
            return poll_result_from_issue(make_protocol_issue(
                "malformed_response", "poll_events expected an events batch payload", envelope,
    ...
    data::TuiRouteCatalogResult
    TuiIpcController::query_route_catalog(const data::TuiRouteCatalogRequest& request) {
        const TuiIpcRequestEnvelope envelope = make_request_envelope(
            request, resolve_deadline(TuiIpcOperation::RouteCatalog, options_.timeout_policy));
        if (const auto validation_issue = validate_request(request, options_.socket_path, envelope);
            validation_issue.has_value()) {
        ...
        if (route_catalog == nullptr) {
            return result_from_issue<data::TuiRouteCatalogResult>(make_protocol_issue(
                "malformed_response", "query_route_catalog expected a route catalog payload",
                envelope, options_.socket_path));
        }
    ...
    data::TuiCloseSessionResult
    TuiIpcController::close_session(const data::TuiCloseSessionRequest& request) {
        const TuiIpcRequestEnvelope envelope = make_request_envelope(
            request, resolve_deadline(TuiIpcOperation::CloseSession, options_.timeout_policy));
        if (const auto validation_issue = validate_request(request, options_.socket_path, envelope);
            validation_issue.has_value()) {
        ...
        const auto* close_ack = std::get_if<TuiIpcCloseSessionAck>(&*roundtrip.response->payload);
        if (close_ack == nullptr || !close_ack->closed) {
            return result_from_issue<data::TuiCloseSessionResult>(make_protocol_issue(
                "malformed_response", "close_session expected a positive close acknowledgement",
                envelope, options_.socket_path));
        }
    ...
    namespace test {
        ...
        ScopedIpcOverride::ScopedIpcOverride(std::shared_ptr<dasall::platform::IIPC> ipc) {
            std::lock_guard<std::mutex> lock(g_test_ipc_mutex);
        ...
        std::string encode_response_envelope_for_test(const TuiIpcResponseEnvelope& envelope) {
            return encode_response_envelope(envelope);
        }

        std::optional<TuiIpcRequestEnvelope>
