...
namespace dasall::tui::ipc {
    ...
    inline constexpr std::string_view kTuiDefaultDaemonSocketPath = "/run/dasall/daemon.sock";
    ...
    using TuiIpcRequestPayload =
        std::variant<TuiIpcOpenSessionPayload, TuiIpcSubmitTurnPayload, TuiIpcPollEventsPayload,
                     TuiIpcRouteCatalogPayload, TuiIpcCloseSessionPayload>;
    ...
    using TuiIpcResponsePayload =
        std::variant<data::TuiSessionView, data::TuiTurnReceipt, TuiIpcPollEventsBatch,
                     data::TuiRouteCatalogView, TuiIpcCloseSessionAck>;
    ...
    struct TuiIpcResponseEnvelope {
        ...
        [[nodiscard]] bool ok() const {
            return outcome == TuiIpcOutcome::Success && payload.has_value() &&
                   !reason_domain.has_value() && !reason_code.has_value() && !message.has_value() &&
        ...
        [[nodiscard]] bool has_consistent_values() const {
            ...
            if (outcome == TuiIpcOutcome::Success) {
                return payload.has_value() && !reason_domain.has_value() &&
                       !reason_code.has_value() && !message.has_value() && !retryable.has_value() &&
                       !error_ref.has_value();
            }

            return !payload.has_value() && reason_domain.has_value() && reason_code.has_value();
        }
    };

    [[nodiscard]] constexpr std::string_view to_string(const TuiIpcOperation operation) noexcept {
        switch (operation) {
    ...
    /**
     * @brief Creates a TuiIpcRequestEnvelope for a given request and deadline.
     * 
     * @param request The request to be wrapped in the envelope.
     * @param deadline_ms The deadline for the request in milliseconds.
     * @return TuiIpcRequestEnvelope The created request envelope.
     */
    [[nodiscard]] inline TuiIpcRequestEnvelope
    make_request_envelope(const data::TuiOpenSessionRequest& request,
     ...
     */
    [[nodiscard]] inline TuiIpcRequestEnvelope
    make_request_envelope(const data::TuiSubmitTurnRequest& request,
                          const std::uint32_t deadline_ms) {
        TuiIpcRequestEnvelope envelope;
     ...
     */
    [[nodiscard]] inline TuiIpcRequestEnvelope
    make_request_envelope(const data::TuiPollEventsRequest& request,
                          const std::uint32_t deadline_ms) {
        TuiIpcRequestEnvelope envelope;
     ...
     */
    [[nodiscard]] inline TuiIpcRequestEnvelope
    make_request_envelope(const data::TuiRouteCatalogRequest& request,
                          const std::uint32_t deadline_ms) {
        TuiIpcRequestEnvelope envelope;
     ...
     */
    [[nodiscard]] inline TuiIpcRequestEnvelope
    make_request_envelope(const data::TuiCloseSessionRequest& request,
                          const std::uint32_t deadline_ms) {
        TuiIpcRequestEnvelope envelope;
    ...
    class TuiIpcController {
      public:
        explicit TuiIpcController(TuiIpcControllerOptions options = TuiIpcControllerOptions{});

        data::TuiOpenSessionResult open_session(const data::TuiOpenSessionRequest& request);
        data::TuiSubmitTurnResult submit_turn(const data::TuiSubmitTurnRequest& request);
        data::TuiPollEventsResult poll_events(const data::TuiPollEventsRequest& request);
        data::TuiRouteCatalogResult
        query_route_catalog(const data::TuiRouteCatalogRequest& request);
        data::TuiCloseSessionResult close_session(const data::TuiCloseSessionRequest& request);
