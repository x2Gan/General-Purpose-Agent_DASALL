...
矩阵中的 `knowledge` 行不再是“无独立正向入口”的显式缺口。当前 authoritative local owner 固定为 `knowledge_local_installed_proof.sh` 落盘的 `knowledge-proof.json` / `installed-normative-assets.json` / `knowledge-retrieve-hybrid-canary.json`、`knowledge_failure_injection_installed_proof.sh` 落盘的 `knowledge-failure-injection-proof.json`，以及 `knowledge_refresh_retrieve_soak.sh` 落盘的 `knowledge-soak-summary.json` / `iteration-XX-retrieve-hybrid-canary.json`；`pkg_smoke_install.sh` 仍可保留 package-smoke 侧原始 Knowledge JSON，但不再作为 Knowledge proof 的唯一 owner。从 `KNO-TODO-040` 起，local proof / soak 即使只得到 lexical fallback，也必须保留 `mode`、`reason_codes`、`selected_corpora`、`vector_backend_ready`、`dense_hit_count` 与 `has_dense_artifact` 等结构化字段，不得把“无混合证据”静默吞掉。Capability Services 的 local owner 现固定为 package-smoke artifact 目录中的 `services-installed-proof.json` 与 release-runner local soak 目录中的 `services-soak-summary.json`；Infrastructure 的 local owner 现固定为 `infra_release_soak_gate.sh` 落盘的 `infra-release-soak-summary.json` 与同目录 iteration JSON / focused binary logs；更高层 qemu / machine isolation 继续属于 packaging / release hardening，不把本机 installed 证据外推为 package-ready。
