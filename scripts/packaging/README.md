...
| infra local release / soak gate | 在 real local installed daemon 上固定 diagnostics/health/readiness 正向、infra failure slices 与 memory soak metrics artifact | `bash scripts/packaging/infra_release_soak_gate.sh --artifact-dir <path> --build-dir <build-dir>` | INF-FIX-006 + WP-MEM-GAP-018 |
...
| release runner workflow | 在 self-hosted runner 固定 local installed package evidence、infra/services/knowledge proof / soak artifact，再执行 qemu gate 与日志归档 | GitHub Actions `DASALL-Release-Package-Gate` workflow_dispatch | FULLINT-BLK-001 blocker-fix |
...
`pkg_smoke_install.sh --explicit-start-check` 与 `debian/tests/pkg-smoke-local-control-plane` 不再只验安装生命周期；二者必须把安装后的本地控制面和主功能语义一起纳入 gate。当前 installed-package 角色固定为：bare `dasall` 只代表 TUI 入口，结构化 control-plane 与 package scripts 一律走 `dasall-cli`。子系统能力收敛可使用本机实际 installed-package smoke 作为 authoritative local evidence；qemu / machine isolation 继续用于 release-runner 环境和隔离可重复性，不作为所有子系统能力闭合的强制前置。

| 功能面 | local lifecycle smoke | `pkg-smoke-local-control-plane` | 验收语义 |
|---|---|---|---|
| TUI entrypoint | bare `dasall` 在 non-TTY 环境下必须 exit 1，并输出 `Use dasall-cli for non-interactive control-plane tasks.` | 同左 | 证明 bare `dasall` 已固定为 TUI 入口，而不是结构化 CLI alias；package smoke 同时守住对 `dasall-cli` 的重定向文案。 |
| `run` / LLM | `dasall-cli run ... --json` 必须同时包含 `"disposition":"completed"`、`"task_completed":true`、`llm.origin=deepseek-prod/`，且不得包含 `agent.dataset` | 同左 | 证明 installed daemon 的主功能链路通过 production `ILLMManager` 调用 DeepSeek；仅有 completed transport 或 builtin dataset 投影不算通过。 |
| memory | `bash scripts/packaging/validate_memory_installed_or_qemu.sh` 必须生成 `~/.cache/dasall/memory/installed-evidence/latest.json`，其中 `checks.init/open_store/prepare_context/write_back/maintenance` 全为 `true`；raw artifact 目录必须保留 `run-first.json`、`run-second.json`、`memory-proof.json`、`memory-maintenance-proof.json` | `pkg_smoke_install.sh --explicit-start-check` 继续作为 raw source evidence producer；focused regression 可用 `validate_memory_installed_or_qemu.sh --reuse-artifacts --artifact-dir <dir>` 复用既有 raw artifacts 校验 wrapper 汇总逻辑；当脚本追加 qemu/autopkgtest 命令时，会在同一轮中串联 `validate_gate_int_10_installed_package_qemu.sh`，但 qemu / machine isolation 继续用于 release-runner 与 packaging hardening | 证明 memory owner 当前 authoritative local evidence 已从 package-smoke 原始 artifact 收敛为独立 gate：same-session 双轮 `prepare_context` / `write_back`、SQLite WAL / core+vector tables 与 maintenance checkpoint / retention / quarantine cleanup 会被汇总进统一 `latest.json`；同时保持 qemu 不回流成 memory 功能 owner 的唯一前置。 |
| runtime | `/usr/lib/dasall/dasall-runtime-installed-proof --json` 必须返回 `"ok": true`、`"tool_status": "Completed"`、`"tool_runtime_path": "runtime_path:tool_positive"`、`"waiting_status": "PartiallyCompleted"`、`"recovery_positive_runtime_path": "runtime_path:recovery_positive"`、`"recovery_negative_binding_rejected": true`；`pkg_smoke_install.sh --explicit-start-check` 必须落盘 `runtime-installed-proof.json` 与汇总 `runtime-proof.json` | package-smoke artifact 目录保留 `run-first.json` / `run-second.json`、`runtime-installed-proof.json` 与 `runtime-proof.json`；L3 cognition-positive 继续由 `Gate-INT-10` / `DaemonBinaryUnarySmokeTest` 的 app-binary owner 维护 | 证明 installed runtime direct LLM、tool-positive 与 recovery-positive/negative 证据已从同一 `dasall-cli run` 结果中拆分：direct path 继续由 `run-first.json`/`run-second.json` 维持，tool/recovery path 则由 runtime owner helper 单独收口；同时明确 cognition-positive 仍是 app-binary L3 owner、qemu/release runner 仍归 packaging gate。 |
...
| `status` | `dasall-cli status receipt:missing token local://uid/0 --json` 必须 exit 5 且 `status_missing` | 同左 | 缺失 receipt fail-closed，不误报成功。 |
| `cancel` | `dasall-cli cancel receipt:missing token local://uid/0 --json` 必须 exit 5 且 `cancel_missing` | 同左 | 缺失 receipt fail-closed，不误报成功。 |
| Access installed security matrix | `pkg_smoke_install.sh --explicit-start-check` 必须落盘 `access-installed-async-receipt-proof.json` 与 `access-installed-gateway-http-proof.json`，并固定 `status_owner_mismatch`、`cancel_owner_mismatch`、`negative_listener_exposed=false` 与 `diag_disabled` 等 installed negative semantics | release-runner local step 归档同名 proof；`DaemonSocketModeIntegrationTest` 与 `AccessPolicyBackendUnavailableIntegrationTest` 继续作为 build-tree complement，不需要 qemu / kvm 才能成为 Access owner 当前验收依据 | 证明 Access owner 当前安全矩阵已由本机 installed proof + focused release-preflight tests 收口；machine isolation 继续归 packaging / release hardening，不回流为 Access owner blocker。 |
| `diag` 默认门控 | 默认安装后 `dasall-cli diag health --json` 必须 exit 4 且 `diag_disabled` | 不适用，autopkgtest 会通过 config apply 启用 diag | 证明 diagnostics 不会绕过 daemon config gate。 |
| `diag` 正向 | 不启用 diag，不跑正向 | config apply 写入 `diag_enabled: true` 后，`dasall-cli diag health --json` 必须 completed 且返回 diagnostics summary | 证明 CLI alias `health`、daemon allowlist、infra diagnostics service 和 UDS JSON 投影都可用。 |
| infrastructure | `pkg_smoke_install.sh --explicit-start-check` 必须保持 `dasall-cli diag health --json` exit 4 且 `diag_disabled`；`infra_release_soak_gate.sh` 需在 `diag_enabled: true` 下重复执行 installed `dasall-cli readiness --json` / `dasall-cli diag health --json`，并顺序跑 `InfraDiagnosticsSmokeTest`、`InfraDiagnosticsIntegrationTest`、`HealthWiringIntegrationTest`、`InfraHealthCadenceIntegrationTest`、`MemoryReleaseSoakProbeTest`、`SecretFailureInjectionTest`、`PluginLifecycleStateTest`、`MetricsFailureInjectionTest`，落盘 `infra-release-soak-summary.json` 与 `memory-release-soak-summary.json` | release-runner local step 继续执行 `infra_release_soak_gate.sh`，归档 iteration JSON、focused binary logs、`infra-release-soak-summary.json` 与 `memory-release-soak-summary.json` | 证明 installed daemon 的 diagnostics/health/readiness 正向、secret unavailable、plugin safe unload、observability sink failure，以及 Memory owner 的 store latency / wal size / maintenance lag / writeback partial rate / vector recall@k / summary fallback rate 已有 local authoritative soak evidence；qemu / machine isolation 继续留给 packaging / release hardening，不把其回流为 infra blocker。 |
...
| knowledge | `dasall-cli knowledge refresh/retrieve/health --json` 必须 completed；`knowledge_local_installed_proof.sh` 需生成 `knowledge-proof.json`、`installed-normative-assets.json` 与 `knowledge-retrieve-hybrid-canary.json`；`knowledge_failure_injection_installed_proof.sh` 需生成 `knowledge-failure-injection-proof.json` | release-runner local step 继续执行 `knowledge_refresh_retrieve_soak.sh` 并生成 `knowledge-soak-summary.json` 与 `iteration-XX-retrieve-hybrid-canary.json` | 证明 installed package 已具独立 Knowledge 正向入口与 active snapshot 损坏后的本机恢复能力；local authoritative evidence 由 refresh/provider-retrieve/health proof、installed normative asset files、failure-injection recovery proof 与 soak artifact 组成，qemu / machine isolation 继续留给 release hardening，不再作为 Knowledge 功能闭环的强制前置。 |

矩阵中的 `knowledge` 行不再是“无独立正向入口”的显式缺口。当前 authoritative local owner 固定为 `knowledge_local_installed_proof.sh` 落盘的 `knowledge-proof.json` / `installed-normative-assets.json` / `knowledge-retrieve-hybrid-canary.json`、`knowledge_failure_injection_installed_proof.sh` 落盘的 `knowledge-failure-injection-proof.json`，以及 `knowledge_refresh_retrieve_soak.sh` 落盘的 `knowledge-soak-summary.json` / `iteration-XX-retrieve-hybrid-canary.json`；`pkg_smoke_install.sh` 仍可保留 package-smoke 侧原始 Knowledge JSON，但不再作为 Knowledge proof 的唯一 owner。从 post-eval 整改起，local proof / soak 的 `--hybrid-canary` 是 strict positive gate：必须同时满足 `mode=hybrid`、`runtime_canary_admitted`、`vector_backend_ready=true` 与 `dense_hit_count>0`，lexical fallback artifact 只能作为问题证据归档，不能作为通过依据。Capability Services 的 local owner 现固定为 package-smoke artifact 目录中的 `services-installed-proof.json` 与 release-runner local soak 目录中的 `services-soak-summary.json`；Infrastructure 的 local owner 现固定为 `infra_release_soak_gate.sh` 落盘的 `infra-release-soak-summary.json`、`memory-release-soak-summary.json` 与同目录 iteration JSON / focused binary logs；更高层 qemu / machine isolation 继续属于 packaging / release hardening，不把本机 installed 证据外推为 package-ready。
...
2. workflow 在 qemu gate 前先固定六类 local installed artifact：`pkg_smoke_install.sh --explicit-start-check` 的 package-smoke 目录（现至少包含 `tui-noninteractive.txt`、`run-first.json`、`run-second.json`、`memory-proof.json`、`memory-maintenance-proof.json`、`tools-installed-proof.json`、`runtime-installed-proof.json`、`runtime-proof.json`、`services-installed-proof.json`）、`infra_release_soak_gate.sh` 的 infra 目录（含 `infra-release-soak-summary.json`、`memory-release-soak-summary.json`、installed `readiness` / `diag health` iteration JSON 与 focused binary logs）、`services_subscription_adapter_soak.sh` 的 soak 目录（含 `services-soak-summary.json`）、`knowledge_local_installed_proof.sh` 的 proof 目录（含 `knowledge-proof.json` / `installed-normative-assets.json` / `knowledge-retrieve-hybrid-canary.json`）、`knowledge_failure_injection_installed_proof.sh` 的 failure 目录（含 `knowledge-failure-injection-proof.json`）与 `knowledge_refresh_retrieve_soak.sh` 的 soak 目录（含 `knowledge-soak-summary.json` / `iteration-XX-retrieve-hybrid-canary.json`）；六者只证明 local authoritative evidence，不直接外推为 qemu PASS。从 `KNO-TODO-040` 起，workflow 默认以 `--hybrid-canary` 调用 knowledge proof / soak。
...
5. workflow 当前会归档 `package-smoke-local.log`、`infra-soak.log`、`services-soak.log`、`knowledge-proof.log`、`knowledge-failure-injection.log`、`knowledge-soak.log`、`package-smoke/runtime-installed-proof.json`、`package-smoke/runtime-proof.json`、`package-smoke/services-installed-proof.json`、`infra-soak/infra-release-soak-summary.json`、`infra-soak/memory-release-soak-summary.json`、`services-soak/services-soak-summary.json`、`knowledge-proof/knowledge-proof.json`、`knowledge-proof/installed-normative-assets.json`、`knowledge-proof/knowledge-retrieve-hybrid-canary.json`、`knowledge-failure/knowledge-failure-injection-proof.json`、`knowledge-soak/knowledge-soak-summary.json` 与 `knowledge-soak/iteration-XX-retrieve-hybrid-canary.json`、`gate-int-10-qemu.log`、结构化 `autopkgtest` 输出目录、`.changes/.buildinfo/.deb/.ddeb` package artifacts、`lintian.log`、secret injection record 与命令元数据；`FULLINT-TODO-019` 后续只需要在真实 runner 上复跑并把归档路径回写 worklog / deliverable，不需要再发明第二套 release harness。
...
10. `scripts/packaging/infra_release_soak_gate.sh`
11. `scripts/packaging/knowledge_refresh_retrieve_soak.sh`
12. `scripts/packaging/services_local_installed_proof.sh`
13. `scripts/packaging/services_subscription_adapter_soak.sh`

仍按需待定的只有：

14. `scripts/packaging/autopkgtest-*.cfg`（仅当 qemu / CI 配置需要固化时新增）
